...
- ⚠️ **Editing skill content requires the skill auditor as a gate** - After editing any `SKILL.md` or skill reference under `src/plugins/`, run `develop:skill-auditor` (or `/auditing-skills`) before shipping. The `/pr` `changes-reviewer` and the CI `spec-tree-review` do NOT load the skill-authoring standards (`standardizing-skills`, `standardizing-agent-prompts`) and will not catch voice (named-subject), structure, or progressive-disclosure violations — only the skill auditor does.
