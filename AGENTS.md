...
- ⚠️ **Protect untracked work during cleanup** - when a gitignored artifact blocks a gate, run `just clean` as the agent's own action, without an operator question or a path-limited substitute. The command removes gitignored artifacts such as `.metadata_never_index`, `.DS_Store`, and `__pycache__` while preserving `.git`, `.gitignore`, `.spx/`, and the active Python environment. Inspect every non-gitignored untracked path before removing it and establish whether it is user work, generated output, or stale workspace state. Empty directories remain visible to filesystem discovery even though Git does not track them; remove a stale empty directory after confirming it is empty and no workflow owns it.
