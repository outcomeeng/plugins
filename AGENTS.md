...
- ✅ **Use this repo's command surface exactly** - Skills decide when validation, tests, review, audit, merge, and marketplace sync are required. `AGENTS.md` records this repo's concrete command forms and how to pass the file set:
  ...
  - Plugin version bumps: `spx/local/open-pr.md` owns `just bump`, followed by `just build-skills`, as the final commit before opening a pull request after base synchronization, and re-runs the bump step after a later rebase before pushing. Ordinary commits never initiate or request a bump. NEVER hand-edit a manifest `version` field; the opening overlay carries the full bump policy and its `just bump-check` refusal.
