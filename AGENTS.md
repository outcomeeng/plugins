...
- ⚠️ **Every change destined for the default branch passes through the local review gate first** - Before any `git push` of a PR branch, run the `changes-reviewer` agent (preferred — isolated context) or the `/review-changes` slash command on the working diff. The gate stops the push on any finding with `severity == "blocking"` or `severity == "debt"`. `follow_up` findings are fixed in-PR unless the fix would widen scope substantively, in which case the deferral is recorded in the relevant node's `ISSUES.md` or `PLAN.md`. `/opening-pr` Step 3 invokes this gate automatically; the spec assertion + `[eval]` evidence live at `spx/21-spec-tree.enabler/76-pr-workflow.enabler/pr-workflow.md` and `evals/local-review-gate/`.
