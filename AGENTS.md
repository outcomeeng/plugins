...
- 🛑 **STOP TRIGGER — NEVER call `sleep` to wait or pace work** - No `sleep 30`, no `sleep 210; echo wake`, no `sleep` inside a backgrounded command, no `sleep` in a `while`/`until` loop. Every shell `sleep` spawns a subprocess (and file descriptors) the harness does not reliably reap; across turns and concurrent agents they accumulate until the host is exhausted and the agent is killed — this has happened in this repo. To wait for a build, CI run, or process to finish, or to re-check on an interval, use the runtime's timer mechanism — in Claude Code, `/loop` for recurring work or `ScheduleWakeup` for a single re-check; in Codex, its scheduling primitive — never a shell `sleep`. If an earlier turn left a `sleep` running, identify it and terminate it by PID before doing anything else.
