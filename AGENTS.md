...
### Codex Root Instruction Rule

At the start of every Codex turn in this repository, before answering, planning, invoking skills, spawning agents, or editing files, Codex MUST read this `AGENTS.md` file from line 1 through EOF in the current turn. A partial read is not valid. Codex must treat the full file as the active repository operating policy for the turn, including product-owned sections and the managed Spec Tree instruction block.
