...
- ⚠️ **NEVER touch a spec-tree node without invoking `/contextualizing` first** - Before implementing work on a node, creating a node, editing a spec file, or opening a PR whose diff sits inside `spx/` or imports modules tested by `spx/`, invoke `/contextualizing <full-path>`. The skill loads the full ancestry (product → decisions → ancestors → target) deterministically. Reading spec files directly leaves you with partial context, missing decisions, and downstream errors. The "Spec-tree navigation" section below explains how to identify the governing node from a diff.
