...
- ✅ **Use this repo's command surface exactly** - Skills decide when validation, tests, review, audit, merge, and marketplace sync are required. `AGENTS.md` records this repo's concrete command forms and how to pass the file set:
  ...
  - Marketplace release installation after merged plugin-distribution changes: `just install-marketplace` with the designated main checkout, fast-forwarded to the merge, as its working directory, as directed by `spx/local/merging.md`. The command refreshes every Claude Code `outcomeeng` install record on the machine through the native plugin update at that record's own scope and project path, bootstraps the invocation checkout's project scope, and refreshes the selected `$CODEX_HOME`; empty agent state receives only `spec-tree` with a warning, and nonempty state without `spec-tree` is rejected before mutation.
