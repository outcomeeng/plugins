...
- ⚠️ **ALWAYS read `spx/AGENTS.md` before spec-tree work** - `spx/AGENTS.md` is the spec-tree directory guide and skill router. Read it before working with files under `spx/` or applying spec-tree lifecycle rules from this root guide.
...
- ✅ **Use this repo's command surface exactly** - Skills decide when validation, tests, review, audit, merge, and marketplace sync are required. `AGENTS.md` records this repo's concrete command forms and how to pass the file set:
  ...
  - Generated spx guide files after guide-template or distribution-render changes: `just build-skills`, then `just build-guides`. Do not hand-edit `spx/CLAUDE.md` or `spx/AGENTS.md`; regenerate them from the rendered runtime templates in `dist/`, then verify with `just guide-check`.
...
- `AGENTS.md` (this file), `CLAUDE.md` (symlink to `AGENTS.md`) — repo-level instruction surfaces.
