...
- ✅ **Use this repo's command surface exactly** - Skills decide when validation, tests, review, audit, merge, and marketplace sync are required. `AGENTS.md` records this repo's concrete command forms and how to pass the file set:
  ...
  - Plugin delivery after a merged plugin-distribution change: no command. Both agents resolve from the `outcomeeng/plugins` marketplace declared in this checkout's committed configuration, so the merge itself publishes; the lifecycle runs no refresh, per `spx/local/merging.md`.
