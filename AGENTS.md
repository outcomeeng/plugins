...
- ⚠️ **Audit skills (`audit-*`) must be read-only** - They produce verdicts, not code changes. `allowed-tools` should not include `Write` or `Edit`. The calling workflow decides what happens after the verdict
...
- ⚠️ **Every change destined for the default branch passes through the `REVIEW_READINESS` gate before every push** - `/merge` dispatches the commit-to-merge lifecycle to the transport it selects from `spx/local/merging.md`: a coordination-note-only changeset (only `PLAN.md` / `ISSUES.md`) to the direct-push transport, an overlay-declared `transport:` when present, else the GitHub-PR transport (`/manage-github-pr`) as the default that carries all reviewable work. The GitHub-PR transport's internal opening protocol opens the PR `ready_for_review` only when local deterministic verification passes and the local review has converged; its internal managing protocol re-establishes the same two predicates before each follow-up push (Step 6), so a fix or a base-sync rebase never reaches CI on a diff no local review has seen. Local deterministic verification is changeset-scoped per `spx/local/merging.md`: run the narrow validation and testing lane that covers touched files, reserving local full-repository `just check` for shared validation/test infrastructure, package manager files, generated catalog output, distribution build machinery, or broad implementation surfaces whose touched-scope commands cannot cover the contract. CI owns the full-repository regression pass. It runs the `changes-reviewer` agent on the working diff (isolated context, so the verdict is not biased by what the operator's main context has been doing; fall back to the `/review-changes` slash command when `changes-reviewer` is not installed — both invoke the same `review-changes` skill chain), invoked at parity with the CI reviewer — only the repository/worktree and diff range are passed, with no interpretive scope, severity pre-filter, or emphasis steering. Findings are acted on by **validity and phase, never severity**: validate against the cited rule, drop the unbacked, apply every valid finding that belongs, sweep the touched node(s) for same-class instances before the next push, and split out of the changeset only a fix too large to belong (recorded in the relevant node's `ISSUES.md` or `PLAN.md`). `MERGE_READINESS` (a clean current-head CI review exists — present, complete, and reporting no unresolved `BLOCKING` or `DEBT` finding (stated directly or with every such finding individually refuted as unbacked; a `DEBT` finding the author tracks out of scope with a recorded reason is not unresolved), with absence never satisfying it — plus every other required check terminal-green, no settle) and `PRODUCTION_READINESS` then govern the merge per `/merging-standards` `<authority_gates>`. The transport-neutral spec assertions + `[eval]` evidence live at `spx/21-spec-tree.enabler/76-merging.enabler/merging.md` and `spx/21-spec-tree.enabler/76-merging.enabler/evals/`; the GitHub-PR transport's gate evals live under `spx/21-spec-tree.enabler/76-merging.enabler/32-github-pr.enabler/32-opening-pr.enabler/evals/` (review-readiness) and `spx/21-spec-tree.enabler/76-merging.enabler/32-github-pr.enabler/54-managing-pr.enabler/evals/` (merge-readiness and the other open-PR evals).
- 🛑 **STOP TRIGGER — NEVER call `sleep` to wait or pace work** - No `sleep 30`, no `sleep 210; echo wake`, no `sleep` inside a backgrounded command, no `sleep` in a `while`/`until` loop. Every shell `sleep` spawns a subprocess (and file descriptors) the harness does not reliably reap; across turns and concurrent agents they accumulate until the host is exhausted and the agent is killed — this has happened in this repo. Wait via the runtime's timer mechanism when one exists; for no-timer PR check waits, use the foreground `gh pr checks <pr-number> --watch --fail-fast --interval 30` path in "Process hygiene" below. If an earlier turn left a `sleep` running, identify it and terminate it by PID before doing anything else.
...
Planning artifacts are transient — `PLAN.md` and `ISSUES.md` are committed coordination notes that `/handoff` leaves in node directories. They are git-tracked only to carry coordination across sessions and go stale unless acted upon, so reconcile each against the specs, decisions, assertions, tests, implementation, and current user intent before it steers work; they never declare product truth. `/contextualize` reads them, conformance checks skip them, and they are removed once resolved.
...
In this product the agent operates the git workflow autonomously. The conservative system default — "ask before every commit, push, and PR action" — is **overridden** here, and `spx/local/merging.md` Pre-mutation confirmation declares no up-front proposal-and-confirm pause, so `/merge` and `/manage-github-pr` state the plan in prose and drive the lifecycle without waiting. Standing authorization covers:
...
- Committing changes via `/commit-changes` (the skill loads the product's commit rules from `spx/local/commit-changes.md`; the agent does not bypass the skill).
- For the GitHub-PR transport that `/merge` delegates to, pushing the feature branch through `/manage-github-pr`'s opening protocol (which runs the branch-hygiene checks and `git push -u origin HEAD:refs/heads/<branch>`).
- Opening the PR ready through `/manage-github-pr`'s opening protocol (which calls `gh pr create` with a curated title and body once `REVIEW_READINESS` holds).
- Iterating on the PR through `/manage-github-pr`'s managing protocol (commit + push + base-sync rebase with a `--force-with-lease` push of the PR's own feature branch + heartbeat + review inspection).
- Merging through `/manage-github-pr`'s managing protocol once `MERGE_READINESS` and `PRODUCTION_READINESS` hold, per `/merging-standards` `<authority_gates>` and refined in `spx/local/merging.md`.
...
`/merge` is the lifecycle entry point: it selects the transport from `spx/local/merging.md` — a coordination-note-only changeset (only `PLAN.md` / `ISSUES.md`) to the direct-push transport, an overlay-declared `transport:` when present, else the GitHub-PR transport — and delegates. For GitHub-PR-transport work (the default for everything reviewable), `/merge` delegates to `/manage-github-pr` and the path is:

1. **Branch.** Cut a feature branch off `origin/main` — `fix/…`, `feat/…`, `docs/…`, or `work/…`.
2. **Bump plugin versions** when the branch's first plugin-distribution commit is about to land. Run `just bump-dry` to preview, then `just bump` to write — the tool detects every plugin with changes under `src/plugins/<name>/**` since `origin/main`, classifies each plugin's change pattern into a semver segment (a plugin that gains, loses, or renames a skill/command/agent/manifest gets `minor`; everything else gets `patch`), and writes the resulting version in every manifest each plugin owns (`.claude-plugin/plugin.json` and `.codex-plugin/plugin.json`). The same run can produce `minor` for one plugin and `patch` for another. When a plugin already carries a bump on this branch, that plugin is skipped with a diagnostic while every other changed-but-unbumped plugin is still bumped in the same pass (per `spx/local/commit-changes.md`'s "bump once per branch" rule), so re-running after a later commit bumps that newly-changed plugin without a manual edit. Auto-detection never selects `major` — pass `--segment major` (or `minor`/`patch`) to force a segment for every changed plugin; explicit overrides emit a stderr warning naming any plugin whose detected segment differed. The mutually-exclusive `--check` mode exits non-zero if any changed plugin still needs a bump; CI calls `just bump-check` to enforce that every PR carries its version bump.
3. **Commit.** Invoke `/commit-changes`. It loads the product's commit rules — Conventional Commits, the version-bump policy, which manifests to touch — from `spx/local/commit-changes.md`.
4. **Open the PR.** `/merge` delegates to `/manage-github-pr` for the GitHub-PR transport. `/manage-github-pr` routes to the opening protocol, runs branch-hygiene checks and evaluates `REVIEW_READINESS` (local deterministic verification green + local review converged), pushes the branch with an explicit destination ref (`git push -u origin HEAD:refs/heads/<branch>`), opens a ready PR with a curated title and body, and creates or requests a thread heartbeat so review/check re-inspection is handled by the runtime timer when one exists. If the runtime can only create a new thread, seed that heartbeat with the repository, PR number, branch, and review-loop instructions. When no runtime heartbeat or timer exists and the PR is blocked by check completion, `/manage-pr` runs one foreground `gh pr checks <pr-number> --watch --fail-fast --interval 30`, inspects the terminal result, then re-runs the full merge-gate inspection before acting. It reads `spx/local/merging.md` for the GitHub-PR transport configuration (merge command, production-relevance recognition, deterministic verification scope, pre-flight, post-merge) through `/open-pr`, `/manage-pr`, and `/merging-standards` — transport selection itself is `/merge`'s — and the opening protocol loads `spx/local/open-pr.md` for product-specific pre-flight checks and template sections.
5. **Review and merge.** Continue through `/manage-github-pr`. It routes to the managing protocol, drives the post-creation loop — three-surface review inspection, two-severity triage of findings, follow-up pushes — and merges autonomously when `MERGE_READINESS` (a clean current-head CI review exists — present, complete, and reporting no unresolved `BLOCKING` or `DEBT` finding (stated directly or with every such finding individually assessed and dropped as unbacked; a `DEBT` finding the author tracks out of scope with a recorded reason is not unresolved), with absence never satisfying it — plus every other required check terminal-green, no settle), `PRODUCTION_READINESS` (no production-relevance mechanism here, so it holds), and the mutation-point guard (`MERGE_READY:<head-sha>` from freshly re-read PR/check/review/head state) all hold, per `/merging-standards` `<authority_gates>`. The product's `spx/local/merging.md` overlay loads via `/merging-standards` `<repo_local_overlay>` and supplies the project-specific merge command, production-relevance recognition, deterministic verification scope, and post-merge sync. The overlay's merge command is `gh pr merge <n> --merge --delete-branch=false` followed by `git push origin --delete <branch>` (the rebase-merge default conflicts with the product's merge-commit history style; the explicit `--delete-branch=false` keeps `gh` from running its local-cleanup branch switch rather than relying on `gh`'s unknowable default for the omitted flag — that switch fails in this multi-worktree checkout where `main` is checked out elsewhere — and the separate remote-delete then deletes the branch deterministically). Verify the merge with `gh pr view <n> --json state,mergedAt,mergeCommit`. A merge is one step, not necessarily the session's end: if in-scope parts of the stated goal remain, continue with them directly; invoke `/handoff` only when the session is genuinely over — the goal is met with nothing remaining, or continuation by the agent is impossible (per the `/handoff` precondition) — and never pass `--no-session` on the agent's behalf.
6. **Sync.** Once a PR with plugin distribution changes merges, bring the **marketplace-source worktree** to the merged `main`, then refresh installs. The source is the Directory-source checkout `claude plugin marketplace list --json` reports for `outcomeeng` — the authoritative default-branch worktree, on branch `main`. Its `dist/` is what the marketplace serves, so fast-forwarding *its* `main` — not the current feature worktree's HEAD — is what makes the merge visible:
   ...
   `just sync-marketplace` refreshes the local Claude marketplace cache, reconciles Claude and Codex `outcomeeng` registrations to the default-branch local marketplace source, reinstalls every generated Codex plugin exposed under `dist/codex` via `codex plugin add <plugin>@outcomeeng`, repairs the Codex compatibility symlinks, installs generated Codex agents, and runs `validate_install` and `check-installed`. It accepts an optional base ref; when the range from that ref to `HEAD` has no changes under `src/`, `dist/`, `.claude-plugin/`, or `.agents/plugins/` and no runtime config repair was needed, it exits without refreshing the marketplace. It does not push or fetch. `main` lives in the marketplace-source worktree only; feature worktrees detach and never attach `main` (`git switch main` there fails with `fatal: 'main' is already used by worktree at <path>`). `spx/local/merging.md` is the authoritative source for the multi-worktree post-merge rationale.
