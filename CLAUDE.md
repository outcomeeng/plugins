...
- ✅ **Use this repo's command surface exactly** - Skills decide when validation, tests, review, audit, merge, and marketplace sync are required. `AGENTS.md` records this repo's concrete command forms and how to pass the file set:
  ...
  - Marketplace release installation after merged plugin-distribution changes: `just install-marketplace` with the designated main checkout, fast-forwarded to the merge, as its working directory, as directed by `spx/local/merging.md`. The command updates every project- or local-scope Claude Code `outcomeeng` install record on the machine whose plugin the committed catalog carries, whose project path is an existing directory, and whose project declares no noncanonical marketplace source, through the native plugin update at that record's own scope and project path; a record outside project and local scope, outside the catalog, at a path that is no directory, in a project declaring a noncanonical source, or in a project whose settings cannot be read is reported and left unchanged. A noncanonical source in the invocation checkout's own settings, or settings it cannot read, stops the run before any plan. It bootstraps the invocation checkout's project scope and refreshes the selected `$CODEX_HOME`; empty agent state receives only `spec-tree` with a warning, and nonempty state without `spec-tree` is rejected before mutation.
...
**Claude Code** uses the project-scoped GitHub marketplace `outcomeeng/plugins`. `just install-marketplace` validates that user scope carries no colliding `outcomeeng` registration, inspects the plugins installed for the invocation checkout, adds the project registration when the checkout declares none, and refreshes exactly those catalog plugins, then updates every project- or local-scope `outcomeeng` install record Claude Code reports on the machine for a cataloged plugin whose project path is an existing directory and whose project declares no noncanonical source, from that record's own project path, so every session on the machine starts and reloads at the version the canonical source serves during the run; a record outside project and local scope, outside the catalog, at a path that is no directory, in a project declaring a noncanonical source, or in a project whose settings cannot be read is reported and left unchanged. A noncanonical source in the invocation checkout's own settings, or settings it cannot read, stops the run before any plan. The run is one bounded pass: it plans from one listing read, reads the listing again after execution, and reports each refreshed record's version before and after together with every project- or local-scope record it did not refresh; it takes no lock and no second update pass, and a record installed after the run holds its own version until the next run.
...
`just verify-marketplace-installation` is the pre-merge proof. Installation and session state are disposable; subscription discovery may read and natively refresh the selected saved login. `just install-marketplace` is the release action and mutates the selected persistent state. A persistent run stops before its first state-changing command when Claude Code user scope contains an `outcomeeng` registration, reporting the exact colliding settings path.

## Missing plugins or skills

With explicit operator authorization to change persistent plugin state, run `just install-marketplace` from the intended checkout. The command inspects each agent's installed `outcomeeng` plugins, bounds and orders them through that agent's committed catalog, adds an absent project registration and refreshes a canonical one, stops before any plan when the invocation checkout declares a noncanonical source or its settings cannot be read, refreshes exactly that set, updates every project- or local-scope `outcomeeng` install record on the machine for a cataloged plugin whose project path is an existing directory and whose project declares no noncanonical source, and reconciles the selected plugins' generated Codex definitions into the selected home under marketplace ownership. Empty agent state receives only `spec-tree` and a warning. Nonempty state without `spec-tree` is rejected before mutation.

Claude Code state is every project- or local-scope install record on the machine, keyed by scope and project path. A user-scoped `outcomeeng` registration blocks the command and must be resolved by the owner of that settings file. Codex installation state belongs to the selected `$CODEX_HOME`; trusted product `.codex/config.toml` controls effective skill activation. Use `just verify-marketplace-installation` when the goal is disposable proof rather than a persistent state change.
