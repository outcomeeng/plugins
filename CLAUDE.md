...
- ✅ **Use this repo's command surface exactly** - Skills decide when validation, tests, review, audit, merge, and marketplace sync are required. `AGENTS.md` records this repo's concrete command forms and how to pass the file set:
  ...
  - Marketplace installation verification before merge: `just verify-marketplace-installation`. The command runs the repository-installation L3 evidence through real agent CLIs in disposable homes and leaves persistent state unchanged.
