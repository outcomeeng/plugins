...
**Sections** are `Breaking`, `Added`, `Changed`, `Deprecated`, `Removed`, `Fixed`, `Requires`. `Breaking` is elevated rather than folded into `Changed`, because a rename breaks invocation outright and must not be discoverable only by careful reading. `Deprecated` is separate for the same reason in the other direction: it is the only section that asks a reader to act before a future release rather than describing one that already happened, and the deprecation rule above gives that request a deadline — a `Deprecated` entry names the edition that removes the form. `Requires` carries floor and compatibility advances so a version constraint is readable without prose.
