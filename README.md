...
| Agent | `pr-review-orchestrator`   | Running a CI-side stateful pull request review — runs the PR review and deterministic six-phase audit over the PR diff, derives resolved and reopened from the pull-request audit journal run set, and posts one fresh combined comment that supersedes the prior display while keeping the latest review prose |
