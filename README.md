...
| Skill | `/operate-agent-mail`   | Registering a mail identity, sending a message record, reading an inbox, or recording a receipt in the agent-mail store                                                                                                                          |
| Skill | `/operate-herdr`        | Operating on agent sessions herdr hosts — agent inventory, read, bounded wait, prompt, start, relaunch, stop, keystroke, or worktree open                                                                                                        |
| Skill | `/operate-prowl`        | Operating Prowl or delivering a correlated delegation handback between Prowl coding agents                                                                                                                                                       |
...
| Skill | `/manage-upstream-issue`  | Continuing an open issue in a repository the operator does not control — answering a maintainer, adding evidence, or reporting the thread's current state                                                                                                                  |
| Skill | `/manage-upstream-pr`     | Continuing an open pull request against a repository the operator does not control — answering review, publishing a revision, or reporting its current state                                                                                                               |
| Skill | `/open-upstream-issue`    | Filing an issue in a repository the operator does not control — a fork's upstream, or any base whose permission is READ, TRIAGE, or NONE                                                                                                                                   |
| Skill | `/open-upstream-pr`       | Opening a pull request against a repository the operator does not control — a fork's upstream, or any base whose permission is READ, TRIAGE, or NONE                                                                                                                       |
| Skill | `/sync-fork`              | Bringing a fork's default branch current with the repository it was forked from                                                                                                                                                                                            |
| Skill | `/upstream`               | Starting a contribution to a repository the operator does not control, or when a contribution step needs that target and no live UPSTREAM_TARGET marker carries it — it resolves the upstream a fork came from, the head to push from, and the operator's permission on it |
...
Spec Tree: /understand, /contextualize, /bootstrap, /author, /decompose, /refactor, /align, /interview, /test, /audit-tests, /audit-eval-evidence, /audit-adr, /audit-pdr, /audit-implementation, /audit-specs, /audit-changeset-coherence, /apply, /commit-changes, /claim-change, /release-change, /close-change, /refocus, /diagnose
...
| Skill | `/audit-change`               | Change record audit methodology                                                                                                                                                                                                                                               |
...
| Skill | `/audit-implementation`       | Implementation audit methodology                                                                                                                                                                                                                                              |
...
| Skill | `/author-change`              | Creating, interviewing, or revising an Outcome Engineering Change record                                                                                                                                                                                                      |
| Skill | `/bootstrap`                  | Setting up a new spec tree or when /author detects an empty spx/ directory                                                                                                                                                                                                    |
| Skill | `/change-standards`           | Change record standards for Output, maturity, lifecycle, refinement, and continuation                                                                                                                                                                                         |
| Skill | `/claim-change`               | Claiming an Available Change from the declared Change store to hold it for refinement or execution                                                                                                                                                                            |
| Skill | `/close-change`               | A held Change reaches a terminal Lifecycle — Applied, Refined, or Abandoned — to write the terminal record, remove the holder, and close it in the declared store                                                                                                             |
...
| Skill | `/refocus`                    | Running ad hoc commands or writing code without a spec inside a repository whose root contains `spx/`                                                                                                                                                                         |
| Skill | `/release-change`             | Stopping work on a held Change so another agent can continue it — it writes the Handoff, removes the holder, and returns the Change to Available                                                                                                                              |
...
| Skill | `/test-evidence-standards`    | Test-evidence seam, case-provenance, oracle-independence, execution-level, per-assertion-type artifact-permission, and pre-authoring assertion-design-record standards enforced across test authoring and auditing                                                            |
