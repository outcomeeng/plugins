...
`open-worktree` opens a workspace for an existing worktree without taking focus and returns its root pane; `start` launches the named agent kind into a pane at its shell prompt and succeeds only when herdr reports the session ready for input within the bound. `stop` closes the pane and the session in it.
...
Every other code stays verbatim under `command-failed`. A stalled or timed-out result does not prove the prompt was never delivered; read the session before submitting it again.
...
- NEVER invoke raw herdr commands, herdr command help, or the bundled herdr skill text.
