...
allowed-tools: Bash(prowl list --json), Bash(prowl agents --json), Bash(prowl read --pane * --last 80 --wait-stable --json), Bash(python3 "${CLAUDE_SKILL_DIR}/scripts/recover_agents.py":*), AskUserQuestion
---
...
**How to avoid:** Leave ambiguous panes stopped and present their complete pane and worktree identities through `AskUserQuestion` before adding them to the target set.
