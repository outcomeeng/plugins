...
| --------------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| `controlled`          | `ADMIN`, `MAINTAIN`, or `WRITE` on a base that is not a fork parent | STOP. This is a controlled-repository contribution; the plugin's flows do not apply. |
| `parent-contribution` | `READ` or `NONE`, or a base resolved through the checkout's parent  | Continue under `<invariants>`, starting with authorization.                          |
...
**Outward-facing text is permanent.** The notification reaches every watcher when the artifact appears, and deleting the artifact does not recall it. Title, body, comments, and review replies each pass a prose review before they are sent. Where the prose plugin is installed, invoke `/audit-prose` through the runtime's skill-composition surface; where it is not, review against `<outward_text>` and report that the review ran unassisted.
