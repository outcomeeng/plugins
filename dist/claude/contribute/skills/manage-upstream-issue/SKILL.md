...
allowed-tools: Read, Skill, Agent, AskUserQuestion, Bash(gh issue view:*), Bash(gh issue comment:*), Bash(gh issue close:*), Bash(gh api user:*), Bash(printf:*)
---
...
With a question to answer, draft per `<reply_shape>` and review — the prose plugin's `prose:prose-auditor` thin agent where installed, `/contribution-standards` `<outward_text>` unassisted where not, stated as such.
