...
allowed-tools: Read, Skill, Agent, AskUserQuestion, Bash(gh issue create:*), Bash(gh search issues:*), Bash(printf:*)
---
...
**Step 6 — GATE: Review the outward text.** Draft the title and body per `<report_shape>`, then review them — the prose plugin's `prose:prose-auditor` thin agent where installed, `/contribution-standards` `<outward_text>` unassisted where not, stated as such in the report.
