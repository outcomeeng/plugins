...
MUST verify refactored code satisfies the `go:code-go` verification checklist (its bundled `references/verification-checklist.md`).
...
<example>
Dispatch prompt: "Simplify `internal/orders/processor.go`."

Test discovery finds `spx/55-example.enabler/tests/processor.scenario.l1_test.go` importing `<module>/internal/orders` and a `mocks.MockStore` generated by `mockery` standing in for the `Store` interface. Verdict: REJECT under `<test_validation>` — the seam is severed, so a behavior change under refactoring would pass. Output: "Cannot refactor: tests use generated mocks instead of DI" and no edit.

Same prompt, but the test injects a hand-written `harnesses.RecordingStore` and compares the returned order against a source-owned `orders.StatusAccepted`. Verdict: ACCEPT. `go test -race ./...` passes before the change; the refactor collapses a nested early-return chain in `Process`, keeps the `Store` parameter and the `context.Context` it forwards, and `go test -race ./...` and `go vet ./...` pass after.
</example>
...
6. **Load standards** — read product CLAUDE.md if present
...
If CLAUDE.md not found: use Go best practices from the `go:code-go` skill, note this in output.
...
- [ ] Satisfies the `go:code-go` verification checklist
