...
- **invoker_coupling**: a skill naming, describing, detecting, constraining, refusing, or branching on the agent, skill, or context that invokes it, or halting because a particular surface loaded it — per `/skill-standards` `<skill_organization>` invoker independence. A description that steers the reader to dispatch an agent is this finding in the frontmatter; a `<dispatch_gate>` or equivalent invoker check in the body is this finding in the body
