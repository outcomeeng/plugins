...
- Dispatch every skill audit through the typed `skill-auditor` role, reading `${CLAUDE_SKILL_DIR}/references/verifier-dispatch.md` before the first dispatch — it owns the dispatch mechanics and the role-task contract, which load with this skill rather than with the managed router block. If the role is unavailable or returns no complete structured verdict, report `BLOCKED`; never invoke `/audit-skill` in the authoring context.
