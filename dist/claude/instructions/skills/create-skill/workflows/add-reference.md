...
Run the target repository's canonical skill build and deterministic checks. Confirm the file exists, every citation resolves, no bundled file is orphaned, the body passes the reference-file checks in `/skill-standards`, and a fresh typed `instructions:skill-auditor` verdict approves the complete bundle.
...
- Repository checks pass and a typed `instructions:skill-auditor` verdict approves the complete bundle.
