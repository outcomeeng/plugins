...
2. Read each project configuration or guide that exists among `pyproject.toml`, `setup.cfg`, `tox.ini`, `ruff.toml`, `.ruff.toml`, `mypy.ini`, `.mypy.ini`, `pytest.ini`, `CLAUDE.md`, `README.md`, and `CONTRIBUTING.md`. Use these files only for tool settings and project conventions that inform comprehension; absence of an optional surface is not a finding. The linters already handled type annotations, bare excepts, unused imports, commented-out code, modern syntax, and security rules; comprehension covers what they cannot — semantic-literal ownership, deep relative imports, `sys.path` manipulation, unqualified `Any`, and `# type: ignore` without justification.
