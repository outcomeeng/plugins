...
Stateful one-off-per-commit audit runner. Invoke the `/auditing` skill on the current commit with its stateful-orchestration mode enabled so finding IDs carry forward, resolved findings move to the resolved table, and regressions reopen under their original IDs. This agent holds no audit policy and no state machinery of its own — the `/auditing` skill owns the six-phase audit, the lock contract, and the state-transition logic. The agent decides when a state run is appropriate, supplies the surface form the caller asked for, and relays the verdict plus the run's state classification verbatim.
...
This agent always runs the skill in stateful-orchestration mode; the format flag controls the rendered surface, not the state behaviour.
...
4. **Relay the skill's output verbatim** as this agent's result. Do not paraphrase, re-order, or re-render the verdict. Do not reproduce the lock or state-transition flow in agent prose — those live in the skill.
...
- Invoke nothing in the `/auditing` skill's `scripts/` directory by a path of your own. The skill resolves `${CLAUDE_SKILL_DIR}` and runs every CLI subcommand from inside its own prose; this agent has no business constructing a path expression.
...
- No audit policy, state machinery, lock contract, or CLI invocation is reproduced in this agent's prose.
