...
| Test evidence    | `spec-tree:verify`, then `spec-tree:test` and each applicable installed language specialist, with `spec-tree:test-evidence-standards` and the language's test standards | Every selected linked test and eval                                                                  | Steps 6 and 8a through `spec-tree:test-evidence-auditor` and `spec-tree:eval-evidence-auditor` when malleability selects them; Step 9 when malleability selects it                   |
| Spec or decision | `spec-tree:author` and `spec-tree:verify`, with the live `/understand` templates and foundation                                                                         | The repository-declared spec validation and status projection                                        | Step 4 through `spec-tree:adr-auditor` or `spec-tree:pdr-auditor` for each changed decision; `spec-tree:spec-auditor` for each changed spec; Step 9 when malleability selects it     |
| Skill            | `instructions:create-skill`, with `instructions:skill-standards` and `instructions:agent-prompt-standards`                                                              | The repository-declared skill build and deterministic skill or documentation checks                  | `instructions:skill-auditor` once over the complete changed skill surface at the Step 8a-equivalent when the `instructions` plugin is installed; Step 9 when malleability selects it |
| Prose            | `prose:author-prose`, with `prose:prose-standards`                                                                                                                      | The repository-declared documentation and prose checks                                               | `prose:prose-auditor` once over the complete changed prose surface at the Step 8a-equivalent when the `prose` plugin is installed; Step 9 when malleability selects it               |
...
Before dispatching an applicable evidence auditor, apply `<verification_checkpoint>`; carry each verdict forward under `<result_carryover>`. When both evidence classes changed, dispatch both auditors against the same checkpoint. Step 8a completes only after every applicable evidence-auditor verdict is clean on the exact committed diff it reviews. The skill row dispatches `instructions:skill-auditor`, the prose row dispatches `prose:prose-auditor`, the spec row dispatches `spec-tree:spec-auditor`, and the decision row dispatches `spec-tree:adr-auditor` or `spec-tree:pdr-auditor` at this same checkpoint after their deterministic lanes. Skill and prose audits run when their plugins are installed and apply independently of whether malleability selects numbered test/eval evidence audit.
