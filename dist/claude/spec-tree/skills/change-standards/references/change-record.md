...
The governing methodology is the Change chapter selected by the consumer repository's `spx.config.yaml`. Read `methodology.version` as a string and compare its complete value with the accepted 4.0-line forms: exactly `4.0`, or exactly `4.0.N` with `N` one or more decimal digits representing a non-negative integer. For either form, resolve `methodology.source` and read `versions/4.0/methodology/change/changes.md` inside that declared source. Reject an absent configuration, an absent `methodology` block, the sentinel `installed`, and every value outside those forms, including another major or minor line, a non-integer patch component, an extra version component, or a prerelease suffix. The rejection names the observed value — using `absent` when no value exists — and the accepted forms `4.0` and `4.0.N`, where `N` is a non-negative integer. This reference operationalizes the selected chapter without replacing it. A Change is mutable coordination for one intended Output in one Product; it declares no product, architecture, or methodology truth.
