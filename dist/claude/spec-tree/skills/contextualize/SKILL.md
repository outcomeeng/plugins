...
- `dirty_tree` → distinguish whose changes block the rebase. **Changes Claude created this session** (a coordination note is the canonical case) are Claude's to clear per `/sync-base`'s `<dirty_tree_resolution>` — commit them to the right branch (objective work to the change branch, an unrelated coordination note to its own branch pending `/merge`), then re-run `/sync-base` so context loads against the current base. **The operator's pre-existing work Claude did not author** is left untouched — proceed to Step 0 noting that loaded context may be stale; context loading never commits or stashes the operator's in-progress work. Neither case is a `SYNC_BASE` conflict, and neither is a reason to ask the operator to resolve a merge conflict.
