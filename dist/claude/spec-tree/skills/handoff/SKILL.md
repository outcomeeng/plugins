...
4. `${CLAUDE_SKILL_DIR}/workflows/04-execute.md` — create or update coordination notes, commit, then write or omit each thread's canonical continuation session file
   - Conditional, inside 04: `${CLAUDE_SKILL_DIR}/workflows/05-change.md` — when `spx/local/coordination.md` exists, post the Handoff on each held Change, release it through the complete Available-state transition, and preserve any terminal condition as the next Activity until the terminal Status protocol is implemented, in place of 04's session-file and archive steps
