...
**Step 6 — Re-establish `REVIEW_READINESS`, then push follow-ups deliberately.** A Step 5 fix or a Step 4 rebase changed the diff, so before any push re-establish the `REVIEW_READINESS` predicates **on the exact tree the push would publish**. All predicates must hold *together* on that final tree — they iterate to a joint fixpoint, not a one-time linear pass:
...
3. **Applicable audit at parity.** When the diff carries implementation, test, or decision-record changes an audit verification type governs, re-dispatch the applicable auditor agents over the changeset per /merging-standards `<auditor_verdicts>` — each in its own isolated context, never an audit skill in this conversation — and handle every `REJECTED`/`UNKNOWN` overall verdict, `FAIL`/`UNKNOWN` row, and `REJECT` finding as in-slice work per `<review_classification>`, committing fixes via /commit-changes until every applicable auditor reports `APPROVED`. A clean local review never substitutes — review and audit inspect different concerns. The same reuse exception applies: when this push follows **only** a base-sync rebase with no content change, reuse the converged audit verdict if the `/sync-base` `preservation` proof and the overlay's governance-surface list permit it per `<base_sync>`. A diff with no auditable content has no applicable audit.

**Any fix in any sub-step mutates the tree, so loop:** a deterministic-verification fix is a new diff the local review and audit have not seen, and a review- or audit-driven fix is a new tree deterministic verification has not covered. Re-run the predicates after every commit until a single tree passes deterministic verification *and* carries no unaddressed valid review or audit finding — that converged tree is what Step 6 pushes. Never push a tree on which a later-fixed predicate was established before the last commit.
...
**Pushed a tree only one predicate had seen.** Claude re-ran deterministic verification after a review- or audit-driven fix, or re-ran the local review after a verification-driven fix, but not all on the final tree — each fix is a new diff the other predicates have not covered, so the pushed tree was never jointly gated. Step 6 iterates the predicates to a joint fixpoint: after every commit, re-run deterministic verification, the local review, and the applicable audit until one tree passes verification *and* carries no unaddressed valid review or audit finding, then push only that tree.

**Substituted a clean review for the audit on a follow-up push.** Claude re-established the deterministic-verification and review predicates on a follow-up diff that changed spec-governed implementation or test files, but did not re-dispatch the applicable auditor agents — letting a clean `changes-reviewer` review stand in for the audit type. Review and audit inspect different concerns. Re-dispatch the applicable audit (Step 6 sub-step 3) on every content-changing follow-up that carries auditable content, reusing the prior verdict only on a pure base-sync rebase the preservation proof permits.
...
- Every follow-up push re-establishes `REVIEW_READINESS` on the diff it would publish — local deterministic verification passes per /merging-standards `<local_deterministic_scope>` (or, for a push that only rebased onto an advanced base, the preservation-proof-scoped lane per /merging-standards `<base_sync>`), the local `review-changes` review (invoked at parity per /merging-standards `<local_review_invocation>`, with no caller narrowing) has converged with no valid finding unaddressed, and the applicable audit per /merging-standards `<auditor_verdicts>` has converged when the diff carries auditable content (reused only on a pure base-sync rebase the preservation proof permits) — re-runs /merging-standards `<branch_hygiene>`, and goes to the ready PR with no draft toggle.
