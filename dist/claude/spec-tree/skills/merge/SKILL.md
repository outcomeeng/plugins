...
allowed-tools: Skill, Agent, AskUserQuestion, Read, Write, Edit, Bash(git branch:*), Bash(git status:*), Bash(git symbolic-ref:*), Bash(git rev-parse:*), Bash(git diff:*), Bash(git push:*), Bash(python3 "${CLAUDE_SKILL_DIR}/scripts/classify_changeset.py":*), Bash(echo:*), Bash(spx diagnose:*), Bash(spx validation markdown:*), Bash(spx spec status:*)
---
...
A task the user explicitly limited to local-only work completes at that local boundary and does not dispatch a merge transport. For default-branch-scoped work, a changeset with commits ahead of its resolved base remains unfinished throughout classification and dispatch. An overlay-required pre-mutation confirmation pauses the next mutation and requires operator input, but it never turns local readiness into completion or becomes a lifecycle gate.
