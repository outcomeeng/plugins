...
The local `spec-tree:changes-reviewer` gate is the Author-side, pre-push instance of the same review kind the CI review runs post-push — the two are the same class of gate on opposite sides of each push. Invoke it the way CI invokes its Reviewer, passing nothing that narrows it:

- **Bind the committed subject and let the review derive its scope.** Commit the exact current version, require a clean worktree, and record the full `HEAD` before dispatch. Pass only `HEAD` for the current branch's default-base scope, or `origin/{base}...HEAD` for a selected base such as a stacked PR. `spec-tree:changes-reviewer` derives the committed diff and governing context from that selector. Supply no file list, changed-area summary, or interpretive prompt.
...
A process exception may substitute for the persisted local-review projection only when the review infrastructure fails after the typed `spec-tree:changes-reviewer` has completed. The operator is the sole approving authority. Approval is explicit, collected through a structured choice, and bound to the exact clean committed head named in that choice.

The exception is available only when all of these conditions hold:

- the typed `spec-tree:changes-reviewer` completed for the exact head and returned a raw review run token;
...
- **required evidence audits have passed** — when the least malleable node the diff touches is implementation-malleable (the absent `malleability` field, so a tree that declares none is implementation-malleable throughout) and the diff creates or modifies `[test]` assertions, linked test files, or test-infrastructure artifacts imported by linked tests, dispatch `spec-tree:test-evidence-auditor`; when the diff creates or modifies `[eval]` assertions, eval artifacts (`eval.toml`, `prompt.md`, `cases.jsonl`, `history.jsonl`), or producer artifacts for eval-backed assertions, dispatch `spec-tree:eval-evidence-auditor`. Each invocation supplies only the owning canonical node path; the invoked skill discovers its assertions and evidence. Apply `<verification_dispatch_readiness>`, then run the applicable evidence Auditors after deterministic verification passes and before `spec-tree:changes-reviewer`. Handle completed rejected, failing, or unknown evidence-auditor verdicts per `<auditor_verdicts>`; and
- **the local review has converged** — `spec-tree:changes-reviewer`, invoked at parity per `<local_review_invocation>` and iterated to convergence, leaves no valid finding unaddressed: each is fixed in the diff, or split out of the changeset and captured in the owning node's `ISSUES.md` or the governing Change. An unbacked finding is dropped. A process exception satisfying `<local_review_process_exception>` substitutes only for the unavailable persisted projection;
...
Local Auditor agents — `spec-tree:test-evidence-auditor`, `spec-tree:eval-evidence-auditor`, `spec-tree:adr-auditor`, `spec-tree:pdr-auditor`, `spec-tree:spec-auditor`, and `spec-tree:implementation-auditor` — emit structured findings for the slice they inspect. Language-specific audit concerns are composed through the installed `audit-{lang}-{code|tests|architecture}` skills, not through language-specific Auditor agents.
...
**Stale-base block.** A Verifier that resolves a committed scope — `spec-tree:implementation-auditor`, `spec-tree:changeset-coherence-auditor`, and `spec-tree:changes-reviewer` — fetches the base first and returns a `stale-base` block, never a verdict, when the head does not descend from the fetched `origin/<base>` tip. The block is no finding and no rejection: run `/sync-base`, re-establish the deterministic results on the rebased head under `<verification_dispatch_readiness>`, and dispatch again. A result established on a behind head satisfies no gate predicate.
...
- The local `spec-tree:changes-reviewer` gate is invoked per `<local_review_invocation>` over an exact committed subject from a clean worktree; the review derives its base/head scope, with no interpretive scope, severity pre-filter, or emphasis steering added.
