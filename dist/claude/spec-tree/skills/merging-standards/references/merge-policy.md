...
- **required artifact audits have passed** — dispatch every artifact Auditor selected by the active workflow's Output lanes for the changed surfaces. This includes `spec-tree:test-evidence-auditor` and `spec-tree:eval-evidence-auditor` when implementation malleability selects their changed evidence surfaces; `spec-tree:adr-auditor` or `spec-tree:pdr-auditor` for each changed decision; `spec-tree:spec-auditor` for each changed spec; `instructions:skill-auditor` for each complete changed skill surface when the instructions plugin is installed; `prose:prose-auditor` for each complete changed prose surface when the prose plugin is installed; and `spec-tree:implementation-auditor` when the code lane and malleability select implementation audit. Supply only the target each Auditor's contract requires, apply `<verification_dispatch_readiness>`, and run every selected artifact Auditor after its deterministic lane passes and before `spec-tree:changes-reviewer`. Handle completed rejected, failing, or unknown artifact-auditor verdicts per `<auditor_verdicts>`; and
...
Local Auditor agents — `spec-tree:test-evidence-auditor`, `spec-tree:eval-evidence-auditor`, `spec-tree:adr-auditor`, `spec-tree:pdr-auditor`, `spec-tree:spec-auditor`, `instructions:skill-auditor`, `prose:prose-auditor`, and `spec-tree:implementation-auditor` — emit structured findings for the slice they inspect. Language-specific audit concerns are composed through the installed `audit-{lang}-{code|tests|architecture}` skills, not through language-specific Auditor agents.
