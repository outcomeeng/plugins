...
description: >-
  ALWAYS invoke this skill to operate the spec-tree plugin's own lifecycle — report its version and manifest-based agent delivery. Invoke it when this plugin's agents are missing from a session. NEVER commit marketplace-delivered agent definitions into a checkout.
