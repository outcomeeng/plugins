...
If a covered link uses a legacy name, record the imperfection and rename the test immediately with `git mv` when the canonical target is unambiguous. Use AskUserQuestion only when the rename changes ownership, scope, cost, risk, or an unresolved product choice. Update the spec link in Step 6.
