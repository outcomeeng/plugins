...
`spx session` operations — including inspection, archive, and release — plus `spx worktree status`, `spx diagnose`, `spx journal` and `spx verification` read, list, and render operations, no-patch Git status, history, and topology, and a skill's read of the `spx/local/` overlay or exclusion mechanism it declares are exempt. Never follow paths from their output into repository content without the marker.
...
An operational continuation — PR inspection, check wait, merge, deploy, release, `spx session` operations, occupancy proof, or dispatching a configured named verifier or reviewer role from recorded exact-commit and path or node coordinates, collecting its result, rendering its returned journal token or projection, and recording its verdict and findings — touches no product content and triggers neither `/understand` nor `/contextualize`. Scope coordinates are operational data only when recorded before dispatch in the exact commit and its `Refs:` trailer, the session store, or sealed journal scope events; deriving a missing coordinate from product content activates the normal gates.

After compaction, rerun the declared deterministic command and observe passing evidence for the exact committed subject before dispatching an agentic verifier or reviewer. A summary or prior-run claim does not re-establish that precondition. A returned token, projection, verdict, finding location, or finding record remains operational while it is recorded or accepted into the conversation-local imperfection ledger. Before rejecting, downgrading, dropping as unbacked, or deferring a finding, or before opening or changing a referenced product artifact, establish a live foundation marker and contextualize every governing node needed for that product judgment or access.
...
- **ALWAYS** confine this context-free authorization to configured named verifier and reviewer roles carrying a managed role-task contract below. It excludes implementation runners such as `spec-tree:applier`, simplifier agents, updater agents, general-purpose workers, explorers, and arbitrary delegated reads.
...
Every managed verifier role-task contract accepts only the repository path and recorded exact-commit, artifact-path, or governing-node coordinates applicable to that role, plus deterministic command output when its gate requires it. The verifier agent session establishes its own live `<SPEC_TREE_FOUNDATION>` marker and contextualized-node set before reading product content, then derives assertion text, eval and producer artifacts, implementation scope, language-scope classification, and owning-plugin classification from those paths. The authoring agent session never preloads or relays those content-bearing values.
...
**Use explicit prompts for audit-agent role tasks.** The `message` field comes from the `multi_agent_v1.spawn_agent` schema. This instruction block owns the prompt content below for required verifier roles. Keep the prompt narrow: repository path, recorded exact-commit, artifact-path, or governing-node coordinates, deterministic verification state when relevant, audit task, and output shape. Do not ask the subagent to edit files.
...
```json
{
  ...
  "arguments": {
    ...
    "message": "Repository: <absolute-repository-path>\nScope: <base>..<head> exact committed changeset scope\nDeterministic verification already run: <commands and observed results, or advisory state>\nTask: Establish your own foundation and node context, derive the governed implementation paths and language partitions from the committed scope, then run the implementation audit through spx verification run with the implementation-auditor run-driver identity. Return the run token and rendered projection; the complete blocked SPX diagnostic with run token or not-started, exact command, payload source, payload key, exit code, and stderr; or the complete pre-run skill-load diagnostic with run token not-started, required skill spec-tree:audit-implementation, and the exact load or availability failure."
  }
}
```
...
**Implementation audit subject.** A gate-eligible implementation audit reads an exact locally committed subject and runs only after applicable deterministic verification passes on that subject. The verifier derives the governed implementation paths from the committed scope after establishing its own foundation and node context. An explicit advisory implementation audit uses a recorded exact committed or working-tree scope; its result supplies no reusable gate evidence.
...
```json
{
  ...
  "arguments": {
    ...
    "message": "Repository: <absolute-repository-path>\nScope: <base>..<head> exact committed changeset scope\nGoverning node: <full spx/... node path>\nSpec file: <full spx/... spec path>\nTask: Establish your own foundation and node context, derive the changed assertions and linked test files from the spec and exact committed scope, then audit whether the test evidence proves those assertions without weakening the selected verification type or test assertion type. Return only the audit-tests JSON verdict, with schema_version 1, skill audit-tests, overall APPROVED or REJECTED, rows, and metadata. Do not add prose outside the JSON object."
  }
}
```
...
```json
{
  ...
  "arguments": {
    ...
    "message": "Repository: <absolute-repository-path>\nScope: <base>..<head> exact committed changeset scope\nGoverning node: <full spx/... node path>\nSpec file: <full spx/... spec path>\nTask: Establish your own foundation and node context, derive the changed [eval] assertions, eval artifacts, and real producer artifacts from the spec and exact committed scope, then audit whether the eval evidence proves those assertions without replacing the real producer with a prompt-only simulation. Return the JSON verdict specified by audit-eval-evidence, with overall PASS, FAIL, or UNKNOWN and row findings for failed evidence properties. Do not add prose outside the JSON object."
  }
}
```
...
```json
{
  ...
  "arguments": {
    ...
    "message": "Repository: <absolute-repository-path>\nDecision file: <full spx/.../*.adr.md path>\nGoverning node: <full spx/... node path>\nAudit scope: <exact committed changeset or artifact scope>\nTask: Establish your own foundation and node context, derive the language-scope classification from the audit coordinates, then audit the ADR for decision structure, atemporal voice, tag validity, and every language-specific architecture concern required by that classification. Return only the structured JSON verdict specified by audit-adr, with no prose outside the JSON object."
  }
}
```
...
```json
{
  ...
  "arguments": {
    ...
    "message": "Repository: <absolute-repository-path>\nDecision file: <full spx/.../*.pdr.md path>\nGoverning node: <full spx/... node path>\nAudit scope: <exact committed changeset or artifact scope>\nTask: Establish your own foundation and node context, then audit the PDR for product-decision structure, atemporal voice, tag validity, downstream alignment, and evidence quality. Return only the audit-pdr JSON verdict, with schema_version 1, skill audit-pdr, overall APPROVED or REJECTED, the content-classification, property-quality, tag-validity, atemporal-voice, and consistency rows, and metadata. Do not add prose outside the JSON object."
  }
}
```

Use this shape for a changeset-coherence audit:

```json
{
  "tool": "multi_agent_v1.spawn_agent",
  "arguments": {
    "agent_type": "spec-tree_changeset-coherence-auditor",
    "message": "Repository: <absolute-repository-path>\nScope: <base>..<head> exact committed changeset scope\nTask: Establish your own foundation and node context, derive the governed nodes from the committed scope, and audit whether the changeset is one coherent review unit. Return only the structured JSON verdict specified by audit-changeset-coherence, with no prose outside the JSON object."
  }
}
```
...
- `changeset-coherence-auditor`: repository path, exact committed `<base>..<head>` scope, and the task to derive the governed nodes and judge whether the changeset is one coherent review unit. Its final message MUST be the structured JSON verdict from `spec-tree:audit-changeset-coherence` with no prose outside it.
...
- `spec-tree_test-evidence-auditor`: repository path, exact committed scope, governing-node path, exact spec path, and the task to derive the changed assertions and linked test files before auditing coupling, falsifiability, alignment, and coverage without weakening the evidence type. Its final message MUST be the `spec-tree:audit-tests` JSON verdict with `schema_version: 1`, `skill: "audit-tests"`, `overall: "APPROVED" | "REJECTED"`, `rows`, and `metadata`, with no prose outside the JSON object. Treat `overall` as authoritative. Malformed JSON, a missing required field, an unexpected `skill`, or an `overall` value outside that vocabulary blocks the gate.
- `spec-tree_eval-evidence-auditor`: repository path, exact committed scope, governing-node path, exact spec path, and the task to derive the changed `[eval]` assertions, eval artifacts, and real producer artifacts before auditing real-producer evidence. Its final message MUST be the audit-eval-evidence JSON verdict with overall `PASS`, `FAIL`, or `UNKNOWN` and no prose outside the JSON object.
- `spec-tree_spec-auditor`: repository path, exact committed scope, full node path, and the task to audit assertion quality, evidence tags, atemporal voice, decision alignment, and structure. Its final message MUST be the `spec-tree:audit-specs` JSON verdict with `schema_version: 1`, `skill: "audit-specs"`, `overall: "APPROVED" | "REJECTED"`, the `section-structure`, `atemporal-voice`, and `tag-fitness` rows, and `metadata`, with no prose outside the JSON object. Treat `overall` as authoritative. Malformed JSON, a missing required field or row, an unexpected `skill`, or an `overall` value outside that vocabulary blocks the gate.
...
- `spec-tree_changeset-coherence-auditor`: repository path, exact committed `<base>..<head>` scope, and the task to derive the governed nodes and judge whether the changeset is one coherent review unit. Its final message MUST be the structured JSON verdict from `spec-tree:audit-changeset-coherence` with no prose outside it.
- A craft plugin's `{plugin}-auditor` — the artifact-type auditor named for the plugin that governs its artifact type, such as a prose plugin's `prose-auditor` — when that configured role is installed: repository path, full artifact paths, and the audit task that skill declares. The verifier consults its owning audit skill and derives every required classification after loading product context. When that skill declares a structured verdict, the final message MUST be that verdict with no prose outside it; treat `overall` as authoritative, and a malformed verdict, a missing required field, a verdict that does not identify itself as the owning plugin's audit skill, or an `overall` value outside the declared vocabulary blocks the gate. When it declares a sealed-run journal token, the final message MUST be that raw token with no prose outside it; render the sealed run through `spx journal render --type <the skill's declared run type> --run <run-token>` and treat the run's terminal status as authoritative, with only `approved` passing, and a blocked, unsealed, or unrenderable run blocking the gate. A final message that is neither the declared verdict nor the declared token blocks the gate.

- `instructions_subagent-auditor`, when that configured role is installed: repository path, exactly one changed custom agent configuration path in the active agent harness's native format, governing nodes when known, deterministic verification state, and the subagent-authoring audit task. Multiple changed configurations require separate `instructions_subagent-auditor` dispatches, one per path; acquire their handles sequentially and let their role tasks run concurrently. Each final message MUST be the `instructions:audit-subagent` JSON verdict with `schema_version: 1`, `skill: "audit-subagent"`, `overall: "APPROVED" | "REJECTED"`, and the `critical-issues`, `recommendations`, `strengths`, and `quick-fixes` rows. Treat `overall` as authoritative. Malformed JSON, a missing required field or row, an unexpected `skill`, or an `overall` value outside that vocabulary blocks the gate.
...
| User Says...          | Skill (composed)           | Composing agent                    |
| --------------------- | -------------------------- | ---------------------------------- |
| "Audit this code"     | `/audit-rust-code`         | `spec-tree_implementation-auditor` |
| "Audit unsafe Rust"   | `/audit-rust-code`         | `spec-tree_implementation-auditor` |
| "Audit ADRs for Rust" | `/audit-rust-architecture` | `spec-tree_adr-auditor`            |
| "Audit these tests"   | `/audit-rust-tests`        | `spec-tree_test-evidence-auditor`  |
