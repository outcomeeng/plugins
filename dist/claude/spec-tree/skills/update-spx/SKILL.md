...
4. **Act on the status.**
   ...
   - **`stale`** — re-render in place, scoped to the enabled languages:
     ...
     ```bash
     python3 "${CLAUDE_SKILL_DIR}/scripts/update_spx.py" --template "${CLAUDE_SKILL_DIR}/../understanding/templates/spx-claude.md" --product <guide> --languages <comma-separated-languages> --write
     ```
