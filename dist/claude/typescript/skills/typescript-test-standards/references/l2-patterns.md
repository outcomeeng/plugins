...
```typescript
import { withHugoHarness } from "@testing/harnesses/hugo";
...
describe("Hugo build", () => {
  it("builds site without error", async () => {
    await withHugoHarness(async (hugo) => {
      await expect(hugo.build()).resolves.toMatchObject({ exitCode: 0 });
    });
  });
  ...
  it("creates index.html in output", async () => {
    await withHugoHarness(async (hugo) => {
      ...
      expect(existsSync(join(hugo.siteDir, "public/index.html"))).toBe(true);
    });
  });
});
```
