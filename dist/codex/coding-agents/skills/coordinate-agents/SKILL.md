...
| Production request          | `Dependency production request`    | the checked need, then `returnPane=<requester-pane>` and `handbackCommand=<exact-command>` | `Send the handback command when the result is written.`                                                     |
