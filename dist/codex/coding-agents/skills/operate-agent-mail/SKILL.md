...
- ALWAYS execute the bundled script through `${SKILL_DIR}`; never import it from another filesystem location or manufacture a path outside this skill directory, because the skill loader substitutes that expression into this body before the command runs, so only this spelling reaches the shell as this skill's real directory; it is no shell variable, and copying it into an agent definition or exporting it yields an empty prefix rather than an error.
