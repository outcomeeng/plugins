...
<objective>
A durable pre-restart allowlist in which every prepared native session carries one exact identity and one Prowl pane correlation, with no duplicate or unauthorized candidate. An idempotent post-restart recovery in which every non-controller session is judged from its own read context — one separately submitted continuation instruction where the restart destroyed something for that one recipient, and a recorded judged-intact identity with no delivery for every other — and no unprepared agent is running.
</objective>
...
8. Launch one planned native session at a time. Apply `<dialog_guard>` to the exact pane immediately before its send. Invoke `/operate-prowl` once for `send` using its complete `paneId`, exact source-owned native command, and immediate-return mode, then require checked transport evidence that a trailing `Enter` was sent. The command contains only the prepared agent type, exact resume locator or session identity, and applicable native home; NEVER append reassessment prose or replace it with a recency selector. Serialize Codex launches sharing one `nativeHome`: before every such launch after the first, invoke `/operate-prowl` `read` once in stable-screen mode against the pane launched just before it on that same home, and require that read to show its Codex TUI accepting input. That one bounded read is the entire wait — `<constraints>` forbids repeating it into a poll — so a read that does not show an input-ready TUI stops the remaining launches on that one `nativeHome` and reports that pane blocked with its complete identity, leaving every other `nativeHome` unaffected. Serializing this way prevents the shared SQLite initialization lock.
