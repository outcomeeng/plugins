...
allowed-tools: Read, Write, Skill, Bash(printf:*), Bash(python3 "${SKILL_DIR}/scripts/recover_agents.py":*)
---

<objective>
A durable pre-restart allowlist in which every prepared native session carries one exact identity and one Prowl pane correlation. An idempotent post-restart recovery in which the prepared set is running under its original identities and no unprepared agent is.
</objective>
...
8. Launch one planned native session at a time. Apply `<dialog_guard>` to the exact pane immediately before its send. Invoke `/operate-prowl` once for `send` using its complete `paneId`, exact source-owned native command, and immediate-return mode, then require checked transport evidence that a trailing `Enter` was sent. The command contains only the prepared agent type, exact resume locator or session identity, and applicable native home; NEVER append reassessment prose or replace it with a recency selector. Serialize Codex launches sharing one `nativeHome`: before every such launch after the first, invoke `/operate-prowl` `read` once in stable-screen mode against the pane launched just before it on that same home, and require that read to show the Codex composer prompt with no initialization or busy indicator above it — the same rendered-prompt state step 10 requires, read once here rather than after the batch. That one bounded read is the entire wait — `<constraints>` forbids repeating it into a poll — so a read that does not show an input-ready TUI stops the remaining launches on that one `nativeHome` and reports that pane blocked with its complete identity, leaving every other `nativeHome` unaffected. Serializing this way prevents the shared SQLite initialization lock.
...
printf '%s\n' '{"prepared":{},"items":[],"agents":[],"controllerPaneId":null}' | python3 "${SKILL_DIR}/scripts/recover_agents.py" activate
printf '%s\n' '{"plan":{},"activationResults":[]}' | python3 "${SKILL_DIR}/scripts/recover_agents.py" bind
printf '%s\n' '{"prepared":{},"bindings":[],"items":[],"agents":[],"controllerPaneId":null}' | python3 "${SKILL_DIR}/scripts/recover_agents.py" recover
...
printf '%s\n' '{"prepared":{},"bindings":[],"verification":{},"paneReadResults":[],"restored":[{"sessionId":"<session>","text":"<what this restart destroyed for this recipient>"}]}' | python3 "${SKILL_DIR}/scripts/recover_agents.py" reassess
```
...
- No two launch sends sharing one `nativeHome` occur without one stable-screen read of the earlier pane between them, so the launch record for each home alternates send and read rather than carrying consecutive sends.
...
- Every non-controller session is judged separately from its own barrier read and settled either as one delivered reassessment instruction — sent only where the restart destroyed something for that one recipient, restoring that unsatisfied operator work or its pending interaction — or as a judged-intact identity carrying no delivery, and the manifest — updated this pass wherever any delivery or judged-intact identity is newly settled, carried forward unmodified where every candidate was already reassessed — records both so a repeated recovery revisits neither.
