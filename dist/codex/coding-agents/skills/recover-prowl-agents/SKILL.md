...
- Every non-controller session is judged separately from its own barrier read and settled either as one delivered reassessment instruction — sent only where the restart destroyed something for that one recipient, restoring that unsatisfied operator work or its pending interaction — or as a judged-intact identity carrying no delivery, and the updated manifest records both so a repeated recovery revisits neither.
