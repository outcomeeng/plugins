...
3. Add an unoccupied candidate only when its complete UUID and visible evidence make the stopped coding-agent session obvious, or when the operator explicitly names that pane. An ordinary shell, conflicting evidence, or uncertain intent remains stopped. Present ambiguous panes through `request_user_input` with complete pane and worktree identities; never guess.
...
```bash
printf '%s\n' '{"items":[],"agents":[]}' | python3 "${SKILL_DIR}/scripts/recover_agents.py" recover --pane <complete-pane-uuid>
```
