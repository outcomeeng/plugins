...
<objective>
The contribute plugin's resolved version and agent-delivery state reported or reconciled in the scope that carries its skills.
</objective>
...
```text
write: /Users/dev/.codex/agents/contribute_reviewer.toml
prune: /Users/dev/.codex/agents/contribute_verifier.toml
collision: /Users/dev/.codex/agents/contribute_notes.toml (unrecorded)
```

`write` names a destination whose bytes differ from the shipped definition, `prune` a recorded `contribute` destination the plugin no longer ships, and `collision` a present file the ownership record does not authorize, with its cause: `symlink`, `unrecorded`, `owned by <plugin>`, `not a regular file`, or `digest mismatch`. A `scope-split directed-removal:` or `scope-split collision:` line names a checkout copy under `.codex/agents/` — matched by shipped bytes, by the `contribute` filename prefix, or by a `skills.config` entry naming a `contribute:` skill — and stops mutation. After the mutating form succeeds, the ownership record carries one entry per written destination:

```json
{ "destination": "agents/contribute_reviewer.toml", "plugin": "contribute", "digest": "<sha256 of the installed bytes>" }
```
