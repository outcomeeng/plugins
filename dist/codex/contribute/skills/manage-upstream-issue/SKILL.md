...
With a question to answer, draft per `<reply_shape>` and review — the prose plugin's `prose_prose-auditor` thin agent where installed, `/contribution-standards` `<outward_text>` unassisted where not, stated as such.
