...
A skill body references a specific file with the `@` prefix (`@path/to/file`), injecting its content — the same affordance a command had. Use it for a file the skill always reads; combine it with an argument (`@$target`) for a caller-named file. Prefer a `${SKILL_DIR}` path for skill-bundled files (see `references/runtime-variables.md`); `@` is for product files in the consumer's tree.
