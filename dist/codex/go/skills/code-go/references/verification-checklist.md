...
If the repository publishes stricter wrapper commands in `AGENTS.md` or `README.md`, use those instead of the fallback commands above.
</tool_commands>
...
- each `Test*` body carries its own comparison and failure call, never a bare `t.Helper()` call that both acts and judges — the shape `${SKILL_DIR}/references/test-patterns.md` `<anti_patterns>` names first
