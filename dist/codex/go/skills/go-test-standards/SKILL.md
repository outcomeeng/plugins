...
<level_1_patterns>
Use Level 1 when the governed logic can run with the Go stdlib, normal developer tooling, a product binary built in-cycle, and temporary local fixtures. The pure-function, dependency-seam, and tempdir examples are worked in `${SKILL_DIR}/references/level-1.md`: the harness owns the temporary product and `t.TempDir()` owns cleanup, the fixture arrives by path, and the `Test*` function calls the governed function and asserts.
</level_1_patterns>
...
- `${SKILL_DIR}/references/level-1.md` - the worked pure-function, dependency-seam, recorder, tempdir, property, and compile-time examples
- `${SKILL_DIR}/references/level-2.md` - the worked installed-binary, database-adapter, and containerized collaborator examples
