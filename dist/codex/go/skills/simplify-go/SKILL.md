...
name: simplify-go
description: >-
  ALWAYS invoke this skill when simplifying Go implementation while preserving behavior.
argument-hint: "<HEAD|base...head>"
allowed-tools: Read, Glob, Grep, Edit, Bash(git status:*), Bash(git rev-parse:*), Bash(git diff:*)
---
...
- MUST preserve context propagation, cancellation paths, goroutine ownership, and channel synchronization semantics.
...
1. Require `$ARGUMENTS` to be `HEAD` or an explicit committed three-dot range. For absent or invalid input, return `blocked` before mutation. Read the product's AGENTS.md and invoke `/understand` when its foundation marker is absent.
...
3. Select changed, present Go implementation files, excluding generated extents and test/evidence files using the product's declarations and `/go-test-standards`. Derive their governing nodes through spec-linked evidence and invoke `/contextualize` on their lowest common ancestor before reading implementation. After context loading, require the same full `HEAD` and a clean worktree again; otherwise return `blocked` with the changed subject. If no implementation qualifies, return `unchanged` with the empty scope. Missing governance returns `blocked`.
4. Inspect the selected implementation and the evidence covering its behavior. Apply `/go-test-standards` to coverage and test quality. Missing or inadequate behavioral coverage returns `blocked` with the affected file or function and evidence gap; do not create tests to bypass this condition.
...
6. Run the product's required baseline tests and record actual exit codes. A failed or unavailable baseline returns `blocked` without edits. Identify only changes whose behavioral and concurrency equivalence can be explained from the code and evidence; when none improves clarity safely, return `unchanged` with the reason.
7. Invoke `go:code-go` with the governing node, selected files, and the behavior-preserving simplification requirement. Follow its complete discovery, standards, and validation contract while preserving the evidence. Retain the exact patch owned by this invocation. Require the same tests afterward and every format, vet, lint, and validation check required by `/code-go`, using product commands and preserving process exit codes. Collect those results once; never mask a failing process through output truncation or a pipeline.
8. Compare the resulting diff with the recorded scope and baseline. Confirm tests, evidence, seams, and concurrency contracts remain unchanged. Follow `<recovery>` on any failed check or uncertain equivalence, then return `<result>`.

</workflow>

<coding_result>

When invoking `go:code-go`, request its completion report with changed paths, the behavior preserved by each edit, the evidence used, executed commands and exit codes, and unresolved constraints. Execute that composed workflow in this session.
...
- Every retained edit belongs to the resolved Go implementation scope and improves clarity without changing behavior, error contracts, seams, or concurrency ownership.
- Unchanged behavioral tests pass before and after edits, and the complete `/code-go` validation requirements pass.
