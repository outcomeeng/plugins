...
Read `/skill-standards` and `/agent-prompt-standards`. Read `spx/local/skills.md` when the target repository provides it. Read `${SKILL_DIR}/templates/router-skill.md` before rewriting the target.
...
Rewrite `SKILL.md` from the router template with `<objective>`, `<essential_principles>`, `<intake>`, `<routing>`, indexes, and `<success_criteria>`. Create each workflow with `<required_reading>`, `<process>`, named `<step>` elements, and `<success_criteria>`. Use pure XML structure and exact `${SKILL_DIR}` paths for bundled files.
