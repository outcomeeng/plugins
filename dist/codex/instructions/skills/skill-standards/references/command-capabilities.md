...
- **Read-only audits** — an `audit-*` skill grants read capabilities and `Skill` when it composes another skill. Add only the specific read-only Bash commands its workflow requires; never grant `Write`/`Edit`.
