...
Soundness audit for `unsafe` blocks and FFI boundaries — the undefined behavior, invariant violations, and FFI contract breaks the compiler cannot catch. Claude runs this pass as part of `/audit-rust-code` whenever the scope contains an `unsafe` block, `unsafe fn`, `unsafe impl`, or `extern "C"` boundary; a scope with no such sites skips it.
