...
name: simplify-rust
description: >-
  ALWAYS invoke this skill when simplifying Rust implementation while preserving behavior.
...
<objective>
Simpler Rust implementation with unchanged behavior and ownership semantics, accompanied by a scope and verification report.
</objective>

<constraints>

- MUST preserve public behavior, dependency-injection seams, error wrapping, and typed errors.
- MUST preserve ownership transfers, lifetime and trait bounds, exhaustive handling, and unsafe/FFI invariants; never add clones merely to simplify borrowing.
...
3. Select changed, present Rust implementation files, excluding generated extents and test/evidence files using the product's declarations and `/rust-test-standards`. Derive their governing nodes through spec-linked evidence and invoke `/contextualize` on their lowest common ancestor before reading implementation. After context loading, require the same full `HEAD` and a clean worktree again; otherwise return `blocked` with the changed subject. If no implementation qualifies, return `unchanged` with the empty scope. Missing governance returns `blocked`.
4. Inspect the selected implementation and the evidence covering its behavior. Apply `/rust-test-standards` to coverage and test quality. Missing or inadequate behavioral coverage returns `blocked` with the affected file or function and evidence gap; do not create tests to bypass this condition.
...
7. Invoke `rust:code-rust` with the governing node, selected files, and the behavior-preserving simplification requirement. Follow its complete discovery, standards, and validation contract while preserving the evidence. Retain the exact patch owned by this invocation. Require the same tests afterward and every format, lint, and validation check required by `/code-rust`, using product commands and preserving process exit codes. Collect those results once; never mask a failing process through output truncation or a pipeline.
8. Compare the resulting diff with the recorded scope and baseline. Confirm tests, evidence, seams, and ownership contracts remain unchanged. Follow `<recovery>` on any failed check or uncertain equivalence, then return `<result>`.

</workflow>

<coding_result>

When invoking `rust:code-rust`, request its completion report with changed paths, the behavior preserved by each edit, the evidence used, executed commands and exit codes, and unresolved constraints. Execute that composed workflow in this session.
...
- Every retained edit belongs to the resolved Rust implementation scope and improves clarity without changing behavior, error contracts, seams, or ownership semantics.
- Unchanged behavioral tests pass before and after edits, and the complete `/code-rust` validation requirements pass.
