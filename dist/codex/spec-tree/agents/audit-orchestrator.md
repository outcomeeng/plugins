...
- Invoke nothing in the `/auditing` skill's `scripts/` directory by a path of your own. The skill resolves `${SKILL_DIR}` and runs every CLI subcommand from inside its own prose; never construct a path expression here.
