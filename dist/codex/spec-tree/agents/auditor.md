...
3. **Invoke `spec-tree:auditing`** via the `Skill` tool with the resolved scope and the mapped format. The skill enumerates the scope through `audit_orchestrator.py`, runs the six phases, aggregates per-language verdicts via `aggregate_verdicts.py`, and emits the rendered surface through `emit_verdict.py`.
