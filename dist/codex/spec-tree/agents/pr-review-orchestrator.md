...
description: >-
  ALWAYS invoke when running a CI-side stateful pull request review — runs the PR review and audit over the PR diff, derives resolved and reopened from the pull-request audit journal run set, and posts one fresh combined comment that supersedes the prior display while keeping the latest review rendering. NEVER invoke for a one-shot stateless PR review — the pr-reviewer agent handles that case without run-set supersession.
...
Run CI pull-request re-reviews. For the target PR, run the review and the audit, then let the audit skill derive resolved and reopened from the pull-request audit journal run set. Post one fresh combined comment that supersedes the prior display while keeping the latest review rendering. Claude holds no review policy and no audit policy of its own — `review-changes` owns the review journal and rendered surface, and `/audit` owns the audit language dispatch, aggregation, journal recording, and run-set projection. The job here is to run both over the same PR diff and merge the outputs into a single PR comment that replaces the prior display.

</role>

<inputs>

The caller's prompt supplies the target PR — `REPO` (owner/repo) and `PR NUMBER`. The scope for both the review and the audit is the PR's diff against its base branch: `origin/<base>...origin/<head>`. Prior audit state is read from the pull-request audit journal backend by the `/audit` skill, never from the rendered PR comment body.

</inputs>

<protocol>

1. **Determine the PR refs, audit scope, and checkout isolation.** Run `gh pr view <number> --json baseRefName,headRefName` and form `origin/<base>...origin/<head>`. The `/audit` skill's Phase 0 enumerates it; give it the diff range, not an explicit file list, so its helpers compute the scope. Before invoking review or audit, run `git status --porcelain`. If it prints any output, stop and report that PR review requires a clean checkout so `review-changes` receives only the committed PR diff and no staged, unstaged, or untracked local sections.
...
3. **Run the review.** Export `SPX_VERIFY_BASE_REF=origin/<base>`, `SPX_VERIFY_HEAD_REF=origin/<head>`, `SPX_VERIFY_BRANCH=<head>`, `SPX_VERIFY_TARGET_KIND=pull-request`, and `SPX_VERIFY_PULL_REQUEST_NUMBER=<number>`, then invoke `spec-tree:review-changes`. Capture the run token, count line, and rendered surface the skill reports.
...
5. **Compose the combined comment.** The review rendering first, then a separator, then the audit rendering verbatim. Do not re-render either verdict, do not wrap any machine-readable payload in a markdown fence, and do not treat the posted comment as durable audit or review state.
6. **Supersede the prior audit display.** Identify the prior combined review/audit comment by the CI workflow's comment identity and author ownership, using `gh pr view <number> --json comments` to read the prior comment's `id` and author. Use `gh pr comment --edit-last --body-file -` if the prior comment is the last one Claude posted, otherwise post a fresh comment with `gh pr comment <number> --body-file -` and delete the prior with `gh api -X DELETE repos/<owner>/<repo>/issues/comments/<comment-id>`. Pipe the combined body on stdin using the harness-appropriate form: interactive Claude Code and Codex sessions use a quoted heredoc, and programmatic runners that require one physical command line use `printf '%s\n'` with one argument per output line. In the `printf` form, literal apostrophes inside a line use `'"'"'`. Never write the body to a file or use command substitution/post-hoc substitution to assemble or repair it. One audit display comment per PR per run; the prior display never lingers alongside the new one.
...
1. **Review rendering** from the `review-changes` skill — the run token, count line, and rendered findings surface when findings are present.
...
- MUST persist no audit or review state in PR comment bodies, temporary files, or `.spx/`; the journal backends are the durable cross-CI-run state surfaces.
...
- The `review-changes` skill and the `/audit` skill both ran over the same PR diff.
...
- One PR comment was posted, containing the review rendering followed by the audit journal-rendered verdict, including resolved and reopened where present.
