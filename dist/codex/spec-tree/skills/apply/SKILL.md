...
Invoke `/verification-run-journal-standards`, then inspect the returned token through its `render_review_run.py` helper exactly as that skill directs. Treat the helper output as the inspection projection of the sealed journal prefix; the sealed prefix remains the only review result. Read the rendered terminal status, full head/base identity, scope coverage, blocking/debt counts, and findings before deciding whether Step 9 converged.
...
- Before declaring the flow complete: if the change touches anything beyond the target node, require a raw Step 9 review run token collected through the typed wait and a rendered sealed projection from `/verification-run-journal-standards`. If either is absent, blocked, or reports unaddressed valid findings, stop — invoke or repair Step 9.
