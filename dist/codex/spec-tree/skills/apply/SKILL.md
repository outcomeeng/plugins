...
Two independent conditions run here, one per selector in `<evidence_auditor_gate>`.

The numbered evidence gate runs `<evidence_auditor_gate>` when the least malleable touched node is `implementation` and the stabilized diff creates or modifies a `[test]` assertion, linked test file, imported test-infrastructure artifact, `[eval]` assertion, eval artifact, or producer artifact for eval-backed evidence.

The artifact-auditor gate runs for every selected skill, prose, spec, or decision row: dispatch the exact artifact Auditor named by `<lane_table>` over the whole changeset after that row's deterministic lane passes. It is selected by Output kind alone, so it runs whether or not malleability selected the numbered evidence gate.
