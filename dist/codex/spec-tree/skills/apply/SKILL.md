...
One narrow exception applies only to a started-run implementation-audit `BLOCKED` result that is unusable because it carries none of the three command-evidence shapes Step 8 defines. After that unusable result is reported, explicit word from the operator or the orchestrating agent session standing in for the operator authorizes one fresh dispatch as a new launch. Without that word, make no fresh dispatch. This exception does not retry the failed invocation, apply to another launch or result failure, or enter the completed-verdict repair loop.
...
Dispatch `spec-tree_adr-auditor` with only the ADR path. The invoked `audit-adr` discovers its governing node, committed changeset, and implementation-language partitions, then composes each applicable `audit-{lang}-architecture` concern inside its isolated agent session. Require its structured JSON verdict.
...
Dispatch `spec-tree_implementation-auditor` with only
...
- Before considering implementation complete: inspect the Step 8 rendered projection. If `terminalStatus` is absent or differs from `approved`, stop — invoke or repair Step 8. A started-run implementation-audit result that is unusable for missing all three command-evidence shapes permits one fresh dispatch only after the explicit authorization in `<launch_contract>`; every other failed launch or unusable result permits none.
