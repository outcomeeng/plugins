...
Invoke `/inspect-review-run` with the returned token. Treat its output as the inspection projection of the sealed journal prefix; the sealed prefix remains the only review result. Read the rendered terminal status, full head/base identity, scope coverage, blocking/debt counts, and findings before deciding whether Step 9 converged.
...
- Before declaring the flow complete: if the change touches anything beyond the target node, require a raw Step 9 review run token collected through the typed wait and a rendered sealed projection from `/inspect-review-run`. If either is absent, blocked, or reports unaddressed valid findings, stop — invoke or repair Step 9.
