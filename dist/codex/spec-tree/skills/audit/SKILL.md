...
2. **Canonical verdict shape.** Every verdict conforms to the schema in `${SKILL_DIR}/scripts/verdict.py`. The orchestrator's wrapper has one row (`determinism-contract`); per-language children have their own rows owned by the dispatched skill. Row names are never invented inline.
...
4. **Compute the scope hash.** Invoke `compute_scope_hash` from `${SKILL_DIR}/scripts/audit_orchestrator.py`. Pass the frozen scope as `list[tuple[path, content]]`; the function returns a 12-character hex string. The hash identifies this exact scope and travels in the wrapper verdict's `metadata.scopeHash`.
...
7. **Compute the config digest.** Invoke `config-digest` from `${SKILL_DIR}/scripts/audit_orchestrator.py` over a stable newline-separated payload naming the repo-local overlays read and the language partitions. The digest identifies the run configuration and travels in the wrapper verdict's `metadata.configDigest`; never reuse the scope hash for this field.
...
BRANCH_NAME=$(python3 "${SKILL_DIR}/scripts/audit_orchestrator.py" current-branch)
BRANCH_SLUG=$(python3 "${SKILL_DIR}/scripts/audit_orchestrator.py" branch-slug \
  --branch "$BRANCH_NAME")
BASE_REF=$(python3 "${SKILL_DIR}/scripts/audit_orchestrator.py" base-ref)
BASE_REMOTE_REF=$(python3 "${SKILL_DIR}/scripts/audit_orchestrator.py" remote-tracking-ref \
  --base "$BASE_REF")
HEAD_SHA=$(python3 "${SKILL_DIR}/scripts/audit_orchestrator.py" commit-oid \
  --ref HEAD)
BASE_SHA=$(python3 "${SKILL_DIR}/scripts/audit_orchestrator.py" commit-oid \
  --ref "$BASE_REMOTE_REF")
SCOPE_HASH=$(printf '%s\n' <frozen-scope-paths> \
  | python3 "${SKILL_DIR}/scripts/audit_orchestrator.py" scope-hash)
SCOPE_JSON=$(printf '%s\n' <frozen-scope-paths> \
...
AUDIT_METADATA=$(python3 "${SKILL_DIR}/scripts/journal_emit.py" metadata \
  --target <scope-target> \
...
python3 "${SKILL_DIR}/scripts/journal_emit.py" scope-advanced \
  --unit "<language>" \
  --now "$PARTITION_COMPLETED_AT" \
  | while IFS= read -r EVENT; do
      printf '%s' "$EVENT" | spx journal append --type audit --run "$RUN_TOKEN" >/dev/null
    done
python3 "${SKILL_DIR}/scripts/journal_emit.py" findings-reported \
  --now "$PARTITION_COMPLETED_AT" \
  < "$CHILD_JSON" \
  | while IFS= read -r EVENT; do
      printf '%s' "$EVENT" | spx journal append --type audit --run "$RUN_TOKEN" >/dev/null
