...
Read the evidence model before auditing: `references/evidence-model.md`
...
If codebase imports exist, classify using the coupling taxonomy in `references/evidence-model.md`:
...
**Exception**: Test doubles used under the 7 legitimate exception cases from the `/testing` methodology are not "coupling severed." The auditor must identify which exception applies and verify the double type matches. See the exception cross-reference in `references/evidence-model.md`.
