...
Use skill `spec-tree:change-standards`. Invoke it with `Sliced`; it loads the common record contract and the cumulative Sliced Definition of Ready only. Read `${SKILL_DIR}/templates/change.md` and the router's store configuration.
...
2. Test whether the framed work forms one coherent, independently integrable unit in one repository. A required split or coalescence routes to the lineage workflow owning that operation; never hide multiple units inside Activities.
