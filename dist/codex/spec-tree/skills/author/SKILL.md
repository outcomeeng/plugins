...
- Apply the forcing question from `${SKILL_DIR}/../understand/references/node-types.md`: try to write it as an enabler first. Why can't this be PROVIDES X SO THAT Y CAN Z? What is uncertain about which output achieves the goal?
...
Use `request_user_input` for genuine gaps. Do not ask about information already provided in the conversation.
...
- [ ] For outcomes: verify the forcing question from step 4 was answered — are the assertions a bet (majority could be swapped for different ones achieving the same goal)? If not, it should be an enabler (see `${SKILL_DIR}/../understand/references/node-types.md`)
...
Write the file. If `spx/AGENTS.md` doesn't exist, note that one should be created as a product guide.
