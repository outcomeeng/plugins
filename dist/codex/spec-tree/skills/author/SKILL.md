...
allowed-tools: Read, Glob, Grep, Write, Edit, Skill, request_user_input, Bash(spx validation markdown:*), Bash(spx spec status:*)
---
...
How to avoid: before authoring a second decision record at the same directory level, ask whether it can be a section inside the first one, or a product-level compliance rule. Closely-related architectural choices (how we package, how we build, how we handle panics, how we log) are one ADR. Product-level guarantees that constrain every node are compliance rules in the product spec, not separate PDRs.

This failure is about *minting* a record, and its scope stops there. It fires when a universal guarantee's only home would be a new decision record authored for it alone; the remedy is the product spec's compliance section, which is where such a guarantee belongs when nothing else owns it. It does not fire on a universal rule that already lives inside an existing decision record whose subject and rationale it shares — that rule is one part of a record earning its place, not a record multiplied. Reading this failure as a placement rule for every universal rule inverts it: relocating such a rule to the product spec separates it from the rationale that justifies it, and the product spec has no section to hold that reasoning. Before citing this failure against an existing record, establish that a record was minted; a rule inside one that predates it is outside the failure's reach.

Keep indices tight (under 55 in small or pre-commit trees) and let them spread only when nodes actually multiply. The spec tree's structure reflects the scope that exists, not the scope that might exist.
