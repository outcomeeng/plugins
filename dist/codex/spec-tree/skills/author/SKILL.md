...
4. If the implementation doesn't exist yet: add the node path to `spx/EXCLUDE`. The `spx` CLI skips excluded nodes when running `spx test passing`. Read `${SKILL_DIR}/../understand/references/excluded-nodes.md` for the convention.
