...
allowed-tools: Read, Glob, Grep, Write, Edit, Skill, Bash(spx validation markdown:*), Bash(spx spec status:*), request_user_input
---
...
Templates and examples are owned and listed by `/understand`:

- `/understand` template `product-name.product.md`
- `/understand` template `decision-name.adr.md`
- `/understand` template `decision-name.pdr.md`
...
If unclear which type, apply the definitions from the live `/understand` `<enabler>` and `<outcome>` sections:
...
- Apply the forcing question from the live `/understand` `<outcome>` section: write it as an enabler first. Why can't this be PROVIDES X SO THAT Y CAN Z? What is uncertain about which output achieves the goal?
...
**Voice rules** (from the live `/understand` `<atemporal_voice>`):
...
- Before drafting each assertion, invoke `/test` with the assertion text and use the verification type, assertion type when applicable, and evidence link it returns — authoring never selects or substitutes those values
- Every assertion carries the exact evidence link returned by `/test`; an evidence target does not need to exist yet because the link is the contract for what `/test` will create
...
- [ ] Nesting rules respected: outcomes CANNOT be children of enablers (see the live `/understand` `<nesting_rules>`)
- [ ] For outcomes: verify the forcing question from step 4 was answered — are the assertions a bet (majority could be swapped for different ones achieving the same goal)? If not, it is an enabler (see the live `/understand` `<outcome>`)
...
- [ ] Every assertion carries the exact evidence link and type selection returned by `/test` (targets don't need to exist yet)
- [ ] Authoring made no independent verification-type or assertion-type selection
- [ ] ADR/PDR rules sit under `## Verification` in MUST/NEVER format and use the subsection, type, and evidence link returned by `/test`
- [ ] Every `[test]` link that resolves to an existing file uses language-canonical naming with evidence ∈ {scenario, mapping, conformance, property, compliance} and level ∈ {l1, l2, l3} encoded in the filename (e.g., TypeScript `<subject>.<evidence>.<level>[.<runner>].test.ts`, Python `test_<subject>.<evidence>.<level>[.<runner>].py`, Rust `<subject>.<evidence>.<level>[.<runner>].rs`; legacy forms `*.unit.test.ts` / `*.integration.test.ts` / `*.e2e.test.ts`, `test_*.unit.py` / `test_*.integration.py` / `test_*.e2e.py`, and `*_test.rs` / `test_*.rs` with no evidence/level are forbidden) — record legacy naming in the imperfection ledger and apply a clear local rename immediately; use request_user_input only when the correction changes ownership, scope, cost, risk, or an unresolved product choice
...
3. Do not create an empty `tests/` directory; `/test` or `/apply` materializes it with the first test file.
4. If the implementation doesn't exist yet: add the node path to `spx/EXCLUDE`. The `spx` CLI skips excluded nodes when running `spx test passing`. See `/understand` operational reference `excluded-nodes`.
...
How to avoid: After drafting, apply the read-aloud test from the live `/understand` `<atemporal_voice>` to every sentence. If it would sound wrong after the feature ships, rewrite it.
...
How to avoid: Apply the live `/understand` `<enabler>` and `<outcome>` definitions before selecting a template. If a natural hypothesis can't be written, it's probably an enabler.
