...
`/understand` owns the foundation templates and examples:

- Product spec template
- ADR template
- PDR template
- Enabler-node template
...
- Apply `/understand`'s node-type forcing question: write it as an enabler first. Why can't this be PROVIDES X SO THAT Y CAN Z? What is uncertain about which output achieves the goal?
...
Read the appropriate foundation template through `/understand`. Fill it using the gathered content.

**Voice rules** (from the durable-map foundation loaded by `/understand`):
...
- [ ] Nesting rules respected: outcomes CANNOT be children of enablers (see `/understand`'s node-types `<nesting_rules>` section)
- [ ] For outcomes: verify the forcing question from step 4 was answered — are the assertions a bet (majority could be swapped for different ones achieving the same goal)? If not, it is an enabler (see `/understand`'s node-types reference)
...
- [ ] Every `[test]` link that resolves to an existing file follows the repository's active language-specific `/test-*` naming contract, including its evidence-kind and execution-level encoding; if a legacy or noncanonical name is found, flag it as an imperfection and surface it via request_user_input before proceeding
