...
Templates and examples live in the understanding skill's directory (`../understanding/`):

- `../understanding/templates/product/product-name.product.md`
- `../understanding/templates/decisions/decision-name.adr.md`
- `../understanding/templates/decisions/decision-name.pdr.md`
- `../understanding/templates/nodes/enabler-name.md`
- `../understanding/templates/nodes/outcome-name.md`
- `../understanding/examples/` — filled specs for reference
...
- Apply the forcing question from `../understanding/references/node-types.md`: try to write it as an enabler first. Why can't this be PROVIDES X SO THAT Y CAN Z? What is uncertain about which output achieves the goal?
...
Read the appropriate template from `../understanding/templates/`. Fill it using the gathered content.
...
- [ ] Nesting rules respected: outcomes CANNOT be children of enablers (see `../understanding/references/node-types.md` `<nesting_rules>` section)
- [ ] For outcomes: verify the forcing question from step 4 was answered — are the assertions a bet (majority could be swapped for different ones achieving the same goal)? If not, it should be an enabler (see `../understanding/references/node-types.md`)
...
4. If the implementation doesn't exist yet: add the node path to `spx/EXCLUDE`. The `spx` CLI skips excluded nodes when running `spx test passing`. See `../understanding/references/excluded-nodes.md`.
