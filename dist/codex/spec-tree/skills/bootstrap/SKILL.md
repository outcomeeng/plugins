...
When candidate top-level areas mix aggregate domains, concrete behaviors, surfaces, actors, and code-shaped names, read `${SKILL_DIR}/../understand/references/product-domain-shapes.md` for the shared classifier and examples.
...
2. Write `spx/{product-name}.product.md` using the template from `${SKILL_DIR}/../understand/templates/product/product-name.product.md`. Fill every section from the interview — leave no `{placeholder}` unresolved:
   - Product name
...
3. Invoke `/update-spx` for the repository root so its generator writes both root guide managed sections — `AGENTS.md` for this runtime and `CLAUDE.md` for the other runtime — from the installed template. `/update-spx` owns guide rendering, enabled-language detection, and any explicit language override.
