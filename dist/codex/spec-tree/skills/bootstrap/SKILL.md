...
3. Invoke `/update-instruction-block` for the repository root so its generator writes both root instruction blocks — `AGENTS.md` for this runtime and `CLAUDE.md` for the other runtime — from the installed template. `/update-instruction-block` owns instruction-block rendering, enabled-language detection, and any explicit language override.
...
- Product spec path
- `AGENTS.md` path and `CLAUDE.md` path
...
- [ ] `AGENTS.md` and `CLAUDE.md` carry managed sections created from template, scoped to the project's enabled languages
