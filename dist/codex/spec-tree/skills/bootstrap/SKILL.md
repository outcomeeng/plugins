...
When candidate top-level areas mix aggregate domains, concrete behaviors, surfaces, actors, and code-shaped names, read the product-domain-shapes operational reference owned by `/understand` for the shared classifier and examples.
...
3. Render the spx-level guides — `spx/CLAUDE.md` for Claude Code and `spx/AGENTS.md` for Codex — from the template via the update-spx helper, passing the project's enabled languages so the guides record their `languages` config (and a later `/update-spx` re-renders from it, scoped to those languages):
   ...
   ```bash
   python3 "${SKILL_DIR}/../update-spx/scripts/update_spx.py" --template "${SKILL_DIR}/../understand/templates/spx-claude.md" --spx-dir <spx-dir> --languages <comma-separated-languages> --write
   ```
...
- [ ] `spx/AGENTS.md` created from template, scoped to the project's enabled languages
