...
1. Read `${SKILL_DIR}/manifest.json` so the diagnostic contract and
   declared `spx` version floor are available for startup-failure remediation.
2. Run `spx diagnose --manifest "${SKILL_DIR}/manifest.json" --format json`.
...
The manifest is `${SKILL_DIR}/manifest.json`. It declares the spx version
...
- `spx diagnose --manifest "${SKILL_DIR}/manifest.json" --format json`
