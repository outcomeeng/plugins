...
- `${SKILL_DIR}/references/claimed-session-resolution.md` before resolving claimed sessions.
...
| **Product rule**      | Convention specific to this codebase, forbidden pattern      | Product `AGENTS.md`                                                             |
...
- Write or update PLAN.md in the node directory (Tier 3 coordination note) — requires `request_user_input` approval and a real stop condition
- Remove PLAN.md when the plan is done — record a clear safe local removal as fix-now for workflow 04 without approval; use request_user_input only when removal changes ownership, scope, cost, risk, or an unresolved product choice
