...
Before the `request_user_input` block, print a plain-text header naming the canonical continuation plan plus every session that will be archived:
...
<process>
Present a single `request_user_input` with `multiSelect: true`. Group items by type: imperfections (with their destination), path-forward insights, and a skip option for coordination-only items.
