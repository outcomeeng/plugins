...
**If the marker is missing**: STOP and return to workflow 02. When context compaction dropped it, rerun workflows 02-03, invoking `/understand`, then `/contextualize` on the governing node, only immediately before this handoff reads or edits coordination notes or other governed product content, so the claimed-session marker and artifact partitions are reconstructed and approved before execution resumes. Do not proceed without the reloaded foundation, node contexts, resolved claimed-session set, and matching partitions.
...
When `spx/local/coordination.md` exists, run `<resolve_changes>` and `<refine_before_handoff>` from `${SKILL_DIR}/workflows/05-change.md` here, skip the session-file paths below, then continue to `<release_work_branch>` — see `SKILL.md` `<change_coordination>`.
...
When `spx/local/coordination.md` exists, run `<post_handoff_or_close>` and `<closeout_rows>` from `${SKILL_DIR}/workflows/05-change.md` here for held Changes, after `<release_work_branch>` has completed; legacy `<CLAIMED_SESSIONS>` ids still archive through the steps below — see `SKILL.md` `<change_coordination>`.
