...
**`gh api` stays inside the declared store.** The `gh api repos/*/issues/*/dependencies/*` grant matches by prefix and cannot bound the repository or method itself, so this workflow passes it only one read, `repos/<store>/issues/<N>/dependencies/blocked_by`, to mirror blockers into a Handoff; no `-X` method, no other endpoint, and no other repository is ever passed to `gh api` here — that constraint is the whole containment.
