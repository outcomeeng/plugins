...
- **Always use request_user_input** — structured choices (2-4 options per question), never open-ended
...
Revision loop: parse feedback, clarify ambiguous comments with request_user_input, regenerate, repeat until approved.
...
- Every question used request_user_input with 2-4 non-obvious options that are materially distinct end-states — no strawman, no false 50/50 — with the recommended option stated first
