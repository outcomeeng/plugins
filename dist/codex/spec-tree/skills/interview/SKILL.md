...
description: >-
  ...
  PDR, test, code, doc). NEVER invoke solely because a workflow uses request_user_input for an operational choice.
...
2. Read product docs — README, AGENTS.md, existing specs
...
Perform this research in the main conversation unless the active repository and runtime instructions explicitly authorize a bounded research subagent for interview pre-analysis. When that authorization exists, use only the authorized subagent shape for bounded read-only research. Read named files, skill files, skill references, the repository's AGENTS.md instruction file, and user-provided files in the main conversation. Summarize findings as a structured brief and share it with the user before the first question.
...
- **Inside an active interview, use request_user_input** — structured choices (2-4 options per question), never open-ended. A workflow's use of request_user_input for an operational choice does not by itself make the prompt an interview.
...
Claude saw a workflow use request_user_input for an operational choice and invoked `/interview`, turning a local workflow decision into requirements gathering.
...
How to avoid: Invoke `/interview` only when the unresolved decision concerns artifact requirements, scope, design, or draft approval. For mechanical workflow choices, follow the governing workflow and use request_user_input directly when that workflow requires it.
