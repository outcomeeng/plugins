...
2. Re-validate against current codebase:
   ...
   - Have product docs (README, AGENTS.md) changed?
...
5. Use request_user_input to confirm: "Resume from [area X], or start over?"
...
3. **Non-spec file** (source code, config, data) — confirm intent via request_user_input: "This looks like [type]. Want me to interview you about [inferred intent]?"
...
4. Read product docs (README, AGENTS.md, existing specs)

Use an Explore agent only when the active repository and runtime instructions explicitly authorize that bounded research subagent for interview pre-analysis. Otherwise perform the same read-only research in the main conversation. Read named files, skill files, skill references, the repository's AGENTS.md instruction file, and user-provided files in the main conversation.
...
2. Ask one question at a time via request_user_input (2-4 options, no obvious choices)
...
2. Ask via request_user_input: "Ready to write, or explore further?"
...
Ask via request_user_input what task format the user wants:
