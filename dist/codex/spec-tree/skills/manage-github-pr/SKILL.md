...
allowed-tools: Skill, request_user_input, Bash(git branch:*), Bash(git status:*), Bash(git diff:*), Bash(git log:*), Bash(git rev-parse:*), Bash(gh repo view:*), Bash(gh pr view:*), Bash(head:*), Bash(echo:*), Read
---
...
- **Open PR** — For a one-token syntactic PR number or PR URL, resolve it with `gh pr view "$ARGUMENTS" --json number,url,state,headRefName,headRefOid,baseRefName`; require `state == OPEN`, and stop mode detection with the host-observed result when resolution fails or returns another state rather than reinterpreting the explicit PR syntax as an instruction. For any other one-token value, probe it as a possible branch with the same command and select this mode only when it resolves to `state == OPEN`; a failed or non-open branch probe falls through to Instructed mode. This mode also applies when `$ARGUMENTS` is empty and the injected state shows an existing PR for this branch. The PR already defines lifecycle state; manage it.
...
**Step 1 — Establish intent and route.** If `<SPEC_TREE_FOUNDATION>` is absent, invoke `/understand` first so the foundation is loaded. Then invoke `/merging-standards` and record its named `<resolved_contracts>` before applying confirmation, safety, verification, publication, merge, deployment, or release specialization. Per the detected mode, gather what is being shipped. In Open PR mode with an explicit PR number, URL, or branch pointer, run `gh pr view "<explicit-pr-pointer>" --json number,url,state,headRefName,headRefOid,baseRefName`, require `state == OPEN`, and record the returned full URL as `pr_pointer`; when the injected current-branch state selected the mode without an explicit pointer, run `gh pr view --json number,url,state,headRefName,headRefOid,baseRefName`, require `state == OPEN`, and record the returned full URL as `pr_pointer`. Then proceed directly to Step 6. In Empty mode, invoke `/interview` to elicit the change. In Instructed mode, resolve the instruction against the repository — when it touches the spec tree, load context through `/contextualize` first per AGENTS.md. When Instructed mode also carries an injected current-branch PR, run `gh pr view --json number,url,state,headRefName,headRefOid,baseRefName`, require `state == OPEN`, and record the returned full URL as `pr_pointer` while continuing the instructed implementation path.

**Step 2 — State the plan; apply the confirmation contract.** When `pre_mutation_confirmation_contract.required=false`, state the plan in prose (the change to make, the branch, the commit shape, and that the flow runs through PR open, merge, and closure unless the user instruction says otherwise) and proceed autonomously. Normal harness approval for a consumer-defined command remains a tool-security boundary and resumes the same governed step after approval. When `required=true`, present that same plan through the runtime's structured-question tool (`AskUserQuestion` on Claude Code, `request_user_input` on Codex) and obtain confirmation before the first mutating action — never branch, commit, push, open, or merge before that confirmation. Establishing *what* to ship in Empty mode (Step 1, `/interview`) is requirements work and always proceeds.

After the plan or required confirmation, run every command in `safety_contract.pre_mutation_checks` immediately before the first branch, commit, or other checkout-sensitive mutation. In Open PR mode, Step 6 delegates this boundary to `/manage-pr`, whose merge cleanup repeats those resolved checks immediately before merge.
...
- MUST drive the lifecycle from a determined changeset autonomously when `pre_mutation_confirmation_contract.required=false`; when `required=true`, present the plan through the runtime's structured-question tool and obtain confirmation before branch creation, commit, push, PR open, or merge.
...
- MUST consume GitHub-PR transport specialization only through /merging-standards `<resolved_contracts>` and pass the relevant contract values to `/open-pr` and `/manage-pr`.
- MUST treat the narrow Bash grants in frontmatter as the approval-free execution surface. Run a command selected from `AGENTS.md` or `project_command_contract` through the harness's normal per-call approval path when it falls outside those grants. When no approval path exists, emit `MERGE_BLOCKED:project-command-approval-unavailable` with the blocked command and contract name; never skip it, widen `allowed-tools` during execution, or add repository-specific grants to this portable skill.
...
- MUST follow AGENTS.md and the loaded skills exactly.
...
**Failure 2: Default autonomy became a confirmation prompt.** Claude stated a plan and then asked whether to push, open, or continue while `pre_mutation_confirmation_contract.required=false`. Signal: an operator question before branch creation, commit, push, PR open, or merge with the default contract. Avoid: state the plan and proceed; use the structured-question tool only when the resolved contract requires confirmation.
...
- The lifecycle ran autonomously when `pre_mutation_confirmation_contract.required=false`; when `required=true`, the plan was presented through the runtime's structured-question tool and confirmed before the first mutation.
- The invocation resolved to the GitHub-PR transport from its arguments and live repository state, and every repository specialization reached `/open-pr` and `/manage-pr` through /merging-standards `<resolved_contracts>`.
