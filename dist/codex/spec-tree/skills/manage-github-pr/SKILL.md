...
allowed-tools: Read, request_user_input, Bash(spx worktree status:*), Bash(spx diagnose:*), Bash(git branch:*), Bash(git status:*), Bash(git diff:*), Bash(git log:*), Bash(git rev-parse:*), Bash(gh repo view:*), Bash(gh pr view:*), Bash(head:*), Bash(echo:*)
---
...
**Step 7 — Continue or close.** Consume `/manage-pr`'s closeout-ready result. When it carries remaining in-scope work — a further PR, a pending `PLAN.md` item, a node in the Specified state, a declared-but-unimplemented assertion — continue with it directly. When no in-scope work remains, produce the operator-useful closeout from the carried branch-state record and live state, and dispose of the held Change per /merging-standards `<close_phase>` — `close-change Applied` when the Change's Output is delivered, `release-change` when continuation remains for another holder. Do not append a separate merge receipt — a Change disposition completed earlier in the same conversation never satisfies this step for work merged after it, because new merged work reopens the disposition. The `--return-closeout` marker makes this outer lifecycle's ownership explicit.
...
**Failure 3: The lifecycle was reimplemented inline.** Claude opened, managed, merged, or cleaned up the PR by running ad hoc `git` or `gh` commands from this skill instead of invoking the governing lifecycle skills. Signal: inline commit, open, manage, merge, branch cleanup, or closeout logic appears in the main flow after mode detection. Avoid: after intent is established, delegate each lifecycle stage to `/commit-changes`, `/open-pr`, `/manage-pr`, and `close-change` or `release-change` as specified; this skill owns orchestration, not the stage protocols.

**Failure 4: A prior closeout substituted for Step 7.** Claude merged new work after closing out earlier in the same conversation, judged a second disposition redundant, and reported the earlier closeout as final. Signal: a closing summary with no Change disposition after the merge. Avoid: new merged work reopens the disposition; build the closeout from live state and dispose of the held Change through `close-change` or `release-change` after the merge.

**Failure 5: The whole methodology reloaded before the first PR inspection.** Claude re-entered an open PR after a compaction by invoking `/understand`, rereading the complete root instruction file, and contextualizing nodes before the first live PR inspection, on a pass whose next actions — current-head review inspection, check state, merge handling — touched no product content. On a 258K-token window that reload — the truncated root instruction file reread whole, the foundation, node contexts, and the lifecycle skills — cost roughly a quarter to a third of the window before any PR state was read. Reload only immediately before the first product content the pass reads or modifies or the first node it discusses; inspection, waiting, merge, deploy, and release proceed on live PR and repository state alone.
