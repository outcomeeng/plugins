...
allowed-tools: Skill, request_user_input, Bash(spx worktree status:*), Bash(spx diagnose:*), Bash(git branch:*), Bash(git status:*), Bash(git diff:*), Bash(git log:*), Bash(git rev-parse:*), Bash(gh repo view:*), Bash(gh pr view:*), Bash(head:*), Bash(echo:*), Read
---
...
**Failure 4: A prior handoff substituted for Step 7.** Claude merged new work after a `/handoff` had already closed the session in the same conversation, judged a second invocation redundant, and wrote the final closeout itself. Signal: a transport-authored closing summary with no `/handoff` invocation after the merge. Avoid: new merged work reopens the session; invoke `/handoff` plain and let its existing-session search classify threads the earlier handoff already owns — that classification is what makes the repeat invocation cheap. Cheapness is the reason to invoke it, never the reason to skip it.
