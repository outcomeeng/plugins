...
The narrow Bash grants in frontmatter authorize approval-free execution; they do not prohibit a consumer-defined command that the harness approves per call. Run commands from `AGENTS.md` or `project_command_contract` through that normal tool-approval path when they fall outside those grants. Never widen `allowed-tools` during execution.
