...
**Step 1 — Load foundation and vocabulary.** If `<SPEC_TREE_FOUNDATION>` is absent, invoke `/understand` first. Invoke `/merging-standards` for the shared gate vocabulary, action tokens, and every named contract in `<resolved_contracts>`. Record the resolved contracts and apply their defaults when a field is absent. NEVER reconstruct transport or merge behavior from incidental repository docs, and NEVER edit a generated guide (`AGENTS.md`) to change it.
...
`${SKILL_DIR}/scripts/classify_changeset.py` is covered by the merging node's scenario evidence before release:
