...
allowed-tools: Read, request_user_input
---
...
- **Deployment declarations** — environment mutation actions and the predicates that authorize them under `DEPLOYMENT_READINESS`. Absence means `DEPLOY` is a no-op and never blocks release or close.
- **Release declarations** — consumer-visible publication or refresh actions and the predicates that authorize them under `RELEASE_READINESS`. Absence means `RELEASE` is a no-op and never blocks close.
