...
- Read `${SKILL_DIR}/references/verifier-dispatch.md` before dispatching any configured verifier or reviewer role. It owns the dispatch mechanics and the per-role role-task contracts, which load with this skill rather than with the managed router block.
