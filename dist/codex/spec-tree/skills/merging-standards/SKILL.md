...
- **Deterministic verification.** Run the narrowest validation lane the project's merge overlay maps `preservation.base_delta_paths` to; fall back to the project's full deterministic-verification command when any path is unclassified, `preservation.path_overlap` is non-empty, or `preservation.branch_patch_changed` is true.
...
Both `REVIEW_READINESS` predicates are re-established before every push, not only the opening push. A follow-up push that changes the branch's own content — a fix for a CI finding — re-runs deterministic verification and the local review per `<local_review_invocation>` on the new diff before it is pushed. A follow-up push that **only** rebased onto an advanced base re-establishes the predicates scoped by the `<base_sync>` preservation proof — reusing the local review and running a narrower validation lane when the proof and the project overlay permit — rather than always re-running both in full. Either way, the author-side review precedes the push that fires CI, so a follow-up diff never reaches CI without an author-side review first.
