...
2. **Reclassify as stacked** — dependency on an unmerged base is intentional. Before a PR exists, obtain an exact stack-base PR pointer from explicit caller state or through the active structured-question capability, resolve its host-observed URL and `headRefName` with `gh pr view`, set that branch as the active base and `<base>` argument for `gh pr create`, and run the pre-create stacked classification against it.

**Pre-create stacked classification** (all must hold): an exact stack-base PR pointer is present in caller state or obtained through the active structured-question capability; `gh pr view` resolves that pointer to an open pull request; its host-observed URL and `headRefName` are recorded as `stack_base_pr_url` and `stack_base`; `origin/${stack_base}` is an ancestor of `HEAD`; and the commit and changed-file lists against `origin/${stack_base}` contain only the intended stacked payload.
