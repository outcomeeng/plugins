...
Steps 3 and 4 are the only commands in the merge lifecycle that touch a path outside the assigned worktree, and neither is pre-authorized in any skill's `allowed-tools`. That omission is deliberate. A harness declares the working directory a session may act in, and a write outside it is the operator's decision to approve, once per write — pre-clearing a parameterized write to an arbitrary resolved path would convert that decision into a blanket grant over any path `spx` happens to report. The refresh therefore surfaces its own approval prompt in a harness that enforces the boundary, and the prompt names the exact checkout being advanced. Never add a grant to suppress it, and never restate the two commands in a form built to match a broader existing pattern.
