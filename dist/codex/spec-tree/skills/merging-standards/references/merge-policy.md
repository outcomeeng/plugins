...
The record is ready only when `runOwnership` names one dispatch owner for the Verifier. For a Verifier that records a run — `changes-reviewer` through the review journal, `implementation-auditor` through `spx verification run`, and any other Verifier whose skill records a run — the run identity is minted by that Verifier's own open or start call and never held by the Author in advance. Before dispatch, the owner reconciles by record kind: for a Verifier that records through the journal — `changes-reviewer` and a craft plugin's `{plugin}-auditor` — it lists that type's unsealed runs for the branch through `spx journal list --type <type> --sealed unsealed`, the `/project-run-journal` `<channel>` verb; for a Verifier that records through `spx verification run` — `implementation-auditor` — it reconciles from the locators it recorded on earlier returns, rendering each through `spx verification run render`, whose projection carries a `terminalStatus` only once the run has finished. Neither the listing nor the projection carries a liveness field, so the owner resolves each unsealed run from its own dispatch state: an unsealed run is a conflicting writer only while a Verifier the owner dispatched in this pass has not yet returned, and every other unsealed run for the type is abandoned — the passing `<occupancy_preflight>` proves no other live session writes this branch's runs, and a Verifier whose dispatch has returned or terminated writes no more. The owner records an abandoned run as abandoned, leaves it unsealed, and never hands a prior run token to a new dispatch for a changed head. After the Verifier returns, the owner records the token it minted. A Verifier that returns one structured verdict and records no run — the evidence, decision, spec, skill, and subagent Auditors — sets `runIdentity` to none and needs no reconciliation.
