...
Apply `<verification_dispatch_readiness>`, then run it via the `changes-reviewer` agent. The Verifier's agent session stays isolated from the Author's agent session that produced the changeset. Iterate to convergence: each round, act on findings by validity and phase per `<review_classification>`, until no valid finding remains unaddressed.
