...
- **required artifact audits have passed** — dispatch every artifact Auditor selected by the active workflow's Output lanes for the changed surfaces. This includes `spec-tree_test-evidence-auditor` and `spec-tree_eval-evidence-auditor` when implementation malleability selects their changed evidence surfaces; `spec-tree_adr-auditor` or `spec-tree_pdr-auditor` for each changed decision; `spec-tree_spec-auditor` for each changed spec; `instructions_skill-auditor` for each complete changed skill surface when the instructions plugin is installed; `prose_prose-auditor` for each complete changed prose surface when the prose plugin is installed; and `spec-tree_implementation-auditor` when the code lane and malleability select implementation audit. Supply only the target each Auditor's contract requires, apply `<verification_dispatch_readiness>`, and run every selected artifact Auditor after its deterministic lane passes and before `spec-tree_changes-reviewer`. Handle completed rejected, failing, or unknown artifact-auditor verdicts per `<auditor_verdicts>`; and
...
Local Auditor agents — `spec-tree_test-evidence-auditor`, `spec-tree_eval-evidence-auditor`, `spec-tree_adr-auditor`, `spec-tree_pdr-auditor`, `spec-tree_spec-auditor`, `instructions_skill-auditor`, `prose_prose-auditor`, and `spec-tree_implementation-auditor` — emit structured findings for the slice they inspect. Language-specific audit concerns are composed through the installed `audit-{lang}-{code|tests|architecture}` skills, not through language-specific Auditor agents.
