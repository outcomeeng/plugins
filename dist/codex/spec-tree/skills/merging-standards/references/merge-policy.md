...
- Cleanup ends by fast-forwarding the worktree attached to the base branch, in place with `merge --ff-only`; no worktree on the base branch, uncommitted work there, or a non-fast-forwardable state each skips with a named reason and leaves that checkout as found, and the base branch is never checked out elsewhere, reset, or force-advanced to refresh it.
