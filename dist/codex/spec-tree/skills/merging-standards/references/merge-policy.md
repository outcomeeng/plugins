...
- Every merge-lifecycle git *mutation* stays inside the assigned worktree; `<merge_cleanup>`'s read-only inspections of other worktrees are how deletion safety is proven and are not a mutation. Advancing another checkout that holds the base branch is environment maintenance a repository declares as a release action in its overlay, never a step this policy performs.
