...
`/sync-base` owns the complete mechanism: it fetches the base, checkpoints authorized dirty work through `/commit-changes`, reruns its synchronizer, rebases the branch onto `origin/<base>`, and never uses `git reset` to integrate base movement. Claude NEVER asks the operator whether to rebase — base-sync is a mechanical consequence of observable git state, not a decision to surface; surfacing "should I rebase?" through a structured question or in prose is a defect. A clean rebase runs to completion with no operator interaction. A conflict `/sync-base` cannot resolve autonomously is reported as `conflict` with a structured `conflict` object; the rebase remains active so the operator can inspect, resolve and continue, or abort. Composing flows never duplicate dirty-tree branch, commit, or retry logic and never stash.
...
**Mutation-point guard.** Immediately before any `gh pr merge` command, the managing flow re-reads live PR state and recomputes `MERGE_READINESS`; it never relies on earlier inspection, conversation memory, or a prior `gh pr view` result. The guard reads `headRefOid`, PR state, `statusCheckRollup`, PR-level comments, formal reviews, review-thread comments, the fetched remote branch head, and the fetched base branch in one fresh pass. It produces `MERGE_READY:<head-sha>` only when the freshly inspected `headRefOid` matches the fetched remote branch head and every `MERGE_READINESS` predicate above holds for that same head; `statusCheckRollup` is the rollup attached to that PR snapshot, not a separate head-identity source.
...
- `MERGE_BLOCKED:<reason>` when a non-review required check is absent or terminal-but-not-success, the inspected `headRefOid` does not match the fetched remote branch head, the PR is closed/draft, the branch is not based on current `origin/<base>`, or any other hard PR-state predicate fails.
...
Claude NEVER asks the operator to choose between auto-merge, hold-at-green, or pause. The merge is a mechanical consequence of `MERGE_READINESS` plus the mutation-point guard returning `MERGE_READY:<head-sha>`, not a decision to surface. A lifecycle stop reports the concrete withheld mutation through its explicit `<action_tokens>`, structured base-sync conflict, or preserved failed safety-check diagnostic.
...
- Merge runs only when `MERGE_READINESS` holds and the mutation-point guard has just produced `MERGE_READY:<head-sha>`: the current-head CI review has no unresolved valid `BLOCKING` or `DEBT` finding, every other required check is terminal-green in the fresh PR snapshot, branch hygiene and PR-state hold on that inspected head, and its `headRefOid` matches the fetched remote branch head. `MERGE_READINESS` carries no time-based settle.
- A committed changeset ahead of its resolved base is treated as unfinished until it reaches the default branch on origin through the selected lifecycle, or an explicit lifecycle stop leaves no independent local action: an action-token emission, a structured base-sync conflict report, or a preserved failed preflight or post-cleanup diagnostic.
...
- No structured question or prose confirmation asks the operator to choose between auto-merge, hold-at-green, or pause; action-token emissions, structured base-sync conflict reports, and preserved failed safety-check diagnostics report their concrete blocked mutation without turning autonomous merge into an operator choice.
