...
<claim>
This whole section is skipped when `spx/local/coordination.md` exists — `${SKILL_DIR}/workflows/change.md` then owns claiming, per `<change_coordination>`.
...
Present options via `request_user_input`:
...
How to avoid: After `/contextualize`, present the loaded state and stop. Use `request_user_input` unless `$ARGUMENTS` explicitly includes `--auto-continue`. Do not invoke `/apply` or edit files before that checkpoint completes.
