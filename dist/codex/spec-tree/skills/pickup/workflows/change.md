...
**Legacy body metadata migrates only after canonical state is settled.** A leading prose line that carries `Product:`, `Maturity:`, `Lifecycle:`, or `Status:` is legacy input, never a fourth metadata store. Compare it with the project fields and the issue's edit, comment, and project-item history. When they disagree, reconstruct the latest intended value from that history; when the history does not decide, stop before editing the body and ask through `request_user_input`. Write and verify the canonical project fields first. Then remove only the legacy metadata line with `gh issue edit <N> --repo <store> --body-file -` and re-read the body. New and refined bodies never add such a line.
...
- `--list` presents candidates. List open Changes from the store with `gh issue list --repo <store> --state open --json number,title,assignees,url --limit 50`, then read Product, Maturity, and Status from their project items. Offer up to three through `request_user_input`, Changes whose Product matches the overlay, Status is `Available`, and assignee list is empty: Executable first, then any Maturity, each labelled with number, title, and Maturity.
...
- The post-context decision is captured through `request_user_input` or an explicit `--auto-continue` override before `<PICKUP_CHECKPOINT change=...>` is emitted, and no Activity executes before that checkpoint.
