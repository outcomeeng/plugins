...
4. When several remain, use `request_user_input` once with 2-3 concrete node choices, each naming the canonical full path and the session evidence that matched it. Include a pause-and-inspect option only when the runtime permits one beyond those candidates.
