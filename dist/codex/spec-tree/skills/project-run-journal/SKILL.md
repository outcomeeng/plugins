...
| `spx journal list --type <t> --sealed <sealed\|unsealed> --limit <n>` | list persisted run metadata for the type — runs opened here and runs `spx verification run` records alike — each with `runToken`, `branchSlug`, `sealed`, `terminalState`, and `startedAt` |
...
When the direct render reports the token is outside the current branch scope, the helper looks up the token's branch slug and retries. Pass an explicit branch slug only when a concrete slug is already known:
