...
Claude left the path of the spec-tree `/applying` flow. One or more of these happened:
...
- Claude made changes or ran operations without loading the spec-tree node with `/contextualizing` first.
...
A proper test written with the `/testing-{language}` skill would serve Claude and other agents again in the future. The debugging or exploration just done without following `/applying` will need to be reworked from scratch when the spec changes — a monumental waste of effort.
...
- Tests written without loading context: the tests may be wrong — verify after Step 3 of `/applying`.
