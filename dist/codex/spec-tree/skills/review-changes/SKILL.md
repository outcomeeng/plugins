...
Five entry points under `${SKILL_DIR}/scripts/` plus prompt and render-template references:
...
4. **Validate** the JSON through the arbiter. Pipe the emitted JSON in on stdin:
   ...
   ```bash
   printf '%s' "$REVIEW_RESULT_JSON" | python3 "${SKILL_DIR}/scripts/validate_review_result.py"
   ```
...
5. **Render** the human-readable surface from the validated JSON:
   ...
   ```bash
   printf '%s' "$REVIEW_RESULT_JSON" | python3 "${SKILL_DIR}/scripts/render_review.py"
   ```
