...
| Line        | Records                                                  | Path                                          |
| ----------- | -------------------------------------------------------- | --------------------------------------------- |
| Marketplace | events no single plugin owns: harnesses, plugins renamed | `${SKILL_DIR}/../../MARKETPLACE-CHANGELOG.md` |
...
```text
write: /Users/dev/.codex/agents/spec-tree_reviewer.toml
prune: /Users/dev/.codex/agents/spec-tree_verifier.toml
collision: /Users/dev/.codex/agents/spec-tree_notes.toml (unrecorded)
```

`write` names a destination whose bytes differ from the shipped definition, `prune` a recorded `spec-tree` destination the plugin no longer ships, and `collision` a present file the ownership record does not authorize, with its cause: `symlink`, `unrecorded`, `owned by <plugin>`, `not a regular file`, or `digest mismatch`. A `scope-split directed-removal:` or `scope-split collision:` line names a checkout copy under `.codex/agents/` — matched by shipped bytes, by the `spec-tree` filename prefix, or by a `skills.config` entry naming a `spec-tree:` skill — and stops mutation. After the mutating form succeeds, the ownership record carries one entry per written destination:

```json
{ "destination": "agents/spec-tree_reviewer.toml", "plugin": "spec-tree", "digest": "<sha256 of the installed bytes>" }
```
