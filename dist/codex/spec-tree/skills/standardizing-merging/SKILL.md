...
Once `MERGE_READINESS ∧ PRODUCTION_READINESS` authorize the merge, Claude merges and then deletes the branch. The universal default — used whenever the overlay declares no merge command — is rebase merge with an explicit **`--delete-branch=false`**, followed by a worktree-safe manual deletion:
