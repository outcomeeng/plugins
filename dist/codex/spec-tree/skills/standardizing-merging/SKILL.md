...
**Loop semantics.** When an invoked workflow surfaces auditor verdicts while preparing or repairing a PR, handle every REJECT and MATERIAL as an in-slice finding under `<review_classification>`: fix it, re-run the auditor, and repeat until no REJECT or MATERIAL remains. APPROVE means the auditor found nothing in its scope. Auditor findings do not add a fourth PR-lifecycle gate and do not change the `MERGE_READINESS` predicate set in `<authority_gates>`.
...
- Every auditor verdict from a local auditor agent (per `<auditor_verdicts>`) is handled as an in-slice finding; REJECT and MATERIAL verdicts are fixed in the slice, not deferred to `ISSUES.md`.
