...
The agent NEVER asks the operator whether to rebase. A behind-base branch is rebased automatically — base-sync is a mechanical consequence of observable git state, not a decision to surface. The only operator touch-point in base-sync is a conflict the agent cannot resolve autonomously, which emits `SYNC_BASE` per `<action_tokens>`; a rebase that applies cleanly runs to completion with no operator interaction. Surfacing "should I rebase?" through a structured question or in prose is a defect.
...
Once `MERGE_READINESS ∧ PRODUCTION_READINESS` authorize the merge, the agent merges and then deletes the branch. The universal default — used whenever the overlay declares no merge command — is rebase merge with an explicit **`--delete-branch=false`**, followed by a worktree-safe manual deletion:
