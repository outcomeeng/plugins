...
**Mutation-point guard.** Immediately before any `gh pr merge` command, /managing-pr re-reads live PR state and recomputes `MERGE_READINESS`; it never relies on earlier inspection, conversation memory, or a prior `gh pr view` result. The guard reads PR state, `statusCheckRollup`, PR-level comments, formal reviews, review-thread comments, the fetched remote branch head, and the fetched base branch. It produces `MERGE_READY:<head-sha>` only when the freshly inspected head SHA, fetched remote branch head, and inspected status-check SHA match and every `MERGE_READINESS` predicate above still holds for that same head.
...
Waiting for CI runs, reviews, or check completion prefers the runtime timer. The consuming flow loads `/tracking-tasks` before creating, refreshing, or deleting runtime tracking after opening a PR and after every follow-up push.

`/tracking-tasks` owns runtime-specific payload shape, stale-context limits, lifecycle rules, failed-check handling, approval-boundary deletion, timer-tool selection, and the no-timer foreground PR-check fallback. This section owns the PR-flow facts: timer-backed waits re-enter /github-pr with the PR pointer, and no-timer PR-check waits use one foreground `gh pr checks` watcher before re-inspecting the merge gate. Review output may be absent during early check completion; when another current-head check is non-terminal, that state is still a PR-check wait and uses the same watcher rather than ending on a wait token.
...
When no runtime heartbeat or timer is available and an open PR is blocked by check completion — a non-terminal required check, a non-terminal reviewing-kind check, or absent review output while another current-head check is non-terminal — run exactly one foreground watcher:

```bash
gh pr checks <pr-number> --watch --fail-fast --interval 30
```

After that command exits, immediately run the full managing inspection again before acting: PR state, check rollup, PR-level comments, formal reviews, and review-thread comments. This fallback is only for PR check completion in the GitHub-PR lifecycle, applies to both Claude Code and Codex, and never runs in the background.
...
| Token                                    | Emitted when                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `WAIT_FOR_CHECKS`                        | A required check is non-terminal (running, queued, pending) while a current-head review has landed; not yet terminal-green. With a runtime heartbeat or timer, the heartbeat re-fires. Without one, the managing flow uses the foreground PR-check watcher from `<heartbeat>` instead of ending on this token.                                                                                                                                                                                                                                                                                                                                                                                                               |
| `WAIT_FOR_REVIEW`                        | No current-head CI review has landed yet on any inspected surface — review-absence takes precedence over the check-not-terminal-green and branch-hygiene predicates for classification. Without a runtime heartbeat or timer, this token is not the end state while the current-head PR is still in check completion: a non-terminal reviewing-kind check, or absent review output while another current-head check is non-terminal, uses the foreground PR-check watcher from `<heartbeat>` instead.                                                                                                                                                                                                                        |
...
- Waiting for CI, review, or checks is delegated to runtime tracking per `<heartbeat>` and using the skill `/tracking-tasks`; when no runtime heartbeat or timer exists, PR check waits — including absent review output while another current-head check is non-terminal — use one foreground `gh pr checks <pr-number> --watch --fail-fast --interval 30` fallback and then re-inspect the full merge gate.
