...
| `git_failure`     | 1    | a diverged detached HEAD carrying its own commits, an unresolved base, a `--base` sync whose default branch does not resolve, or a failed fetch — a clean behind-base detached HEAD is advanced, not failed | report `detail`; NEVER rebase                                                                                                                              |
