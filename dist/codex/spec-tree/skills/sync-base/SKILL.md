...
| `git_failure`     | 1    | a diverged detached HEAD carrying its own commits, an unresolved base, a `--base` sync of an attached branch whose default branch does not resolve, or a failed fetch — a clean behind-base detached HEAD is advanced, not failed | report `detail`; NEVER rebase                                                                                                                              |
...
A branch with no record derives its predecessor from local topology. The candidates are the local branches, other than the default, that do not contain the branch's head and that forked from the default before the branch forked from them — a branch stacked above this one contains its head and is never a candidate; the candidate whose fork with the branch descends from every other candidate's is the nearest predecessor, and the synchronizer records it with that fork as the tip and proceeds as above. Two candidates whose forks are unordered, or whose forks coincide because they sit at one commit, yield no predecessor and no record. A conflict on a branch with no stack record and no derivable predecessor lists `git rebase --onto origin/<base> <fork>` among its operator options; the fork is the commit the branch's own commits sit on, which the operator names.
...
2. **Operator-owned or unknown work.** When existing authorization covers committing the exact paths, invoke `/commit-changes` on their owning branch and continue. Otherwise preserve those files and their index state, report the blocked commit and exact paths, and request authority through `request_user_input` with a recommended commit option and a pause-and-inspect option. Apply the same boundary when an explicit operator limit withholds authority over session-owned paths. Never classify absent authority as a rebase conflict.
...
- A stacked branch is restacked from its recorded predecessor tip by `git rebase --onto`, never by replaying the predecessor's commits against their rewritten form; the record is written from git facts alone — a rewrite's pre-rebase head, the nearest-fork derivation's merge-base, or the tip a non-default `--base` lands on — and the synchronizer alone writes or removes it.
...
| `--no-fetch` with an explicit base whose remote-tracking ref does not resolve | exit 1; `status=git_failure`; no fetch attempted                                                                                        |
...
| rebased branch at the bottom of an unrecorded three-branch chain              | exit 0; `status=rebased`; the middle branch records the rebased branch, the top branch records the middle branch                        |
| unrecorded branch above two candidates at one commit                          | exit 0; no record written; synced against the default base                                                                              |
| clean detached HEAD with `--base` and no resolvable default branch            | exit 0; `status=rebased`; HEAD advanced to `origin/<base>`                                                                              |
| recorded branch synced with `--base <default>` while its predecessor is open  | exit 0; `status=rebased`; only the branch's own commits above `origin/<default>`; record removed                                        |
...
- Every clean outcome's `preservation` object carries `schema_version`, full old/new base and head OIDs, base and branch path sets, overlap, patch-identity booleans, and `stack_predecessor`, `stack_tip_before`, and `stack_tip_after` — null for a branch with no stack, and `stack_tip_after` null once a restack onto the default cleared the record; it carries no project lane name.
