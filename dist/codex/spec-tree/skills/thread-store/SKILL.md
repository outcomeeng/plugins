...
The active backend resolves from `SPX_VERIFY_BACKEND`. Default `local` selects the filesystem backend rooted at `.spx/reviews/<branch-slug>/`; `SPX_VERIFY_LOCAL_ROOT` overrides the root for tests and runtime customization. Unknown backend names exit non-zero with a configuration error that enumerates the registered names.

</backend_selection>

<slug_derivation>

Slug derivation re-exports the canonical helper from `${SKILL_DIR}/../changeset-scope/scripts/changeset_scope.py`. The slug:
