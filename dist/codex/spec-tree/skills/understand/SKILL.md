...
6. List each template and example by path — evidence each was located, not assumed — then read in full immediately when authoring:
   ...
   When `$profile` is `align`, also read `${SKILL_DIR}/references/what-goes-where.md` and all five listed templates in full. Steps 2 and 6 then guarantee that `durable-map.md`, `node-types.md`, `what-goes-where.md`, and every structural template are present in the caller's conversation.
