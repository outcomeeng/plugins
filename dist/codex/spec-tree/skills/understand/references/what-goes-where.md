...
| **Rust**       | A workspace-member crate at `<package>-testing/` (Cargo package `<package>-testing`, Rust import path `<package>_testing`), declared as a `[dev-dependencies]` entry of consumers; modules `<package>_testing::harnesses::*`, `<package>_testing::generators::*`, `<package>_testing::fixtures::*` |
| **Go**         | `internal/testinfra/` (package `testinfra` — not `test`, which collides with the standard library): `internal/testinfra/harnesses/`, `internal/testinfra/generators/`, `internal/testinfra/fixtures/`, imported as `<module>/internal/testinfra/...`                                               |
...
**Verified by:** Reconcile against the durable layers and current user intent before use. `/contextualize` reads them automatically; `/pickup` checks for them.
