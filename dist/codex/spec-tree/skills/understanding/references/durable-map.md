...
5. Use node-local `PLAN.md` for concrete pending implementation steps when tests or code are not part of the slice.
...
- If tests or code are not part of the slice, add or update `PLAN.md` at the first affected lower node with the next implementation step and the higher-level artifact that created it.
