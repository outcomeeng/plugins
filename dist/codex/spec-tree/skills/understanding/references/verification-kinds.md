...
The assertion tag `[review]` is the legacy spelling of the `[audit]` tag and resolves to it during migration; it does not denote the `reviewing` type. The collision is between a tag and a type — `[review]`-the-tag is auditing evidence, `reviewing`-the-type is the open-ended gate that backs no tag.
