...
- The same-plugin `select-artifacts` provider, whose script and rendered `artifact-registry.json` the generator imports by their installed layout to map test-file extensions to kinds; a partial plugin install without it stops language detection at GATE 1.
...
- The closing report carries each root file's `budget:` measurement, and every `breach` line observed at check or write time is named in it with the file's exact size, the ceiling, and the overage.
