...
1. **Resolve the paths.** Template: `${SKILL_DIR}/../understand/templates/instruction-block.md`. Repository root: the product's root directory, referred to below as `<repo-root>`; the generator writes instruction blocks into `<repo-root>/CLAUDE.md` and `<repo-root>/AGENTS.md` and removes the retired generated instruction files under `<repo-root>/spx/` when present.
...
<constraints>
- NEVER edit the deterministic parse, compare, filter, block replacement, or render logic here — it lives in `${SKILL_DIR}/scripts/instruction_block.py`, governed by the node's Instruction Block Render Model ADR.
...
- The template has one home, the understanding skill's `templates/`. Read it through `${SKILL_DIR}/../understand/templates/instruction-block.md`; never copy it into this skill.
</constraints>
