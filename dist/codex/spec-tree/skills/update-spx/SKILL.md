...
<objective>
Keep a product's two spx-level directory guides current with the installed spec-tree template. The guide is two generated files — `spx/CLAUDE.md` for Claude Code and `spx/AGENTS.md` for Codex — because one repository is worked by both agents at once and each reads its own filename. Both render from one template: the body is shared, the spans that differ by agent runtime are authored as per-runtime blocks rendered into each file, and the only per-product variation is the enabled-language list, scoped to the languages the project's tests use. Generation is deterministic and needs no agent judgment; the regenerate-and-diff gate keeps the guides current on every commit, and this skill is the manual trigger over the same generator. The parse, compare, filter, and render logic lives in `scripts/update_spx.py`, governed by the node's Guide Render Model ADR.
</objective>
...
1. **Resolve the paths.** Template: `${SKILL_DIR}/../understand/templates/spx-claude.md`. Guide directory: the product's `spx/` directory, referred to below as `<spx-dir>`; the generator writes `<spx-dir>/CLAUDE.md` and `<spx-dir>/AGENTS.md`.

2. **Detect status.** Run:

   ```bash
   python3 "${SKILL_DIR}/scripts/update_spx.py" --template "${SKILL_DIR}/../understand/templates/spx-claude.md" --spx-dir <spx-dir> --check
   ```
...
3. **Act on the status.**
   ...
   - **`stale` or `absent`** — regenerate both files:
     ...
     ```bash
     python3 "${SKILL_DIR}/scripts/update_spx.py" --template "${SKILL_DIR}/../understand/templates/spx-claude.md" --spx-dir <spx-dir> --write
     ```
