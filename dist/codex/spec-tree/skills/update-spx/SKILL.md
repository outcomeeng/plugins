...
3. **Act on the status.**
   ...
   - **`stale`** — re-render in place; the helper reads the guide's existing `product_name` and `languages` and re-renders the new template with them:
     ...
     ```bash
     python3 "${SKILL_DIR}/scripts/update_spx.py" --template "${SKILL_DIR}/../understanding/templates/spx-claude.md" --product <guide> --write
     ```
   ...
   - **`absent`** — scaffold. When interactive, ask the user for the product name and the enabled languages with `AskUserQuestion`, then render with them:
     ...
     ```bash
     python3 "${SKILL_DIR}/scripts/update_spx.py" --template "${SKILL_DIR}/../understanding/templates/spx-claude.md" --product <guide> --name "<product name>" --languages <comma-separated-languages> --write
     ```
...
- The template has one home, the understanding skill's `templates/`. Read it through `${SKILL_DIR}/../understanding/templates/spx-claude.md`; never copy it into this skill.
