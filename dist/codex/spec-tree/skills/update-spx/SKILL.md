...
3. **Detect status.** Run, passing the determined languages so the check catches a language drift as well as a version gap:
   ...
   ```bash
   python3 "${SKILL_DIR}/scripts/update_spx.py" --template "${SKILL_DIR}/../understand/templates/spx-claude.md" --product <guide> --languages <languages> --check
   ```
...
4. **Act on the status.**
   ...
   - **`stale`** — re-render in place, scoped to the enabled languages:
     ...
     ```bash
     python3 "${SKILL_DIR}/scripts/update_spx.py" --template "${SKILL_DIR}/../understand/templates/spx-claude.md" --product <guide> --languages <languages> --write
     ```
