...
- **NEVER** pass dynamic branch names to `git branch -d` or `git branch -D`: variables, command substitutions, arrays, and globs are denied. Type every branch name literally; delete several literal names in one command.
