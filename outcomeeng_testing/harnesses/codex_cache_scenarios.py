"""Harness-owned installation scenarios for chain-recovery cache preservation.
...
protocol DI) in place of the production git-history walker. CLI-surface
scenarios that verify ``main()`` output call ``main()`` directly with the
production ``GitPluginHistory`` against a synthetic non-git repository.
"""
...
from outcomeeng_testing.generators.codex_cache_scenarios import (
    CURRENT_VERSION,
    NEW_CURRENT_VERSION,
    NOT_INSTALLED_CURRENT_VERSION,
    NOT_INSTALLED_PLUGIN,
    NOT_INSTALLED_STALE_VERSION,
    OLDER_VERSION,
    ORPHAN_PLUGIN_NAME,
    PLUGIN_NAME,
    UNINSTALLED_PLUGIN_NAME,
)
...
def codex_cache_scenarios_pass() -> bool:
...
def _run_scenario(scenario: Callable[..., object]) -> None:
