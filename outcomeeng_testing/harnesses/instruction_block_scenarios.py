"""Harness-owned scenarios for the instruction-block render model.
...
class CapturedOutput:
    """Captured process streams returned with the pytest-compatible shape."""
...
class CapturedStreams:
    """Harness-owned stdout and stderr capture for CLI scenarios."""
    ...
    def active(self) -> Iterator[None]:
        """Capture both process streams for one scenario invocation."""
        with redirect_stdout(self._stdout), redirect_stderr(self._stderr):
            yield

    def readouterr(self) -> CapturedOutput:
        """Return and clear output accumulated since the previous read."""
...
def instruction_block_scenarios_hold() -> bool:
    """Run every scenario with harness-owned temporary state and stream capture."""
    checks = tuple(
        cast(Callable[..., None], check)
    ...
    for check in checks:
        ...
        if parameters == ("tmp_path",):
            ...
            continue
        if parameters == ("tmp_path", "capsys"):
            with TemporaryDirectory() as directory:
                streams = CapturedStreams()
                with streams.active():
                    check(pathlib.Path(directory).resolve(), streams)
            continue
        message = f"Unsupported scenario parameters: {parameters!r}"
