"""Harness-owned scenario execution for the instruction-block render model.
...
def _case_write_produces_both_files_language_and_harness_filtered(
...
def _case_write_preserves_shared_region_and_independent_prose(
    tmp_path: pathlib.Path,
) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
...
def _case_router_marker_format(tmp_path: pathlib.Path) -> None:
...
def _case_both_files_identical_except_harness_spans(tmp_path: pathlib.Path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    harness.run_generator_write_primary(repo, _template(tmp_path))
    claude = (repo / harness.INSTRUCTION_CLAUDE).read_text(encoding="utf-8")
...
def _case_newer_template_adds_section_preserving_shared_region(
    tmp_path: pathlib.Path,
) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
...
def _case_template_symlink_is_rejected(tmp_path: pathlib.Path) -> None:
...
def _case_cli_rejects_missing_repo_root(
...
def _case_cli_rejects_non_directory_repo_root(
    tmp_path: pathlib.Path, capsys: pytest.CaptureFixture[str]
) -> None:
...
def _case_cli_rejects_missing_template(
    tmp_path: pathlib.Path, capsys: pytest.CaptureFixture[str]
) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
...
def _case_cli_rejects_root_symlink_escaping_repo(
    tmp_path: pathlib.Path, capsys: pytest.CaptureFixture[str]
) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
...
def _case_cli_rejects_spx_symlink_during_language_detection(
    tmp_path: pathlib.Path, capsys: pytest.CaptureFixture[str]
) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
...
def _case_cli_detects_languages_from_test_extensions(tmp_path: pathlib.Path) -> None:
...
def _case_cli_write_without_repo_root_exits(
    tmp_path: pathlib.Path, capsys: pytest.CaptureFixture[str]
) -> None:
...
def _case_cli_check_reports_absent_when_one_file_missing(
    tmp_path: pathlib.Path, capsys: pytest.CaptureFixture[str]
) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
...
def _case_cli_check_marks_router_not_first_as_stale(tmp_path: pathlib.Path) -> None:
    block = MODULE.render(
        harness.build_template(harness.NEW_VERSION),
        (harness.LANG_PRIMARY,),
        harness.NEW_VERSION,
...
def _case_symlinked_root_file_becomes_regular_file(tmp_path: pathlib.Path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / harness.INSTRUCTION_AGENTS).write_text(
        harness.ROOT_SHARED_BODY, encoding="utf-8"
...
def _case_legacy_marker_block_reported_stale_and_replaced(
    tmp_path: pathlib.Path,
) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
...
def _case_quoted_router_marker_in_prose_is_preserved(tmp_path: pathlib.Path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    quoted = (
...
def _case_malformed_shared_fence_is_reported_stale(tmp_path: pathlib.Path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    block = MODULE.render(
        harness.build_template(harness.NEW_VERSION),
...
def _case_bootstrap_refuses_a_malformed_seed_fence() -> None:
...
def _case_duplicate_shared_region_name_is_malformed() -> None:
    open_marker = MODULE.shared_open_marker("commands")
...
def _case_blank_run_in_independent_content_preserved(tmp_path: pathlib.Path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    seed = "# Product\n\nfirst\n\n\n\nsecond\n"
    for name in (harness.INSTRUCTION_CLAUDE, harness.INSTRUCTION_AGENTS):
...
def _case_bootstrap_preserves_lines_when_common_span_ends_mid_line() -> None:
...
def _case_bootstrap_finds_whole_line_block_over_longer_straddling_match() -> None:
...
def _case_bootstrap_snaps_span_to_line_boundaries_in_both_files() -> None:
...
def _case_diverged_shared_region_reconciles_to_more_recent_side(
    tmp_path: pathlib.Path,
) -> None:
...
def _case_reconcile_replaces_losing_region_whole_without_blending(
    tmp_path: pathlib.Path,
) -> None:
    repo = _init_repo_with_committed_shared_region(
        tmp_path,
...
def _case_reconcile_uses_region_recency_not_whole_file_recency(
    tmp_path: pathlib.Path,
) -> None:
...
def _case_region_line_range_covers_content_lines_only() -> None:
...
def _case_recency_tie_is_reported_ambiguous(tmp_path: pathlib.Path) -> None:
    repo = _init_repo_with_committed_shared_region(
        tmp_path,
        claude_region=harness.SHARED_REGION_BODY,
...
def _case_one_sided_shared_region_is_reported_ambiguous(
    tmp_path: pathlib.Path,
) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
...
def _case_reconcile_reports_malformed_fence_as_ambiguous(
    tmp_path: pathlib.Path,
) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
...
def _case_reconcile_skips_a_malformed_duplicate_name(tmp_path: pathlib.Path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    harness.init_git_identity(repo)
    open_marker = MODULE.shared_open_marker("commands")
...
def _case_cli_reconcile_requires_repo_root(
    tmp_path: pathlib.Path, capsys: pytest.CaptureFixture[str]
) -> None:
...
def _case_cli_reconcile_from_applies_operator_tie_break(
    tmp_path: pathlib.Path, capsys: pytest.CaptureFixture[str]
) -> None:
...
def _case_cli_reconcile_reports_no_change_when_regions_agree(
    tmp_path: pathlib.Path, capsys: pytest.CaptureFixture[str]
) -> None:
    repo = _init_repo_with_committed_shared_region(
        tmp_path,
...
def _case_reconcile_makes_no_change_to_a_dirty_file(tmp_path: pathlib.Path) -> None:
    repo = _init_repo_with_committed_shared_region(
        tmp_path,
        claude_region=harness.SHARED_REGION_BODY,
        agents_region=harness.SHARED_REGION_BODY,
...
class _Capture:
    """Minimal capture adapter for harness-owned direct CLI execution."""
...
def _run_case(case: Callable[..., None]) -> None:
    """Run one scenario with harness-owned temporary paths and stream capture."""
    stdout = io.StringIO()
    stderr = io.StringIO()
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        if "tmp_path" in parameters:
            arguments["tmp_path"] = pathlib.Path(directory).resolve()
        if "capsys" in parameters:
            arguments["capsys"] = _Capture(stdout, stderr)
        with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
            case(**arguments)


def assert_write_produces_both_files_language_and_harness_filtered() -> None:
    """Run the write produces both files language and harness filtered scenario."""
    _run_case(_case_write_produces_both_files_language_and_harness_filtered)


def assert_write_preserves_shared_region_and_independent_prose() -> None:
    """Run the write preserves shared region and independent prose scenario."""
    _run_case(_case_write_preserves_shared_region_and_independent_prose)


def assert_router_marker_format() -> None:
    """Run the router marker format scenario."""
    _run_case(_case_router_marker_format)


def assert_both_files_identical_except_harness_spans() -> None:
    """Run the both files identical except harness spans scenario."""
    _run_case(_case_both_files_identical_except_harness_spans)


def assert_newer_template_adds_section_preserving_shared_region() -> None:
    """Run the newer template adds section preserving shared region scenario."""
    _run_case(_case_newer_template_adds_section_preserving_shared_region)


def assert_template_symlink_is_rejected() -> None:
    """Run the template symlink is rejected scenario."""
    _run_case(_case_template_symlink_is_rejected)


def assert_cli_rejects_missing_repo_root() -> None:
    """Run the cli rejects missing repo root scenario."""
    _run_case(_case_cli_rejects_missing_repo_root)


def assert_cli_rejects_non_directory_repo_root() -> None:
    """Run the cli rejects non directory repo root scenario."""
    _run_case(_case_cli_rejects_non_directory_repo_root)
...
def assert_cli_rejects_directory_template() -> None:
    """Run the cli rejects directory template scenario."""
    _run_case(_case_cli_rejects_directory_template)


def assert_cli_rejects_root_symlink_escaping_repo() -> None:
    """Run the cli rejects root symlink escaping repo scenario."""
    _run_case(_case_cli_rejects_root_symlink_escaping_repo)


def assert_cli_rejects_spx_symlink_during_language_detection() -> None:
    """Run the cli rejects spx symlink during language detection scenario."""
    _run_case(_case_cli_rejects_spx_symlink_during_language_detection)


def assert_cli_detects_languages_from_test_extensions() -> None:
    """Run the cli detects languages from test extensions scenario."""
    _run_case(_case_cli_detects_languages_from_test_extensions)


def assert_cli_write_without_repo_root_exits() -> None:
    """Run the cli write without repo root exits scenario."""
    _run_case(_case_cli_write_without_repo_root_exits)


def assert_cli_check_reports_absent_when_one_file_missing() -> None:
    """Run the cli check reports absent when one file missing scenario."""
    _run_case(_case_cli_check_reports_absent_when_one_file_missing)


def assert_cli_check_treats_language_order_as_set() -> None:
    """Run the cli check treats language order as set scenario."""
    _run_case(_case_cli_check_treats_language_order_as_set)


def assert_cli_check_marks_router_not_first_as_stale() -> None:
    """Run the cli check marks router not first as stale scenario."""
    _run_case(_case_cli_check_marks_router_not_first_as_stale)


def assert_unparseable_version_is_stale() -> None:
    """Run the unparseable version is stale scenario."""
    _run_case(_case_unparseable_version_is_stale)


def assert_symlinked_root_file_becomes_regular_file() -> None:
    """Run the symlinked root file becomes regular file scenario."""
    _run_case(_case_symlinked_root_file_becomes_regular_file)


def assert_markerless_generated_body_is_replaced() -> None:
    """Run the markerless generated body is replaced scenario."""
    _run_case(_case_markerless_generated_body_is_replaced)


def assert_legacy_marker_block_reported_stale_and_replaced() -> None:
    """Run the legacy marker block reported stale and replaced scenario."""
    _run_case(_case_legacy_marker_block_reported_stale_and_replaced)


def assert_quoted_router_marker_in_prose_is_preserved() -> None:
    """Run the quoted router marker in prose is preserved scenario."""
    _run_case(_case_quoted_router_marker_in_prose_is_preserved)


def assert_quoted_router_closing_marker_after_block_is_preserved() -> None:
    """Run the quoted router closing marker after block is preserved scenario."""
    _run_case(_case_quoted_router_closing_marker_after_block_is_preserved)


def assert_quoted_shared_fence_in_prose_is_not_a_region() -> None:
    """Run the quoted shared fence in prose is not a region scenario."""
    _run_case(_case_quoted_shared_fence_in_prose_is_not_a_region)


def assert_malformed_shared_fence_is_reported_stale() -> None:
    """Run the malformed shared fence is reported stale scenario."""
    _run_case(_case_malformed_shared_fence_is_reported_stale)


def assert_bootstrap_refuses_a_malformed_seed_fence() -> None:
    """Run the bootstrap refuses a malformed seed fence scenario."""
    _run_case(_case_bootstrap_refuses_a_malformed_seed_fence)


def assert_duplicate_shared_region_name_is_malformed() -> None:
    """Run the duplicate shared region name is malformed scenario."""
    _run_case(_case_duplicate_shared_region_name_is_malformed)


def assert_blank_run_in_independent_content_preserved() -> None:
    """Run the blank run in independent content preserved scenario."""
    _run_case(_case_blank_run_in_independent_content_preserved)


def assert_bootstrap_preserves_lines_when_common_span_ends_mid_line() -> None:
    """Run the bootstrap preserves lines when common span ends mid line scenario."""
    _run_case(_case_bootstrap_preserves_lines_when_common_span_ends_mid_line)


def assert_bootstrap_finds_whole_line_block_over_longer_straddling_match() -> None:
    """Run the bootstrap finds whole line block over longer straddling match scenario."""
    _run_case(_case_bootstrap_finds_whole_line_block_over_longer_straddling_match)


def assert_bootstrap_snaps_span_to_line_boundaries_in_both_files() -> None:
    """Run the bootstrap snaps span to line boundaries in both files scenario."""
    _run_case(_case_bootstrap_snaps_span_to_line_boundaries_in_both_files)


def assert_diverged_shared_region_reconciles_to_more_recent_side() -> None:
    """Run the diverged shared region reconciles to more recent side scenario."""
    _run_case(_case_diverged_shared_region_reconciles_to_more_recent_side)


def assert_reconcile_replaces_losing_region_whole_without_blending() -> None:
    """Run the reconcile replaces losing region whole without blending scenario."""
    _run_case(_case_reconcile_replaces_losing_region_whole_without_blending)


def assert_reconcile_uses_region_recency_not_whole_file_recency() -> None:
    """Run the reconcile uses region recency not whole file recency scenario."""
    _run_case(_case_reconcile_uses_region_recency_not_whole_file_recency)


def assert_region_line_range_covers_content_lines_only() -> None:
    """Run the region line range covers content lines only scenario."""
    _run_case(_case_region_line_range_covers_content_lines_only)


def assert_recency_tie_is_reported_ambiguous() -> None:
    """Run the recency tie is reported ambiguous scenario."""
    _run_case(_case_recency_tie_is_reported_ambiguous)


def assert_one_sided_shared_region_is_reported_ambiguous() -> None:
    """Run the one sided shared region is reported ambiguous scenario."""
    _run_case(_case_one_sided_shared_region_is_reported_ambiguous)


def assert_reconcile_reports_malformed_fence_as_ambiguous() -> None:
    """Run the reconcile reports malformed fence as ambiguous scenario."""
    _run_case(_case_reconcile_reports_malformed_fence_as_ambiguous)


def assert_reconcile_skips_a_malformed_duplicate_name() -> None:
    """Run the reconcile skips a malformed duplicate name scenario."""
    _run_case(_case_reconcile_skips_a_malformed_duplicate_name)


def assert_cli_reconcile_requires_repo_root() -> None:
    """Run the cli reconcile requires repo root scenario."""
    _run_case(_case_cli_reconcile_requires_repo_root)


def assert_cli_reconcile_from_applies_operator_tie_break() -> None:
    """Run the cli reconcile from applies operator tie break scenario."""
    _run_case(_case_cli_reconcile_from_applies_operator_tie_break)


def assert_cli_reconcile_reports_no_change_when_regions_agree() -> None:
    """Run the cli reconcile reports no change when regions agree scenario."""
    _run_case(_case_cli_reconcile_reports_no_change_when_regions_agree)


def assert_reconcile_makes_no_change_to_a_dirty_file() -> None:
    """Run the reconcile makes no change to a dirty file scenario."""
    _run_case(_case_reconcile_makes_no_change_to_a_dirty_file)
