...
def run_link_integrity_contract(
    ...
    """Deliver every source-owned link case to the linked test predicate."""
...
def _observe_link_case(root: Path, case: LinkIntegrityCase) -> LinkIntegrityObservation:
    ...
    if case in {LinkIntegrityCase.TEST_INLINE, LinkIntegrityCase.TEST_FENCED}:
        ...
        (root / "doc.md").write_text(
            ...
            if case is LinkIntegrityCase.TEST_INLINE
    ...
    if case in {LinkIntegrityCase.TEST_NON_NAME, LinkIntegrityCase.TEST_NON_PYTHON}:
        root.mkdir(parents=True)
        target = (
            root
            / "tests"
            / (
                "helper.py"
                if case is LinkIntegrityCase.TEST_NON_NAME
                else "test_thing.txt"
            )
        )
        target.parent.mkdir(parents=True)
