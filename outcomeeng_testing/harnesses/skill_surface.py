...
``/pickup`` workflow file plus the spec-tree commands directory to verify
each skill's declared invocation surface. The path constants and the small
frontmatter/context readers live here per ``spx/15-test-infrastructure.pdr.md``
...
PICKUP_WORKFLOW = _SKILLS_DIR / "pickup" / "workflows" / "pickup.md"
...
def file_text(path: Path) -> str:
    """Return authored text for a skill-surface artifact."""
