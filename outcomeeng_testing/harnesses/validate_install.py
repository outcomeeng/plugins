"""Harness-owned installation scenarios for validate_install behavior.
...
from outcomeeng.distribution.codex_cache import CODEX_LIST_COMMAND
...
from outcomeeng_testing.generators.validate_install import (
    MARKETPLACE_NAME,
    ORPHAN_PLUGIN_NAME,
    PLUGIN_NAME,
    PUBLISHED_VERSION,
...
class _CaptureHarness:
    ...
    def readouterr(self) -> _CapturedOutput:
        return _CapturedOutput(out=self.stdout.getvalue(), err=self.stderr.getvalue())
...
def validate_install_scenarios_pass() -> bool:
    for scenario in _scenario_functions():
        _run_scenario_cases(scenario)
    return True


def _scenario_functions() -> tuple[Callable[..., object], ...]:
    return tuple(
        value
...
def _run_scenario_cases(scenario: Callable[..., object]) -> None:
    parametrize_marks = [
        mark
        for mark in getattr(scenario, "pytestmark", ())
    ...
    for mark in parametrize_marks:
        ...
        for values in mark.args[1]:
            case_values = values if isinstance(values, tuple) else (values,)
            _run_scenario(scenario, dict(zip(names, case_values, strict=True)))


def _run_scenario(
    scenario: Callable[..., object], case_kwargs: dict[str, object] | None = None
) -> None:
    signature = inspect.signature(scenario)
    with TemporaryDirectory() as temp_dir:
        tmp_path = Path(temp_dir).resolve()
        capture = _CaptureHarness()
        with pytest.MonkeyPatch.context() as monkeypatch:
            kwargs: dict[str, object] = dict(case_kwargs or {})
...
def test_codex_cache_incomplete_entry_is_an_error(tmp_path: Path) -> None:
    ...
    assert str(CODEX_PLUGIN_MANIFEST) in error
...
def test_codex_reported_versions_forwards_the_marketplace_to_the_cli() -> None:
    ...
    assert captured == [[*CODEX_LIST_COMMAND, MARKETPLACE_NAME]]
