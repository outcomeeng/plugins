...
Each finding carries `file`, `line`, `rule` (the concern name or specific violation), `severity: "reject"`, and `message` (the one-line "why this fails"). Include correct-approach Rust samples and required-changes summary in supplementary markdown appended after the carrier when format is `markdown` or `markdown+json`.
