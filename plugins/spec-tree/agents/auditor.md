...
The wrapper verdict is APPROVED iff `## Open findings` is empty after this run for the language partition; otherwise REJECTED. One verdict per language partition.
...
**State:** `.spx/audits/<language>/<slug>.md`
...
- The wrapper verdict block is emitted to the conversation for every language partition (not just to state files).
...
- Each verdict contains all six concern rows and all three findings sections (Open, Resolved this run, Reopened this run), each with its `(<count>)` header — including `(0)` empty tables on re-runs.
- The wrapper verdict is APPROVED iff `## Open findings` is empty after the run for that language; REJECTED otherwise.
