...
Exposes a stdlib ``argparse`` CLI so the ``/auditing`` skill can drive
...
def _read_frontmatter_branch(path: pathlib.Path) -> str | None:
    ...
    try:
        ...
        return None
    if not content.startswith(STATE_FRONTMATTER_DELIMITER):
        return None
    in_frontmatter = False
    for line in content.splitlines():
        if line.strip() == STATE_FRONTMATTER_DELIMITER:
            if in_frontmatter:
                return None
            in_frontmatter = True
            continue
