...
# Load the canonical verdict module from the sibling script. Direct
# ``import verdict`` works when this file is run as a script (Python
# inserts its directory into sys.path) but not when it is loaded by
# ``importlib.util.spec_from_file_location`` from a test harness. The
# explicit loader below makes both paths work the same way.
_VERDICT_PATH = pathlib.Path(__file__).resolve().with_name("verdict.py")
if "verdict" in sys.modules:
    verdict = sys.modules["verdict"]
else:
    _spec = importlib.util.spec_from_file_location("verdict", _VERDICT_PATH)
    if _spec is None or _spec.loader is None:
        raise RuntimeError(f"Cannot load verdict module from {_VERDICT_PATH}")
