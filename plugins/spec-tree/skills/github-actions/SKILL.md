...
```bash
uv run python "${CLAUDE_SKILL_DIR}/scripts/gh_access.py"
```
...
- NEVER call `gh auth login`, `gh auth switch`, `gh auth refresh`, `gh run rerun`, `gh workflow run`, or `gh run cancel` without an explicit user instruction in the same turn. The user answering an `AskUserQuestion` is explicit instruction.
