...
| `MENTION_REVIEW_NEEDED:<trigger-phrase>` | Auto-review job intentionally skipped (e.g., PR modifies the reviewer's own workflow file, triggering the GitHub Actions "head differs from main" security skip). Post one PR-level comment containing the configured trigger phrase to fire the mention-triggered reviewer (which has no identical-content gate); reschedule the heartbeat to await its findings. |
