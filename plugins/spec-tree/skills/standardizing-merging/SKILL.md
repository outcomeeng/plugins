...
- **Merge command** — rebase merge (`gh pr merge <pr-number> --rebase`) is the universal default; the merge flow runs it unless the overlay opts in to a different command. The overlay may opt in to merge commit (`--merge`) or squash (`--squash`) only with explicit rationale tied to the project's history model — merge commits and squashes are not the agent's choice to make from the gate alone. The overlay also declares whether `--delete-branch` runs inline or as a separate `git push origin --delete <branch>` to avoid multi-worktree cleanup failures.
...
- Merge runs via rebase merge (`gh pr merge --rebase`) unless the overlay declares a different command with explicit rationale — merge commit and squash are overlay opt-ins, not the agent's choice.
