<overview>
Numeric prefixes drive deterministic context loading within each directory. This reference explains how an existing tree is read. It does not decide new structure; use `/decomposing` for child boundaries, ordering evidence, and index assignment.
</overview>
...
Higher-index siblings may depend on the target, but they do not constrain the target. `/contextualizing` lists them in the manifest and does not read their specs while loading target context.
