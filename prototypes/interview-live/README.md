# Live spec-tree surface prototype

A standalone Python 3 stdlib prototype of the browser surface governed by
`spx/16-interfaces.enabler/21-browser.enabler`. It renders a spec-tree projection,
streams agent updates into the browser, and sends browser interactions back to
the agent through the MCP transport.

The prototype lives outside `src/plugins/`. It de-risks transport, rendering, and
state reconciliation before the shipped `interfaces` plugin is created.
...
- The left pane keeps chat and interview questions independently scrollable from
  the spec-tree pane.
- The tree renders SPX projection fields for state, category, and index. The
  current projection lacks node-detail fields for opener text, assertions, and
  evidence links, so the detail pane waits on a richer SPX CLI projection.
...
| `boot.py` and `poll_client.py` | Script-model fallback kept for comparison and focused HTTP testing.                                                                         |
...
## Script-model fallback

The older script path still works for local inspection without an MCP runtime:
