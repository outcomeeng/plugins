...
These encode the browser node's assertions about sourcing structure and derived
state from the SPX CLI's JSON projection: the adapter preserves hierarchy and
order, carries each node's derived state through, and uses the projection slug as
the display title because the projection exposes none. Run:
