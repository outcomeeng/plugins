...
- ALWAYS: one invocation owns the whole readiness attempt up to its four-hour bound; a `not_ready` result is terminal for that attempt and the guarded command does not start ([audit])
