...
- Given normalized host load whose computed interval is longer than the time left before the deadline, when the waiter sleeps, then it sleeps only the time remaining and reaches its deadline exactly ([test](tests/test_host_readiness.scenario.l1.py))
