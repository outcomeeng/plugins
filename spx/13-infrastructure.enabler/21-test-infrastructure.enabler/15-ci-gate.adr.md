...
- NEVER: the `check` job carries a job-level `if:` or `continue-on-error` — a job-level condition could skip the entire gate on `pull_request` or let a failed job report success, defeating the gate as surely as a soft-passed step ([compliance])
