...
- ALWAYS: a gate step fails when the `@outcomeeng/spx` version pinned in the CI workflow is below the marketplace's declared minimum spx version — the lowest version whose capabilities the shipped skills and their tests depend on — so a dependency on an unpinned or unpublished spx capability fails the gate rather than shipping, per [15-ci-gate.adr.md](15-ci-gate.adr.md) ([test](tests/test_spx_version_floor.compliance.l1.py))
