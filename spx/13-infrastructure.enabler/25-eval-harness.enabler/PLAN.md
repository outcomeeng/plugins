...
- **Verdict-format carrier exception for audit-skill mode**: the eval-harness slice emits pure JSON because the assistant message IS the verdict — there is no carrier. Audit skills running in CI deliver their verdict to a PR comment (the only durable cross-CI-run surface) and therefore wrap the JSON in a markdown carrier with HTML-comment delimiters. The PDR (`spx/15-audit-verdict-format.pdr.md`) overreached during the JSON flip by prohibiting markdown fences in absolute terms; it gets refined in the next-session work tracked at [`spx/21-spec-tree.enabler/65-auditing.enabler/PLAN.md`](../../21-spec-tree.enabler/65-auditing.enabler/PLAN.md). The eval-harness assertions in `eval-harness.md` are correct for the pure-JSON mode the harness uses; no change is needed here once the PDR refinement clarifies that the no-carrier model is the slice's case, not a universal rule.
