...
`work/eval-evidence-hygiene` is a candidate extraction branch for eval-runner hygiene. Before opening a pull request, compare its current 19-path diff with `origin/work/strict-finding-disposition` by behavior and establish which producer-coupled eval capability belongs to this node.

Keep only eval definition, runner, fixture, and test-infrastructure changes required to prove one eval-harness behavior. Audit or review policy that merely consumes the capability stays in the owning audit or reviewing node.

**Revisit condition:** replace this paragraph with the resulting PR reference after the branch has one independent verification and rollback story, or remove it when comparison proves the branch unrelated to the preserved aggregate.
