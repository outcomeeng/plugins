...
PROVIDES checkout-bounded reading of each coding-agent runtime's installed plugin set
SO THAT marketplace install verification and the isolated installation harness
CAN confirm which plugins a runtime reports installed without misreading a changed CLI contract as an empty or stale set

Install verification and the isolated installation harness read a runtime's installed plugin set through that runtime's own CLI. The read is bounded to observation: it reports what a runtime holds and never reconciles, repairs, or refreshes a developer's user-scope marketplace registrations, plugin caches, or agent directories, per `spx/12-marketplace-state.adr.md`.
