...
| The CI `spec-tree-review` duplicates the local review the author already ran | The duplication is deliberate and per-push: the local review is the author-side gate run before every push (not observable from the PR); the CI review is the observable `MERGE_READINESS` predicate any independent reader can verify after each push. Pairing them keeps the same reviewing kind on both sides of every push |
