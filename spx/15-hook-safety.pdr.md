...
Every hook any marketplace plugin ships, on every agent surface, observes or injects context on a non-blocking event only — it can never deny, block, or stall an agent's forward progress. A hook command is a self-contained inline guard whose reachable floor is a valid empty result, depends on no shipped script file resolving at a substituted path, treats any external command it uses as an optional dependency it probes before use, and carries an explicit short timeout and an environment kill switch. The guarantee binds at build time: every version the product builds — authored source rendered into both generated runtime trees — carries only hooks that satisfy this contract, so no version the product ships can trap a session.

## Rationale

A hook the agent consults before an action can deny that action, so a hook on a blocking-capable event turns any hook failure into a wall the agent cannot pass — and the failure that matters happens beyond the hook script's reach. A command that invokes a script by its resolved path fails the instant that path drifts: a pinned plugin cache, a changed plugin root, or a missing scripts directory leaves the script absent, and on a blocking event that absence denies the action before any in-script fail-open can run. Only two properties together make a hook structurally unable to trap a session: it fires on a non-blocking event, where the agent treats any failure as advisory and continues; and its command carries its own inline floor, so the worst case it can produce is a valid empty result rather than an error. Methodology enforcement belongs in skills and the spx CLI — surfaces the agent invokes explicitly and the operator can decline — never in a hook the agent runs implicitly. An external CLI a hook consults is therefore optional: probed for existence and executability and resolved from the environment or PATH, never a version-pinned cache path, with every failure wrapped to the floor. The contract binds each version at the moment it is built: a shipped version is immutable, so the product never reaches back to alter, collapse, or withdraw a version already distributed — the agent owns the lifecycle of the versions it has cached or pinned. Forward enforcement is sufficient, because the gate blocks any version that violates the contract from shipping at all, and a version that satisfies it cannot later become unsafe.

## Product properties

1. Every shipped hook is observe-or-inject-only on a non-blocking event; on no agent surface can a hook deny, block, defer, or stall an action.
...
- ALWAYS: every shipped hook entry declares an explicit timeout, never inheriting the agent's multi-minute default ([compliance])
