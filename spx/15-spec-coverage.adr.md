...
Classification follows the behavior under assertion. Deterministic CLI state carries `[test]` evidence when an LLM only provides its readout; model-generated behavior carries `[eval]` evidence, including when a CLI exposes it. The readout reports existing state without interpreting its meaning or deciding the verdict, and the test compares structured observations against independently established expectations.
...
- ALWAYS: use `[test]` evidence for assertions about deterministic executable behavior, including CLI state exposed through an LLM used solely as a readout; compare structured observations against independent expectations, with the test owning the verdict ([audit])
