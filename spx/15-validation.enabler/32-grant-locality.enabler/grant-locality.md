...
- Given a `SKILL.md` whose frontmatter grants are local and whose body writes an escaping `allowed-tools` declaration at column zero, when the validator scans it, then it reports nothing and exits zero ([test](tests/test_grant_locality.scenario.l1.py))
...
- ALWAYS: the validator reads grants only from an `allowed-tools` field declaration inside the file's opening frontmatter, so a skill body that quotes or documents an escaping grant — including at column zero, the spelling a standard uses to show the shape it prohibits — is not itself a violation ([test](tests/test_grant_locality.compliance.l1.py))
