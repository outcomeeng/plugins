...
- NEVER: the validator passes a reference that points into this marketplace's own files — a spec-tree node or decision named by its sibling-local numeric prefix with real digits (`spx/\d+-…`, such as `spx/15-validation.enabler/…` or `spx/13-plugin-and-runtime-conventions.adr.md`), or a path under one of this marketplace's own roots (`src/plugins/`, `dist/claude/`, `dist/codex/`, or an `outcomeeng` toolchain package such as `outcomeeng/validation/…` or `outcomeeng_testing/…`, caught even inside an absolute checkout path) — because none resolves in a consumer checkout ([test](tests/test_reference_portability.compliance.l1.py))
- ALWAYS: the validator passes a reference a consumer checkout resolves — a generic placeholder (`spx/{…}`) or the index-placeholder sentinel (`spx/NN-…`) standing in for an illustrative node, a methodology-universal tree path every consumer holds (a non-numbered `spx/…` such as `spx/EXCLUDE`, `spx/CLAUDE.md`, `spx/local/…`, `spx/sessions/`), a universal source-tree convention outside the marketplace roots (a bare `src/…` or `dist/…` such as `src/index.ts`), the marketplace's own GitHub org/repo slug (`outcomeeng/plugins`, `outcomeeng/spx`), and the plugin's own files via `${CLAUDE_SKILL_DIR}` or `${CLAUDE_PLUGIN_ROOT}` ([test](tests/test_reference_portability.compliance.l1.py))
