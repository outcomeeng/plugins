...
The set of valid SKILL.md frontmatter fields is the union of the fields enforced by a vendored copy of Anthropic's `quick_validate.py` (Apache 2.0), which defines the Agent Skills open-standard floor, and a hand-curated allowlist of Claude Code-specific fields maintained in this marketplace. A skill is valid when the vendored validator accepts it, or when the only unexpected keys it reports are in the Claude Code allowlist and all other standard-field format rules pass. A validation wrapper delegates open-standard field checking to the vendored validator, then consults the curated allowlist for any fields the vendored validator flagged; the vendored script and its license file are co-located under a vendor directory and are not modified in place.

## Rationale

A hardcoded field list drifts from reality the moment Anthropic tightens or extends the open standard. Scanning an opaque CLI binary for field patterns is unreliable; a public, versioned validator script is the correct contract. Using only the vendored script rejects `disable-model-invocation`, `argument-hint`, `hooks`, and other fields Claude Code accepts and this marketplace uses. Vendoring the upstream validator plus a small allowlist gives a single stable source of truth for the open standard with an explicit, reviewable extension for Claude Code-specific fields; updates arrive by re-pulling the upstream file, and the allowlist is audited whenever a new Claude Code field appears.
...
- ALWAYS: maintain the Claude Code-specific field allowlist in the wrapper, not in the vendored script — separation keeps upstream syncs mechanical ([audit])
