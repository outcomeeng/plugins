...
- Given a SKILL.md with a description containing angle brackets, when validated, then an error is reported referencing the description rule ([test](tests/test_skill_frontmatter.scenario.l1.py))
- Given a SKILL.md with no frontmatter, when validated, then an error is reported referencing the missing frontmatter ([test](tests/test_skill_frontmatter.scenario.l1.py))
- Given a file that is not named SKILL.md, when passed to the validator, then it is skipped ([test](tests/test_skill_frontmatter.scenario.l1.py))
