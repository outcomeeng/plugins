...
- ALWAYS: every import statement form — `import a.b`, `from a.b import c` naming a submodule, `from a.b import c` naming an attribute, `from . import c`, and `from .c import d` — resolves to the test-infrastructure module names it depends on: the imported module and every package on its dotted path, plus the submodule when the imported name resolves to one ([mapping])
- ALWAYS: reach closes transitively over test-infrastructure modules before it is projected onto executed tests — for any chain of test-infrastructure modules in which each imports the next, an executed test importing the first reaches every module in the chain ([property])
