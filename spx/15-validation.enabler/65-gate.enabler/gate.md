...
- Given the `validation` primitive recipe and all preflight and recipe steps exiting 0, when the orchestrator runs, then preflight runs inside the orchestrator before validation steps, each step writes combined stdout/stderr to a temp log while it runs, each completed step removes its log and prints a `PASS  {label}  {elapsed}s` status line without printing captured child output, the run summary records the recipe name, the recipe's declared verification type and purpose, phase, commands, statuses, durations, and exit codes without discarded log paths, and the orchestrator exits 0 ([test](tests/test_gate.scenario.l1.py))
- Given the `test` primitive recipe and all preflight and recipe steps exiting 0, when the orchestrator runs, then preflight runs inside the orchestrator before the pytest-backed `[test]` step, the run summary records the recipe name, the recipe's declared verification type and purpose, phase, commands, statuses, durations, and exit codes without discarded log paths, and the orchestrator exits 0 ([test](tests/test_gate.scenario.l1.py))
...
- ALWAYS: the primitive recipe command surface aligns with its declared contract — `validation` includes `fmt-check` (`dprint check`), `ruff format --check`, `ruff check`, `mypy --strict`, `pyright`, `spx validation markdown`, and hook safety while excluding pytest-backed `[test]` evidence; `test` executes only the configured pytest-backed `[test]` step; the full wrapper composes both primitive recipes in that order ([audit])
- ALWAYS: primitive recipe metadata aligns with the verification taxonomy — `validation` declares `verification_type: validation` and `purpose: conformance`, while `test` declares `verification_type: testing` and `purpose: correctness` ([audit])
