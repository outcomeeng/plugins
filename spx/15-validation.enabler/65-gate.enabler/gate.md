...
- ALWAYS: the declared step list includes a `ruff check` step and a `spx validation markdown` step — `spx/ISSUES.md` records prior lint and link drift attributed to their absence from `just check` ([test](tests/test_gate.compliance.l1.py))
