...
- ALWAYS: the `validation` primitive recipe includes a `fmt-check` (`dprint check`) step, a `ruff format --check` step, a `ruff check` step, a `mypy --strict` package step, a `pyright` package step, a `spx validation markdown` step, and an evidence-link integrity step, and excludes the pytest-backed `[test]` evidence step ([test](tests/test_gate.compliance.l1.py))
