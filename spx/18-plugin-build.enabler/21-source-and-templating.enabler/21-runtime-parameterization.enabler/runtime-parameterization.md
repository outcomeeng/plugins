...
- ALWAYS: a registry-backed token renders the build target's name for its capability from the source-owned per-runtime registry of its token kind — `tool('ask_user')` emits `AskUserQuestion` for the Claude target and `request_user_input` for the Codex target, and the `field(…)`, `term(…)`, and `file(…)` tokens render their own kind's registry the same way — `file('spx_guide')` emits `spx/CLAUDE.md` for the Claude target and `spx/AGENTS.md` for the Codex target ([test](tests/test_runtime_parameterization.compliance.l1.py))
