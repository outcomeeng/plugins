...
- NEVER: a runtime-divergent unique token registered as a capability's per-runtime name (for example `AskUserQuestion` or `ScheduleWakeup`) appears in a guarded plugin's authored `src/plugins/` content (including text pulled in via an include) outside a registry token or a per-runtime conditional — the source-layer guard fails the build ([test](tests/test_runtime_parameterization.compliance.l1.py))
