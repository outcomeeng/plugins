...
CAN write one source that renders each tool name, field name, concept term, and per-runtime filename into every target agent's native surface without privileging one runtime.
...
- ALWAYS: configuration examples and descriptions select a complete central
  profile and render the destination harness's native configuration from its
  owner; changing that profile propagates to every consumer in both generated
  trees without an authored-source change ([audit]).
- NEVER: a template independently selects model and reasoning values or supplies
  a native configuration override ([audit]).
- ALWAYS: a registry-backed token renders the build target's name for its capability from the source-owned per-runtime registry of its token kind — `tool('ask_user')` emits `AskUserQuestion` for the Claude target and `request_user_input` for the Codex target, and the `field(…)`, `term(…)`, and `file(…)` tokens render their own kind's registry the same way — `file('root_guide')` emits `CLAUDE.md` for the Claude target and `AGENTS.md` for the Codex target ([test](tests/test_runtime_parameterization.compliance.l1.py))
