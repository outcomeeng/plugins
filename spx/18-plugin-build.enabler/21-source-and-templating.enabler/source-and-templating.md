...
- Given an included fragment whose body itself contains an `{!% include %!}` or `{!% require_skill %!}` directive, when the source renders, then the nested directive is expanded too — rendering re-processes an inlined body so directives resolve at any nesting depth ([test](tests/test_render_text.scenario.l1.py))
