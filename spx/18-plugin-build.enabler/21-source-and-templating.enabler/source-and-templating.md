...
- ALWAYS: source-tree validation treats a skill directory holding no file the emission walk's authored-source predicate accepts as absent, whether it holds only `__pycache__` files or no file at all; a skill directory holding an authored file without `SKILL.md` fails validation ([test](tests/test_source_and_templating.compliance.l1.py))
...
- ALWAYS: `{!% require_skill 'plugin:skill' %!}` expands to the owned guidance sentence carrying the named skill reference, identically in both targets — full sister-skill content stays in its own skill ([test](tests/test_source_and_templating.compliance.l1.py))
- ALWAYS: the owned guidance sentence is ``Use skill `{plugin}:{skill}`.`` — the agreement between this declaration and the build's guidance template is audit evidence ([audit])
