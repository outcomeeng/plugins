...
- ALWAYS: `{!% require_skill 'plugin:skill' %!}` expands to identical coding-agent-neutral invocation text in both targets, and an unavailable required skill is reported as terminal — the generated instruction stops instead of substituting another workflow; full sister-skill content stays in its own skill ([test](tests/test_source_and_templating.compliance.l1.py))
