...
- ALWAYS: `{!% require_skill 'plugin:skill' %!}` expands to the exact instruction ``Use skill `{plugin}:{skill}`.`` in both targets — full sister-skill content stays in its own skill ([test](tests/test_source_and_templating.compliance.l1.py))
