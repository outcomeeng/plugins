...
- ALWAYS: target-specific frontmatter fields (`disable-model-invocation`, `argument-hint` for Codex) are absent from a target that does not consume them, while portable skill capability fields such as `allowed-tools` appear in both generated runtime trees ([test](tests/test_target_emission.compliance.l1.py))
