...
- Claude Code tool allowlists that omit web-capable tools map to `web_search = "disabled"`, while allowlists that include `WebSearch` leave web search unset for runtime defaults ([test](tests/test_agents.mapping.l1.py))
- Claude Code read-only tool allowlists map to `sandbox_mode = "read-only"` when no explicit `permissionMode` is present; write-capable allowlists and explicit unmapped `permissionMode` values leave sandbox mode unset for runtime defaults plus manual-review guidance ([test](tests/test_agents.mapping.l1.py))
