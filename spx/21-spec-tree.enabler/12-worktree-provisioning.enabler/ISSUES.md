...
**Evidence**: in a pool provisioned at `/Users/shz/Code/outcomeeng/plugins/plugins.git`, `git config --show-origin --get-all core.bare` reports `file:/Users/shz/Code/outcomeeng/plugins/plugins.git/config	true` from a linked worktree, `git config --get extensions.worktreeConfig` is empty, and `git -c core.bare=false rev-parse --is-inside-work-tree` answers `true` where the unmodified command answers `false`. Observed across the pool's linked worktrees while running the merge lifecycle for PR #524.
