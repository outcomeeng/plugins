...
- ALWAYS: `init-worktrees` keeps the prior-checkout removal the operator's action — the skill itself runs only the classification, provisioning, and remote-check commands and never the removal, and after emitting the removal command it blocks on an explicit operator confirmation before re-classifying, never reporting the layout complete until the operator confirms the prior checkout is removed ([audit])
