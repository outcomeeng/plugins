...
- Given a `SessionStart` payload and `spx` resolvable on `PATH`, when the shipped hook command runs, then the `spx` hook runner delivers the session environment — `CLAUDE_SESSION_ID` and a worktree-claim indicator reach `$CLAUDE_ENV_FILE` and a worktree-occupancy claim is recorded under the project's `.spx/worktrees/` ([test](tests/test_agent_environment.scenario.l1.py))
