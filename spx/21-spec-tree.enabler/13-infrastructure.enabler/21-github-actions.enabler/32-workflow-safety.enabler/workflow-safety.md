...
- ALWAYS: pin third-party actions by commit SHA in `uses:` references — version tags are mutable and supply-chain attacks publish under existing tags; `outcomeeng/gh-actions` beta-test callers may track `@main` only when the caller is under the same trust boundary and carries an explicit `# BETA TESTER:` marker naming the risk ([test](tests/test_workflow_safety.compliance.l1.py))
