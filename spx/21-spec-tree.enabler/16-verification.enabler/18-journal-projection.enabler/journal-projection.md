...
- Given a sealed review run token, when the review-run inspection helper renders it, then it reads the review journal prefix through `spx journal render --type review --run <token>`, resolves a not-found current-scope miss through `spx journal list --type review` and re-renders with the listed branch slug, and prints the run token, terminal status, full head/base identity, scope coverage, finding counts, and any findings from the shared projection, so the manager can inspect the raw `changes-reviewer` token without hand-reading JSON ([test](tests/test_render_review_run.scenario.l1.py))
