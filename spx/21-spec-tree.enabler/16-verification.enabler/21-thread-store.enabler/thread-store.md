...
- ALWAYS: `branch_slug` is the symbol re-exported from `plugins/spec-tree/skills/scope-changeset/scripts/changeset_scope.py` — slug derivation lives in exactly one function source ([test](tests/test_thread_store.compliance.l1.py))
- ALWAYS: every CRUD CLI under `plugins/spec-tree/skills/manage-thread-store/scripts/` performs its filesystem effects through `thread_store.get_backend()` — no CLI calls `open()`, `pathlib.Path.write_*`, `os.remove`, or any other direct filesystem primitive ([test](tests/test_cli.compliance.l1.py))
...
- ALWAYS: every CRUD CLI under `plugins/spec-tree/skills/manage-thread-store/scripts/` makes its `--slug` argument optional and derives the slug from `thread_store.current_slug()` when omitted — agents and operators do not name the on-disk addressing scheme to address the current branch's thread ([test](tests/test_cli.compliance.l1.py))
- NEVER: a script under `plugins/spec-tree/skills/manage-thread-store/scripts/` imports a third-party package, depends on `uv` at runtime, or imports any `outcomeeng_*` module — stdlib only, per the Plugin Portability Constraints in `AGENTS.md` and `spx/21-spec-tree.enabler/16-verification.enabler/21-thread-store.enabler/21-backend-abstraction.adr.md` ([test](tests/test_plugin_portability.compliance.l1.py))
- NEVER: a verification skill imports a backend module directly — every executable read and write routes through the `thread_store` facade per `spx/21-spec-tree.enabler/16-verification.enabler/21-thread-store.enabler/21-backend-abstraction.adr.md` ([test](tests/test_plugin_portability.compliance.l1.py))
- NEVER: a wrapper agent instructs the runtime to access a concrete backend directly — wrapper-agent persistence guidance routes through the `thread_store` facade per `spx/21-spec-tree.enabler/16-verification.enabler/21-thread-store.enabler/21-backend-abstraction.adr.md` ([audit])
