...
- NEVER: a script under `plugins/spec-tree/skills/thread-store/scripts/` imports a third-party package, depends on `uv` at runtime, or imports any `outcomeeng_*` module — stdlib only, per the Plugin Portability Constraints in `AGENTS.md` and `spx/21-spec-tree.enabler/16-verification.enabler/21-thread-store.enabler/21-backend-abstraction.adr.md` ([test](tests/test_plugin_portability.compliance.l1.py))
