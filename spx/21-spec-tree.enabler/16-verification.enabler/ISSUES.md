...
`spx/21-spec-tree.enabler/16-verification.enabler/13-run-journal.adr.md` and `verification.md` declare the append-only run-journal contract in atemporal voice — the present-tense product truth, not a conditional. Truth flows down, so the lower layers that have not yet migrated are currently non-conformant by design, not defective: the shipped `review-changes` skill still persists through thread-store's CRUD-overwrite facade; `spx/21-spec-tree.enabler/17-auditing.adr.md` and `spx/21-spec-tree.enabler/16-verification.enabler/15-verdict-toolchain.enabler` still encode the end-of-run verdict-toolchain and `.spx/audits` model. An audit of these surfaces against the new contract reports the migration in flight, expected per the durable-map future-product-truth rule — not a defect to fix in place. The migration itself is tracked in this node's `PLAN.md`.

**Unblocked: `@outcomeeng/spx@0.6.0` is published and ships `spx journal`** (`open`/`append`/`read --from <cursor>`/`seal`/`render`, CloudEvents events, the verification kind an opaque `<type>` segment). The repo floor (`REQUIRED_SPX_VERSION`) and CI `SPX_VERSION` pin still sit at `0.5.6` and must advance to `0.6.0` in the first channel-binding change. The migration is now executable per `PLAN.md`.

**Direction correction — `spx journal` makes the Python verdict toolchain redundant.** `spx journal render` is the projection renderer and CloudEvents is the event schema, so `15-verdict-toolchain.enabler` (`verdict.py` + the four CLIs) is to be **removed**, not reframed as "the projection." PR #300 (`7ab9ba66`) did the wrong thing — it preserved the Python, reworded `verdict-toolchain.md` to call it "the verdict projection a sealed journal renders," and renamed its tests `scenario`→`compliance`; it changed no behavior and must be reverted or superseded by the removal. The spec assertions are NOT weakened with a "contingent on X" qualifier — this note carries the timeline.
