...
`spx/21-spec-tree.enabler/16-verification.enabler/13-run-journal.adr.md` and `verification.md` declare the append-only run-journal contract in atemporal voice — present-tense product truth, not a conditional. Truth flows down, so the lower layers that have not yet migrated are non-conformant by design, not defective: the shipped `review-changes` skill still persists through thread-store's CRUD-overwrite facade, and `spx/21-spec-tree.enabler/17-auditing.adr.md` and `spx/21-spec-tree.enabler/16-verification.enabler/15-verdict-toolchain.enabler` still encode the end-of-run verdict-toolchain and `.spx/audits` model. An audit of these surfaces against the contract reports the migration in flight — expected per the durable-map future-product-truth rule, not a defect to fix in place. The migration is tracked in this node's `PLAN.md`.
