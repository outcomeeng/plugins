...
`spx/21-spec-tree.enabler/16-verification.enabler/13-run-journal.adr.md` and `verification.md` declare the append-only run-journal contract in atemporal voice — the present-tense product truth, not a conditional. Truth flows down, so the lower layers that have not yet migrated are currently non-conformant by design, not defective: the shipped `review-changes` skill still persists through thread-store's CRUD-overwrite facade; `spx/21-spec-tree.enabler/17-auditing.adr.md` and `spx/21-spec-tree.enabler/16-verification.enabler/15-verdict-toolchain.enabler` still encode the end-of-run verdict-toolchain and `.spx/audits` model. An audit of these surfaces against the new contract reports the migration in flight, expected per the durable-map future-product-truth rule — not a defect to fix in place. The migration itself is tracked in this node's `PLAN.md`. It is gated on the `spx journal` channel — the single type-agnostic run-journal surface (`open`/`append`/`read --from <cursor>`/`seal`/`render`) the contract binds, where the verification kind is an opaque `<type>` scope segment. That channel is **unpublished**: it lives on the `spx` `work/journal` branch, unmerged, and the installed `spx 0.5.6` (the `REQUIRED_SPX_VERSION` floor and the CI `SPX_VERSION` pin) ships only the now-removed `spx audit init|progress|close|status|list` lifecycle. Per the published-capability rule, the migration is unblocked only by the forthcoming spx-journal release — an `@outcomeeng/spx` publication that ships `spx journal`, with `REQUIRED_SPX_VERSION` advanced and the CI pin bumped above `0.5.6`. The `spx` merge is not gated on this plugin work (this repo invokes the channel in zero executable paths), so `spx` publishes `spx journal` first and this migration adopts it after, per `PLAN.md`. The spec assertions are NOT weakened with a "contingent on X" qualifier — that would inject temporal hedging the atemporal-voice rule forbids; this note carries the timeline instead.
