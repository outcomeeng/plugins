...
- The `audit-orchestrator` stateful mode. Cross-run resolved/reopened (a finding fixed across commits) reads across the scope's sealed runs through the channel's cross-run access, decided in `spx/21-spec-tree.enabler/16-verification.enabler/14-cross-run-access.adr.md`. **This migration is blocked until `@outcomeeng/spx` publishes that cross-run access operation and the floor (`REQUIRED_SPX_VERSION`) advances to the publishing version**: `spx@0.6.0` (the published version and the floor) exposes only per-run journal verbs (`open`/`append`/`read --run`/`seal`/`render --run`) with no run-set enumeration, so the consumer cannot read across runs without it, and per `AGENTS.md` the consumer may depend on the capability only once it is published and the floor advanced. Reading run files under `.spx/branch/<slug>/<type>/runs/` directly, or keeping a separate run index, is forbidden by `14-cross-run-access.adr.md` and `13-run-journal.adr.md`.
- The `pr-reviewer` / `pr-review-orchestrator` PR-thread mode, including the human-readable PR verdict and its delivery (Channel reality above). Its prior-run state is the same cross-run access (`14-cross-run-access.adr.md`) over the pull-request backend's run history rather than parsing the rendered PR comment, so it carries the same `spx` cross-run-access dependency.
