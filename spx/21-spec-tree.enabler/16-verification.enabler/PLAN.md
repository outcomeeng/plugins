...
- The `audit-orchestrator` stateful mode. Cross-run resolved/reopened (a finding fixed across commits) is the open design here — a single journal run is one commit's events, so cross-run classification reads across runs.
