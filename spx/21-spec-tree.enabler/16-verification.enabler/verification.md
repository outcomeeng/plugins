...
- ALWAYS: every skill-owning wrapper agent under `src/plugins/spec-tree/agents/` declares an explicit non-inherited model and a non-empty required skill set, and target conversion emits the mapped native model plus enabled skills in source order ([test](tests/test_wrapper_agent_configuration.compliance.l1.py))
- ALWAYS: a thin wrapper agent under `src/plugins/spec-tree/agents/` drives each agentic verification skill and holds no verification or I/O policy ([audit])
