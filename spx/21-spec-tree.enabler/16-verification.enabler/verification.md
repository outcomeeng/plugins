...
Each rule enforces a guarantee of `spx/21-spec-tree.enabler/16-verification.enabler/13-run-journal.adr.md` or `spx/21-spec-tree.enabler/16-verification.enabler/14-cross-run-access.adr.md`.
...
- ALWAYS: a cross-run projection reads across the scope's sealed runs — an open run is excluded until its terminal seal — through the one backend-neutral channel, computing resolved and reopened by folding the run set with no state held outside the journal ([audit])
...
- NEVER: a consumer reads across runs by enumerating backend storage paths directly or maintaining a run index outside the journal ([audit])
