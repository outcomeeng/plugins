...
- ALWAYS: when sync-base reports `dirty_tree` during context loading, abort before reading product truth; route the tracked changes through `/commit-changes`, re-run sync-base, and restart context loading for the same target, so every emitted context marker comes from a clean checkout whose currency is established ([audit])
- NEVER: emit a context marker after sync-base reports `dirty_tree` or `git_failure` — stale or unverified checkout state cannot produce authoritative product context ([audit])
