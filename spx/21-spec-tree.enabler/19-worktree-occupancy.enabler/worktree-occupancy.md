...
- NEVER: `/handoff` releases the running worktree's live claim through `spx worktree release`; it archives or rewrites session documents and frees the Git branch by stepping off it, while the live process keeps its occupancy claim until a later claim replaces it or liveness marks it free ([audit])
