...
- NEVER: a plugin reads, writes, or removes a `.spx/worktrees/` claim by direct filesystem access — occupancy is reached only through the `spx` CLI (`spx worktree status`/`release` from the `/pickup` and `/handoff` skills, and the claim that `spx hook run session-start` records from the `SessionStart` hook) ([audit])
