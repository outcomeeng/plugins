...
- NEVER: the wrapper agent reads or writes files under the thread-store backend's storage paths directly — every read and write routes through the script chain, per the shared verification contract in `spx/21-spec-tree.enabler/32-evidence.enabler/21-verification.enabler/verification.md` ([eval](evals/wrapper-protocol/eval.toml))
