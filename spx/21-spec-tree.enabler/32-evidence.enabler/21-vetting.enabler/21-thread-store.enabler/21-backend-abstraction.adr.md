...
This decision governs how thread-store persists branch-scoped vetting records across multiple operating modes — the developer's worktree, CI runners, hosted services — without coupling the consuming lens or its wrapper agent to the chosen storage surface.

## Context

**Business impact:** Vetting lenses produce records (machine-readable JSON, human-readable markdown) that outlive the agent invocation. The developer running a lens locally wants those records under their worktree, gitignored. The same lens, run from CI, wants the records under a runner-cacheable path that survives across job invocations. A hosted-service deployment wants them in a remote object store. A persistence layer that fixes one storage surface forces N parallel lens implementations or N copies of every lens's I/O logic.
...
- The canonical slug function takes a branch name and an optional state-directory path; the state-directory argument participates in collision disambiguation when an existing state file records a different branch under the same base slug. Lens code that does not maintain audit-state files passes the argument as `None` and gets the same length-bounded, path-safe slug as the audit-state code path.
- Lens skills are language-agnostic at the prose level; the wrapper agents invoke CLIs and never branch on backend identity.
...
Env-var-driven selection over skill-argument-driven keeps the lens unaware of the backend. A lens skill that knows it's running against filesystem-vs-cache would either branch in prose (drifts across lenses) or thread a `--backend` flag through every CLI call (every lens reimplements the plumbing). Env var resolved once inside `thread_store.get_backend()` keeps the lens definition single-shape.

Alternatives rejected:

- **Hardcode filesystem with no abstraction.** Adding a second backend then requires re-authoring every lens's persistence calls or introducing a parallel surface. The cost of one Protocol class plus one env-var lookup stays much smaller than the cost of that re-authoring across every lens.
- **Each lens owns its persistence.** Backend swap becomes N-place change. Slug-rule drift becomes inevitable. The cross-lens contract in `spx/21-spec-tree.enabler/32-evidence.enabler/21-vetting.enabler/vetting.md` exists specifically to prevent this.
...
| -------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| One indirection layer between every lens I/O call and storage                                      | The interface is small (write, read, delete, list, thread_path); the facade is a single function call; performance cost is negligible relative to the LLM-emit step every lens already performs           |
| Lens cannot observe which backend is in play                                                       | Lenses have no legitimate reason to know; any genuinely backend-specific operation lives behind a CRUD method whose semantics every backend honors uniformly                                              |
...
| Env-var selection is invisible to a lens reader inspecting just the skill prose                    | The wrapper agent's prose names the env var and the default; the lens itself doesn't reference backend selection at all, which is the desired separation of concerns                                      |
...
A conformant backend module implements every method declared by the `Backend` protocol in `plugins/spec-tree/skills/thread-store/scripts/backend.py`. The `thread_store` facade selects backends via `SPX_VET_BACKEND` and defaults to `local`. Every CRUD CLI under `plugins/spec-tree/skills/thread-store/scripts/` invokes the facade and never touches the filesystem directly. The slug helper at `plugins/spec-tree/skills/thread-store/scripts/branch_slug.py` re-exports the function from `plugins/spec-tree/skills/auditing/scripts/audit_orchestrator.py` via `importlib` without redefinition.
...
- Backend selection runs through `SPX_VET_BACKEND` resolved inside `thread_store.get_backend()` — neither a lens skill, a wrapper agent, nor any CLI argument selects the backend ([review])
...
- A lens skill or wrapper agent imports a backend module directly (`fs_backend` or any other concrete backend module) — every read and write routes through the `thread_store` facade ([test](tests/test_plugin_portability.compliance.l1.py))
