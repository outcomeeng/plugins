...
- Given `SPX_VET_BRANCH` set in the environment, `thread_store.current_slug()` returns `branch_slug(<env value>)` and the CRUD CLIs invoked without `--slug` address that derived slug ([test](tests/test_cli.scenario.l1.py))
- Given `SPX_VET_BRANCH` unset AND `git symbolic-ref --short HEAD` resolves to a branch name, `thread_store.current_slug()` returns `branch_slug(<git branch>)` and the CRUD CLIs invoked without `--slug` address that derived slug ([test](tests/test_cli.scenario.l1.py))
- Given the repository is on a detached HEAD AND `SPX_VET_BRANCH` unset, `thread_store.current_slug()` raises a structured error naming the `SPX_VET_BRANCH` override and the CRUD CLIs invoked without `--slug` exit non-zero with the same message ([test](tests/test_cli.scenario.l1.py))
- Given `git` is not on `PATH` AND `SPX_VET_BRANCH` unset, `thread_store.current_slug()` raises a structured error identifying git as the missing dependency and the CRUD CLIs invoked without `--slug` exit non-zero with the same message ([test](tests/test_cli.scenario.l1.py))
