...
- A pull-request review against a `base_ref` requires a real git diff, which the orchestrator computes once per run; the model never derives diffs from text descriptions.
...
- `compute_diff.py` — orchestration helper. Reads `pr.json` from the thread store (via `read_record.py` invocation or by delegating to the facade), extracts `baseRefName`, runs `git diff <base_ref>..HEAD` via `subprocess.run`, writes the diff to stdout.
...
**Why `compute_diff` reads `pr.json` rather than taking `base_ref` directly.** The wrapper agent operates on a branch's PR thread; the canonical record of the branch's intended base is the `pr.json` document the orchestrating workflow seeded into the thread store. A `--base-ref` flag would let the agent pass a value out of step with the persisted record; reading from the thread store closes that gap. The script extracts `baseRefName` because that is the field `gh pr view --json baseRefName` (and the test harness `make_pr_json` factory) writes.
...
- `compute_diff.py` reads `pr.json` through the `thread_store` facade, extracts `baseRefName`, and runs `git diff <base_ref>..HEAD` via `subprocess.run` — every filesystem effect routes through `thread_store`, and the diff command line is the only `subprocess` invocation in the script ([test](tests/test_skill_orchestration.scenario.l2.py))
