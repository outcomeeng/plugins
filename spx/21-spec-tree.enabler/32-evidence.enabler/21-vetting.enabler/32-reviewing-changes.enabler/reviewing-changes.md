...
- ALWAYS: the swappable render templates live under `plugins/spec-tree/skills/reviewing-changes/references/render/` as one markdown file per render section (`document.md`, `finding-blocking.md`, `finding-followup.md`, `no-blockers.md`, `acknowledgements.md`) and `render_review.py` loads them via stdlib `string.Template` — the rendered surface is data the script substitutes, not f-string concatenation in code ([test](tests/test_reviewing_changes.compliance.l1.py))
- ALWAYS: `render_review.py` maps finding `severity` to four-class render labels (`must_fix` → BLOCKING, `suggestion` → FOLLOW-UP, `nit` → FOLLOW-UP) and emits the `no-blockers.md` content when no BLOCKING finding is present — the rendered shape matches the GH `spec-tree-review` workflow's four-class taxonomy ([test](tests/test_skill_orchestration.scenario.l2.py))
