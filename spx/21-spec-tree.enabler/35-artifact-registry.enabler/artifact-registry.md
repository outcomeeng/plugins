...
- NEVER: the implementation audit's orchestration, its wrapper agent, or the instruction-block generator carries its own list of kinds, artifacts, or extensions, or its own reader of the rendered registry — each reaches the provider skill's reader by import ([audit])
