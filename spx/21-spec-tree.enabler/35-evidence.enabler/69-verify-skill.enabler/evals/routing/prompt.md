...
src/plugins/spec-tree/skills/verify/SKILL.md
dist/claude/spec-tree/skills/verify/SKILL.md
dist/codex/spec-tree/skills/verify/SKILL.md
-->
...
- **probe** — record the `[probe](probes/{probe-slug}/probe.md)` link with routing status `routed` when a probe-authoring specialist in the runtime skill catalog produces the protocol; when none is installed, preserve the probe evidence shape and report `capability-required` with the subject and the protocol path. Never author a protocol or attest a run inside `/verify`.
