...
- ALWAYS: treat a specified node's missing-owner RED result as authoring state — record the exact owner, focused diagnostics, and relative `spx/EXCLUDE` entry; defer test-evidence audit until implementation exists, remove the exclusion when implementation begins, pass normal deterministic gates, and audit the complete production chain before implementation audit ([audit])
...
- NEVER: approve test evidence while its assertion-relevant production owner is absent — test shape and RED diagnostics cannot substitute for an inspectable production path ([audit])
