...
- ALWAYS: `/testing` is the single authority that selects an assertion's evidence type — one of scenario, mapping, conformance, property, compliance — from the shape of the claim it proves ([review])
- NEVER: exclude specified nodes from linting — style is checked regardless of implementation existence ([review])
- NEVER: infer an evidence type from the section a rule appears in — a MUST/NEVER rule under a `## Compliance` heading does not imply `compliance` evidence; the type follows the claim's shape ([review])
