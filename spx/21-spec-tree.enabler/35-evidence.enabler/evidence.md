...
`/testing` (with `/testing-{language}` binding the test file) is the single authority that selects the verification type and, under testing, the assertion type. No decision record, template, section heading, or other skill selects either.

## Assertions

### Compliance

- ALWAYS: `/testing` (with `/testing-{language}`) is the single authority that selects an assertion's verification type and, under testing, its assertion type — no other skill, decision record, template, or section heading selects either ([audit])
