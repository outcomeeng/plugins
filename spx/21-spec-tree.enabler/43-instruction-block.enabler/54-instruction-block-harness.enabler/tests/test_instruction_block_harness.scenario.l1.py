...
def test_symlinked_harness_instruction_files_materialize_as_regular_files() -> None:
    ...
    assert observation.declared_claude_is_symlink
    assert not observation.declared_agents_is_symlink
    assert observation.declared_claude_link_target == harness.INSTRUCTION_AGENTS
