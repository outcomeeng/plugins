...
Observed in a consumer repository: `author-prose` required a `prose-auditor` dispatch, the role was unavailable, and the router's Codex section — "If a plugin's agents are missing, run that plugin's `/<plugin>-plugin init` … The definitions it places are durable checkout configuration: commit them" — led the session to commit `.codex/agents/prose_prose-auditor.toml` plus two `.gitignore` exceptions into an open, unrelated methodology PR, then to strip them back out after operator challenge. Two template rules made the recovery ambiguous. The init instruction says commit the placed definitions but scopes neither the changeset that carries them nor the authorization check that precedes placement. The Codex "Read named files yourself" rule ("Never use subagents to read … files named by the user") literally forbids what every configured auditor role-task contract requires, and a reviewer in that repository initially rejected the prose-auditor's path-reading on that basis before the specific contracts were read as clarifying the general rule. The craft-auditor role-contract clause resolves the authorization gap; the init-instruction rule remains open, and the named-file rule's disposition is recorded in the resolution below.

**Resolution shape**: the operator interview is complete and `spx/12-marketplace-state.adr.md` now decides the direction — a plugin's agent definitions live in the same scope as the skill content they invoke, so persistent installation places Codex agent definitions in the selected agent home's agents directory beside the skills, a checkout carries them only where it also carries the invoked skill content, and no generated instruction surface directs a session to commit agent definitions; a missing agent role is repaired by refreshing the selected agent home and then reloading the harness plugin index or starting a new session, because a running session retains already-loaded plugin content. The template pass rewrites the Codex agent-init instruction to that home-refresh-and-reload repair path, removing the commit guidance entirely. Until that pass ships, the live generated routers still carry the retired commit-directing guidance (this repository's `AGENTS.md` "commit them" agent-init line and its checkout-placement description), and the authored lifecycle-skill template `src/templates/plugin/SKILL.md` carries the same directive rendered into every shipped `<plugin>-plugin` skill; both surfaces contradict the decision; the decision supersedes the router, so a session that hits a missing agent role repairs it by refreshing the selected agent home and reloading, never by committing checkout definitions. The template pass is sequenced behind home delivery because its replacement guidance repairs nothing until home delivery populates the selected agent home's agents directory — rewritten earlier it would prescribe a repair that cannot succeed — so the rewrite ships with the slice that makes it true, per the dependency order in `spx/32-distribution.enabler/21-installation.enabler/21-repository-installation.enabler/PLAN.md`. The named-file rule is resolved: the router carries the designated-context rule shipped by the `work/designated-context-rule` changeset. Implementation order lives in `spx/32-distribution.enabler/21-installation.enabler/21-repository-installation.enabler/PLAN.md`.
