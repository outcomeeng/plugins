...
1. Repository fit: this repository's own root instruction content shrinks until the rendered `AGENTS.md` fits the ceiling, and the gate flips from report to fail for fitting surfaces. The router reduction — dispatch mechanics and per-role role-task contracts authored once under `src/_shared/agentic-execution/` and build-injected as the `verifier-dispatch` reference of `/merging-standards`, `/apply`, and `/create-skill`, with the pinned assertions, validation tuples, and `[eval]` dispatch evidence moved in the same changeset — is delivered; the rendered surfaces still exceed the ceiling on this repository's own product content, which this remaining unit owns.
