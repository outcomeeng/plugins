...
- Given the `CLAUDE.md` and `AGENTS.md` outputs of one generation, when they are compared, then they share every line except the per-runtime blocks the template marks — today, the Codex auditor-subagent mandate; the two files differ only there and in their own filename, which is the output path, not a span of the shared body ([test](tests/test_update_spx.scenario.l1.py))
