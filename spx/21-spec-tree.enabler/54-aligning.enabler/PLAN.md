# PLAN: downstream-alignment conformance in `/aligning`
...
`spx/21-spec-tree.enabler/spec-tree.md` (the `/understanding` references declare the
downstream-alignment contract) declare that a change to a higher-level declaration
aligns the first affected lower specs in the same change. The `/aligning` enforcement
of that contract is concrete downstream implementation this slice does not carry.

## Next implementation step

Add a downstream-alignment conformance check to `/aligning`: over a changeset's
...
node-local `PLAN.md` grounding. Derive the changed-file set from the `changeset-scope`
skill's `branch_scope(base, repo)` — never hand-roll git base-ref or diff derivation
inside `/aligning`, per `spx/21-spec-tree.enabler/17-auditing.adr.md`.
