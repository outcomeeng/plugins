...
- ALWAYS: before `/author` writes an assertion, `/test` `select-evidence` accepts the target `spx/...` path, exact assertion text, and product language; returns exactly `mode`, `verification_type`, `assertion_type`, `execution_level`, `evidence_form`, and `rationale`; performs no repository mutation; and `/author` copies `evidence_form` without substitution ([audit])
