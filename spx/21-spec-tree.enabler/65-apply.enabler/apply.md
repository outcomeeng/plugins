...
- ALWAYS: the apply lifecycle routes each Output kind present in the changeset — code, test evidence, spec or decision, skill, prose — through one lane table naming the authoring skill and standards that kind loads, the deterministic lane it runs, and the artifact auditors that kind owes independent of malleability; the selected rows compose, the deterministic lane is their union, each artifact auditor runs once over the whole changeset for its kind, and the one-language rule holds inside the code row alone, so an Output with no code row never stops at language detection ([audit])
...
- ALWAYS: select the numbered evidence gates and the whole-changeset review from the least malleable node the changeset touches, and the artifact auditors from the Output kinds it contains; when the change spans nodes, widen each selected audit to every affected governed subject and run Step 9 over the full diff only when malleability selects it — cross-node scope changes gate subjects, never gate applicability ([audit])
