...
PROVIDES the apply lifecycle — selecting the next executable observable slice, then driving each node in that slice through its composed Output-kind lanes — bounded by the agentic gates its least malleable touched node selects and a terminal merge-lifecycle gate
...
- ALWAYS: the apply lifecycle selects its agentic gates from the least malleable node the changeset touches, read from each spec's front matter with the absent field as `implementation`: the deterministic lane always; Step 9 for a verification-malleable or implementation-malleable node; Steps 6, 8, and 8a for an implementation-malleable node only; product and outcome-record changes run the deterministic lane and Step 9 regardless; Step 4 runs whenever a decision changed, independent of malleability ([audit])
- ALWAYS: the apply lifecycle routes each Output kind present in the changeset — code, test evidence, spec or decision, skill, prose — through one lane table naming the authoring skill and standards that kind loads, the deterministic lane it runs, and the auditors Steps 4, 6, 8, 8a, and 9 dispatch for it; the selected rows compose, the deterministic lane is their union, each auditor runs once over the whole changeset for its kind, and the one-language rule holds inside the code row alone, so an Output with no code row never stops at language detection ([audit])
