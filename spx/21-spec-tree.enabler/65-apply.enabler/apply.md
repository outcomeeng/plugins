...
- ALWAYS: with a node-path argument the work queue is that single full `spx/...` path, and with no argument it is derived from the conversation, falling back by expanding every `spx/EXCLUDE` entry from its required path relative to `spx/` into a full `spx/...` context path ([audit])
...
- ALWAYS: every persisted audit or review gate binds to an exact committed head after deterministic verification passes; a rejection first reloads the matching authoring counterpart and every standards skill shared with the auditor, then produces a standards-derived defect-class repair plan and a new local checkpoint before the gate reruns, while an audit over modified or untracked files is advisory and never satisfies a gate ([audit])
...
- ALWAYS: before test authoring or repair, `/apply` requires `/test` to complete the clause-evidence matrix and pass-while-false analysis for every assertion; subpart-only evidence leaves the test design unstabilized ([audit])
- ALWAYS: every test-evidence rejection triggers an autonomous `/test` methodology reset that rereads the shared test standards and rebuilds the complete assertion and evidence chain before another stabilized audit run; repeated concrete findings prove incomplete standards application and do not trigger an operator question while governing standards, assertions, source contracts, and deterministic evidence determine the repair ([audit])
