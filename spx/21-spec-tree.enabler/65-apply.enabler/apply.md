...
- ALWAYS: before its language-specific test-writing step, `/apply` reaches the foundational `/test` evidence-design gate, prevents direct delegation around it, and carries the resulting packet plus any fixture decision into the committed test-evidence-auditor handoff as context rather than proof; when the change touches files or specs beyond the target node, required evidence-auditor gates and the `changes-reviewer` inspect the whole changeset ([eval](evals/evidence-design-handoff/eval.toml))
