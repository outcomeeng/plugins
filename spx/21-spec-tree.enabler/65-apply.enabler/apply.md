...
PROVIDES the apply lifecycle — selecting the next executable observable slice, then driving each node in that slice through its composed Output-kind lanes — bounded by the agentic gates its malleability and Output-kind selectors compose and a terminal merge-lifecycle gate
...
- ALWAYS: one stop rule with two triggers governs every gate — a second consecutive completed rejected, unknown, or blocked verdict on one gate (Steps 4, 6, 8, 8a), or a second consecutive completed Step 9 review with unresolved valid findings, stops the flow with a diagnosis carrying the classes that survived, the repeated-class check, and one proposal (split, track, or stop) raised through the structured question; a rejection in a previously recorded class stops at any pass with the amendment `spx/15-merging.pdr.md` requires; a launch with no run token and no verdict is no result and counts as neither a pass nor a rejection ([audit])
- ALWAYS: the apply lifecycle composes its agentic gates from two selectors that neither subsume nor deselect each other, and owes their union: malleability, read from each touched spec's front matter with the absent field as `implementation`, selects the numbered evidence gates and the whole-changeset review from the least malleable node the changeset touches — the deterministic lane always; Step 9 for a verification-malleable or implementation-malleable node; Steps 6, 8, and 8a for an implementation-malleable node only; product and outcome-record changes run the deterministic lane and Step 9 regardless; Step 4 runs whenever a decision changed, independent of malleability — while the Output kinds the changeset contains select the artifact auditors that judge those artifacts, independent of malleability; the numbered evidence gates malleability selects precede Step 9, as the foundation orders the evidence audits before Review for an implementation-malleable node, and an artifact auditor is never a review-dispatch predicate ([audit])
- ALWAYS: the apply lifecycle routes each Output kind the lane table enumerates — code, test evidence, spec or decision, skill, prose — through that table, naming the authoring skill and standards the kind loads, the deterministic lane it runs, and the artifact auditors it owes independent of malleability; the selected rows compose, the deterministic lane is their union, each artifact auditor covers the whole changeset for its kind — once per kind where its subject is that kind's changed surface, and once per artifact where its subject is a single artifact, as a decision auditor's is — the one-language rule holds inside the code row alone so an Output with no code row never stops at language detection, and a changed surface matching no row selects no authoring row while still entering the deterministic lane, entering the whole-changeset review only when malleability selects Step 9 ([audit])
- ALWAYS: when the repository requires the full deterministic gate, run `just check-full` only after every artifact audit selected by the Output lanes, every numbered evidence gate selected by malleability, and the whole-changeset review selected by malleability have converged, and run no agentic verification after it; any change after the full gate invalidates it and requires the affected agentic gates to converge again before a new full-gate run ([audit])
- ALWAYS: when the change spans nodes, widen each audit the two selectors chose to every affected governed subject and run Step 9 over the full diff only when malleability selects it — cross-node scope changes gate subjects, never gate applicability ([audit])
- ALWAYS: for default-branch work, the flow is incomplete until the change reaches the default branch on origin through `/merge`; an approved code audit, a converged whole-changeset review when selected, passing tests, and a clean committed branch are local readiness, not completion — the flow continues into `/merge` unless the user explicitly scoped the work to a proposal, analysis, review, or local-only change, or an explicit lifecycle gate blocks with no independent local action remaining, per `spx/15-merging.pdr.md` and the `/understand` default-branch completion boundary ([audit])
