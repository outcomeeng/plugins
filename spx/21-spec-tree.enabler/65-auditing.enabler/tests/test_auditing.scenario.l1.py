...
# Verdict spellings live in the canonical verdict module that
# audit_orchestrator imports. Bind once at module-load time so the rest of
# the test module references the canonical instances rather than re-typing
# the literal strings.
_VERDICT_MODULE = _load_audit_orchestrator()
APPROVED_VERDICT = _VERDICT_MODULE.verdict.Status.APPROVED
REJECTED_VERDICT = _VERDICT_MODULE.verdict.Status.REJECTED
...
def test_helpers_under_audit_are_exposed_by_module() -> None:
    ...
    assert hasattr(module.verdict, "Status")
