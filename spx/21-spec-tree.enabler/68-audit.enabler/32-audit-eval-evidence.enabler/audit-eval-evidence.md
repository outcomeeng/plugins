...
- The `audit-eval-evidence` skill is an agent-preloaded audit skill with a dispatch gate, read-only tool grants, and a reference model for eval-evidence properties ([test](tests/test_audit_eval_evidence.conformance.l1.py))
- The `eval-evidence-auditor` agent is a thin wrapper that preloads `spec-tree:audit-eval-evidence`, declares `model: sonnet`, and carries no independent audit policy beyond invoking the skill ([test](tests/test_audit_eval_evidence.conformance.l1.py))
