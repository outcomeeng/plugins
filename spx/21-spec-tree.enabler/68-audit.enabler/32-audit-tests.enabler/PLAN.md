...
Steps, carried by the plugin lane of `https://github.com/outcomeeng/changes/issues/10` alongside the go plugin skills:
...
3. Regenerate `dist/` with `just build-skills`, bump the spec-tree plugin with `just bump`, and gate the surface with the configured `skill-auditor`.
