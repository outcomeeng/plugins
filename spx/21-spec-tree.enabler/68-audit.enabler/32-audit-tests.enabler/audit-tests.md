...
- ALWAYS: judge a source symbol the test cites by declared-contract ownership, inspecting dynamic registration, the published package surface, generated and reflective use, and consumers outside the checkout before reporting it as laundered, so an absent in-repository caller opens the ownership question rather than settling it ([audit])
