...
- Given an executed test file declaring a variable or constant that owns test data, expected output, runner settings, property-test configuration, setup policy, reusable cases, fixtures, generators, or harness behavior, when audited by `/audit-tests`, then the verdict is REJECT with finding category "test-owned declaration" ([test](tests/test_test_auditing.scenario.l1.py))
