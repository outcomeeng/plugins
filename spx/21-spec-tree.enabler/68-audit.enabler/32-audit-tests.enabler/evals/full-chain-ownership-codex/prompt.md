...
Read detected language or language partitions from the audit inputs. When absent, derive partitions from the linked-test extensions and the installed language plugins: list the installed `audit-<lang>-tests` skills, read the test-file extension each language's `<lang>-test-standards` skill declares in its filename instantiation of `<subject>.<evidence>.<level>[.<runner>]`, and map every linked test whose extension an installed plugin declares to that language. Reject an extension no installed plugin declares, or an ambiguous partition, with property `unsupported-language` and remediation target `language-partition` instead of guessing; never map an extension from a list this skill carries.
