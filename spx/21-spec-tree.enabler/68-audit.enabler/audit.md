...
- ALWAYS: the `implementation-auditor` wrapper is a thin projection relay that invokes `spec-tree:audit`, carries the caller's changeset scope and live file list, owns no audit policy, and relays the exact `spx verification run` token and rendered projection ([test](tests/test_implementation_audit_contract.compliance.l1.py))
- ALWAYS: the `spec-tree:audit` implementation-audit prompt contract requires request validation, one started audit run, language and concern partitioning for every implementation scope, required `audit-{lang}-{code|tests|architecture}` concern dispatch, scope and finding recording through `spx verification run`, missing-skill rejection before dispatch, terminal status derived from accepted evidence, and final projection relay ([test](tests/test_implementation_audit_contract.compliance.l1.py))
- ALWAYS: each `audit-{lang}-architecture` concern skill accepts composition by `implementation-auditor` for implementation architecture scope and by `adr-auditor` for ADR language-specific architecture concerns ([test](tests/test_implementation_audit_contract.compliance.l1.py))
