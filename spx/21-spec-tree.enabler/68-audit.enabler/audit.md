...
- ALWAYS: the `implementation-auditor` wrapper is a thin projection relay that invokes `spec-tree:audit`, carries the caller's changeset scope and optional advisory live file list, owns no audit policy, and relays the exact `spx verification run` token and rendered projection ([audit])
