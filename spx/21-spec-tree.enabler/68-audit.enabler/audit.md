...
- ALWAYS: the published `spx verification run` audit lifecycle accepts implementation-audit input, scope, and finding payloads, extracts the reported `runToken`, reports monotonic evidence sequence numbers, accepts the caller-computed evidence-derived terminal status, and renders the sealed projection with the authoritative finding count ([audit])
