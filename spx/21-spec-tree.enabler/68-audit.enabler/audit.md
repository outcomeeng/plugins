...
- ALWAYS: implementation-audit coverage classifies changed artifacts by ownership before creating required units — implementation code, linked tests, and implementation or test infrastructure receive required language concerns; specs, decisions, coordination notes, skill or agent prompts, eval artifacts, generated copies, workflow configuration, documentation, and inert fixtures are absent from required implementation coverage or recorded as optional `not-applicable`; `unsupported` rejects only an implementation-owned artifact that lacks an executable concern producer ([eval](evals/implementation-ownership/eval.toml))
