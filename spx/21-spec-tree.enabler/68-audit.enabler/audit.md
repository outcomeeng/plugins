...
- ALWAYS: implementation-audit coverage units contain implementation code, executable tests, and implementation architecture only; specs, decisions, skills, agent configurations, prose, instruction guides, manifests, catalogs, and their generated copies remain governed by their dedicated audit, validation, and whole-changeset review surfaces ([audit])
...
- ALWAYS: the published `spx verification run` audit lifecycle accepts implementation-audit input, scope, and finding payloads, extracts the reported `runToken`, reports monotonic evidence sequence numbers, accepts the caller-computed evidence-derived terminal status, and renders the sealed projection with the authoritative finding count ([test](tests/test_implementation_audit_contract.compliance.l1.py))
