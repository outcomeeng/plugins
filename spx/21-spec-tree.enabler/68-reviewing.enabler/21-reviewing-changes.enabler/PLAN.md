...
**D3. (Superseded by D4.)** D3 originally proposed extracting `FOLLOWUPS_HEADER` to a separate render template `references/render/followups-header.md`. D4's three-severity realignment replaced that plan with one template per render class. The shipped render templates are `document.md`, `finding-blocking.md`, `finding-debt.md`, `finding-followup.md`, the per-severity empty-bucket census markers `none-blocking.md` / `none-debt.md` / `none-followup.md`, and `acknowledgements.md` — eight in total; the census markers replaced the earlier single blocking-only `no-blockers.md`. No `followups-header.md` exists; `reviewing-changes.md` and `21-script-decomposition.adr.md` enumerate these templates as shipped. D3 in its original form is not applicable.
