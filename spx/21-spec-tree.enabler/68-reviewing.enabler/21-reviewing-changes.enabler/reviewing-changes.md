...
- ALWAYS: known-clean diffs yield zero `blocking` findings and known-broken diffs yield at least one `blocking` finding at the per-eval threshold — the suite gate value `0.85` governs CI pass/fail, not a single trial
