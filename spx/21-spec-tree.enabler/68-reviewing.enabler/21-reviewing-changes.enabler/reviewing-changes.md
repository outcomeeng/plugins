...
- ALWAYS: the wrapper agent invokes the `review-changes` skill, the skill uses only `review_run.py` as its command surface, and the wrapper returns only the raw `runToken` without caller-facing rendering, summarization, counts, or restated findings ([eval](evals/wrapper-protocol/eval.toml))
...
- ALWAYS: each pass against a given changeset surfaces every finding the changeset exhibits in that single pass — there is no cross-pass continuity, and a finding missed on this pass has no second chance unless the diff itself changes ([eval](evals/findings-direction/eval.toml))
- ALWAYS: the review prompt yields zero `blocking` findings for known-clean diffs and at least one `blocking` finding for known-broken diffs at the per-eval threshold — the suite gate value `0.85` governs CI pass/fail, not a single trial ([eval](evals/findings-direction/eval.toml))
