...
- ALWAYS: the `review-changes` skill frontmatter grants only the runner command and `Read` ([test](tests/test_reviewing_changes.audit.l1.py))
...
- ALWAYS: the wrapper agent at `plugins/spec-tree/agents/changes-reviewer.md` declares `model: sonnet`, `tools: Bash, Read, Skill`, and `skills:` listing `spec-tree:review-changes` ([test](tests/test_reviewing_changes.audit.l1.py))
