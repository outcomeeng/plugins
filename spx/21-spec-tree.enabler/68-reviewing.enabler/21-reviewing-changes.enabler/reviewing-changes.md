...
- ALWAYS: `Finding.rule` is a non-empty string whose form is a path-style citation into an existing rule in the spec-tree or skill ecosystem — `spx/<path>:<MUST\|NEVER\|ALWAYS\|SCENARIO\|MAPPING\|CONFORMANCE\|COMPLIANCE\|PROPERTY\|AUDIT>:<n>`, a path to a spec-tree ADR/PDR, `plugins/<plugin>/skills/<skill>/SKILL.md:<rule-slug>`, `plugins/<plugin>/skills/<skill>/<references\|workflows\|templates\|scripts>/<file>[:<rule-slug>]`, `AGENTS.md:<rule-slug>`, or `CLAUDE.md:<rule-slug>`. `rule` is never a free-form description, a required-action string, a repository-root review policy citation, or a tracking-location string ([test](tests/test_review_result.scenario.l1.py))
