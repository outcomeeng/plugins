...
class TestReviewRunnerBoundary:
    ...
    def test_runner_preserves_journal_namespace_across_subcommands(
        ...
        runner = review_runner_harness(tmp_path)

        started = run_script(
            REVIEW_RUN_SCRIPT, "start", env=runner.env, cwd=runner.repo
        )
    ...
    def test_runner_requires_rename_source_and_destination_scope(
        ...
        runner = review_runner_harness(tmp_path, renamed=True)
        ...
        assert finished.returncode == 0, finished.stderr
        assert finished.stdout == f"{runner.run_token}\n"
