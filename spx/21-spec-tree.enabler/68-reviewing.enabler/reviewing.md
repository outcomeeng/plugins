...
- ALWAYS: reviewing classifies every finding by the shared review taxonomy — one of two severities (`blocking`, `debt`) paired with one of six concerns (`consistency`, `security`, `performance`, `evidence`, `standards`, `architecture`) per `REVIEW.template.md` and `spx/21-spec-tree.enabler/68-reviewing.enabler/15-severity-disposition.pdr.md` — so findings stay comparable across every reviewing surface ([review])
...
- ALWAYS: the reviewer judges finding validity and severity, and the author judges disposition — whether each `debt` finding is fixed in the pull request or tracked out of scope with a recorded reason — per `spx/21-spec-tree.enabler/68-reviewing.enabler/15-severity-disposition.pdr.md` ([review])
