...
`spx/21-spec-tree.enabler/76-merge.enabler/32-github-pr.enabler/32-opening-pr.enabler/opening-pr.md` and `.../54-managing-pr.enabler/managing-pr.md` declare the readiness record and the finish-before-wait ordering as their own node assertions, so each node's spec states what its skill already does rather than inheriting the claim from the ancestor. The decision's rules bind every workflow on a changeset's path to the default branch, so the apply flow carries them too.
