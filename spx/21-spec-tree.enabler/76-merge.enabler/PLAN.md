...
Remaining lower-layer work, tracked as a later slice of Change 8 in the `outcomeeng/changes` store: `spx/21-spec-tree.enabler/76-merge.enabler/32-github-pr.enabler/32-opening-pr.enabler/opening-pr.md` and `.../54-managing-pr.enabler/managing-pr.md` declare the readiness record and the finish-before-wait ordering as their own node assertions, so each node's spec states what its skill already does rather than inheriting the claim from the ancestor.
