...
Remaining lower-layer work, tracked as a later slice of Change 8 in the `outcomeeng/changes` store: `spx/21-spec-tree.enabler/76-merge.enabler/32-github-pr.enabler/32-opening-pr.enabler/opening-pr.md` and `spx/21-spec-tree.enabler/76-merge.enabler/32-github-pr.enabler/54-managing-pr.enabler/managing-pr.md` declare that each pre-push Verifier dispatch in `/open-pr` and `/manage-pr` is preceded by the readiness record and that the check wait starts only after independent Author-side work completes; the two skills then cite `<verification_dispatch_readiness>` at their dispatch and wait steps.
