<!-- Prompt template for the review-readiness eval.
     ...
     reviewer's own behavior (changes-reviewer producing findings) has
...
- **the local review has converged** — every *valid* finding from `changes-reviewer` is either fixed in the diff or split out of the changeset and captured in `ISSUES.md` / `PLAN.md`. A finding the agent judged unbacked (not valid) is dropped and does not block. A *valid* finding still `unaddressed` (neither fixed nor split out) means the review has not converged.
