...
- ALWAYS: `/open-pr` receives repository-specific publication preflights and PR-body sections only through `/merging-standards` `<pr_opening_specialization>`, runs each applicable preflight in declared order before publication, composes each applicable section exactly once in declared order, verifies the observed body after creation, and never discovers, reads, or interprets `spx/local/merging.md` itself ([audit])
