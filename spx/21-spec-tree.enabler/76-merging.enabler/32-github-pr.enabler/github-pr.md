...
- Given `/manage-github-pr` invoked with arguments, when it runs, then it interprets the arguments as instructions and drives the lifecycle by invoking `/commit-changes`, `/open-pr`, and `/manage-pr`, applying `/merging-standards` `pre_mutation_confirmation_contract` before the first mutation ([audit])
- Given `/manage-github-pr` invoked with no arguments and an existing changeset — uncommitted working-tree changes, or a branch ahead of its base — when it runs, then it derives intent from that changeset and drives the same lifecycle under the resolved pre-mutation confirmation contract ([audit])
...
- Given a confirmed GitHub-PR changeset, when `/manage-github-pr` drives the lifecycle, then it runs `/commit-changes` and `/open-pr`, then tail-delegates the remaining lifecycle to `/manage-pr`; after merge `/manage-pr` continues through any declared deploy and release phases, carries the branch-state closeout record and Remaining Branches groups into `/handoff`, continues in-scope work rather than closing early, and invokes `/handoff` plain (never a hardcoded `--no-session`) only when the session is over — the goal is met with no in-scope work remaining, or continuation by the agent is impossible — so one caller-independent management protocol supplies the operator-useful final closeout rather than a merge receipt ([audit])
...
- ALWAYS: the GitHub-PR transport's `/manage-github-pr` orchestration is selected by `/merge`, assumes this transport, and consumes repository specialization only through `/merging-standards` named contracts ([audit])
- ALWAYS: drive the lifecycle from a determined changeset without an up-front operator proposal when `pre_mutation_confirmation_contract.required=false`; when `required=true`, present the changeset and intended lifecycle through the runtime's structured-question tool and obtain confirmation before the first mutating action — branch creation, commit, push, PR open, or merge — per `spx/15-merging.pdr.md` ([audit])
...
- Argument instruction mode: a user asks `/manage-github-pr` to implement and ship work, and the response applies the resolved pre-mutation confirmation contract before driving the lifecycle.
