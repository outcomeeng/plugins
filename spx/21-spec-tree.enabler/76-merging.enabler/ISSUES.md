...
The `terminal-green` case-space is now complete: the missing cases were added — status-context `state == EXPECTED` and `state == ERROR`, check-run `QUEUED`, and check-run `CANCELLED`/`TIMED_OUT`/`ACTION_REQUIRED` — so the suite now covers every status-context state (`EXPECTED`, `PENDING`, `SUCCESS`, `ERROR`, `FAILURE`) and every check-run conclusion the `terminal-green` definition in `spx/15-agent-pr-authority.pdr.md` names (15 cases).
