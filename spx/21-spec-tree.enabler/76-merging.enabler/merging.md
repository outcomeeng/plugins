...
- Given the current-head CI `spec-tree-review` reports no valid finding and every other required check is terminal-green, when `/managing-pr` evaluates `MERGE_READINESS` and `PRODUCTION_READINESS` holds, then it merges autonomously without separate human instruction ([eval](evals/merge-readiness/eval.toml))
