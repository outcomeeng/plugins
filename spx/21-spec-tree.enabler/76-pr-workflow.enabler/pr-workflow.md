...
- Given a PR whose closure gate has passed, all required checks are terminal-green, a current-head four-class review on at least one inspected surface has no unresolved `BLOCKING` or `NEEDS-ANSWER`, the latest push is at least five minutes prior to evaluation, branch hygiene including upstream-safety holds, and no project-declared production-class markers apply, when the PR-management skill evaluates the PR authority gate, then the gate authorizes both draft → ready promotion and merge from one verdict ([eval](evals/authority-gate-green/eval.toml))
