...
PROVIDES the `.spx/sessions/` document store and the `spx session` command contract — document creation from a stdin JSON header, `git_ref` derivation and its refusals, and the `todo`/`doing`/`archive` transitions
