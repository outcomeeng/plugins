...
- ALWAYS: under `spx/local/coordination.md`, `/pickup` claims a Change by reading and validating canonical Product, Maturity, Status as Available, and empty assignee state; adding the assignee; posting the earliest `Claim:` comment since the latest release with the agent session id; writing Status as Claimed; and verifying the three project fields, assignee, and Claim before executing an Activity — one account's sessions cannot be told apart by assignee alone ([audit])
