...
- NEVER: the `/handoff` skill injects `spx session list` output into its context block; session queue reads occur only in the workflow step that consumes them and use status filters when queue inspection is needed ([test](../tests/test_sessions.compliance.l1.py))
