...
- A marketplace registration entry's declared source fields — the registered source, the materialized root, and the source type, each present, empty, or absent for its runtime — map to the resolved target checkout path, the registered source taking precedence over the materialized root and a Directory source matching its token exactly, an empty value falling through wherever an absent one would, and every other combination maps to a none-available diagnostic; a field belonging to the other runtime leaves the resolved path unchanged ([test](tests/test_resolve_marketplace.mapping.l1.py))

- A requested marketplace name maps across a registration listing to the first entry that both carries that name and resolves a path, an omitted request name standing for the default marketplace name and a listing with no such entry mapping to a diagnostic naming every marketplace in the listing that does resolve a path, or none when the listing resolves none ([test](tests/test_resolve_marketplace.mapping.l1.py))
...
- ALWAYS: each authorized `/issue` invocation creates exactly one fresh `todo` follow-up; before every authorized write it reads only the header fields `spx session list --json` returns for the target queue's `todo` and `doing` sessions, reports as possible overlaps the full ids whose `goal` or `next_step` names an affected path or skill of the observation, and never reads a session body, reuses a session, probes origin for a stored branch, or withholds the write on that basis — queue consumers reconcile overlap at pickup ([audit])
- ALWAYS: `/issue` reads an external target's queue only after that target's mutation is confirmed, so an unconfirmed repository yields no header listing, no overlap ids, and no observation of its session state ([audit])
- ALWAYS: `/issue` makes a second handoff attempt only after a fresh header listing shows the target queue's `todo` and `doing` ids unchanged from the set captured before the first attempt; a new id is taken as that attempt's own record and verified rather than written again, and a listing that fails to return authorizes no write ([audit])
