...
- ALWAYS: before a same-repository write, `/issue` searches both `todo` and `doing` sessions for dependency-followup bodies describing the same observation and affected surfaces, checks semantic matches in `doing`-then-`todo`, oldest-first order, reuses the first match whose stored branch remains reachable on origin, preserves and reports stale matches, and creates exactly one follow-up when no reachable match remains; an unverifiable origin check stops without mutation, and wording or title differences never make the observation distinct ([audit])
- ALWAYS: `/issue` obtains operator confirmation before its first mutating command against every repository whose resolved git common directory differs from the invoking repository's, presenting the absolute target root, that repository's normalized origin identity, the resolved git ref, and the follow-up's goal, and stopping on anything but explicit approval with both repositories unchanged; the explicit `/issue` invocation itself authorizes one deduplicated write only to the invoking repository's own shared queue ([audit])
...
- NEVER: `/issue` edits, commits to, or pushes the owning repository's tracked source — its only possible effect there is one new handoff session document in that repository's queue when no active semantic match exists ([audit])
