...
`/pickup` reconciles every recorded session claim against observed repository and external state before it presents the session and before the post-context checkpoint, and presents one verdict per claim — `Confirmed`, `Discrepancy`, or `Unverifiable` — in place of the recorded snapshot. The reconciliation is performed by a single plugin-shipped, stdlib-only `python3` script — `scripts/verify_session_claims.py` under the pickup skill, invoked through each runtime's skill-directory variable — which reads session frontmatter through the published `spx session show --json` contract, reads session prose through `spx session show`, observes the checkout, and emits the verdicts as structured output. The script reaches `spx session show`, `spx spec status`, `gh`, and `git` only through a dependency-injected command runner, performs no working-tree mutation, and runs no node test suite.

## Rationale

A session document is a pointer whose detail the resuming agent re-derives from the spec tree, while repository and external observations supply reconciliation facts (`spx/21-spec-tree.enabler/76-sessions.enabler/sessions.md`, `spx/21-spec-tree.enabler/76-sessions.enabler/13-handoff-persistence.adr.md`). Recorded fields can diverge as the base, commit, or tree state changes. Reconciling each claim against observed state before the checkpoint makes repository-supported facts govern continuation.
...
- A claim verdict is a pure function of the recorded claim and observed state; re-running the script against an unchanged checkout and unchanged external state yields the same verdict for that claim.
...
- ALWAYS: the default runner implementation uses `subprocess` with array arguments, and tests inject a controlled recording runner under the `/test` Stage 5 failure-simulation, interaction-protocol, and observability exceptions -- no mocking ([audit])
