...
**Evidence**: the oversize was surfaced by the changeset reviewer on PR 494 (run `2026-07-28_18-06-07-731-0251e1d43e02`) as an `[architecture]` `DEBT` finding. The first composition pass moved 24 assertions into `spx/21-spec-tree.enabler/76-sessions.enabler/15-session-store.enabler` and `spx/21-spec-tree.enabler/76-sessions.enabler/28-pickup.enabler/30-claim-verification.enabler`, and one cross-cutting currency rule onto `spx/21-spec-tree.enabler/76-sessions.enabler/28-pickup.enabler` itself. The node's assertion total fell from 60 to 59 because the spec audit identified one moved rule as a duplicate of `spx/21-spec-tree.enabler/18-context-loading.enabler/context-loading.md`, which states the same foundation-marker exemption over a wider command set; that rule was deleted here rather than restated.
