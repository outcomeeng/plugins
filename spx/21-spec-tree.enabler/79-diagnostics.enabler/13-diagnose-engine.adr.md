...
The manifest is declarative because the facts that vary by consumer — the floor the installed plugin requires, the marketplace it depends on, the methodology plugins it expects, and the checks it runs — are the plugin's, not the `spx` CLI's. The plugin declares them; `spx diagnose` executes against them, keeping the CLI generic across the consumers a methodology marketplace installs into. The floor stays a single source of truth: the build renders the product's `REQUIRED_SPX_VERSION` into the manifest, exactly as `spx/21-spec-tree.enabler/79-diagnostics.enabler/15-version-floor.adr.md` requires the floor rendered into the shipped tree, so the shipped floor cannot drift from the floor the product enforces in CI. The manifest is a product-owned data file the skill passes by path; the field that `15-version-floor.adr.md` rejects is a field in the *runtime* plugin manifest, whose schema the runtime owns and validates — a different artifact from this contract. Under this decision the floor renders into the diagnose manifest and the floor comparison runs inside `spx diagnose`; `15-version-floor.adr.md` governs floor delivery for the in-body-classifying skill, where the floor renders into skill prose and the comparison runs in the skill, and the single-source, build-rendered, no-drift invariant holds in both.
...
and carries no in-body check classification. The implementation conforms to
this decision: the skill passes the plugin-shipped manifest to `spx diagnose`,
relays the deterministic report, and adds remediation judgment from the report's
...
- ALWAYS: the consumer-varying inputs — the spx-version floor, the marketplace identity, the expected plugin set, and the check set — reach `spx diagnose` through a plugin-shipped declarative manifest the skill passes by path, the floor build-rendered from the product's single source of truth per `spx/21-spec-tree.enabler/79-diagnostics.enabler/15-version-floor.adr.md` ([audit])
- NEVER: the `spx` CLI hard-codes a consumer's floor, marketplace identity, expected plugin set, or check set — those are the manifest's, so `spx diagnose` stays generic across the consumers a methodology marketplace installs into ([audit])
