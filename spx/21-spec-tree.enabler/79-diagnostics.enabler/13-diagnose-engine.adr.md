...
- ALWAYS: marketplace-install diagnosis derives the expected selection from Claude Code project-installed inventory and raw selected-home Codex plugin declarations, including disabled and uncached entries, bounded by the corresponding committed marketplace catalog.
...
- NEVER: the shipped diagnose manifest embeds an expected or required plugin set — the check derives selection from the agent-owned state declared in `spx/12-marketplace-state.adr.md` ([audit])
