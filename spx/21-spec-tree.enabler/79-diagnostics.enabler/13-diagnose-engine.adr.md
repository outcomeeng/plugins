...
- The marketplace-install check's expected plugin state is derived from the checkout's per-runtime project declarations — the committed repository configuration each coding-agent runtime owns — never from a plugin set embedded in the shipped manifest or read from the marketplace catalog.
...
- ALWAYS: the marketplace-install check derives expected plugin state from the checkout's per-runtime project declarations — the committed repository configuration each coding-agent runtime owns — rather than from a plugin catalog embedded in shipped diagnostic output, per `spx/12-marketplace-state.adr.md` ([audit])
- NEVER: the shipped diagnose manifest embeds an expected or required plugin set — expected plugin state for the marketplace-install check is derived from the checkout's per-runtime project declarations, not carried in shipped diagnostic output, per `spx/12-marketplace-state.adr.md` ([audit])
