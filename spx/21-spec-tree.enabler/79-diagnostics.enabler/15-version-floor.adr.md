...
check that re-reads the same command. A field in the agent's plugin manifest is
rejected: that manifest's schema is the agent's, an unknown field risks the
agent's validator rejecting it, and the value would still need syncing with the
...
- The floor the shipped check judges against is identical on every build target — it is a single source-owned value, not an agent-divergent name.
...
- NEVER: the shipped floor is hand-duplicated in the plugin tree or declared in a field of the agent's plugin manifest whose schema the agent owns — it is build-rendered into the diagnose manifest from the single source of truth so it cannot drift from the floor the product enforces ([audit])
