...
Resolved by removing the hooks. The spec-tree plugin now ships a single `SessionStart` hook (`src/plugins/spec-tree/scripts/session-start.py`) whose only effect is writing `CLAUDE_SESSION_ID` to `$CLAUDE_ENV_FILE` — no `<SPEC-TREE_*>` markers, no gate argv tokens, no transcript parsing. The `PreToolUse` gate, the compaction hooks, and the `PostToolUse` gate-reminder were removed (`spx/21-spec-tree.enabler/15-hook-state-delegation.adr.md`), so the cross-hook inline-token coupling this item described no longer exists.
