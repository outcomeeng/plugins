...
## 23. Pre-existing skill-quality debt surfaced by the skill-auditor during the guide-filename token conversion

The guide-filename portability conversion (the `file` runtime-token kind; hardcoded `CLAUDE.md`/`AGENTS.md` references in shipped skills rendered as `{{! file('root_guide') !}}` / `{{! file('spx_guide') !}}`) mandated a `develop:skill-auditor` pass over every edited `SKILL.md`. The conversion itself was APPROVED in all 13 audited skills. The audit also surfaced pre-existing defects across whole files the conversion touched only mechanically — none caused by the token change. Tracked here for a dedicated skill-quality pass rather than mixed into the portability changeset. Every deterministic gate (`check-skills`, `check-manifests`, `guide-check`, runtime-token, validation) passes, so none of these block shipping.

**Must-fix (REJECTED) findings, by skill:**

- `plugins/spec-tree/skills/decompose/SKILL.md` — 15 orphaned `references/archetypes/` files (three archetypes × five files), uncited in the skill body and two levels deep (violating the one-level-deep references rule); plus a procedural-to-operational imbalance. Resolution: either wire the archetypes into the `/decompose spx/` flow and flatten to one level deep, or delete them; rebalance `<success_criteria>` to verifiable output properties.
- `plugins/spec-tree/skills/audit-tests/SKILL.md` — `<objective>` carries two sentences of evaluation methodology plus a read-directive that belong in `<audit_workflow>`/`<essential_principles>`. Resolution: keep only the verdict-output sentence; relocate the rest.
- `plugins/spec-tree/skills/open-pr/SKILL.md` — procedural-to-operational ratio ~7.4:1 (one failure mode, no formal gates) against the 3:1 threshold for durable-change skills. Resolution: add the remaining failure modes (push rejection, duplicate-PR, stacked-draft topology, convergence stall) and pre-write GATE checkpoints.
- `plugins/spec-tree/skills/handoff/SKILL.md` — the word "Claude" appears in the YAML `description` field. The shipped `skill_frontmatter` validator currently passes it (handoff ships today), so this is a standards-text finding, not a publication blocker. Resolution: fold the parenthetical cases ("context exhausted, user halted, external blocker") directly into the trigger clause and drop "continuation by Claude is impossible".

**Notable WARNING-level findings** (worth-improving, all pre-existing): objective bloat in `understand`, `align`, `author`, `audit`, `init-worktrees`, `merging-standards`, `task-tracking-standards`, `commit-changes`; missing `argument-hint` in `contextualize` and `review-pr`; missing `<failure_modes>` in `commit-changes`, `manage-github-pr`, `merging-standards`, `review-pr`; overbroad `Bash` grants in `audit`, `manage-github-pr`, `init-worktrees`, `open-pr`, `commit-changes`; duplicate/unmerged `<constraints>` and lowercase `never` in `review-pr`; markdown headings in `interview`'s `direct-interview.md` and `preview-template.md`; `skill-standards/SKILL.md` is exactly 500 lines where its own criterion says "under 500".

**Not yet audited:** the 12 language-plugin skills edited by the conversion (`architect-*`, `audit-*`, `code-*`, `test-python`, and the `*-simplifier` agents) took purely mechanical filename→token edits identical to the audited set; their audits were skipped, so any pre-existing debt in them is unrecorded.

Surfaced during the guide-filename portability conversion (2026-06-25/26).
