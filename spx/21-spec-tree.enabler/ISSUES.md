...
This repository has no `spx/EXCLUDE` and no root `conftest.py`: no node is in specified state, every declared `[test]` and `[eval]` link resolves, and the `eval-links` validation step fails the gate on a dangling link. The shipped methodology still describes exclusion as the specified-state mechanism — `src/plugins/spec-tree/skills/understand/references/excluded-nodes.md`, the inline `/understand` `<node_states>` and `<decision_to_spec_alignment>`, and the `apply`, `author`, `test`, `handoff`, `manage-github-pr`, `merging-standards`, and `test-typescript` skills — and `spx test passing` reads it. The specs `spx/21-spec-tree.enabler/65-apply.enabler/apply.md`, `spx/21-spec-tree.enabler/76-sessions.enabler/25-handoff.enabler/20-closure.enabler/closure.md`, and `spx/15-validation.enabler/32-reference-portability.enabler/reference-portability.md` name the file as a mechanism.
...
**Resolution shape.** Once the gate holds: replace `excluded-nodes.md` with a "specified via committed `spx.status.json`" reference; rewrite the `/understand` `<node_states>` and `<decision_to_spec_alignment>` sentences; sweep the seven skills above (each plugin takes the `skill-auditor` gate and a bump); re-point the three specs above from the file to the `specified` state; drop `EXCLUDE` from `PORTABLE_SPX_FILES` in `outcomeeng/validation/reference_portability.py` once no shipped text names it. Per-mechanism evidence readers for eval and audit, and a cost-reward CI reproduction policy, are not required for the retirement.
