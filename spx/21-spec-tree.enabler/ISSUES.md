...
**Resolution shape.** Once the gate holds: replace `excluded-nodes.md` with a "specified via committed `spx.status.json`" reference; rewrite the `/understand` `<node_states>` and `<decision_to_spec_alignment>` sentences; sweep the seven skills above (each plugin takes the `skill-auditor` gate and a bump) and the `/apply` fallback sentence in `docs/tutorial.md` that describes the mechanism to consumers; re-point the three specs above from the file to the `specified` state; drop `EXCLUDE` from `PORTABLE_SPX_FILES` in `outcomeeng/validation/reference_portability.py` once no shipped text names it. Per-mechanism evidence readers for eval and audit, and a cost-reward CI reproduction policy, are not required for the retirement.
