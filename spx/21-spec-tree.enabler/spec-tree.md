...
- ALWAYS: for default-branch work, the `/understanding` skill declares passing local verification and review gates as progress and requires continuing through `/merge` until the change is merged or no independent local action remains without operator input or an external-state change ([review])
