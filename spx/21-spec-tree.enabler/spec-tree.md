...
- ALWAYS: the `/understand` skill's imperfection and closing protocols make an orchestrating session with officers in flight the sole exception to structured operator questions: it writes an escalation as text in its own pane, assumes the operator is away, holds only the raised decision while continuing work that loaded truth settles, and leads with evidence, consequence, options, and one recommendation; every other session uses the structured-question tool ([audit])
