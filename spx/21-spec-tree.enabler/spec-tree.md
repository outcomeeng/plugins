...
- ALWAYS: the `/understand` skill's references declare that test harnesses, generators, and inert fixtures are governed by naturally placed spec nodes rather than by a mandatory top-level category subtree, per `spx/15-test-infrastructure.pdr.md` ([review])
