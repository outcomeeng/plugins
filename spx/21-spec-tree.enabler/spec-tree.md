...
- ALWAYS: the `/understand` skill defines product content as every product artifact a spec node governs or must govern — source, tests, evals, generated output, specs, decisions, coordination notes, and spec-declared configuration, implementation being its code layer — with governance derived from node placement or the test-link chain as a search under the live foundation marker, an ungoverned artifact recorded as a coverage gap and left unread and unmodified, and operational configuration under `spx/local/` and the exclusion mechanism — read by its declaring skill under the live foundation marker — together with the agent harness's own instruction and settings files, tool output, session store, and scratch space excluded ([audit])
