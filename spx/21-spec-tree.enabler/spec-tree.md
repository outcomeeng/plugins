...
- ALWAYS: the `/understand` skill declares that no product content is read or modified before its governing spec node has been contextualized in the conversation since the last compaction, that `/contextualize` on a node precedes any discussion of that node, and that a compaction empties the set of contextualized nodes; the managed router extends operational-continuation handling to configured named verifier and reviewer dispatch from recorded exact-commit and path or node coordinates, result collection, journal-token or projection rendering, and verdict or finding recording, while following a returned path into product content or making a product judgment re-enters those gates ([audit])
