...
- ALWAYS: the `/understand` skill's artifact-placement foundation declares the artifact taxonomy closed — `spx/` admits no artifact outside it with `spx/local/` overlays as operational configuration, placement decides only between the governing layer (ADR or PDR) and the declaring layer (spec), and verification and implementation artifacts are never placed by classification because assertion tags derive evidence locations and node ownership with the language's declared infrastructure home derives implementation locations ([audit])
