...
- ALWAYS: the `/understand` skill's `<orchestrating_session_escalation>` block makes an orchestrating session with officers in flight the sole structured-question exception: it writes its escalation as text in its own pane and never through the structured-question tool, and says to hold only the decision and keep every officer moving on what loaded truth settles ([audit])
