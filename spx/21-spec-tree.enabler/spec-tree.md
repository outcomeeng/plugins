...
- ALWAYS: the `/understand` skill's truth hierarchy declares spec-first routing for enforcement surfaces — a change that makes a behavioral rule enforceable in a skill, agent, or instruction surface is spec-governed work that enters through `/contextualize` on the owning node and authors the governing assertion before any enforcement surface is edited — and names the inversion, editing enforcement surfaces first with the missing assertion left for a reviewer to discover, as the failure mode ([audit])
