...
- ALWAYS: the `/understand` skill's artifact-placement foundation declares the artifact taxonomy closed — `spx/` admits no content outside it, placement decides only between the governing layer (ADR or PDR) and the declaring layer (spec), and verification and implementation artifacts are never placed by classification because assertions derive their locations ([audit])
