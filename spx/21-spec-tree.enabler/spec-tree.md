...
- ALWAYS: the `/understand` skill's coordination model declares the Change as the coordination object that carries work ordering outside the durable map, with its Maturity and Lifecycle, the capitalized roles Refiner, Executor, Author, Fixer, and Verifier, and `ISSUES.md` as the only node-local note, per `spx/15-agent-terminology.pdr.md` ([audit])
