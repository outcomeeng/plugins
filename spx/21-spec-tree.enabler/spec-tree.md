...
- ALWAYS: the `/understand` skill defines implementation as every product artifact a spec node governs or must govern — source, tests, evals, generated output, specs, decisions, coordination notes, and spec-declared configuration — with governance derived from node placement or the test-link chain as a search under the live foundation marker, an ungoverned artifact recorded as a coverage gap and left unread and unmodified, and the agent harness's own instruction and settings files, tool output, session store, and scratch space excluded ([audit])
- ALWAYS: the `/understand` skill declares that no implementation is read or modified before its governing spec node has been contextualized in the conversation since the last compaction, that `/contextualize` on a node precedes any discussion of that node, that a compaction empties the set of contextualized nodes, and that an operational continuation — PR inspection, check wait, merge, deploy, release, `spx session` operations, occupancy proof — opens no implementation and triggers neither `/understand` nor `/contextualize` ([audit])
