...
- ALWAYS: a repair of a subject an agentic verdict rejected is produced, whenever a Fixer produces it, by an Author's agent session independent of the round's Author, holding the earlier round's artifacts and verdicts — the roles are those `spx/15-agent-terminology.pdr.md` defines ([audit])
