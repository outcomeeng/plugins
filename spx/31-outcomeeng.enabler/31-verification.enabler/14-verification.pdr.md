...
3. An audit verdict is produced in a Verifier's agent session isolated from the Author's agent session that produced the work under audit. The Author's agent session never grades its own work in place; it dispatches the audit to a separate Verifier's agent session. The isolation separates Author from Verifier, never one Verifier from another, so a Verifier already free of Author bias may compose further verification.
...
- ALWAYS: an audit verdict is produced in a Verifier's agent session isolated from the Author's agent session that produced the work under audit — the isolation separates Author from Verifier, never one Verifier from another ([audit])
...
- ALWAYS: a repair of a subject an agentic verdict rejected enters the next agentic verification round as a new committed subject; when the session holding the Fixer role produces it, that session receives the rejected round's subject and verdicts, and its independence from the round's Author is the rule `spx/15-agent-terminology.pdr.md` owns ([audit])
