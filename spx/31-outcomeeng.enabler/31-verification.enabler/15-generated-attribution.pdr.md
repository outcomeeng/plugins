...
A region marker bounds an extent only where its generator emits it: a marker line matches at the start of a line, permitting leading whitespace only for a relation whose generator emits the marker indented and trailing content only where the declared begin or end is a prefix of a version- or name-bearing marker line; a marker quoted inside prose or an example fence bounds nothing, and where the relation's generator implements stricter fence matching for the same marker family, the generator's matching is authoritative. The extent spans from the begin line through the end line inclusive, for every such pair in the file. A path's attribution derives from the declaration: wholly generated when covered by an `outputs` path pattern, generated-region-bearing when named by a region entry, authored otherwise. Sources and generator implementations are authored content; their generation roles exist only within their relation, and a relation's source may itself be another relation's output.
...
- ALWAYS: deterministic verification (validate, test, evaluate) runs over the complete changeset including generated extents, and each relation's regeneration command backs regeneration-parity evidence — rerunning the command leaves the committed generated extents byte-identical — established before agentic verification dispatch per `spx/31-outcomeeng.enabler/31-verification.enabler/14-verification.pdr.md` ([audit])
