...
**Harnesses manage context, resources, and access.** A harness mediates access to real behavior or real local/remote infrastructure. It owns setup, teardown, lifecycle, cleanup, mandatory dependency checks, diagnostics, and controlled doubles for dependencies outside the assertion boundary for resources such as temporary filesystems, browsers, product binaries, local services, APIs, databases, Docker containers, and remote credentialed endpoints. It does not own arbitrary domain data, expected outcomes, the test predicate, or the assertion calls that decide the verdict, and it does not replace the behavior an assertion claims to verify.
...
**Executed tests own predicates and assertions.** The executed test function or a callback lexically authored in the test file owns the predicate and every assertion-framework call that decides whether the evidence passes. A harness may establish context, invoke real behavior, return observations, or pass observations into a test-owned callback; it never calls `expect`, `assert`, matchers, or equivalent verdict APIs on the test's behalf and never accepts an expected outcome that lets it choose the verdict. Moving assertion calls behind a harness wrapper relocates test logic into infrastructure and hides the evidence boundary.

Declarations are judged by the choice they own, not by their syntax. A test-local parameter, destructuring binding, or result alias is valid when it merely names a value supplied by a source contract, generator, fixture, or harness and chooses no domain member, expected outcome, protocol vocabulary, reusable case, setup policy, lifecycle policy, runner setting, seed, retry count, or verdict rule. A binding that makes one of those choices belongs with its owning source contract, generator, inert fixture, harness, or curated eval case.
...
**Generators produce variable input domains and construction-derived expectations.** A generator represents a domain with meaningful variation, composition, shrinking, or systematic exploration. It may pair generated input with an expected value derived independently from the generated construction when that derivation does not call the behavior under test. It is appropriate for paths, names, identifiers, content, syntax trees, option sets, structured request shapes, and other inputs where evidence improves because the test searches a space rather than naming one example.
...
- **Ownership and one-way dependency**: source-owned domain truth comes from source modules; generators own variable domains and independently construction-derived expectations; fixtures stay inert whole-payload inputs; harnesses manage resources, access, lifecycle, controlled doubles outside the assertion boundary, property-run configuration, replay, and diagnostics; and executed tests own predicates plus every assertion call. The dependency direction is one-way (test assertion files depend on test infrastructure; product modules never import it) — so a methodology user inspects the evidence chain to identify which layer owns each choice and relies on shipping code carrying no test-infrastructure references.
...
- ALWAYS: the executed test function or a callback lexically authored in the executed test file owns the predicate and every assertion-framework call that decides the verdict; harnesses expose context and observations without calling assertion APIs or accepting expected outcomes on the test's behalf ([audit])
- ALWAYS: judge test-file declarations by the choice each binding owns; permit parameters, destructuring bindings, and result aliases that merely name supplied values, and route bindings that choose domain members, expected outcomes, protocol vocabulary, reusable cases, setup or lifecycle policy, runner settings, seeds, retries, or verdict rules to the owning source contract, generator, fixture, harness, or eval case ([audit])
...
- ALWAYS: generators represent variable input domains with meaningful variation, composition, shrinking, or systematic exploration and may own expected values independently derived from generated construction; source-owned singleton shapes come from source constructors, registries, or typed factories ([audit])
...
- NEVER: a harness, fixture, generator, or shared test module calls assertion APIs or decides the predicate for an executed test — delegated assertions are relocated test logic rather than infrastructure ([audit])
- NEVER: reject a test-file binding solely because it is a variable, constant, fixture parameter, property-generated parameter, destructuring binding, or result alias — ownership follows the semantic choice the binding makes ([audit])
