...
Test authoring determines the evidence architecture before mutating an executed test file. The determination emits one row per assertion and records the assertion quantifier and domain, independent oracle, a concrete way the proposed evidence could pass while the assertion remained false, execution level, source-contract needs, harness needs, generator needs, and fixture status. A missing independent oracle, a generator without meaningful variation, or a property test without harness-owned seed, replay, and failure diagnostics stops authoring before file mutation.

An open or composable input space defaults to generated or property-based evidence. An inert whole-payload fixture is an exception requiring a structured operator decision for the named assertion and payload role. The decision states why the complete payload shape is material, why a variable generator or property test is infeasible or wasteful, the state-space coverage surrendered, the harness that owns setup, cleanup, seed policy, replay, and diagnostics, and the recommended generated or property-based alternative. Approval never covers fixture modules, constant bags, copied protocol values, expected-output files, or a deterministic payload used as a finite substitute for an open domain.

## Reference Contract

Local references in an evidence-design packet use product-root-relative Markdown links. A prose name, inline-code path, absolute path, `file://` URI, leading `/`, `./`, or path containing `../` does not establish traceability.

| Reference role                                  | Primary reference                                                                       | Secondary reference                                                                                                             | Validation                                                                                                                            |
| ----------------------------------------------- | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| Governed test infrastructure or source contract | Link to the exact assertion-bearing spec file or full decision-record path under `spx/` | Link to the implementation artifact when it exists                                                                              | The governance target exists and is a spec or decision file; a node directory, bare node name, or implementation-only reference fails |
| Assertion subject                               | Exact assertion text plus link to its assertion-bearing spec file under `spx/`          | Stable heading fragment when the heading is unique and durable                                                                  | The target exists and contains the assertion; a bare assertion label fails                                                            |
| Deterministic test evidence                     | The assertion's `[test]` link to the exact co-located typed test file                   | Links to imported infrastructure implementations                                                                                | The target exists under the governing node's `tests/` directory and its filename encodes assertion type and execution level           |
| Eval evidence                                   | The assertion's `[eval]` link to the exact `eval.toml`                                  | Links to producer artifacts and sibling eval artifacts                                                                          | The definition and declared sibling artifacts exist; an eval directory link is insufficient                                           |
| External conformance authority                  | Stable canonical URL, schema identifier, or standard identifier                         | Product-root-relative Markdown link to the local spec or full decision that adopts it                                           | An external authority never replaces the local governing declaration                                                                  |
| Runtime replay identity                         | Literal seed, replay token, run token, or source-emitted identifier                     | Product-root-relative Markdown links to the harness or run-journal implementation and its exact governing spec or full decision | Runtime identities remain verbatim values rather than invented file links                                                             |

Every local primary or secondary reference uses a product-root-relative Markdown link and passes the same path-shape, existence, target-kind, and declared-role validation. When an implementation artifact exists, its product-root-relative Markdown link is mandatory secondary traceability and never substitutes for the governing spec-node link. When planned infrastructure has no implementation, the packet links its governing spec file, records the implementation as absent, and stops dependent test-file authoring until that node's TDD flow supplies the artifact.

Reference validation checks the declared role, Markdown-link syntax, product-root-relative path shape, target existence, target artifact kind, and role pairing. It rejects a harness, generator, fixture, or source-contract reference whose only target is implementation; rejects evidence links whose target kind or location does not match the declared verification type; and rejects broken links for planned implementations rather than manufacturing a path to a nonexistent file.
...
- **Pre-mutation design**: every assertion receives an evidence-design determination before an executed test file changes; open and composable domains default to variable generation or property evidence, and a fixture exception requires scoped operator approval with the surrendered state-space coverage visible.
- **Typed traceability**: every local reference is a product-root-relative Markdown link whose target matches its declared role; governed artifacts link to the exact spec or decision file and existing implementation, evidence links resolve to their typed definition, external authorities retain canonical identifiers, and runtime replay identities remain verbatim values.
...
- ALWAYS: test authoring emits the complete evidence-design row before mutating an executed test file; missing independent oracles, constant-only generators, absent property replay harnesses, and unapproved fixture exceptions stop the mutation ([audit])
- ALWAYS: an open or composable input space defaults to generated or property-based evidence, while an inert whole-payload fixture requires scoped structured operator approval that records the complete-payload rationale, infeasibility of variable evidence, surrendered coverage, owning harness, and recommended alternative ([audit])
- ALWAYS: local evidence-design references are product-root-relative Markdown links validated by declared role, target existence, target kind, and required pairing; governed infrastructure and source contracts link to their exact spec or decision file and also to existing implementation ([audit])
