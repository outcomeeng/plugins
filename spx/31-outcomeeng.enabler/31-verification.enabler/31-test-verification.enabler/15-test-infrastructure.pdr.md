...
- ALWAYS: a Go product's test-infrastructure package at `internal/testinfra/` is declared `package testinfra` — never `package test`, which collides with the standard library — and its subpackages `harnesses`, `generators`, and `fixtures` are imported only from `_test.go` files ([audit])
