...
- ALWAYS: before test authoring or repair, an assertion-to-evidence matrix records every assertion's type, verification lane, linked evidence file, language-standard evidence form, type-specific obligations, coupling path, falsifying mutations, and assertion-relevant source branches; no assertion proceeds from a partial matrix ([audit])
