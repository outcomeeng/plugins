...
A path prefix carrying the plugin's own name cannot express the shared tree, where one authored fragment renders into several plugins' shipped output. Attribution by prefix alone therefore misses a change that alters what a plugin ships, and the miss is silent at the documented command order: bumping precedes building, so no generated path carries the plugin's name yet. Resolving the consumer set from the include directives in authored sources reaches the same answer the build reaches, from the same source of truth, without running the build or maintaining a second mapping that can drift from it. The resolution is a probe rather than a pure path rule because the answer lives in repository content, and it stays behind a Protocol so the attribution logic remains verifiable against an in-memory index.
...
- ALWAYS: `plugins_from_shared_change(change, include_index)` is pure — it resolves a shared-root path to plugin names from the supplied index alone and reads no repository content ([mapping])
- ALWAYS: the `IncludeIndexProbe` adapter derives its mapping from the include directives present in authored plugin sources, so the index names the plugins the build renders each fragment into ([conformance])
...
- ALWAYS: every git invocation and filesystem read/write passes through the injected `ChangeProbe`, `ContentProbe`, `IncludeIndexProbe`, `ManifestReader`, `ManifestWriter`, and `ToolProbe` Protocols ([audit])
...
- NEVER: a shared-root change is attributed by a hardcoded fragment-to-plugin table — a table drifts from the include directives the build actually resolves ([audit])
