...
- ALWAYS: the branch-already-bumped predicate requires each plugin's `working_tree_version` to be ahead of its `base_ref_version`, so a manual edit that leaves the branch ahead counts as a prior bump while a stale lower version after base advancement is bumped from the base version ([compliance])
