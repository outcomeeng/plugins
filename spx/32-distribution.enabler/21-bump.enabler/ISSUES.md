...
**Why backfilling is the wrong repair.** A later changeset reconstructing an earlier release's entry from commit messages and diffs is guessing at what that release's consumers must know, and the author who knew is gone from the changeset. It also attaches an unbounded obligation to whichever branch is open: each base advance brings another release to document. The entry belongs to the changeset that ships the change.
...
**Also decide**: whether the existing gaps are filled by their authors, left as gaps with a note in the changelog itself, or closed by declaring the entries start at the version the check first covers. Reconstruction by a later changeset is not among the options.
...
Surfaced on the release-overlay changeset, where an attempt to backfill 0.88.3, 0.88.5, and 0.88.6 produced one entry with fabricated content before it was caught and the backfill was withdrawn.
