...
- NEVER: bump a plugin whose working-tree version is already ahead of its `base_ref` version — the branch already carries a valid bump for that plugin; `spx/local/open-pr.md` owns the bump policy and opening step after base synchronization, and `spx/local/merging.md` repeats the step after a later rebase under that policy, preserving versions still ahead of the base and writing the next version from the base when it catches up or advances; outside CHECK mode, that plugin is skipped with a diagnostic while every other changed-but-unbumped plugin is still bumped — or, under `--dry-run`, reported — in the same pass ([test](tests/test_bump.compliance.l1.py))
