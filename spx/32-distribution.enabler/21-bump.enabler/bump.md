...
- Given no plugin under `src/plugins/` has any change since `base_ref`, when bump runs, then it exits 0 and no manifest is written ([test](tests/test_bump.scenario.l1.py))
...
- ALWAYS: changes are detected only under `src/plugins/{name}/**` — any file change inside that prefix counts as a distribution-surface change for plugin `{name}`, and no path outside that prefix triggers any bump ([test](tests/test_bump.property.l1.py))
...
- NEVER: bump a plugin whose working-tree version already differs from its `base_ref` version — the branch already carries a bump for that plugin (re-bumping during PR review is the failure `spx/local/committing-changes.md` prohibits); outside CHECK mode, that plugin is skipped with a diagnostic while every other changed-but-unbumped plugin is still bumped — or, under `--dry-run`, reported — in the same pass ([test](tests/test_bump.compliance.l1.py))
- NEVER: write a manifest for a plugin with no changes under `src/plugins/{name}/**` since `base_ref` — change detection is the sole trigger for writing ([test](tests/test_bump.compliance.l1.py))
...
- ALWAYS: when `--segment` is unspecified, the resolved segment is the auto-detected per-plugin segment from the file-status pattern of changes under `src/plugins/{name}/**` ([test](tests/test_bump.compliance.l1.py))
