...
- NEVER: bump a plugin whose working-tree version already differs from its `base_ref` version — the branch already carries a bump for that plugin (re-bumping during PR review is the failure `spx/local/committing-changes.md` prohibits); in a write pass (WRITE or DRY_RUN) that plugin is skipped with a diagnostic while every other changed-but-unbumped plugin is still bumped ([test](tests/test_bump.compliance.l1.py))
