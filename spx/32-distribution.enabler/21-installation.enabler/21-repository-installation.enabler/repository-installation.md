...
- Each isolated verification selection — the complete committed catalogs and a generated valid subset containing `spec-tree` — maps to registration of the invocation checkout and exactly that selection reported as installed and enabled by the corresponding real agent CLI, with an explicitly selected valid subset planning exactly its members in catalog order. ([test](tests/test_repository_installation.mapping.l3.py)) ([test](tests/test_repository_installation.mapping.l1.py))
