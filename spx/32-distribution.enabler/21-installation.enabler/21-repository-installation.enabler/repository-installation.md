...
- Given an installed plugin that owns Codex agent definitions, when that plugin's checkout placement runs, then its generated definitions are placed in the invocation checkout's `.codex/agents/` namespace while definitions outside that plugin's ownership remain unchanged — when placement may run is governed by `spx/12-marketplace-state.adr.md`. ([test](tests/test_repository_installation.scenario.l2.py))
- Given unchanged committed catalogs and checkout content, when isolated installation, together with any checkout placement the run exercises, runs twice against the same disposable homes, then the second run succeeds with the same installed and placed state. ([test](tests/test_repository_installation.scenario.l2.py))
