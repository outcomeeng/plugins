...
- Each registered marketplace entry shape — a GitHub repository, a git URL, a local path, or no entry — maps to the source persistent refresh refreshes from and names in its report, or, when no entry exists, to the bootstrap registration from the invocation checkout's own declaration.
...
- NEVER: the persistent run exits zero while a project- or local-scope install record of a cataloged plugin in the closing listing lists a version other than the target.
- NEVER: the persistent run executes an agent command whose working directory is a checkout other than the invocation checkout.
- ALWAYS: the install-record file rewrite replaces the file atomically and preserves every entry and field it does not set, including entries of other marketplaces.
- Given a project-scope record of a cataloged plugin whose project directory does not exist, when the persistent run executes, then the closing listing shows it at the target and no command named that directory.
- Given a real Claude Code install-record file holding a record whose directory is gone and a record at an older version, when the persistent recipe runs, then `claude plugin list --json` reports both at the target and the run exits zero.
