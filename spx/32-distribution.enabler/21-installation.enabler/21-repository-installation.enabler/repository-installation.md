...
- Given a disposable `CODEX_HOME` that isolated installation populated and the agent CLI's login authenticated from the CI credential variable, when a fresh non-interactive Codex session in that home is asked for its available subagents as structured output, then the returned subagent set contains every canonical subagent name the installation placed under that home's `agents/` directory. ([test](tests/test_repository_installation.scenario.l3.py))
