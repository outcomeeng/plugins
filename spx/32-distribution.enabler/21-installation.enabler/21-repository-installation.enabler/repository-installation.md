...
- Given Claude Code install records for one cataloged plugin at differing recorded versions across checkouts on the machine, when the persistent recipe runs, then every refreshed record lists the version the canonical marketplace serves during the run afterwards, and the report names each refreshed record's version before and after. ([test](tests/test_repository_installation.scenario.l3.py))
...
- Given a project- or local-scope `outcomeeng` install record that another agent session writes after the run reads its listing, when the run's post-execution listing reports it, then the run names that record's scope, project path, and version as unrefreshed and issues no further update. ([test](tests/test_repository_installation.scenario.l1.py))
