...
- Given a native probe command whose parent exits while a descendant keeps the captured output stream open, when the execution timeout expires, then the runner reports the timeout promptly and terminates the descendant before returning. ([test](tests/test_native_profile_process.scenario.l1.py))
