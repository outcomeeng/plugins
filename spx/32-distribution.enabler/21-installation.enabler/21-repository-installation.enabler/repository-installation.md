...
- Given unchanged committed catalogs and checkout content, when isolated installation and each placement-owning plugin's checkout placement run twice against the same disposable homes, then the second run succeeds with the same installed and placed state. ([test](tests/test_repository_installation.scenario.l2.py))
