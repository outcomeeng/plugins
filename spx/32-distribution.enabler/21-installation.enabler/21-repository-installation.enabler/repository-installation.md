...
- Given `just verify-marketplace-installation`, when the recipe runs, then it executes the repository-installation evidence files through the repository test command. ([test](tests/test_repository_installation.scenario.l1.py))
...
- Each pair of generated valid installed Claude Code and Codex subsets containing `spec-tree` maps through `just install-marketplace` to exactly those catalog plugins for its own agent, with selected unpublished plugins reported as pending and the project's activation selection preserved. ([test](tests/test_repository_installation.mapping.l3.py))
