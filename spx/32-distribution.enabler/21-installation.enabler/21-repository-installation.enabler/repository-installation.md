...
- Given canonical project configuration and existing Claude Code and Codex installed subsets that include `spec-tree`, when `just install-marketplace` runs, then each agent refreshes exactly its own installed catalog plugins, installs no unselected catalog plugin, reports any selected plugin pending publication, and preserves the project's activation selection. ([test](tests/test_repository_installation.scenario.l3.py))
...
- Given `just verify-marketplace-installation`, when the recipe runs, then it executes the repository-installation L3 evidence in disposable homes through the repository test command. ([test](tests/test_repository_installation.scenario.l1.py))
- Given every plugin in the Claude Code and Codex marketplace catalogs and a generated valid subset containing `spec-tree`, when the isolated installation harness installs each selection, then each agent registers the invocation checkout and exactly that selection is observable as installed and enabled through the corresponding real agent CLI. ([test](tests/test_repository_installation.scenario.l3.py))
...
- Given an installed plugin that owns Codex agent definitions, when that plugin's checkout materialization runs, then its generated definitions are placed in the invocation checkout's `.codex/agents/` namespace while definitions outside that plugin's ownership remain unchanged — when materialization may run is governed by `spx/12-marketplace-state.adr.md`. ([test](tests/test_repository_installation.scenario.l3.py))
