...
- For each supported agent, a generated valid installed subset containing `spec-tree` and published by the canonical marketplace maps through `just install-marketplace` to exactly those catalog plugins for that agent, with the project's activation selection preserved. ([test](tests/test_repository_installation.mapping.l3.py))
