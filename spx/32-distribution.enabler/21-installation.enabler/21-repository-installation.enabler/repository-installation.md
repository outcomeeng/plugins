...
- Given Claude Code install records for one cataloged plugin at differing recorded versions across checkouts on the machine, when the persistent recipe runs, then every refreshed record lists the version the canonical marketplace serves during the run afterwards, and the report names each refreshed record's version before and after.
- Given a Claude Code install record whose native update reports the record already current, when the persistent recipe runs, then the record holds that version, the run continues, and the report names it refreshed at an unchanged version.
- Given a project- or local-scope `outcomeeng` install record that another agent session writes after the run reads its listing, when the run's post-execution listing reports it, then the run names that record's scope, project path, and version as unrefreshed and issues no further update.
- ALWAYS: a persistent refresh run plans from exactly one install-record listing read at its start, reads the listing exactly once more after execution, and issues no lock, wait, or retry against the agent's record store.
- NEVER: the persistent recipe reports a record as current on the strength of an earlier run or of the marketplace cache; currency is established only from the listing the same run reads after execution.
