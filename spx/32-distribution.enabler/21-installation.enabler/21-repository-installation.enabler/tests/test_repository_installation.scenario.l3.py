...
def test_fresh_codex_session_discovers_every_placed_canonical_subagent() -> None:
    ...
    assert observation.placed_subagents
    assert observation.discovered_subagents is not None, (
        observation.session_last_message
    )
    assert observation.placed_subagents <= observation.discovered_subagents
