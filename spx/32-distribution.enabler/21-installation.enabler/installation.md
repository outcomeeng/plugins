...
- Persistent refresh reaches every project- or local-scope Claude Code install record on the machine for a cataloged plugin whose project path exists and whose project declares no noncanonical marketplace source for the native update, reporting every other record unchanged, and Codex's selected-home configured catalog members include entries with missing caches; the repository-installation node declares and evidences those dispositions.
