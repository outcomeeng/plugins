...
- Persistent Claude Code refresh is one bounded pass: it plans from the listing it reads at its start, moves every planned record to the version the canonical marketplace serves during the run so that records entering at differing versions leave at one version, reads the listing again after execution, and reports every project- or local-scope `outcomeeng` record present then that the plan did not refresh; it takes no lock and no second update pass, and a record's currency between runs is whatever its last native install or update wrote; the repository-installation node declares and evidences those behaviors.
