...
- Persistent refresh reaches every project- or local-scope Claude Code install record on the machine for a cataloged plugin whose project path exists and whose readable project settings declare no noncanonical marketplace source, or that lies in the invocation checkout whose registration the run reconciles first, for the native update, reporting every other record unchanged, and Codex's selected-home configured catalog members include entries with missing caches; the repository-installation node declares and evidences those dispositions.
