...
- A delegation to a herdr agent and its one correlated terminal handback travel as message records through the message capability, each carrying the complete initiating coordination reference; the environment surface carries the launch prompt and keystrokes only and produces no pane-borne handback block
