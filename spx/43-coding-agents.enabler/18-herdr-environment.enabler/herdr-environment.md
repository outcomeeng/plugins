...
- Every supported operation — open-worktree, start, stop, relaunch, inventory, read, wait, prompt, and key — maps one source-owned request shape through the source-owned operation registry to one herdr argument vector and checked response result
...
- ALWAYS: open-worktree, start, relaunch, stop, and key require explicit mutation authorization in the operation request before any herdr command runs
