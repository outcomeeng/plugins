...
One Python 3.13+ standard-library adapter shipped inside `/operate-prowl` owns the complete public Prowl command grammar, response validation, participant projection, structured handback generation, and delegation-state reduction. A caller supplies semantic completion text; the adapter renders the exact one-line return command and its success, retry, socket, and participant conditions into a versioned handback block. It exposes typed importable operations and a versioned JSON CLI; other coding-agents skills invoke `/operate-prowl` as a capability and never construct Prowl argument vectors, handback commands, or cross-skill script paths.

## Rationale

One capability keeps Prowl command and return-path knowledge testable and portable while preserving the skill-directory boundary required by plugin packaging. Semantic completion text stays separate from executable grammar, so a caller cannot add an argument or corrupt shell quoting while copying a return command. A pure delegation reducer makes duplicate and conflicting terminal handbacks deterministic without making the adapter own workflow persistence or execution decisions.

## Invariants

- One source-owned operation registry covers list, agents, read, send, key, focus, tab create, tab close, pane close, and open.
- One handback builder maps complete sender and recipient identities plus semantic completion text to a structured block containing the absolute adapter path, an exact command that ends at the `run` subcommand, checked success criteria, the default socket, both expected panes, and a no-retry-after-submission policy.
...
- ALWAYS: public Prowl responses map to versioned source-owned results or named schema and command failures without value rewriting ([mapping])
- ALWAYS: default subprocess execution maps absent operation input to null-device stdin and explicit operation input to an exact captured-text pipe ([mapping])
...
- ALWAYS: an open result preserves whether the path resolved to an exact known root and whether Prowl created a tab, distinguishing lazy sidebar activation from adding a new root ([mapping])
...
- NEVER: a delegation or coordination caller supplies executable handback data — `/operate-prowl` generates the structured handback block from semantic input before transport ([compliance])
...
- ALWAYS: `/operate-prowl` owns handback command rendering and publishes the generated command with its success, retry, socket, and expected-pane conditions as one structured block ([audit])
