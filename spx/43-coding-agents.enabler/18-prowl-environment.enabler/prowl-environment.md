...
- NEVER: a delegation request submitted over stdin carries a field the envelope does not forward — an unsupported key is rejected rather than dropped, so a caller never sends a delegation missing the data it believed it supplied ([test](tests/test_prowl_environment.compliance.l1.py))
