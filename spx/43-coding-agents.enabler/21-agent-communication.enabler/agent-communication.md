...
A same-worktree delegation request carries the authority `spx/43-coding-agents.enabler/32-same-worktree-coordination.enabler` declares: the sender as the tracked-file and Git owner, no write scope, and no Git mutation by the recipient; the recipient's result is the handback record's body. The record renders that authority in its body; a same-worktree delegation request without it, or whose authority names a write scope, reaches no record.
...
- NEVER: a same-worktree delegation request whose authority omits the sender as owner, names a write scope, or admits Git mutation reaches a record ([test](tests/test_mail_delivery.compliance.l1.py))
