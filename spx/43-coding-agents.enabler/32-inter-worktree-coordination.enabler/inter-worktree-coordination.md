...
- ALWAYS: a dependency handoff that asks another workflow to produce something carries the requester's pane and the exact command reaching it, and resolves to a signal gap when the requester's pane is not an authoritative participant ([eval](evals/coordination-decision/eval.toml))
