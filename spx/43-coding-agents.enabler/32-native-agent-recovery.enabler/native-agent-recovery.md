...
- ALWAYS: a complete read barrier delivers one separately submitted instruction only to each non-controller candidate the controller supplies a destroyed fact for, records every other verified candidate as judged intact with no delivery, and delivers nothing to any candidate when a pane read is absent or failed ([test](tests/test_native_agent_recovery.compliance.l1.py))
