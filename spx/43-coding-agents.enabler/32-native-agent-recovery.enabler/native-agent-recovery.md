...
PROVIDES two-phase native-agent preparation and recovery through exact pre-restart identity and launch evidence, lazy post-restart Prowl activation, pane rebinding, exact native-session launch, exact correlation verification, and separate continuation delivery
...
- ALWAYS: recovery activates absent worktrees through `/operate-prowl`, preserves Prowl-returned post-restart pane identities, sends only one source-owned exact native resume command to each selected unoccupied pane, and sends reassessment only after exact native correlation verifies ([test](tests/test_native_agent_recovery.compliance.l1.py))
- ALWAYS: exact native launch and reassessment use separate checked sends so interactive launch cannot buffer or reinterpret continuation prose ([test](tests/test_native_agent_recovery.compliance.l1.py))
