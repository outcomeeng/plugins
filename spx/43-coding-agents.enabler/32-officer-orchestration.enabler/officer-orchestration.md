...
- ALWAYS: every order requires reuse of a Verifier verdict on a byte-identical subject; after a rebase it permits reuse only when the preservation proof in `spx/15-merging.pdr.md` establishes an unchanged branch diff, unrelated base movement, and no governance surface in the base delta, with the base delta's narrower validation still run; every commit and verdict is mailed before compaction, and every idle officer is restarted after a release
