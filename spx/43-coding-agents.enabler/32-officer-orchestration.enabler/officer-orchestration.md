...
- ALWAYS: after a release, the orchestrating session relaunches every idle officer before its next order, because a running session keeps the plugin catalog it loaded; this is the herdr relaunch operation on a live pane, distinct from the housekeeping restart of a session that is gone
