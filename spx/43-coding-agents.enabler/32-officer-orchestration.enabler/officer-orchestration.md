...
- ALWAYS: the orchestrating session launches every officer through the environment capability, delivers the launch prompt that names the Change and the mail name the officer registers under, and delivers every later order as a message record; an order with an unfilled field or an unproven worktree is refused, an officer without a mail identity receives only the launch prompt, and one that has not registered within the bounded wait is stopped
...
- NEVER: a third Verifier pass runs on one Change without the operator's word; the second rejection produces the repeated-class check, a reading of the pushed changeset, a reading of the sealed journal runs, and a split, track, or stop proposal
...
- ALWAYS: officers are reported by worktree name and Change, every escalation leads with evidence, consequence, options, and one recommendation, what loaded truth settles is decided in the session, and the rest is raised to the operator through the structured question
