...
- ALWAYS: after a release, an idle officer's next order runs in a fresh session: the orchestrating session orders /release-change, stops the session through the herdr capability, and starts a new one in the same pane before the order, because a running session keeps the plugin catalog it loaded
