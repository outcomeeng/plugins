...
- One monitor poll maps the agent-mail inbox, the Captain's judged-record marker, the officer and pull-request watch file, the monitor state file, and each watched pull request's state to signal lines for newly listed mail, newly crossed officer-silence spans, changed pull-request fields, and a new sync hour; unchanged inputs produce no line, and the state records each emitted condition.
- The monitor performs one bounded read of each source and exits. The harness monitor command owns the polling loop and the operator-named sleep; the script owns no loop, sleep, background process, or daemon.
- Officer silence is measured from the store's per-sender record timestamps against the span the watch file gives that officer, never from pane state.
- The monitor alone writes its state file. The Captain alone writes the judged-record marker and watch file, and the monitor never advances the marker.
- Each watched pull request is named as `owner/repo#N`, and one poll reads its state, merge state, head, and check conclusions through `gh` behind an injected runner with one bounded call per watched pull request.
- An officer's message to the orchestrating session submits no doorbell into the orchestrating session's pane. Every order carries that rule, and the delivered store record is the signal.
...
- ALWAYS: reads happen on events — a monitor signal, an officer state change, or a bound crossed — or on a cadence the operator names; the cadence is the sleep in the loop command the skill gives the harness monitor facility, never a timer owned by the session or monitor script, and a poll without a signal line produces no report unless the operator asked for one
