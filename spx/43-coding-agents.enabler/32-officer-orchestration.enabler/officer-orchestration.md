...
- Wall time totals every event's duration across mail records and journal runs, at the durations' own decimal precision [test](tests/test_ledger_derivation.property.l1.py)
- Every total the ledger carries is emitted as the exact decimal text of its value, so a consumer parsing the result reads every digit the sources carried [test](tests/test_ledger_derivation.property.l1.py)
- Source records sharing one identity contribute once — mail records under one store id, journal runs under one run token: the derived ledger equals the ledger of the same sources with every repeated identity dropped [test](tests/test_ledger_derivation.property.l1.py)
...
- A source document the JSON parser refuses — malformed syntax, an integer literal wider than the interpreter converts from a digit string, nesting past its recursion guard, or bytes no codec decodes — refuses the document as the versioned invalid-input result on stdout with exit two and empty stderr, so no source shape reaches the caller as a traceback [test](tests/test_ledger_derivation.property.l1.py)
...
- ALWAYS: every bundled path the supervision skill names — in its body, in a workflow, or in a reference — is written in the skill-directory token form of `spx/13-plugin-and-runtime-conventions.adr.md`, so the read resolves against the installed bundle rather than the session's working directory ([audit])
- NEVER: a bundled reference directs a read of another bundled reference; the workflow that needs the second reference declares that read itself, so the read set of an operation is stated in one place and a reference added later inherits no pointer chain ([audit])
