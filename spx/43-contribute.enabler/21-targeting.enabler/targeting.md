...
- ALWAYS: the checkout's own repository is read by naming it, because a nameless read answers for whichever repository `gh` resolves as the base — in a fork checkout that can be the parent, and the parent's own fork state and parent then stand in for the head the contribution pushes from; the value `origin` reports is named unparsed, because `gh` resolves every remote form to the same repository ([test](tests/test_target_resolution.compliance.l1.py))
