...
`/upstream` is named for the artifact one invocation produces rather than as an imperative phrase, and it is the marketplace's only user-invocable skill named that way. `spx/local/skills.md` names invoked workflow skills as imperative phrases matching user speech, and `spx/14-skill-naming.pdr.md` reserves the noun-phrase form for reference skills. The departure is deliberate: `upstream` is the word a developer types when they want this, so the name satisfies the activation half of that rule while departing from its form, and a verb-led alternative names an action nobody asks for by name.
