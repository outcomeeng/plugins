...
`/upstream` carries the name of the artifact one invocation produces, which `spx/14-skill-naming.pdr.md` requires of every workflow skill. An operator already types `upstream` for that artifact, so that term is the name; a verb-led alternative would name an action nobody asks for and bury the target the invocation resolves. It is the marketplace's only user-invocable skill whose artifact carries such a term, and `spx/local/skills.md` states the form this case takes.
