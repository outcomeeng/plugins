...
`audit-pdr`, `audit-skills`, `audit-commands`, `audit-subagents`, `auditing-python`,
`auditing-python-architecture`, `auditing-rust`, `auditing-rust-architecture`,
`audit-typescript`, `audit-typescript-architecture`, `reviewing-pr`), `the orchestrating
...
- **Standalone `you` / `the model`** beyond prose: **DONE — complete across all six plugins.** The
  ...
  `auditing-python`/`audit-typescript` ("what the auditor catches beyond automated tools") was
  ...
  **Tracked follow-ups — DONE (named-subject voice axes, branch `work/named-subject-voice-followups`):**
  ...
  - **Uniform `<what_you_do_not_do>` → `<out_of_scope>` tag rename — DONE.** Renamed the section
    tag in the only two skills that carry it — `architecting-rust` and `architect-typescript`
  ...
  - **`<failure_modes>` "the auditor" role-noun → "Claude" — DONE.** Converted every executing-
    ...
    `auditing-python` (5 sites) and `audit-typescript` (5 sites). The `develop:skill-auditor`
  ...
  **New tracked follow-up (surfaced by the skill-auditor gate during the voice sweep — separate, structural):**
  ...
  - **Audit-skill structural conformance — marketplace-wide.** `skill-standards` mandates
    ...
    and `auditing-python-architecture` + `audit-typescript-architecture` completed their
    ...
    complex code auditors `auditing-python`, `auditing-rust`, `audit-typescript` (whether a
