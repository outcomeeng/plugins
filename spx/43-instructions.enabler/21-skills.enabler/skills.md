...
- ALWAYS: `/skill-standards` owns every rule `/audit-skills` enforces — standards and enforcement stay in one place so drift cannot open between them ([audit])
- ALWAYS: `/create-skill` and `/audit-skills` load `/skill-standards` before doing any authoring or evaluation work — prevents memory-based assessment ([audit])
- ALWAYS: `/audit-skills` emits structured verdicts and performs no file modifications — audits inform decisions; they do not implement them ([audit])
...
- NEVER: restate `/skill-standards` rules inside `/create-skill` or `/audit-skills` — a single source of truth prevents drift between standard and enforcer ([audit])
