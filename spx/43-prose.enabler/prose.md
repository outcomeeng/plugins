...
- ALWAYS: text the detection procedure leaves ambiguous is resolved by the interactive caller asking the user to select a kind from the taxonomy — never by guessing and never by inventing a style outside it; a dispatched audit honors the kind its dispatch declares, and undeclared ambiguity is reported in the verdict with the plausible kinds' shared rules audited ([audit])
