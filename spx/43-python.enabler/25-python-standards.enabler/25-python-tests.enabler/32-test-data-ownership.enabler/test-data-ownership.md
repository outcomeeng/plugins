...
- ALWAYS: executed test functions and callbacks own every behavioral predicate and assertion API call; imported harnesses and recording collaborators expose observations rather than verdicts ([review])
...
- NEVER: expected outputs are selected by the same implementation table, algorithm, parser, branch logic, or collaborator verdict method that produces the actual output — the oracle remains independently derived ([review])
