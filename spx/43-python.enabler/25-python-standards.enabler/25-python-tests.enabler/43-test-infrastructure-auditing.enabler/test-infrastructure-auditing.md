...
- ALWAYS: audit of an inert fixture checks that the fixture is a real-world payload whose complete shape matters to the assertion — isolated strings and numbers belong in source contracts or generators ([review])
- ALWAYS: audit rejects imports from inert fixture files in executed Python tests — fixtures are inert file inputs read, copied, or passed by path, never sources of exports consumed by the test runner ([review])
- ALWAYS: test-infrastructure findings identify the exact test-infrastructure file and the evidence property affected, such as source ownership, coupling, falsifiability, domain variation, oracle independence, cleanup safety, pytest discovery safety, or coverage — review output names the evidence failure, not only the artifact path ([review])
