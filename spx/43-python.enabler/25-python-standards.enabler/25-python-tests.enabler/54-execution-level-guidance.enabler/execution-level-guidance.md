...
- ALWAYS: the Python expression of the levels: `l1` covers pure functions, `tmp_path` filesystem work, standard repository-required tools and subprocesses, and dependency-injected controlled implementations under a recorded exception; `l2` covers Docker containers, local databases or queues, local dev servers, browsers against local services, and product binaries installed during bootstrap; `l3` covers remote, shared, credentialed, or network-dependent systems ([audit])
