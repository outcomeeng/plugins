...
- ALWAYS: Python execution-level guidance realizes the neutral levels through Python surfaces — `tmp_path` and `tmp_path_factory` handles for `l1` filesystem work, dependency-injected Stage 5 collaborators behind `Protocol` boundaries at `l1`, bootstrap-installed product binaries at `l2` — and never restates the neutral category lists, whose semantics derive from `spx/31-outcomeeng.enabler/31-verification.enabler/31-test-verification.enabler/21-evidence-types.pdr.md` ([audit])
