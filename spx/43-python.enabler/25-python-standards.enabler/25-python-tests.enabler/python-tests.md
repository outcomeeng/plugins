...
- ALWAYS: Python test infrastructure follows `spx/15-test-infrastructure.pdr.md`: harnesses manage resource lifecycle and access to behavior, generators produce variable input domains, fixtures stay inert, and source-owned domain truth comes from source modules ([review])
...
- NEVER: executed Python test files declare variables or constants for test data, expected outputs, runner settings, property-test configuration, setup policy, reusable cases, fixtures, generators, harness behavior, diagnostics, or source-owned singleton shapes — those choices live in spec-governed harnesses, generators, inert whole-payload fixtures, source contracts, or justified eval case data ([review])
