...
- ALWAYS: the Python filename instantiation of the canonical test-filename model in `spx/31-outcomeeng.enabler/31-verification.enabler/31-test-verification.enabler/21-evidence-types.pdr.md` is `test_<subject>.<evidence>.<level>[.<runner>].py` — expression of the model, altering no token semantics ([audit])

- ALWAYS: Python test guidance starts from the spec assertion and selected evidence type before choosing file names, runners, harnesses, generators, pytest fixtures, or examples — evidence shape follows the claim being proved ([audit])
