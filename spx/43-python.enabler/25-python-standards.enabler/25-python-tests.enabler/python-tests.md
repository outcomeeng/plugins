...
- ALWAYS: Python specified-node test authoring records the exact missing production owner, focused RED diagnostics, and relative `spx/EXCLUDE` entry without dispatching test audit; implementation removes the exclusion, passes normal deterministic gates, and returns to test audit before implementation audit ([audit])
