...
PROVIDES Python test standards for verification type, assertion type, execution level, testability, test data ownership, test-infrastructure auditing, and generator use
...
- ALWAYS: Python test guidance starts from the spec assertion and selected verification and assertion types, then emits the evidence-design row before choosing file names, runners, harnesses, generators, pytest fixtures, or examples — evidence shape follows the assertion being proved ([review])
...
- ALWAYS: Python test infrastructure follows `spx/31-outcomeeng.enabler/31-verification.enabler/31-test-verification.enabler/15-test-infrastructure.pdr.md`: harnesses manage resource lifecycle and access to behavior, generators produce variable input domains, fixtures stay inert, source-owned domain truth comes from source modules, and every evidence-design reference pairs the product-root-relative Markdown link to the governing spec or decision with an implementation link when that implementation exists ([review])
...
- ALWAYS: pytest fixture callables that perform setup, teardown, cleanup, or dependency access are harness entrypoints and live under `product_testing/harnesses/`; `conftest.py` imports them explicitly only for pytest discovery ([review])
