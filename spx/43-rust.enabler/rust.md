...
- ALWAYS: the Rust plugin's testing skills (`/rust-test-standards`, `/test-rust`, `/audit-rust-tests`) teach the separate workspace-member crate contract from `spx/31-outcomeeng.enabler/31-verification.enabler/31-test-verification.enabler/15-test-infrastructure.pdr.md` through the portable Cargo package placeholder `<package>-testing` and Rust import placeholder `<package>_testing` ([review])
...
- ALWAYS: Rust specified-node test authoring records the exact missing production owner, focused RED diagnostics, coverage as not applicable, and relative `spx/EXCLUDE` entry without dispatching test audit; implementation removes the exclusion, passes normal deterministic gates, and returns to test audit before implementation audit ([audit])
- NEVER: executed Rust test files declare `const`, `static`, or `let` bindings; every value or configuration choice those declarations would bind lives in the `<package>-testing` workspace crate, source contracts, inert whole-payload fixtures, or justified eval case data ([review])
