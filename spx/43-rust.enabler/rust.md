...
- NEVER: executed Rust test files declare `const`, `static`, `let`, or helper bindings for test data, expected outputs, runner settings, property-test configuration, setup policy, reusable cases, fixtures, generators, harness behavior, diagnostics, or source-owned singleton shapes — those choices live in the `product-testing` workspace crate, source contracts, inert whole-payload fixtures, or justified eval case data ([review])
