...
- ALWAYS: controlled implementations and recording collaborators implement the same typed interface boundary as production, preserve behavior-relevant state, and expose observations while the linked test owns every `expect` and behavioral predicate, per `spx/31-outcomeeng.enabler/31-verification.enabler/31-test-verification.enabler/test-verification.md` ([audit])
...
- NEVER: a controlled implementation or recording collaborator accepts an expected outcome, calls `expect`, an assertion API, or a matcher, exposes a verdict-shaped method, or replaces the behavior the assertion claims to verify ([audit])
