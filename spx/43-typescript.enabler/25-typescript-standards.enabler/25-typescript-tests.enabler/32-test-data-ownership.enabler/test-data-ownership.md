...
- ALWAYS: property-based test configuration — run counts, seed selection, replay output, and failure diagnostics — lives in a spec-governed harness so every property failure reports the seed and replay path ([review])
...
- NEVER: declare variables or constants inside executed TypeScript test files to hold test data, expected outputs, runner settings, property-test configuration, setup policy, reusable cases, fixture paths, generator choices, or harness behavior — test files contain assertion flow, not ownership ([review])
