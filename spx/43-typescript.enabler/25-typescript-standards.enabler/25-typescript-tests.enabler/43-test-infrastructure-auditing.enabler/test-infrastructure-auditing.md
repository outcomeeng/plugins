...
- NEVER: approve an executed TypeScript test file that declares variables or constants for test data, expected outputs, runner settings, property-test configuration, setup policy, reusable cases, fixtures, generators, or harness behavior — declarations identify ownership in the wrong layer ([review])
