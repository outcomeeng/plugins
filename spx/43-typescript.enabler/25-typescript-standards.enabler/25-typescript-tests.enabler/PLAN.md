...
1. Move one compact topic out of `standardizing-typescript-tests/SKILL.md` into a reference file so the overview stays under the `/standardizing-skills` line-count target.
...
5. Gate with `spx validation markdown`, `spx spec status --format json`, `just check-skills`, `just docs-check`, and `typescript:auditing-typescript-tests`.
