...
PROVIDES TypeScript test standards for evidence type, execution level, testability, test data ownership, support-module auditing, and generator use
...
- ALWAYS: harnesses manage external resources while generators produce input values — neither layer stores arbitrary domain literals as inert test-owned fixtures ([review])
- ALWAYS: fixture files are inert input artifacts read from disk, copied into temp projects, or passed by path to code or tools under test — fixtures may be valid `*.ts` source files for linters, parsers, pre-commit hooks, or scanners, but executed tests never import them as modules or consume their exports ([review])
- ALWAYS: TypeScript test audit opens imported support modules, including generators, harnesses, fixtures, and helpers — laundering and severed coupling can live outside the test file ([review])
