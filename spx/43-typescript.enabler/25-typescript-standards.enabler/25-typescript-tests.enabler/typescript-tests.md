...
- ALWAYS: TypeScript test audit opens imported test-infrastructure modules — generators, harnesses, fixtures — before approving an assertion; laundering and severed coupling can live outside the test file, and the canonical home for these modules is the `@testing/` path mapping per `spx/15-test-infrastructure.adr.md` ([review])
