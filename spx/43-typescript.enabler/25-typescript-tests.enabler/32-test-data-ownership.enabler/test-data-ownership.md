...
CAN distinguish source contracts from generated input domains and avoid hiding literals in test-owned infrastructure
...
- ALWAYS: values that only the test needs are generated when they represent a variable input domain such as paths, names, identifiers, option sets, file contents, encodings, counts, or project shapes — generated values expand the exercised space ([review])
...
- ALWAYS: harnesses manage setup, teardown, and access to external resources such as filesystems, browsers, APIs, Docker, project binaries, or local services — harnesses do not own arbitrary test data ([review])
...
- ALWAYS: executed tests access fixtures only by reading files, copying files into temp projects, or passing fixture paths to the code or tool under test — fixture contents remain inputs rather than executable test dependencies ([review])
...
- NEVER: create shared test-owned constants such as `TEST_FIXTURES`, `SAMPLE_PATHS`, `TYPICAL`, or `EDGES` to satisfy literal-reuse checks — named example bags preserve hand-picked values ([review])
