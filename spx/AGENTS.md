...
5. **Deterministic context injection**: The tree structure defines what context gets loaded for work on a target and is injected by the `/contextualize` skill.
...
Use `/decompose` to create or restructure child nodes. It owns concern boundaries, node types, ordering evidence, and sparse index assignment.
