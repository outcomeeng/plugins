...
## 21 bundled reference files over 100 lines carry no table of contents

`/skill-standards` `<progressive_disclosure>` requires a table of contents at the top of every reference file over 100 lines, "so partial reads still see the full scope". Twenty-one files across five plugins exceed the threshold without one, ranging from 109 to 971 lines:
...
## Thirty-three `PLAN.md` files remain in a tree that declares methodology 4.0

`spx.config.yaml` declares methodology 4.0, under which pending work is a Change — realized here by `spx/local/coordination.md` as one issue per Change in `outcomeeng/changes` — and `ISSUES.md` carries known imperfections; `PLAN.md` is not a version 4 artifact. Six `PLAN.md` files in the `spx/EXCLUDE` removal's path were converted to `ISSUES.md` entries or deleted as realized. Thirty-three remain, from `spx/PLAN.md` and `spx/21-spec-tree.enabler/76-merge.enabler/PLAN.md` (158 lines) to `spx/43-typescript.enabler/25-typescript-standards.enabler/PLAN.md` (259 lines).

**Resolution shape**: one sweep, or one changeset per top-level node: contextualize each owning node, move still-true pending work into a Change in the change store or, where it records an imperfection, into that node's `ISSUES.md`, drop what is realized or superseded, delete the file, and re-point every reference (the shipped `contextualize` skill still reads `PLAN.md`, which is harmless once none exist). The shipped methodology's own `PLAN.md` references are the 4.0 migration's concern, tracked in `spx/PLAN.md` under "Refine the methodology 4.0 migration as a Change" until that file is itself converted.
