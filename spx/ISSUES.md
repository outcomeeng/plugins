...
Each command already maps to a governing skill (e.g. `/commit` → `committing-changes`, `/apply` → `applying`, `/author` → `authoring`, `/bootstrap` → `bootstrapping`, `/clarify` → `clarify`/`interviewing`, `/review-changes` → `reviewing-changes`, `/rtfm` → `refocusing`). The `develop` plugin also ships `creating-commands` and `auditing-commands` skills whose status this convention affects — fold their disposition into the migration. `/open-pr` is removed by `spx/21-spec-tree.enabler/76-merging.enabler/32-github-pr.enabler/`, which makes `/pr` the shipping route and treats opening/management as internal protocols.
