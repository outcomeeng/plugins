...
`spx.config.yaml` declares methodology 4.0.0, under which pending work is a Change — realized here by `spx/local/coordination.md` as one issue per Change in `outcomeeng/changes` — and `ISSUES.md` carries known imperfections; `PLAN.md` is not a version 4 artifact. Six `PLAN.md` files in the `spx/EXCLUDE` removal's path were converted to `ISSUES.md` entries or deleted as realized. Thirty-three remain, from `spx/PLAN.md` and `spx/21-spec-tree.enabler/76-merge.enabler/PLAN.md` (158 lines) to `spx/43-typescript.enabler/25-typescript-standards.enabler/PLAN.md` (259 lines).
