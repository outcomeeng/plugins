...
The command derives both complete plugin sets from the merged checkout's committed catalogs, enforces the canonical GitHub source, and installs every plugin into Claude Code project scope and the selected `CODEX_HOME`. The run still executes shipped Codex lifecycle placement against the invocation checkout; `spx/12-marketplace-state.adr.md` makes checkout placement the owning plugin lifecycle skill's explicit opt-in, and `spx/32-distribution.enabler/21-installation.enabler/21-repository-installation.enabler/PLAN.md` retires that automatic step from the installation run — treat any `.codex/agents/` churn a release run produces as pending-removal behavior, never as definitions to commit. A colliding Claude Code user-scope `outcomeeng` registration withholds `RELEASE_READINESS` before mutation. Preserve the structured first-failure diagnostic when the command fails.
