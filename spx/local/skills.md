...
ALWAYS: An invoked workflow skill drops the imperative form and takes the artifact's own term when no verb names what the operator asks for, because every candidate imperative names the skill's own mechanism instead. Decide it by whether operators say the verb: they ask to open a pull request, manage an issue, and sync a fork, so those skills stay imperative; nobody asks to resolve a target, so `/upstream` takes the term they do say.
