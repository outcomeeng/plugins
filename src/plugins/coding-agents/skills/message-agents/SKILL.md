...
allowed-tools: Bash(printf:*), Bash(python3 "${CLAUDE_SKILL_DIR}/scripts/agent_message.py":*)
---
...
1. Discover the caller and recipients:
   ...
   `python3 "${CLAUDE_SKILL_DIR}/scripts/agent_message.py" discover`
...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/agent_message.py" send <<'JSON'
...
```bash
printf '%s\n' '{"toPane":"<complete-pane-uuid>","kind":"fact","subject":"<subject>","facts":["<authoritative fact>"],"request":null}' | python3 "${CLAUDE_SKILL_DIR}/scripts/agent_message.py" send
```
