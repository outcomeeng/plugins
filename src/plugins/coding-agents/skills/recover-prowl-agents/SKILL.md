...
allowed-tools: Bash(prowl list --json), Bash(prowl agents --json), Bash(prowl read --pane * --last 80 --wait-stable --json), Bash(python3 "${CLAUDE_SKILL_DIR}/scripts/recover_agents.py":*), {{! tool('ask_user') !}}
---
...
3. Add an unoccupied candidate only when its complete UUID and visible evidence make the stopped coding-agent session obvious, or when the operator explicitly names that pane. An ordinary shell, conflicting evidence, or uncertain intent remains stopped. Present ambiguous panes through `{{! tool('ask_user') !}}` with their complete pane and worktree identities; never guess.
...
```text
python3 "${CLAUDE_SKILL_DIR}/scripts/recover_agents.py" recover --pane <pane-uuid> [--pane <pane-uuid> ...]
```
...
```text
python3 "${CLAUDE_SKILL_DIR}/scripts/recover_agents.py" verify --pane <pane-uuid> [--pane <pane-uuid> ...]
```
...
**How to avoid:** Leave ambiguous panes stopped and present their complete pane and worktree identities through `{{! tool('ask_user') !}}` before adding them to the target set.
