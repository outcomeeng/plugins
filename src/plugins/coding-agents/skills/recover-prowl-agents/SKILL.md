...
14. Run `reassess` with the verified result and the complete ordered `paneReadResults`. `already-current` emits no delivery. `reassessment-ready` emits one source-owned continuation instruction for each verified non-controller session absent from `reassessedSessionIds`. The instruction requires the resumed agent to reconcile the native conversation and every explicitly presented plan or context artifact against the work actually delivered before continuing or marking anything complete; separate useful work never absorbs an unreconciled plan. Every emitted instruction also states that its recipient is not the recovery controller and must neither invoke recovery nor send text or keys to any pane; a recipient carrying this skill otherwise reads restart framing as its own recovery mandate. No emitted instruction names the recipient's next action, and uniform wording across recipients is correct: the controller works from a rendered screen while the recipient holds the conversation that decides, so a per-recipient next step ships the controller's inference as the recipient's decision.
...
printf '%s\n' '{"prepared":{},"bindings":[],"verification":{},"paneReadResults":[]}' | python3 "${CLAUDE_SKILL_DIR}/scripts/recover_agents.py" reassess
```
...
- NEVER send a resumed session an instruction to exit, stop, or stand down, and NEVER prescribe its next action, disposition, or conclusion — a continuation instruction states only what the restart destroyed and routes the recipient's doubt to the operator, because the controller reads a rendered screen while the recipient holds the conversation that decides.
