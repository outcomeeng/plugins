...
With findings to answer, draft the comment per `<reply_shape>` and review it per `/contribution-standards` `<invariants>` "Outward-facing text is permanent" — the prose plugin's `{{! subagent_name('prose', 'prose-auditor') !}}` thin agent where installed, `<outward_text>` unassisted where not.
