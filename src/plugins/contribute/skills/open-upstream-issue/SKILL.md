...
**Step 1 — Load the standards and read the invocation input.** {!% require_skill 'contribute:contribution-standards' %!}
...
**Step 6 — GATE: Review the outward text.** Draft the title and body per `<report_shape>`, then review them — the prose plugin's `{{! subagent_name('prose', 'prose-auditor') !}}` thin agent where installed, `/contribution-standards` `<outward_text>` unassisted where not, stated as such in the report.
