...
1. Require `$ARGUMENTS` to be `HEAD` or an explicit committed three-dot range. For absent or invalid input, return `blocked` before mutation. Read the product's {{! file('root_guide') !}} and invoke `/understand` when its foundation marker is absent.
