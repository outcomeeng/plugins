...
4. Read the {{! term('configured_agent') !}} configuration file at `$configured_agent_path`.
5. Follow the owning skill's link for each remaining guide whose area the configuration puts in play, and skip the rest. A guide whose condition holds is required: when it cannot be read, STOP with `REJECTED` and a critical issue naming the unreadable guide and the area left unjudged.
   ...
   - `orchestration-patterns.md` when the configuration delegates, coordinates, or spawns other {{! term('configured_agents') !}}
...
- **Description style**: The directive form `/agent-prompt-standards` `<description_style>` prescribes, naming the triggers that select this {{! term('configured_agent') !}}. That standard owns the convention for skills and {{! term('configured_agents') !}} alike.
