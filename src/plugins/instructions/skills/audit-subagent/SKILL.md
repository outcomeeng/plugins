...
<objective>
An `APPROVED` or `REJECTED` verdict on one {{! term('configured_agent') !}} configuration file against `/subagent-standards` and `/agent-prompt-standards`, with findings grouped as critical-issues, recommendations, strengths, and quick-fixes, each naming its location, violated convention, evidence, and consequence.
</objective>
