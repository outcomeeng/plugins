...
Run the script tests and the target repository's canonical skill build and deterministic checks, then obtain a fresh typed `{{! subagent_name('instructions', 'skill-auditor') !}}` verdict over the complete bundle. Remove the script when it cannot meet the declared test or portability contract.
...
- Every bundled citation resolves, repository checks pass, and a typed `{{! subagent_name('instructions', 'skill-auditor') !}}` verdict approves the complete bundle.
