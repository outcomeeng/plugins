...
Render one representative instance, verify that no unresolved placeholder remains, run the target repository's canonical skill checks, and obtain a fresh typed `{{! subagent_name('instructions', 'skill-auditor') !}}` verdict over the complete bundle.
