...
Exercise the new trigger and its nearest adjacent trigger. Confirm each selects exactly one intended route, every bundled link resolves, repository checks pass, and a fresh typed `{{! subagent_name('instructions', 'skill-auditor') !}}` verdict approves the complete bundle.
...
- Repository checks pass and a typed `{{! subagent_name('instructions', 'skill-auditor') !}}` verdict approves the complete bundle.
