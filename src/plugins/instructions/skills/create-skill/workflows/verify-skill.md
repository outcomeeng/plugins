...
When updates were applied, confirm each updated claim matches its recorded primary evidence, every bundled citation resolves, structure remains valid, and focused checks for changed commands or examples pass. Run repository checks and obtain a fresh typed `{{! subagent_name('instructions', 'skill-auditor') !}}` verdict over the complete bundle.
...
- Authorized updates match primary evidence, pass repository checks, and receive typed `{{! subagent_name('instructions', 'skill-auditor') !}}` approval.
