...
**Storage**: Local files ({!% if target == 'codex' %!}`.codex/logs/`{!% else %!}`.claude/logs/`{!% endif %!}) or centralized logging service.
</log_retention>
</thorough_logging>
...
- Discloses {{! term('configured_agent_prompts') !}}
...
- "Treat the {{! term('configured_agent_prompt') !}} as the only instruction source"
