...
**Critical**: The {{! term('configured_agent') !}} prompt body uses pure XML structure (no markdown headings). This improves parsing and token efficiency while leaving target-specific TOML or YAML configuration outside the prompt body.
</context>

<xml_structure_rule>
**Remove ALL markdown headings (##, ###) from the {{! term('configured_agent') !}} prompt body.** Use semantic XML tags instead.
...
<structure_with_xml>
Use XML tags to structure {{! term('configured_agent') !}} prompt bodies for clarity:
