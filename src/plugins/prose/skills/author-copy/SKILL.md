...
{!% require_skill 'prose:copy-standards' %!}
...
Adverbs earn their place by adding real information. "The server responded in 14ms" is precise. An adverb that exists only to make something sound more meaningful than the sentence demonstrates is dead weight. When a sentence demonstrates a quality through its content, it does not need an adverb to assert that same quality.
...
Groups of items have the size the content demands — sometimes two, sometimes five — never a preference for any particular count.

Transitions connect ideas, not slots. A paragraph that follows logically often needs none; when one is needed it names the actual relationship. A transition word deletable without losing meaning is deleted.

Participial phrases at sentence endings add new information, or they go.
