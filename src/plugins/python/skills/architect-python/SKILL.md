...
- **Product context** — `{{! file('root_guide') !}}` for navigation, node status, and sparse-integer index dependencies. For testing methodology, invoke `/test` (foundational), `/python-test-standards` (standards), and `/test-python` (patterns).
