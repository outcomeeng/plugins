...
fn processes_simple_input() {
    <product>_testing::harnesses::processing::assert_processes_case(simple_input_case(), process);
}
```
...
fn rejects_empty_input() {
    <product>_testing::harnesses::processing::assert_rejects_empty_input(process);
}
```
...
fn processes_known_cases() {
    <product>_testing::harnesses::processing::assert_processes_known_cases(
        <product>_testing::generators::processing::known_process_cases(),
        process,
    );
}
```
...
fn canonical_key_roundtrips() {
    <product>_testing::harnesses::properties::assert_canonical_key_roundtrips(
        <product>_testing::generators::keys::canonical_key_strings(),
        CanonicalKey::parse,
    );
}
```
