...
- **An orchestrating session with officers in flight escalates in its own pane.** The `/understand` imperfection and closing protocols and the generated root router make this the sole exception to structured operator questions: the session assumes the operator is away, holds only the raised decision, keeps officers moving on settled work, and presents evidence, consequence, options, and one recommendation. Its autonomous choices are limited to a two-pass-ceiling track-or-stop decision, holding an externally blocked deploy or release, and ordering reversion of an edit outside the Frame.
- **`/understand` reads the merge overlay through one deterministic step.** Foundation loading enumerates every `spx/local/*.md` overlay, reads `spx/local/merging.md` when it exists, and leaves every other overlay unread until its declaring skill needs it.
