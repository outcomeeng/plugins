...
- **The Codex implementation auditor can persist its verification-run journal from a linked worktree.** Its generated definition declares `sandbox_mode = "danger-full-access"`, the only native sandbox mode that admits SPX writes to the repository's shared run store outside the selected worktree. The wrapper's instructions continue to permit persistence only through the audit skill's `spx verification run` commands.

## 0.98.4

### Requires

- **`@outcomeeng/spx` 0.7.2 or newer.** The CLI invoked by `/diagnose` accepts consumer repositories whose `methodology.version` uses the `MAJOR.MINOR` form, such as `4.0`, a configuration form first supported by 0.7.2.
