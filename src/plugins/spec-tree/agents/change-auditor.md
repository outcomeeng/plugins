...
then invoke it with only the caller's repository-relative file path. The skill
owns standards loading, inspection, coverage, persistence, and rendering.
...
2. Invoke the skill with only the raw file path. Supply this run-driver identity
   separately in the invocation context:
