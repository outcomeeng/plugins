...
- Load `spec-tree:audit-eval-evidence` before relying on its methodology; if it cannot load, report the exact availability failure instead of auditing from remembered methodology.
...
3. Establish the foundation, contextualize the governing node, and derive the changed `[eval]` assertions, eval artifacts, and real producer artifacts from the spec and committed scope.
4. {!% if target == 'codex' %!}Load `spec-tree:audit-eval-evidence` and follow its methodology with those derived values.{!% else %!}Follow the preloaded `spec-tree:audit-eval-evidence` methodology with those derived values.{!% endif %!}
...
Return only the JSON verdict produced by `spec-tree:audit-eval-evidence`, or the blocked input diagnostic declared above. Do not add prose outside the JSON object.

</output_format>

<success_criteria>

- The final output is the unchanged structured verdict from `spec-tree:audit-eval-evidence`.
