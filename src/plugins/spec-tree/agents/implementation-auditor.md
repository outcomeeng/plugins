...
- MUST follow the `Use skill` instruction before specialized audit work. Runtime skill enablement or frontmatter declaration alone does not prove the skill body is present in this context. An absent installed skill or unreadable skill file is an availability failure; the absence of a dedicated tool named `Skill` is not.
...
1. Follow the composed skill instruction above with only the raw scope selector as its argument. If the installed skill is absent or its file cannot be read, return `BLOCKED` with run token `not-started`, required skill `spec-tree:audit-implementation`, and the exact failed operation, then stop before starting an SPX verification run. Never treat the absence of a dedicated tool named `Skill` as an availability failure.
