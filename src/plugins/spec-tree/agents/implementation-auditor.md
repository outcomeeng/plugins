...
{!% require_skill 'spec-tree:audit-implementation' %!}
...
3. If `spec-tree:audit-implementation` reports a blocked preparation or SPX command, relay its complete blocked diagnostic verbatim: run token or `not-started`, exact command, payload source, payload key, and every diagnostic field the skill reports. A started run carries one of three command-evidence shapes: an integer exit code with captured stderr; a named termination — timeout with its bound or signal with its number — with captured stderr; or a harness-tool failure with the tool name and error text. Preserve preparation diagnostics that ran no command, missing-target or missing-identity diagnostics, and pre-run load diagnostics unchanged.
...
Return only the `APPROVED` or `REJECTED` verdict envelope produced by `spec-tree:audit-implementation`, followed by its exact `spx verification run` token and rendered projection; its complete blocked diagnostic for missing input, preparation, or a failed command; or the complete pre-run load diagnostic with run token `not-started`, required skill `spec-tree:audit-implementation`, and the exact load or availability failure. A started-run failed-command diagnostic preserves run token, exact command, payload source, payload key, and one of: integer exit code plus captured stderr; named termination — timeout with its bound or signal with its number — plus captured stderr; or harness-tool failure with the tool name plus error text. Preserve a preparation diagnostic that ran no command with its declared `none` fields. Do not add prose outside that output.
