...
4. {!% if target == 'codex' %!}Load `spec-tree:audit-pdr` and follow its methodology with those path-only values.{!% else %!}Follow the preloaded `spec-tree:audit-pdr` methodology with those path-only values.{!% endif %!}
...
Return only the JSON verdict produced by `spec-tree:audit-pdr`, or the blocked input diagnostic declared above. Do not add prose outside the JSON object.
