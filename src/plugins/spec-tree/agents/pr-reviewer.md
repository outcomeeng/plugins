...
Run `spec-tree:review-pr` (composed mode) and `spec-tree:audit` on the same PR diff, then post one combined PR comment: review prose, separator, audit verdict rendered as the `markdown+json` carrier.
...
3. **Run the audit.** Invoke `spec-tree:audit` with the diff range as scope and `--format markdown+json`. Capture the rendered output (markdown table plus the HTML-comment-delimited JSON block).
