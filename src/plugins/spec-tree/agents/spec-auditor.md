...
3. {!% if target == 'codex' %!}Load `spec-tree:audit-specs`; return the skill-unavailable diagnostic and stop if loading fails.{!% else %!}Confirm the preloaded `spec-tree:audit-specs` methodology is available; return the skill-unavailable diagnostic and stop if it is unavailable.{!% endif %!}
