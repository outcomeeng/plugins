...
**Spec file** means the file whose name matches the directory slug: `auth.spec.md` inside `10-auth.capability/`, or `auth.md` inside `10-auth.enabler/` in the transition form. Other `.md` files in the directory (like `{{! file('root_guide', 'claude') !}}` and `{{! file('root_guide', 'codex') !}}`) are not spec files — skip them.
