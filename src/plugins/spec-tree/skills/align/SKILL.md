...
4. **Discover**: For a file scope, use that file directly. For a directory scope, glob `{scope}/**/*.md`. For a changeset scope, use every Markdown path in the caller-supplied changed-file set directly. In every mode, exclude `{{! file('root_guide', 'claude') !}}` and `{{! file('root_guide', 'codex') !}}`, `PLAN.md`, `ISSUES.md`, files inside `tests/`, and files inside `spx/local/`.
