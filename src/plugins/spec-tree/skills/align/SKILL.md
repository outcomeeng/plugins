...
3. **Scope**: Materialize one subject list and one report-scope token according to the requested mode:
   ...
   - **Branch-changeset mode**: Invoke `/scope-changeset`, run `python3 "${CLAUDE_SKILL_DIR}/scripts/derive_changeset_scope.py" .`, and read `base_ref` plus the complete `changed_files` array. Set the report-scope token to `origin/{base_ref}...HEAD`. Keep the complete array for downstream-alignment analysis; derive conformance candidates only from its existing `.md` paths under `spx/`. Do not glob a changeset token or replace the returned array with a caller-supplied subset.
4. **Discover**: Before classification, remove every candidate whose basename is `{{! file('root_guide', 'claude') !}}`, `{{! file('root_guide', 'codex') !}}`, `PLAN.md`, or `ISSUES.md`; whose path is inside a `tests/` directory; or whose path is inside `spx/local/`. The remaining paths are the complete conformance subject list for the selected mode.
