...
{!% if target == 'codex' %!}Read the explicit invocation arguments attached to `/apply` exactly once before Step 0: optional `--agent`, followed by an optional node path. Do not scan quoted examples, handoff context, or unrelated prompt prose for node paths. When no explicit invocation argument surface is present, treat it as absent and derive work from the conversation only through the fallback below.{!% else %!}The complete raw invocation from `$ARGUMENTS` controls what runs before the per-node flow below. Parse it exactly once before Step 0.{!% endif %!}

- {!% if target == 'codex' %!}explicit invocation arguments begin with `--agent`{!% else %!}`$ARGUMENTS` begins with `--agent`{!% endif %!} -> launch the `applier` agent on the optional node path that follows it. Do not run the per-node authoring steps in the main context. The `applier` role does not review the whole changeset or merge. On return, treat its live-file audit handoffs as advisory work summaries: run focused deterministic verification, apply `<verification_checkpoint>` to commit the stabilized tree, confirm the worktree is clean, and replace each live-file request with the resulting committed `<base>..<head>` scope and no live file list before dispatching the auditor. Then continue with Step 9 (when the change is cross-node) and Step 10 over the resulting changeset.
- {!% if target == 'codex' %!}explicit invocation arguments contain a node path without `--agent`{!% else %!}`$ARGUMENTS` contains a node path without `--agent`{!% endif %!} -> the work queue is that single node.
- {!% if target == 'codex' %!}the explicit invocation argument surface is absent or contains no node path{!% else %!}empty `$ARGUMENTS`{!% endif %!} -> determine the work from the conversation; if nothing is clear, read `spx/EXCLUDE`, treat each non-comment, non-blank entry as relative to `spx/`, prefix it with `spx/`, and queue the resulting full node paths. If no work is found, report "Nothing to apply" and stop.
...
When a gate returns `REJECTED`, `UNKNOWN`, or `BLOCKED`, or when a review surfaces a valid finding, treat it as evidence of a defect class. For `REJECTED`, re-invoke the matching authoring counterpart and reread every `*-standards` skill shared by that authoring and audit pair before planning a repair; the rejection proves the prior authoring pass failed to apply its governing standards, so the finding alone and remembered rules are insufficient. Then read the touched node(s) — the files they govern — find same-class instances, write the standards-derived repair plan, and fix the class before re-running the gate. Same-class means the same rule, source contract, evidence pattern, lifecycle step, generated-source relationship, or architectural boundary. A patch to the cited line alone is sufficient only when the sweep proves the defect isolated.
...
Dispatch `test-evidence-auditor` before Step 9 when the diff creates or modifies any `[test]` assertion, linked test file, or test-infrastructure artifact imported by a linked test. Include the governing node, assertion text or spec path plus assertion headings, and the test files in the dispatch prompt. If the auditor returns `overall: REJECTED`, a `FAIL` row, or a reject finding, fix the evidence defect class, re-run deterministic verification, and re-dispatch before Step 9.
