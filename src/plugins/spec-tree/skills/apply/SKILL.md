...
- {!% if target == 'codex' %!}invoking prompt contains no node path{!% else %!}empty `$ARGUMENTS`{!% endif %!} -> determine the work from the conversation; if nothing is clear, read `spx/EXCLUDE` and queue every node path it lists (one per non-comment, non-blank line). If no work is found, report "Nothing to apply" and stop.
