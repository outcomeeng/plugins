...
2. Dispatch `{!% if target == 'codex' %!}spec-tree_change-auditor{!% else %!}spec-tree:change-auditor{!% endif %!}` once through the native subagent capability with only the normalized repository-relative candidate path. Start without authoring history or a suggested verdict.
