...
- [ ] Every `[test]` link that resolves to an existing file follows the repository's active language-specific `/test-*` naming contract, including its evidence-kind and execution-level encoding; if a legacy or noncanonical name is found, flag it as an imperfection and surface it via {{! tool('ask_user') !}} before proceeding
