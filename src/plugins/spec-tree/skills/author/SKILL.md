...
**PREREQUISITE**: Invoke `/understand author` on every run. Continue only when it returns both a live `<SPEC_TREE_FOUNDATION>` marker and a `<SPEC_TREE_FOUNDATION_MATERIALS profile="author">` receipt covering the authoring references, all five structural templates, and every filled example.

Use the loaded template and examples that match the artifact being drafted. Never derive a path into the understanding skill's bundle.
...
If unclear which type, apply the loaded node-types decision table:
...
- Apply the forcing question from the loaded node-types reference: write it as an enabler first. Why can't this be PROVIDES X SO THAT Y CAN Z? What is uncertain about which output achieves the goal?
...
Use the appropriate structural template loaded by `/understand author`. Fill it using the gathered content.

**Voice rules** (from the loaded durable-map reference):
...
**Assertion rules** (from the loaded assertion-types reference):
...
- [ ] Nesting rules respected: outcomes CANNOT be children of enablers, per the loaded node-types `<nesting_rules>` section
- [ ] For outcomes: verify the forcing question from step 4 was answered — are the assertions a bet (majority could be swapped for different ones achieving the same goal)? If not, it is an enabler, per the loaded node-types reference
...
- [ ] No content misplacement under the loaded what-goes-where taxonomy
...
4. If the implementation doesn't exist yet: add the node path to `spx/EXCLUDE`. The `spx` CLI skips excluded nodes when running `spx test passing`, per the exclusion rules loaded by `/understand author`.
