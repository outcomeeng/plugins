...
3. Render the spx-level guides — `{{! file('spx_guide', 'claude') !}}` for Claude Code and `{{! file('spx_guide', 'codex') !}}` for Codex — from the template via the update-spx helper, passing the project's enabled languages so the guides record their `languages` config (and a later `/update-spx` re-renders from it, scoped to those languages):
