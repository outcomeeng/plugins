...
- `{{! file('root_guide') !}}` and `{!% if target == 'claude' %!}`{{! file('root_guide', 'codex') !}}`{!% else %!}`{{! file('root_guide', 'claude') !}}`{!% endif %!}` contain current managed Spec Tree sections scoped to the product's enabled languages while preserving independent product instructions.
