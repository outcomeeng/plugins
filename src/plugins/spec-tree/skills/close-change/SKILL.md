...
3. **Verify the terminal precondition from current state**, never from this conversation's account of it:
   ...
   - `Refined`: at least one successor exists in the store, and every known successor names this Change in its `refined_from`. Find them with `gh issue list --repo <store> --state all --search '<issue-url>' --json number,state,body,url` and read each with `gh issue view <S> --repo <store> --json number,state,body,url`; zero successors refuses the close, because a Change whose Output continues nowhere is not refined, and one absent known successor refuses the close and names it.
