...
allowed-tools: Read, Glob, Grep, Skill{!% if target == 'claude' %!}, Bash(python3:*){!% endif %!}
---
...
Read: {{! file('root_guide') !}}  (if exists)
...
**Guide files**: Read `{{! file('root_guide') !}}` when present and record it in the manifest. A freshly bootstrapped tree may lack the guide; absence is normal.
...
Read: spx/{path-to-dir}/{{! file('root_guide') !}}  (if exists)
```
