...
Verifies that the runtime's `SessionStart` hook delivered the session environment for the current session. The spec-tree plugin ships this hook for the Claude Code runtime: it writes the Claude session identity and project directories into the harness environment and records a worktree-occupancy claim. This check reads the observable traces of that work.
...
`spx worktree status --format json` reports the worktree state in its `.status` field as the two-state value `running` or `free` (spx 0.6.3 retired the `occupied`/`unclaimed`/`stale` three-state vocabulary). A `free` reading for the current worktree when the hook reports the claim written (`claimed=1`) — the live session's own claim missing — matches no row below and falls to `unknown` per step 4. Classify:
...
Verifies that the `spx` CLI is installed and on PATH, reports its version, and judges that version against the floor the spec-tree skills depend on. That floor is `{{! spx_floor !}}` — the lowest `spx` version whose capabilities the shipped skills assume.

```bash
command -v spx && spx --version
```

Classify the resolution, reporting the resolved path and version verbatim and comparing the reported version against the floor `{{! spx_floor !}}` by dotted-numeric order:

| Reading                                                                           | Verdict         | Bucket   | Remediation                                                                                                                                |
| --------------------------------------------------------------------------------- | --------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `spx` resolves on PATH and its version is at or above `{{! spx_floor !}}`         | **reachable**   | healthy  | None — report the resolved path and version verbatim.                                                                                      |
| `spx` resolves on PATH and reports a version, but it is below `{{! spx_floor !}}` | **below-floor** | degraded | Update `spx` to at least `{{! spx_floor !}}`; the spec-tree skills assume its capabilities. Report the resolved path and version verbatim. |
...
A reported version that is not dotted-numeric — a prerelease or build-tagged value that cannot be ordered against `{{! spx_floor !}}` — matches no row and falls to `unknown` per step 4, reported with the version verbatim.
...
| The store reads but one or more `doing` sessions are orphaned — the worktree on the session's `git_ref` reads `free`, or no worktree exists on that branch | **orphaned-claims** | degraded | Release or archive each orphaned session — `spx session release <id>` returns it to the queue, or `spx session archive <id>` once its work has landed. |
...
diagnose — environment report
  ...
  worktree-pool         compliant — bare-repository pool, 8 worktrees
...
diagnose — environment report
  ...
  spx-reachability      reachable — /opt/homebrew/bin/spx, 0.61.0
  worktree-pool         compliant — bare-repository pool, 7 worktrees
