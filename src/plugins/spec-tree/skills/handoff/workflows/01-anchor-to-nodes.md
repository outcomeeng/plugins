...
<no_nodes_case>
If NO spec-tree nodes were involved in this session, use `{{! tool('ask_user') !}}` with stable question id `node_anchor`, header `Node anchor`, and the question `This session's work isn't anchored to any spec-tree node. Why?`. Present these mutually exclusive options:
