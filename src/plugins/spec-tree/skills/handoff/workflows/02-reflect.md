...
- Write or update PLAN.md in the node directory (Tier 3 coordination note) — requires `{{! tool('ask_user') !}}` approval and a real stop condition
- Remove PLAN.md when the plan is done — record a clear safe local removal as fix-now for workflow 04 without approval; use {{! tool('ask_user') !}} only when removal changes ownership, scope, cost, risk, or an unresolved product choice
