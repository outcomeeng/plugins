...
<process>
When one or more persistence items require user approval, present a single `{{! tool('ask_user') !}}` with `multiSelect: true`. Group items by type: imperfections (with their destination), path-forward insights, and a skip option for coordination-only items.
...
```json
{
  "questions": [{
    ...
    "multiSelect": true,
    "options": [
      { "label": "[Imperfection → destination] summary", "description": "→ target named by nature (e.g., 'code-typescript refs', '{{! file('root_guide') !}}', 'typescript-standards', 'ISSUES.md in spx/{node}')" },
