...
If `has_access` is `false`, `is_tty` is `true`, and `available_accounts` is non-empty, ask the user via `{{! tool('ask_user') !}}` which account to switch to. Calling `gh auth switch -u <account>` is permitted only after the user has answered — that answer is the explicit instruction the safety rule requires.
