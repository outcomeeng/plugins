...
allowed-tools: Read, Grep{!% if target == 'claude' %!}, Bash(python3 "${CLAUDE_SKILL_DIR}/scripts/gh_access.py":*), Bash(git branch --show-current), Bash(git rev-parse:*), Bash(gh run view:*), Bash(gh run list:*), Bash(gh pr view:*), Bash(gh pr checks:*), Bash(gh auth switch:*){!% endif %!}, {{! tool('ask_user') !}}
model: haiku
---
...
If `has_access` is `false`, `is_tty` is `true`, and `available_accounts` is non-empty, ask the user via `{{! tool('ask_user') !}}` which account to switch to. Calling `gh auth switch --hostname <host> -u <account>` is permitted only after the user has answered — that answer is the explicit instruction the safety rule requires.
