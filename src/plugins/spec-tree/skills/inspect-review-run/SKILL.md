...
allowed-tools: Bash(python3 "${CLAUDE_SKILL_DIR}/scripts/inspect_review_run.py":*)
---

{!% require_skill 'spec-tree:verification-run-journal-standards' %!}
...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/inspect_review_run.py" <run-token>
```
...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/inspect_review_run.py" <run-token> --branch-slug <slug>
```
