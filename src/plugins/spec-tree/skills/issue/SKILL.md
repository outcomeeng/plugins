...
- `core.bare` is `true` — a bare-repository pool: run `spx -C <target-dir> diagnose --format json`, read the sole `worktree-pool` record, and require `verdict=compliant` and a non-empty absolute `readings.mainCheckoutPath`; `<queue-host>` is that path. Require that `git -C <main-checkout-path> rev-parse --path-format=absolute --git-common-dir` equals the common directory resolved in Step 1. Do not switch, detach, commit, or otherwise move the invoking or target worktree.
...
Then resolve the current runtime identity verbatim with {!% if target == 'codex' %!}`printenv CODEX_THREAD_ID`{!% else %!}`printenv CLAUDE_SESSION_ID`{!% endif %!} and STOP when it is empty. Run `spx -C <queue-host> session handoff`, passing the JSON header line then the body on stdin:
