...
**Step 6 — GATE: Confirm the target, then file the follow-up.** The handoff writes into a repository the operator did not name in this turn, resolved from a marketplace registration rather than supplied as a path. Resolving a path is not authorization to write to it, so obtain confirmation through `{{! tool('ask_user') !}}` before the first mutating command, presenting:
...
- [ ] A marketplace-resolved target was approved by the operator through the step 6 confirmation, which named the absolute target root verbatim, before any `spx -C <target-dir>` mutation ran.
