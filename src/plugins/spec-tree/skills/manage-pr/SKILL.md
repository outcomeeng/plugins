...
arguments: pr
allowed-tools: Read, Glob, Grep, Edit, Write,{!% if target == 'claude' %!} Agent,{!% else %!} {{! tool('spawn_agent') !}}, {{! tool('wait_agent') !}}, {{! tool('close_agent') !}},{!% endif %!} Skill, Bash(gh auth status:*), Bash(gh repo view:*), Bash(gh pr view:*), Bash(gh pr edit:*), Bash(gh pr ready:*), Bash(gh pr checks:*), Bash(gh pr comment:*), Bash(gh pr review:*), Bash(gh pr merge:*), Bash(gh run view:*), Bash(gh api repos/*/pulls/*/comments:*), Bash(gh api repos/*/actions/jobs/*:*), Bash(python3 "${CLAUDE_SKILL_DIR}/scripts/resolve_review_thread.py":*), Bash(git fetch:*), Bash(git branch:*), Bash(git status:*), Bash(git log:*), Bash(git diff:*), Bash(git rev-parse:*), Bash(git merge-base:*), Bash(git rebase:*), Bash(git push:*), Bash(git switch:*), Bash(git ls-remote:*), Bash(git cherry:*), Bash(git worktree list:*), Bash(printf:*)
---
...
Resolve the repository default branch in the same pass. Inspect the captured `body` for the complete `## Stack` section and its merge-order line before classifying topology. When that line names a previous stack branch, record `topology=stacked` and `stack_base=<named-branch>` regardless of the current `baseRefName`, then inspect the PR for that stack-base branch. While the stack-base PR is unmerged, require `baseRefName == stack_base`; after GitHub auto-retargets a merged stack to the default branch, the preserved Stack note keeps the PR classified as stacked until Step 4 completes reconstruction. A Stack note whose named branch disagrees with a non-default `baseRefName`, or an unmerged stack base paired with `baseRefName == <default>`, is `MERGE_BLOCKED:stack-topology-inconsistent`. Classify the PR as `topology=peer` only when `baseRefName` equals the default and the body carries no Stack note. A non-default `baseRefName` without the required Stack note is also `MERGE_BLOCKED:stack-topology-inconsistent` rather than guessed into either topology.
...
- The same closeout contract applies for every invocation: continue remaining in-scope work, then invoke `/handoff` plain with the branch-state closeout record when the session is complete.
