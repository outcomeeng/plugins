...
Resolve the repository default branch in the same pass. When `baseRefName` equals that default, `isDraft` is false, and the captured `body` contains no complete `## Stack` section, classify the PR as `topology=peer` immediately; this host-observed state is authoritative after stack reconstruction, so never inspect branch naming for a retired stack identity. Otherwise resolve a previous stack branch in the fallback order from /merging-standards `<branch_topology>`: first inspect the captured `body` for the complete `## Stack` section and its merge-order line; when absent, inspect the repository's branch-naming convention; then use an explicit operator instruction already present in the invocation or conversation. Prefer the body note whenever it exists because it preserves stack identity after GitHub auto-retargets the PR. When none of those sources yields a ref for a non-default-base PR, ask the operator through the runtime's structured-question capability rather than guessing; a pass that still has no resolved ref is `MERGE_BLOCKED:stack-topology-unresolved`.

When any permitted source yields a previous stack branch, record `topology=stacked` and `stack_base=<resolved-branch>`, then inspect the PR for that stack-base branch. Contradictory sources, a resolved stack branch that disagrees with a non-default `baseRefName`, or an unmerged stack base paired with `baseRefName == <default>` are `MERGE_BLOCKED:stack-topology-inconsistent`. While the stack-base PR is unmerged, require `baseRefName == stack_base`; after GitHub auto-retargets a merged stack to the default branch, the resolved stack identity keeps the PR classified as stacked until Step 4 completes reconstruction. Outside the authoritative reconstructed-peer state above, classify the PR as `topology=peer` only when `baseRefName` equals the default and no permitted source resolves a stack branch.

**Step 2 — Inspect three surfaces and check base drift.** Run /merging-standards `<review_inspection>` queries. Compare timestamps against the most recent push; entries after that push are re-reviews of the latest state. In the same checkpoint, select the drift ref from the topology and stack-base PR state captured in Step 1: a peer fetches `origin/<base>` and records `active_base=base`; a stacked PR whose base is unmerged fetches `origin/<stack-base>` and records `active_base=stack_base`; a stacked PR whose base is merged — including one GitHub already auto-retargeted while its body preserves the Stack note — skips the retired stack ref, fetches `origin/<default>`, and records `active_base=default`. Determine whether the branch is behind that selected ref — review state and base drift are read together so the rebase can proceed during the wait for reviews, not only after they land.
