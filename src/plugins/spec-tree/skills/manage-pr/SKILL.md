...
arguments: pr
allowed-tools: Read, Glob, Grep, Edit, Write,{!% if target == 'claude' %!} Agent,{!% else %!} {{! tool('spawn_agent') !}}, {{! tool('wait_agent') !}}, {{! tool('close_agent') !}},{!% endif %!} {{! tool('ask_user') !}}, Skill, Bash(gh auth status:*), Bash(gh repo view:*), Bash(gh pr view:*), Bash(gh pr edit:*), Bash(gh pr ready:*), Bash(gh pr checks:*), Bash(gh pr comment:*), Bash(gh pr merge:*), Bash(gh run view:*), Bash(gh api repos/*/pulls/*/comments:*), Bash(gh api repos/*/actions/jobs/*:*), Bash(python3 "${CLAUDE_SKILL_DIR}/scripts/resolve_review_thread.py":*), Bash(git fetch:*), Bash(git branch:*), Bash(git status:*), Bash(git log:*), Bash(git diff:*), Bash(git rev-parse:*), Bash(git merge-base:*), Bash(git push:*), Bash(git switch:*), Bash(git ls-remote:*), Bash(git cherry:*), Bash(git worktree list:*), Bash(printf:*)
---

<objective>
The pull request lifecycle advanced through merge, declared deploy and release phases, continuation, and operator-useful handoff, or stopped at one terminal action token naming the withholding gate.
</objective>
...
Resolve the repository default branch in the same pass. When `baseRefName` equals that default and the captured `body` contains no complete `## Stack` section, classify the PR as `topology=peer` immediately and record `ready_transition_pending=isDraft`; this host-observed state is authoritative after stack reconstruction, including recovery when the body-and-base edit succeeded but the separate ready mutation did not. Otherwise parse the complete `## Stack` section's merge-order line for `stack_base_pr_url` and `stack_base`. When the section or either value is absent for a non-default-base or draft PR, use an exact base-PR pointer already supplied by the operator; if none exists, ask through the runtime's structured-question capability rather than guessing. Resolve that pointer with `gh pr view <stack-base-pr-pointer> --json number,url,body,headRefName,headRefOid,baseRefName,state,isDraft,mergeStateStatus,statusCheckRollup,reviewDecision,reviews,comments,mergedAt`, record its host-observed URL and `headRefName`, and never inspect a branch-naming convention or reconstruct merged-PR identity from a branch label. A pass that still lacks the exact PR URL and branch emits `MERGE_BLOCKED:stack-topology-unresolved` and stops before Step 2.

When the exact base PR resolves, require its `headRefName` to equal the recorded `stack_base`, record `topology=stacked` and its returned `state` and `mergedAt`, and use that exact PR identity for every pending-or-merged decision. Contradictory metadata, a resolved stack branch that disagrees with a non-default `baseRefName`, or an unmerged stack-base PR paired with `baseRefName == <default>` emit `MERGE_BLOCKED:stack-topology-inconsistent` and stop before Step 2. While the stack-base PR is unmerged, require `baseRefName == stack_base`; after GitHub auto-retargets a merged stack to the default branch, the persisted stack-base PR URL keeps the PR classified as stacked until Step 4 completes reconstruction. Outside the authoritative reconstructed-peer state above, classify the PR as `topology=peer` only when `baseRefName` equals the default and no complete stack metadata exists.

**Step 2 — Inspect three surfaces and check base drift.** Run /merging-standards `<review_inspection>` queries. Compare timestamps against the most recent push; entries after that push are re-reviews of the latest state. In the same checkpoint, select the drift ref from the topology and exact stack-base PR state captured in Step 1: a peer fetches `origin/<base>` and records `active_base=base`; a stacked PR whose exact base PR is unmerged fetches `origin/<stack-base>` and records `active_base=stack_base`; a stacked PR whose exact base PR is merged — including one GitHub already auto-retargeted while its body preserves the Stack note — skips the retired stack ref, fetches `origin/<default>`, and records `active_base=default`. Determine whether the branch is behind that selected ref — review state and base drift are read together so the rebase can proceed during the wait for reviews, not only after they land.
...
**Step 4 — Run the safety preflight, then sync to base.** Immediately after the read-only inspection and classification in Steps 1–3, run every command in `safety_contract.pre_mutation_checks`. Run this preflight on every management pass, including when the branch is already current, so it is the first checkout-sensitive action before any rebase, finding repair, commit, push, or merge. A failed check stops before mutation with its output preserved. `<merge_cleanup>` repeats the resolved checks immediately before the merge command because the earlier follow-up work may have changed the inspected environment.

For a stacked PR identified in Step 1, use the captured stack-base PR state before ordinary base sync. While the stack-base PR is unmerged and `baseRefName` still equals `stack_base`, ensure the current PR remains draft: when the captured `isDraft` is false, run `gh pr ready <pr-number> --undo`, discard the pass, and return to Step 1 before base sync or any merge-readiness decision. When `isDraft` is true, keep the current PR draft and sync only against `origin/<stack-base>` when Step 2 found drift. Record `stack_base_pending=true`; Step 8 emits `MERGE_BLOCKED:stack-base-unmerged` rather than treating draft state as an unexplained hygiene failure. Once the stack-base PR is merged — whether `baseRefName` still names it or GitHub already auto-retargeted the PR to the default — rebase the current branch onto the `origin/<default>` ref Step 2 fetched, per /merging-standards `<base_sync>`. Re-run the peer gate from `<branch_topology>` against that default, then apply the active /commit-changes product specialization to re-evaluate any project-declared manifest or release version against the updated default base. Regenerate and commit any resulting distribution change before recording `stack_transition=true`. The transition continues in Step 6; never fetch the retired stack ref, retain a version derived from the superseded base, or leave a reconstructed branch targeting the merged stack branch.

For a peer PR, if Step 2 found the branch behind `origin/<base>`, rebase per /merging-standards `<base_sync>` now — independent of whether a review has landed and independent of whether any landed review carries findings. The preceding stacked path exclusively owns unmerged-stack synchronization and uses only `origin/<stack-base>` while `stack_base_pending=true`. A branch behind its active base is superseded before it can merge, so rebasing immediately aims CI and reviewers at the head that will actually merge and surfaces a nasty rebase early. An unresolvable conflict stops with `/sync-base`'s structured `conflict` report and active rebase state; a `dirty_tree` outcome is committed through `/commit-changes` then re-synced per `<base_sync>`, never surfaced as a conflict. Otherwise Step 6 re-establishes `VERIFICATION_READINESS` against the rebased tree — scoped by the `/sync-base` `preservation` proof per `<base_sync>`, so an unrelated base movement does not force a full re-run — and pushes it with `--force-with-lease`.
...
For each line-level review thread in the ledger, retain the inspected review-comment ID and any already-known review-thread node ID. Resolve a thread only when its finding is either unbacked with a cited governance rationale or visibly fixed in the inspected current remote head; never resolve a valid unresolved finding or a fix that exists only in an unpushed local commit. Reply in the thread with the disposition before resolving it. Invoke `resolve_review_thread.py` in direct mode only when the inspection already supplied the review-thread node ID; otherwise use discovery mode with the repository, PR number, and inspected review-comment ID. After any thread reply or resolution mutation, discard every action token and guard verdict from the pass and return to Step 1 for complete PR, check, formal-review, PR-comment, review-thread, and base-drift reinspection before any further action.
...
1. **Deterministic verification.** Run `verification_contract`'s local deterministic commands per /merging-standards `<local_deterministic_scope>` — validation and testing for the touched scope, escalating only when the contract or risk evidence requires a wider local run. Redirect verbose command output to a temporary log path and inspect only the exit status, summary, and failing sections. One scoping exception: when this push follows **only** a base-sync rebase with no Step 5 content fix, scope this command to the lane the `/sync-base` `preservation` proof and `verification_contract` select per `<base_sync>`.
...
3. **Local review at parity.** Run the local review to convergence per /merging-standards `<local_review_invocation>` — the `changes-reviewer` agent. When the active topology is peer, including `stack_transition=true` after reconstruction onto the default branch, pass only the raw scope `HEAD`; while `stack_base_pending=true` and `active_base` is the non-default stack branch, pass only the raw scope `origin/<active-base>...HEAD`, replacing `<active-base>` with the recorded literal branch. Add no interpretive scope, severity pre-filter, or instruction on what to emphasize. This re-applies to the new diff the same author-side gate /open-pr ran before the opening push; act on its findings by validity and phase per /merging-standards `<review_classification>`, committing fixes via /commit-changes. One reuse exception: when this push follows only a base-sync rebase with no content change, reuse the converged verdict only when the `/sync-base` preservation proof and `verification_contract.governance_surfaces` permit it per `<base_sync>`. Any content fix re-runs the review on the new diff.
4. **Terminal full deterministic gate.** When `verification_contract` carries a terminal full command, commit the converged subject, require a clean worktree, and run that command once after every applicable evidence auditor and the local review converge on that exact head. Never run it concurrently with another heavy command. A later change invalidates the result and reopens every affected agentic predicate before this sub-step runs again.

**Any fix in any sub-step mutates the tree, so loop:** a deterministic-verification fix is a new diff the evidence auditors and local review have not seen, an evidence-audit fix changes the evidence surface, and a review-driven fix is a new tree deterministic verification and evidence auditors have not covered. Re-run all applicable predicates after every commit until a single clean committed tree passes deterministic verification, every required evidence-auditor predicate is clean, the local review carries no unaddressed valid finding, and the terminal full deterministic gate passes when required — that converged tree is what Step 6 pushes. Never push a tree on which the later-fixed predicate was established before the last commit.

Then set `publication_phase=follow-up`, re-run /merging-standards `<branch_hygiene>` with that phase and the `active_base` recorded in Step 2, and re-run the active topology gate before the push — every hygiene predicate except the initial-publication duplicate-PR predicate applies on every follow-up push. Push via /merging-standards `<push_semantics>`; a pass that rebased in Step 4 pushes with the `--force-with-lease` form.

A `ready_transition_pending=true` peer completes the interrupted transition after its current head passes `VERIFICATION_READINESS`. Immediately before mutation, re-read the complete PR identity fields, require `headRefOid` to equal the verified current head, `baseRefName` to equal the repository default, and the body to contain no complete `## Stack` section or retired stack-base reference, then run only `gh pr ready <pr-number>`. Discard the inspection and return to Step 1 after the command. Never ask for a stack-base pointer or repeat the body-and-base edit for this durable recovery state.

A `stack_transition=true` pass is the sole draft follow-up exception. After its reconstructed head passes `VERIFICATION_READINESS` and the peer gate, force-with-lease push that head. Immediately before editing the PR, run `gh pr view <pr-number> --json number,url,body,headRefName,headRefOid,baseRefName,state,isDraft,mergeStateStatus,statusCheckRollup,reviewDecision,reviews,comments`, require the returned `headRefOid` to equal the reconstructed head just pushed, and use that fresh returned `body` as the rewrite source. Re-evaluate every entry in `publication_contract`'s `<pr_opening_specialization>.required_body_sections` against the reconstructed head and updated default base. Regenerate each applicable configured section from its body template and the current facts, remove each configured section whose applicability rule no longer holds, and place the resulting sections exactly once in declared order after `## Changes` and before the portable tail. Remove the complete `## Stack` section — from that heading through the line before the next level-two heading, or through the end of the body when no later level-two heading exists — and remove every remaining reference to the retired stack base. Preserve every other live, nonconfigured section, its order, and its content. Retarget the PR and replace its body in one `gh pr edit` mutation, then move it to ready.

Interactive Claude Code and Codex sessions use a quoted heredoc carrying the complete reconstructed body:

```bash
gh pr edit <pr-number> --base <default-branch> --body-file - <<'EOF' &&
<reconstructed live body with regenerated contract sections and the entire Stack section removed>
EOF
gh pr ready <pr-number>
```

Programmatic Claude Code, Codex, and hosted runners encode each reconstructed body line as one POSIX single-quoted shell word, replacing every literal apostrophe with the exact shell sequence `'"'"'`. Shell parsing restores the original apostrophe; every other character inside the single-quoted word remains literal. Never place an unescaped live body line inside the command. Use one physical command line for the same complete body and ready transition:
...
When `stack_base_pending=true`, withhold before this table with `merge_readiness: "WITHHOLD"`, `blocking_predicate: "stack-base-unmerged"`, `guard_verdict: "MERGE_BLOCKED:stack-base-unmerged"`, `merge_command_allowed: false`, `autonomous_action: "block"`, and `pr_comment_body: null`. Resume from Step 1 after the stack base changes state. A completed stack transition returns to Step 1 as a ready peer and uses the ordinary table below.
...
1. Read the trigger phrase from /merging-standards `reviewer_trigger_contract`.
...
Apply the first matching row in `<merge_readiness_decision_table>`. For a wait or mention result, run Step 7 and return to Step 1 in the same turn. The post-wait pass re-reads every review surface and check before deciding the next action.
...
- Use the exact command and cleanup mode from /merging-standards `merge_execution_contract`.
- Apply the universal default from /merging-standards `<merge_cleanup>` when the contract carries its default: selected merge flag `--rebase`, explicit delete-branch flag `--delete-branch=false`, and worktree-safe manual branch deletion.
- Never select merge commit (`--merge`) or squash (`--squash`) from the gate alone; those flags require a non-default resolved contract.

</step>

Run the mutation-point guard inspection per /merging-standards `<authority_gates>` and continue only after it returns `MERGE_READY:<head-sha>`. Enter the single-source /merging-standards `<merge_cleanup>` sequence; its first action runs every applicable `safety_contract` preflight immediately before the merge command, and its post-detach boundary runs every applicable post-cleanup check before branch deletion. Do not transcribe a second copy of those commands here. All cleanup stays in the assigned worktree per /merging-standards `<assigned_cwd_worktree_discipline>`.
...
**Step 9 — Continue or close.** Once the PR is merged, build the branch-state closeout record from /merging-standards `<branch_state_closeout>` and run its safe cleanup policy before deploy or release routing so every later token carries the cleanup state. If a declared deploy action exists and its authorization predicate is unsatisfied, emit `AWAIT_DEPLOYMENT_AUTHORIZATION` with the branch-state closeout record, run no deploy action, run no release action, and wait for operator authorization before re-entering Step 1. If a declared release action exists after deploy completion or a deploy no-op and its authorization predicate is unsatisfied, emit `AWAIT_RELEASE_AUTHORIZATION` with the branch-state closeout record, run no release action, and wait for operator authorization before re-entering Step 1. When declared deploy and declared release phases are complete or no-op, continue any remaining in-scope work directly. Preserve the PR URL, merged head SHA, merge commit when available, cleanup state, branch-state closeout record, and deploy or release result as observable state for `/handoff` to read. When the session is complete, invoke `/handoff` plain with no caller-assembled payload; `/handoff` owns the operator-useful summary, **Remaining Branches** groups, and continuation disposition. Apply this single closeout contract on every invocation. Do not emit a receipt-only response made only of PR state, branch cleanup, or merge commit mechanics.

**Exit when:** the PR is closed, Step 9 has invoked `/handoff`, or Step 9 has emitted `AWAIT_DEPLOYMENT_AUTHORIZATION` or `AWAIT_RELEASE_AUTHORIZATION` with branch-state closeout evidence. Otherwise continue the remaining in-scope work, or return to Step 1 after Step 7 or after the operator resolves a token boundary.

</workflow>

<script_testing>

`${CLAUDE_SKILL_DIR}/scripts/resolve_review_thread.py` has mapping evidence in this plugin's source test suite. The covered behavior is the review-thread resolution workflow this skill invokes.
...
The narrow Bash grants in frontmatter authorize approval-free execution; they do not prohibit a consumer-defined command that the harness approves per call. Run commands from `{{! file('root_guide') !}}` or `project_command_contract` through that normal tool-approval path when they fall outside those grants. Never widen `allowed-tools` during execution.
...
**Pushed a tree only one predicate had seen.** Claude re-ran deterministic verification after a review-driven fix, re-ran an evidence auditor after a verification-driven fix, or re-ran the local review after an evidence-audit fix, but did not run every applicable predicate on the final tree — each fix is a new diff the other predicates have not covered, so the pushed tree was never jointly gated. Step 6 iterates the reusable predicates to a joint fixpoint: after every commit, re-run deterministic verification, required evidence-auditor predicates, and local review until one tree passes them all, then run the terminal full deterministic gate on that clean committed head when required and push only that tree.
...
- A reconstructed stack was triggered by the persisted exact base PR reporting merged, targets the repository default branch, is ready for review, contains every applicable contract-driven body section exactly once in declared order with no inapplicable configured section, and contains no dependency on the retired stack ref before it can produce a merged result.
- The same closeout contract applies for every invocation: continue remaining in-scope work, preserve closeout facts as observable state, then invoke `/handoff` plain with no caller-assembled payload when the session is complete.
