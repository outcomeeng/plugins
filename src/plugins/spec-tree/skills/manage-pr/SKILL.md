...
arguments: pr
allowed-tools: Read, Glob, Grep, Edit, Write,{!% if target == 'claude' %!} Agent,{!% else %!} {{! tool('spawn_agent') !}}, {{! tool('wait_agent') !}}, {{! tool('close_agent') !}},{!% endif %!} {{! tool('ask_user') !}}, Skill, Bash(gh auth status:*), Bash(gh repo view:*), Bash(gh pr view:*), Bash(gh pr edit:*), Bash(gh pr ready:*), Bash(gh pr checks:*), Bash(gh pr comment:*), Bash(gh pr review:*), Bash(gh pr merge:*), Bash(gh run view:*), Bash(gh api repos/*/pulls/*/comments:*), Bash(gh api repos/*/actions/jobs/*:*), Bash(python3 "${CLAUDE_SKILL_DIR}/scripts/resolve_review_thread.py":*), Bash(git fetch:*), Bash(git branch:*), Bash(git status:*), Bash(git log:*), Bash(git diff:*), Bash(git rev-parse:*), Bash(git merge-base:*), Bash(git push:*), Bash(git switch:*), Bash(git ls-remote:*), Bash(git cherry:*), Bash(git worktree list:*), Bash(printf:*)
---
...
4. **Terminal full deterministic gate.** When the project overlay requires a full deterministic bundle, commit the converged subject, require a clean worktree, and run the declared full-gate command once after every applicable evidence auditor and the local review converge on that exact head. Never run it concurrently with another heavy command. A later change invalidates the result, reopens every affected agentic predicate, and requires this sub-step again only after those predicates reconverge.
...
The narrow Bash grants in frontmatter authorize approval-free execution; they do not prohibit a consumer-defined command that the harness approves per call. Run commands from `{{! file('root_guide') !}}` or `spx/local/merging.md` through that normal tool-approval path when they fall outside those grants. Never widen `allowed-tools` during execution.
