...
Resolve the repository default branch in the same pass. When `baseRefName` equals that default, `isDraft` is false, and the captured `body` contains no complete `## Stack` section, classify the PR as `topology=peer` immediately; this host-observed state is authoritative after stack reconstruction. Otherwise parse the complete `## Stack` section's merge-order line for `stack_base_pr_url` and `stack_base`. When the section or either value is absent for a non-default-base or draft PR, use an exact base-PR pointer already supplied by the operator; if none exists, ask through the runtime's structured-question capability rather than guessing. Resolve that pointer with `gh pr view <stack-base-pr-pointer> --json number,url,body,headRefName,headRefOid,baseRefName,state,isDraft,mergeStateStatus,statusCheckRollup,reviewDecision,reviews,comments,mergedAt`, record its host-observed URL and `headRefName`, and never inspect a branch-naming convention or reconstruct merged-PR identity from a branch label. A pass that still lacks the exact PR URL and branch is `MERGE_BLOCKED:stack-topology-unresolved`.
