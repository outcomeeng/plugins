...
When `stack_base_pending=true`, withhold before this table with `guard_verdict: "MERGE_BLOCKED:stack-base-unmerged"`, `merge_command_allowed: false`, and `autonomous_action: "block"`. Resume from Step 1 after the stack base changes state. A completed stack transition returns to Step 1 as a ready peer and uses the ordinary table below.
