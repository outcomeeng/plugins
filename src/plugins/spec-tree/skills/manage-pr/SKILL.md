...
- After merge and declared deploy or release handling, the same closeout-ready result is returned for every invocation: PR URL, full merged head SHA, merge commit when available, cleanup state, branch-state record with **Remaining Branches**, deploy and release results, and remaining-work disposition. The branch-state closeout record from /merging-standards `<branch_state_closeout>` has been built and safe cleanup has run.
