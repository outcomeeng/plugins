...
For a stacked PR, inspect the stack-base PR before ordinary base sync. While the stack-base PR is unmerged, keep the current PR draft and sync only against `origin/<stack-base>` when Step 2 found drift. Record `stack_base_pending=true`; Step 8 emits `MERGE_BLOCKED:stack-base-unmerged` rather than treating draft state as an unexplained hygiene failure. Once the stack-base PR is merged, fetch the updated repository default branch and rebase the current branch onto `origin/<default>` per /merging-standards `<base_sync>`. Re-run the peer gate from `<branch_topology>` against that default and record `stack_transition=true` only when it passes. The transition continues in Step 6; never leave a reconstructed branch targeting the merged stack branch.
