...
**Step 4 — Continue or close.** Reaching merged state ends the transport, not necessarily the session. When in-scope parts of the user's stated goal remain, the transport continues with them rather than closing; it closes through `/handoff` only when the session is complete — the goal is met with no in-scope work remaining, or continuation by Claude is impossible (per live `/understand` `<closing_protocol>` and the `/handoff` precondition). Do not emit an independent merge receipt after the transport returns. The final operator-facing closeout comes from `/handoff` or from continuing the remaining governed work — the transport never authors it. A `/handoff` completed earlier in the same conversation never satisfies this step for work merged after it: new merged work reopens the session, and the handoff workflow's existing-session search is what makes the repeat invocation cheap — it reconciles the earlier handoff's artifact as a same-owner continuation rather than duplicating it.
...
**A prior handoff substituted for CLOSE.** Claude merged new work after a `/handoff` had closed the session earlier in the conversation, judged a second invocation redundant, and hand-authored the final closeout. The message looked operator-useful, but the duties behind it — claimed-session accounting, worktree-release verification, continuation disposition — never ran. New merged work reopens the session; only `/handoff` closes it, and its existing-session search reconciles the earlier handoff's artifact as a same-owner continuation, which is what makes the repeat invocation cheap. That cheapness is the reason to invoke it, never the reason to skip it. Avoid: at CLOSE, either continue in-scope work or invoke `/handoff` plain — never author the final closeout in the transport.
