...
**A prior handoff substituted for CLOSE.** Claude merged new work after a `/handoff` had closed the session earlier in the conversation, judged a second invocation redundant, and hand-authored the final closeout. The message looked operator-useful, but the duties behind it — claimed-session accounting, worktree-release verification, continuation disposition — never ran. New merged work reopens the session; only `/handoff` closes it, and its existing-session search classifies threads the earlier handoff already owns, which is what makes the repeat invocation cheap. Avoid: at CLOSE, either continue in-scope work or invoke `/handoff` plain — never author the final closeout in the transport.
