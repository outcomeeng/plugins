...
Identify the previous stack branch from context: the PR description's `Stack` / `Merge order` note, the branch-naming convention, or an explicit user instruction. If none of those yields a ref, invoke `{{! tool('ask_user') !}}` rather than guess.
...
When `/sync-base` reports `rebased`, the rebased tree is a fresh integration — this branch replayed on newly merged work — and the consuming flow re-establishes the `REVIEW_READINESS` predicates on it before the `--force-with-lease` push from `<push_semantics>`, fixing any failure or unaddressed valid review or audit finding in the same pass. The `preservation` proof in the `/sync-base` result scopes how much of that work the base movement actually invalidated, so a rebase that moved an unrelated part of the tree does not force a full re-run:

- **Local review.** Reuse the converged `review-changes` verdict when `preservation.branch_diff_unchanged` is true **and** no `preservation.base_delta_paths` entry is a governance surface the reviewer judges against (named in the project's merge overlay). Otherwise re-establish the review per `<local_review_invocation>` on the rebased diff.
- **Applicable audit.** Reuse the converged auditor verdict when `preservation.branch_diff_unchanged` is true **and** no `preservation.base_delta_paths` entry is a governance surface the auditor judges against (named in the project's merge overlay). Otherwise re-dispatch the applicable auditor agents per `<auditor_verdicts>` on the rebased diff; a rebased diff with no auditable content has no applicable audit.
...
The proof scopes pre-push local work only. After the push, `MERGE_READINESS` still requires every current-head required check terminal-green and a clean current-head CI review — a preservation proof never substitutes for either. When the project declares no overlay lane mapping, run the full deterministic-verification command and re-establish the review and the applicable audit on every rebase.
...
The local `review-changes` gate is the author-side, pre-push instance of the same review kind the CI review runs post-push — the two are the same class of gate on opposite sides of each push. Invoke it the way CI invokes its reviewer, passing nothing that narrows it:
...
**`REVIEW_READINESS`** authorizes opening the PR. It holds when these predicates hold:

- **deterministic verification passes** — the project's local validation and testing commands for the touched scope per `<local_deterministic_scope>` report success. A failing touched-scope test means this predicate does not hold, including a TDD-red opener authored intentionally ahead of an implementation slice. The remedy is either land the implementation in the same PR so the test passes, or add the owning node to the project's spec-tree EXCLUDE mechanism (for example `spx/EXCLUDE`) so the test runner skips the node until implementation arrives. See `references/excluded-nodes.md` in `/understand`. Per-line suppression (`# noqa`, `# type: ignore`, `@pytest.mark.skipif`, `@pytest.mark.xfail`, equivalents in other languages) does not satisfy this predicate because those suppressions are scattered and invisible to the spec-tree status surface;
- **the local review has converged** — `review-changes` (via the `changes-reviewer` agent or `/review-changes`), invoked at parity per `<local_review_invocation>` and iterated to convergence, leaves no valid finding unaddressed: each is fixed in the diff, or split out of the changeset and captured in the owning node's `ISSUES.md` / `PLAN.md`. An unbacked finding is dropped; and
- **the applicable audit has converged** — when the changeset carries implementation, test, or decision-record changes an audit verification type governs, the applicable auditor agents (per `<auditor_verdicts>`) are dispatched over the changeset and converge: no `REJECTED`/`UNKNOWN` overall verdict, `FAIL`/`UNKNOWN` row, or `REJECT` finding is left unresolved. A changeset with no auditable content — spec-only, coordination-note-only, or documentation-only — has no applicable audit, so this predicate is satisfied with nothing to dispatch. A converged `changes-reviewer` review never substitutes for this predicate: review and audit inspect different concerns, so a clean review establishes nothing about whether the audit type ran.
...
The `REVIEW_READINESS` predicates are re-established before every push, not only the opening push. A follow-up push that changes the branch's own content — a fix for a CI finding — re-runs local deterministic verification per `<local_deterministic_scope>`, the local review per `<local_review_invocation>`, and the applicable audit per `<auditor_verdicts>` on the new diff before it is pushed. A follow-up push that **only** rebased onto an advanced base re-establishes the predicates scoped by the `<base_sync>` preservation proof — reusing the local review and audit and running a narrower local validation/testing lane when the proof and the project overlay permit — rather than always re-running them in full. Either way, the author-side review and audit precede the push that fires CI, so a follow-up diff never reaches CI without author-side review and audit first.
...
- `WAIT_FOR_REVIEW` when current-head review output is absent, the review-kind check is missing/non-terminal, or the review-kind check skipped for any cause except the reviewer-skipped-by-design exception.
...
**The audit is a `REVIEW_READINESS` predicate.** When the changeset carries implementation, test, or decision-record changes an audit verification type governs, the flow dispatches the applicable auditor agents over the changeset before every push — a converged audit is the third `REVIEW_READINESS` predicate in `<authority_gates>`, alongside deterministic verification and the local review. The flow owns running the audit type locally over the changeset before publication; it never waits for an audit to surface incidentally and never lets a clean `changes-reviewer` review stand in for it, because review and audit inspect different concerns. A changeset with auditable content reaches no push until its applicable audit has converged. A spec-only, coordination-note-only, or documentation-only changeset carries no auditable content, so there is no audit to dispatch and the predicate is satisfied with nothing to run.
...
- The PR opens `ready_for_review` once `REVIEW_READINESS` holds — local deterministic verification per `<local_deterministic_scope>` passes, the local review has converged, and the applicable audit per `<auditor_verdicts>` has converged — with no draft phase as a gating mechanism (a stacked PR held draft per `<branch_topology>` is the one exception).
- The `REVIEW_READINESS` predicates — local deterministic verification per `<local_deterministic_scope>`, a converged local review, and the applicable converged audit per `<auditor_verdicts>` — are re-established on the diff every push publishes: the opening push and every content-changing follow-up push; a push that only rebased onto an advanced base re-establishes them scoped by the `<base_sync>` preservation proof.
...
- When the changeset carries auditable content, the applicable auditor agents are dispatched over the changeset before every push as the third `REVIEW_READINESS` predicate per `<auditor_verdicts>`, and every auditor verdict is handled as an in-slice finding; `REJECTED` or `UNKNOWN` overall verdicts, `FAIL` or `UNKNOWN` rows, and `REJECT` findings are fixed or resolved in the slice, not deferred to `ISSUES.md`.
