...
Both `REVIEW_READINESS` predicates are re-established before every push, not only the opening push. A follow-up push that changes the branch's own content — a fix for a CI finding — re-runs local deterministic verification per `<local_deterministic_scope>` and the local review per `<local_review_invocation>` on the new diff before it is pushed. A follow-up push that **only** rebased onto an advanced base re-establishes the predicates scoped by the `<base_sync>` preservation proof — reusing the local review and running a narrower local validation/testing lane when the proof and the project overlay permit — rather than always re-running both in full. Either way, the author-side review precedes the push that fires CI, so a follow-up diff never reaches CI without an author-side review first.
...
After `<merge_cleanup>` completes, run the post-merge steps declared by the project overlay. Treat `spx/local/merging.md` as the authority for these steps when it exists; when the overlay is absent or silent on post-merge verification, no post-merge command is required.
...
**Cross-reviewer union and convergence.** Build one finding ledger from all current-head review surfaces and reviewers, then classify each item once. A no-findings review from the designated CI reviewer, a clean local review, a passing deterministic check, or an approved audit never cancels a valid finding from another reviewer. Multiple review rounds that keep surfacing valid variants in the same area are not reviewer noise and not an operator decision point; they prove the prior fix or sweep was too narrow. Treat the next valid variant as the same defect class until the underlying lifecycle contract is repaired and a new review round finds no valid in-scope variant. "Not wired into production yet" and "deferred next slice" are not dispositions for code in the diff — production relevance is handled only by `PRODUCTION_READINESS`, after `MERGE_READINESS` is clean.
...
| `POST_MERGE_VERIFY`                      | PR merged and every autonomous post-merge command has run, but a remaining post-merge action is operator-owned, credential-blocked, or externally blocked. Runnable overlay commands do not emit this token; Claude executes them in `<post_merge_verification>` before exiting.                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
- The PR opens `ready_for_review` once `REVIEW_READINESS` holds — local deterministic verification per `<local_deterministic_scope>` passes and the local review has converged — with no draft phase as a gating mechanism (a stacked PR held draft per `<branch_topology>` is the one exception).
- Both `REVIEW_READINESS` predicates — local deterministic verification per `<local_deterministic_scope>` and a converged local review — are re-established on the diff every push publishes: the opening push and every content-changing follow-up push; a push that only rebased onto an advanced base re-establishes them scoped by the `<base_sync>` preservation proof.
...
- After merge cleanup, every autonomous post-merge command the overlay declares runs through `<post_merge_verification>` before the managing pass exits; a checkout refresh in the marketplace-source worktree is incomplete until the overlay's install-refresh command exits.
