...
When `spx/local/merging.md` declares preflight checks, run all of them immediately before the first checkout-sensitive mutation owned by each lifecycle entry. The GitHub-PR orchestration flow and direct-push transport run them before branch or commit work, the opening flow runs them before push, direct-push runs them again before its default-branch push, the managing flow runs them after its initial read-only inspection and before base sync, finding repair, commit, push, or merge work, and `release-change` runs them before it detaches the worktree off the released branch. `<merge_cleanup>` repeats the checks immediately before the merge command. A failed check stops before mutation with its output preserved.

When the overlay declares post-cleanup checks, run all of them immediately after every detach-based cleanup and before branch deletion, Change disposition, deploy, release, or closeout. This applies both to `<merge_cleanup>` and whenever a Lifecycle skill detaches a checkout. A failed post-cleanup check stops the remaining cleanup and preserves the detached checkout for inspection.
</overlay_safety_checks>
...
The changeset's git work — branch, commit, push, base-sync, PR management, merge, and its cleanup — happens in the **assigned worktree**, the repository working directory the session started in. Occupancy is established only from a fresh `spx worktree status` reading at session start, after compaction or restart, and immediately before any checkout transition. The reading must show the assigned root and the current session's live claim; cleanliness, branch name, detached state, and worktree naming never prove that a checkout is free.

- NEVER run the changeset's git work in a worktree another live session holds. When the assigned-root claim is absent or a different holder appears, stay in the assigned worktree, preserve its state, and record the occupancy defect in the governing node's `ISSUES.md`; never infer permission from a clean tree. NEVER create a worktree to carry the work, and NEVER use `git stash`; a dirty tree is cleared by `<base_sync>`, never by stashing.
...
`/sync-base` owns the complete mechanism: it fetches the base, checkpoints only paths the session is authorized to mutate through `/commit-changes`, requires that checkpoint to complete successfully before retrying its synchronizer, and performs topology-appropriate base movement. It never uses `git reset` to integrate base movement. Routine synchronization proceeds autonomously from observable Git state; never ask whether to perform an already-authorized rebase. Composing flows never duplicate dirty-tree branch, commit, or retry logic and never stash.

Continue dependent work only after `/sync-base` completes with `already_current` or `rebased` for the recorded checkout and selected base. A checkpoint alone establishes no currency, and a currency result establishes neither working-tree cleanliness nor verification readiness. Preserve unresolved authority boundaries, failed checkpoints, hard Git failures, and product-intent conflicts as distinct recovery conditions with the exact blocked action, paths, and evidence. `/sync-base` follows each condition's owning workflow for available authorized repair; involve the operator only when no authorized autonomous continuation remains. Preserve files and index entries outside mutation authority. Never bypass a failed hook or commit, retry synchronization after an incomplete checkpoint, or reinterpret another condition as a rebase conflict. An unresolved conflict retains its structured `conflict` report and active rebase state for inspection, resolution and continuation, or abort.
...
- **Local review.** Reuse the converged `{{! subagent_name('spec-tree', 'changes-reviewer') !}}` verdict when `preservation.branch_diff_unchanged` is true **and** no `preservation.base_delta_paths` entry is a governance surface the Reviewer judges against (named in the project's merge overlay). Otherwise re-establish the review per `<local_review_invocation>` on the rebased diff.
- **Evidence-auditor predicates.** Reuse a prior evidence-auditor verdict only when `preservation.branch_diff_unchanged` is true and no `preservation.base_delta_paths` entry touches a governed evidence surface. Otherwise apply `<verification_dispatch_readiness>` and re-dispatch the applicable evidence Auditors before local review.
...
```json
{
  ...
  "worktreeClean": true,
  "baseCurrency": { "status": "already_current or rebased, read from the /sync-base result for subjectHead", "remoteRef": "origin/<base>", "baseOid": "full fetched base tip SHA" },
  ...
  "consistencyMatrix": {
    "subjectHead": "the same full committed HEAD SHA as subjectHead",
    "crossSection": { "status": "zero-unresolved", "pairs": [{ "left": "section or contract", "right": "section or contract", "status": "resolved" }], "unresolved": [] },
    "crossSurface": { "status": "zero-unresolved", "pairs": [{ "left": "artifact or generated surface", "right": "artifact or generated surface", "status": "resolved" }], "unresolved": [] }
  },
  "rowRuleSweep": {
    "subjectHead": "the same full committed HEAD SHA as subjectHead",
    "status": "zero-unresolved",
    "pairs": [{ "row": "one result or diagnostic table row or field", "rule": "one governing rule in the changed artifact or its consumers", "status": "resolved or unresolved" }],
    "unresolved": []
  },
  "runOwnership": { "verificationType": "audit or review", "runIdentity": "the token the Verifier's own open or start call mints, recorded from its result; none when the Verifier records no run", "writer": "one dispatch owner", "state": "owner named; token recorded when the Verifier returns" },
  "priorRejections": [],
  "unresolved": []
}
```

The record is complete only when `baseCurrency` carries the `already_current` or `rebased` result `/sync-base` returned for `subjectHead` — invoked before every deterministic command and before this dispatch, never filled from memory — all changed paths serve the active objective or have been split from the changeset, every applicable deterministic command passed on `subjectHead`, every known finding has a same-class scan and disposition — the same-class sweep `<authority_gates>` and `<review_classification>` require, recorded as `sameClassScans` — every added or repaired test carries the assertion-design record required by `/test-evidence-standards`, and `unresolved` is empty, with every relocated case or expected value judged by the relocation rule `/test-evidence-standards` owns. The `consistencyMatrix` is bound to the same `subjectHead`, enumerates every applicable cross-section pair inside each changed artifact and every applicable cross-surface pair across declarations, authored sources, generated outputs, and result consumers, records each pair's status, and reports `zero-unresolved` with an empty `unresolved` list for both `crossSection` and `crossSurface`. Its cross-section pairs include dispatch readiness against `<auditor_verdicts>` and `<verification_result_projection>` whenever those contracts apply. The `rowRuleSweep` is bound to the same `subjectHead` and enumerates one entry for every pairing of a row or field in each changed result or diagnostic table or record with each governing rule in that artifact or its consumers; each entry is `resolved` or `unresolved`, and readiness requires `status: zero-unresolved` with an empty `unresolved` list. A section-level pair never substitutes for the row-rule entries. Every external tool capability the changeset's own text asserts traces to the project's declared version floor or to that tool's documented contract; a capability confirmed only against the version installed on the running machine is unrecorded and leaves the record incomplete.

The record is ready only when `runOwnership` names one dispatch owner for the Verifier. For a Verifier that records a run — `changes-reviewer` through the review journal, `implementation-auditor` through `spx verification run`, and any other Verifier whose skill records a run — the run identity is minted by that Verifier's own open or start call and never held by the Author in advance. The owner therefore knows its own outstanding writers without inspecting any store: a writer conflicts only while a Verifier this owner dispatched in the current pass has not yet returned, and the passing `<occupancy_preflight>` establishes that no other live session writes this branch's runs. The owner never hands a prior run token to a new dispatch for a changed head, and records the token the Verifier minted once that Verifier returns. A Verifier that returns one structured verdict and records no run — the evidence, decision, spec, skill, and subagent Auditors — sets `runIdentity` to none.
...
Missing fields, missing applicable consistency or row-rule pairs, a `consistencyMatrix.subjectHead` or `rowRuleSweep.subjectHead` that differs from `subjectHead`, any matrix or sweep status that differs from `zero-unresolved`, any unresolved pair, a `baseCurrency` absent or filled from memory, a dirty or changed subject, a nonzero deterministic result, a prior run token handed to a new dispatch, a conflicting writer, an objective/path mismatch, an unresolved item, or a repeated class with no amended invariant produces `VERIFICATION_DISPATCH_BLOCKED`; do not dispatch.

</verification_dispatch_readiness>

<local_review_invocation>

The local `{{! subagent_name('spec-tree', 'changes-reviewer') !}}` gate is the Author-side, pre-push instance of the same review kind the CI review runs post-push — the two are the same class of gate on opposite sides of each push. Invoke it the way CI invokes its Reviewer, passing nothing that narrows it:

- **Bind the committed subject and let the review derive its scope.** Commit the exact current version, require a clean worktree, and record the full `HEAD` before dispatch. Pass only `HEAD` for the current branch's default-base scope, or `origin/{base}...HEAD` for a selected base such as a stacked PR. `{{! subagent_name('spec-tree', 'changes-reviewer') !}}` derives the committed diff and governing context from that selector. Supply no file list, changed-area summary, or interpretive prompt.
...
Apply `<verification_dispatch_readiness>` before dispatch. Launch `{{! subagent_name('spec-tree', 'changes-reviewer') !}}` through exactly one native call and collect its native final result. The Verifier's agent session stays isolated from the Author's agent session that produced the changeset. Accept only its raw review run token, then inspect that token through `/project-run-journal`. If the launch fails or the final result supplies no usable raw token, preserve the observed failure and report the blocked gate without retry, substitution, model override, another launch mechanism, or a replacement review in the Author's conversation. If inspection of a valid token fails, retain the token and diagnose that inspection boundary; never launch a replacement review for it.

For completed usable reviews, act on findings by validity and phase per `<review_classification>`. Verify and checkpoint each repaired subject before its new review. Continue until no valid finding remains unaddressed; this finding-repair workflow never retries a failed launch or unusable result.
...
A process exception may substitute for the persisted local-review projection only when the review infrastructure fails after the typed `{{! subagent_name('spec-tree', 'changes-reviewer') !}}` has completed. The operator is the sole approving authority. Approval is explicit, collected through a structured choice, and bound to the exact clean committed head named in that choice.

The exception is available only when all of these conditions hold:

- the typed `{{! subagent_name('spec-tree', 'changes-reviewer') !}}` completed for the exact head and returned a raw review run token;
...
- **deterministic verification passes** — the project's local validation and testing commands for the touched scope per `<local_deterministic_scope>` report success. A failing touched-scope test means this predicate does not hold, including a TDD-red opener authored intentionally ahead of an implementation slice. The remedy is either land the implementation in the same PR so the test passes, or leave the owning node without a passing claim — or, on a toolchain that has not adopted the status claim, list it in the project's passing-scope list — so the test runner skips the node until implementation arrives. See `/understand` `<passing_scope_list>` in its status-claims reference. Per-line suppression (`# noqa`, `# type: ignore`, `@pytest.mark.skipif`, `@pytest.mark.xfail`, equivalents in other languages) does not satisfy this predicate because those suppressions are scattered and invisible to the spec-tree status surface; and
- **required evidence audits have passed** — when the least malleable node the diff touches is implementation-malleable (the absent `malleability` field, so a tree that declares none is implementation-malleable throughout) and the diff creates or modifies `[test]` assertions, linked test files, or test-infrastructure artifacts imported by linked tests, dispatch `{{! subagent_name('spec-tree', 'test-evidence-auditor') !}}`; when the diff creates or modifies `[eval]` assertions, eval artifacts (`eval.toml`, `prompt.md`, `cases.jsonl`, `history.jsonl`), or producer artifacts for eval-backed assertions, dispatch `{{! subagent_name('spec-tree', 'eval-evidence-auditor') !}}`. Each invocation supplies only the owning canonical node path; the invoked skill discovers its assertions and evidence. Apply `<verification_dispatch_readiness>`, then run the applicable evidence Auditors after deterministic verification passes and before `{{! subagent_name('spec-tree', 'changes-reviewer') !}}`. Handle completed rejected, failing, or unknown evidence-auditor verdicts per `<auditor_verdicts>`; and
- **the local review has converged** — `{{! subagent_name('spec-tree', 'changes-reviewer') !}}`, invoked at parity per `<local_review_invocation>` and iterated to convergence, leaves no valid finding unaddressed: each is fixed in the diff, or split out of the changeset and captured in the owning node's `ISSUES.md` or the governing Change. An unbacked finding is dropped. A process exception satisfying `<local_review_process_exception>` substitutes only for the unavailable persisted projection;
- **the terminal full deterministic gate has passed when required** — the full-repository bundle the overlay declares (see `<repo_local_overlay>`) ran after every applicable evidence audit and agentic review converged, against the current clean committed head, with no subsequent change and no concurrent heavy command.
...
**Mutation-point guard.** Immediately before any `gh pr merge` command, the managing flow re-reads live PR state and recomputes `MERGE_READINESS`; it never relies on earlier inspection, conversation memory, or a prior `gh pr view` result. The guard reads PR state, `statusCheckRollup`, PR-level comments, formal reviews, review-thread comments, the fetched remote branch head, and the fetched base branch. It produces `MERGE_READY:<head-sha>` only when the freshly inspected head SHA, fetched remote branch head, and inspected status-check SHA match and every `MERGE_READINESS` predicate above still holds for that same head.
...
Claude NEVER asks the operator to choose between auto-merge, hold-at-green, or pause. Merge follows `MERGE_READINESS` plus the mutation-point guard returning `MERGE_READY:<head-sha>` autonomously. Gate conditions use the explicit `<action_tokens>`; unresolved synchronization follows `<base_sync>` and reaches the operator only when no authorized autonomous continuation remains.
...
Before carrying this projection forward, pair its exact head, verdict vocabulary, finding identifiers, defect classes, and next action with every consuming section in the readiness record's `consistencyMatrix.crossSection`; record each pair resolved on `subjectHead`. A missing or unresolved projection pair produces `VERIFICATION_DISPATCH_BLOCKED` before another dispatch or push.
...
Local Auditor agents — `{{! subagent_name('spec-tree', 'test-evidence-auditor') !}}`, `{{! subagent_name('spec-tree', 'eval-evidence-auditor') !}}`, `{{! subagent_name('spec-tree', 'adr-auditor') !}}`, `{{! subagent_name('spec-tree', 'pdr-auditor') !}}`, `{{! subagent_name('spec-tree', 'spec-auditor') !}}`, and `{{! subagent_name('spec-tree', 'implementation-auditor') !}}` — emit structured findings for the slice they inspect. Language-specific audit concerns are composed through the installed `audit-{lang}-{code|tests|architecture}` skills, not through language-specific Auditor agents.

Make exactly one native launch for each required audit, with only its target as the prompt. A failed launch or unusable result blocks the gate immediately: analyze and report the observed failure without retry, substitution, model override, another launch mechanism, or an audit in the Author's conversation. A started-run implementation-audit `BLOCKED` diagnostic carrying none of the three command-evidence shapes — an exit code with stderr, a named termination with stderr, or a harness-tool failure with the tool's name and error text — is unusable on its first occurrence regardless of whether its `stderr` line says `none`; report it to the operator or the orchestrating agent session standing in for the operator, and make no workflow retry. Only their word authorizes one fresh dispatch as a new launch; without it the gate remains blocked and the Change is released with the run token in its Handoff. The verdict and repair rules below apply to completed usable results. Verify and checkpoint a repaired subject before a new audit of that subject.

Before dispatch or push, pair every applicable Auditor verdict shape and handling rule in this section with its readiness and bounded-projection consumers in `consistencyMatrix.crossSection`, and pair every authored declaration, wrapper, generated definition, and emitted result contract it crosses in `consistencyMatrix.crossSurface`. Both dimensions must be `zero-unresolved` on `subjectHead`; an absent, stale-head, or unresolved pair withholds dispatch and publication.

For every audit and review, persist accepted requirements in decisions and specs before dispatch and follow the root guide's isolation mechanics. Start the Verifier without authoring history and never append an author-written context packet, reasoning, summary, or suggested verdict. The invoked skill independently discovers evidence from its target and configured instructions. An approval produced with inherited authoring context supplies no independent gate evidence.

**Stale-base block.** A Verifier that resolves a committed scope — `{{! subagent_name('spec-tree', 'implementation-auditor') !}}`, `{{! subagent_name('spec-tree', 'changeset-coherence-auditor') !}}`, and `{{! subagent_name('spec-tree', 'changes-reviewer') !}}` — fetches the base first and returns a `stale-base` block, never a verdict, when the head does not descend from the fetched `origin/<base>` tip. The block is no finding and no rejection: run `/sync-base`, re-establish the deterministic results on the rebased head under `<verification_dispatch_readiness>`, and dispatch again. A result established on a behind head satisfies no gate predicate.

**Verdict handling.** A verdict is in-slice when its subject artifact is in the changeset's diff, and out of the PR when its subject artifact lies outside the diff. A `REJECTED` overall verdict, an `UNKNOWN` overall verdict, a `FAIL` row, an `UNKNOWN` row, or a `REJECT` finding on an in-slice subject is in-slice unresolved work, identical in handling to a valid `BLOCKING` or `DEBT` finding in `<review_classification>`: fix the bug or resolve the audit uncertainty, update `<verification_dispatch_readiness>` — a repeated defect class blocks redispatch there until the invariant is amended — re-run the Auditor, and repeat until clean. `APPROVED` means the Auditor found nothing in scope. "Capture in `ISSUES.md`" is NOT an option for rejected or unknown in-slice audit work on a slice currently under review — `ISSUES.md` is for items outside the slice, never for in-slice bugs or audit uncertainty the Auditor surfaced. A rejected, failing, or unknown row on an out-of-PR subject — an evidence artifact, run history, or case set the diff does not touch — is tracked in the owning node's `ISSUES.md` with the reason recorded and never blocks the merge.
...
**Failure 5: Claude re-dispatched after a repeated rejection without widening the repair.**
What happened: Three consecutive review runs on one changeset each requested the same literal pin, and the evidence audit rejected that literal each time; Claude answered every round with a local patch and a fresh dispatch, so the same defect class came back in a new sentence each round until the operator resolved the conflicting rules by hand.
Why it failed: A repair that removes one occurrence carries an implicit invariant that the class is gone, and a second rejection in the same class falsifies it; re-dispatching on the same invariant sends the Verifier back to the same defect and spends a full round to learn what the second rejection already proved.
How to avoid: Record every rejection's defect class in the `<verification_dispatch_readiness>` record, treat a recurrence as `VERIFICATION_DISPATCH_BLOCKED`, widen the root-cause analysis and same-class scan, and amend the governing workflow, standard, or source contract before the next dispatch.

**Failure 6: Claude wrote a tool capability into shipped text after confirming it only on the running machine.**
What happened: Claude probed the locally installed CLI, watched one subcommand return what a new instruction claimed, and shipped that claim in a reference every consumer loads. The project pinned an older floor version, and no test or documented contract carried the behavior. Four consecutive integration reviews rejected the same paragraph, each naming a different unbacked verb or field, and each repair renamed the capability instead of removing the claim.
Why it failed: A consumer runs the pinned floor, not the Author's machine, so a capability observed locally is evidence about one installation rather than about the shipped contract. The Author-side gates could not catch it either: the skill Auditor judges authoring standards and the local Reviewer reads the diff, and neither compares a cited capability against the project's version-floor record.
How to avoid: Trace every external tool capability the changeset asserts to the project's declared version floor or to that tool's documented contract before the record is complete, and when a claim cannot be traced, remove the claim rather than substitute another verb for it.
...
- The local `{{! subagent_name('spec-tree', 'changes-reviewer') !}}` gate is invoked per `<local_review_invocation>` over an exact committed subject from a clean worktree; the review derives its base/head scope, with no interpretive scope, severity pre-filter, or emphasis steering added.
- Every local Auditor and Reviewer dispatch has a complete `VERIFICATION_DISPATCH_READY` record for the exact head, with objective/path alignment, deterministic success, same-class dispositions, required assertion-design records, a head-bound row-rule sweep with one entry per applicable pair and zero unresolved, one run writer, and no unresolved work.
...
- Every detach-based cleanup runs overlay-declared post-cleanup checks before branch deletion, Change disposition, deploy, release, or closeout; failure preserves the detached checkout and stops remaining mutation.
...
- Each pass that does not fire an autonomous action emits exactly one token from `<action_tokens>`, except unresolved base synchronization, which preserves `/sync-base`'s distinct recovery condition and diagnostic evidence, including active rebase state for an unresolved conflict.
