...
- **Artifact-auditor predicates.** Reuse a prior artifact-auditor verdict only when `preservation.branch_diff_unchanged` is true and no `preservation.base_delta_paths` entry touches the governed artifact surface that Auditor judged. Otherwise apply `<verification_dispatch_readiness>` and re-dispatch the applicable artifact Auditors before local review.
...
- every applicable evidence, skill-content, subagent-configuration, decision, specification, prose-content, and implementation audit passed on the exact head; and
...
- **required artifact audits have passed** — dispatch every artifact Auditor selected by the active workflow's Output lanes for the changed surfaces. This includes `{{! subagent_name('spec-tree', 'test-evidence-auditor') !}}` and `{{! subagent_name('spec-tree', 'eval-evidence-auditor') !}}` when implementation malleability selects their changed evidence surfaces; `{{! subagent_name('spec-tree', 'adr-auditor') !}}` or `{{! subagent_name('spec-tree', 'pdr-auditor') !}}` for each changed decision; `{{! subagent_name('spec-tree', 'spec-auditor') !}}` for each changed spec; `{{! subagent_name('instructions', 'skill-auditor') !}}` for each complete changed skill surface when the instructions plugin is installed; `{{! subagent_name('prose', 'prose-auditor') !}}` for each complete changed prose surface when the prose plugin is installed; and `{{! subagent_name('spec-tree', 'implementation-auditor') !}}` when the code lane and malleability select implementation audit. Supply only the target each Auditor's contract requires, apply `<verification_dispatch_readiness>`, and run every selected artifact Auditor after its deterministic lane passes and before `{{! subagent_name('spec-tree', 'changes-reviewer') !}}`. Handle completed rejected, failing, or unknown artifact-auditor verdicts per `<auditor_verdicts>`; and
...
- **the terminal full deterministic gate has passed when required** — the full-repository bundle the overlay declares (see `<repo_local_overlay>`) ran after every applicable artifact audit and agentic review converged, against the current clean committed head, with no subsequent change and no concurrent heavy command.
...
All `VERIFICATION_READINESS` predicates are re-established before every push, not only the opening push. A follow-up push that changes the branch's own content — a fix for a CI finding — re-runs local deterministic verification per `<local_deterministic_scope>`, re-runs any artifact-auditor predicate whose judged surface changed, and re-runs the local review per `<local_review_invocation>` on the new diff before it is pushed, each dispatch preceded by `<verification_dispatch_readiness>`. A follow-up push that **only** rebased onto an advanced base re-establishes the predicates scoped by the `<base_sync>` preservation proof — reusing the local review and artifact-auditor verdicts when the branch diff is unchanged and the base movement does not touch the governed artifact surface, and running a narrower local validation/testing lane when the proof and the project overlay permit — rather than always re-running every predicate in full. Either way, the Author-side artifact audits and review precede the push that fires CI, so a follow-up diff never reaches CI without Author-side agentic verification first.
...
Local Auditor agents — `{{! subagent_name('spec-tree', 'test-evidence-auditor') !}}`, `{{! subagent_name('spec-tree', 'eval-evidence-auditor') !}}`, `{{! subagent_name('spec-tree', 'adr-auditor') !}}`, `{{! subagent_name('spec-tree', 'pdr-auditor') !}}`, `{{! subagent_name('spec-tree', 'spec-auditor') !}}`, `{{! subagent_name('instructions', 'skill-auditor') !}}`, `{{! subagent_name('prose', 'prose-auditor') !}}`, and `{{! subagent_name('spec-tree', 'implementation-auditor') !}}` — emit structured findings for the slice they inspect. Language-specific audit concerns are composed through the installed `audit-{lang}-{code|tests|architecture}` skills, not through language-specific Auditor agents.

Make exactly one native launch for each required audit, with only its target as the prompt. A failed launch or unusable result blocks the gate immediately: analyze and report the observed failure without retry, substitution, model override, another launch mechanism, or an audit in the Author's conversation. The verdict and repair rules below apply to completed usable results. Verify and checkpoint a repaired subject before a new audit of that subject.
