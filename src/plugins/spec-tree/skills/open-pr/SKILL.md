...
allowed-tools: Read, Glob, Grep,{!% if target == 'claude' %!} Agent,{!% else %!} {{! tool('spawn_agent') !}}, {{! tool('wait_agent') !}},{!% endif %!} Bash(spx worktree status:*), Bash(spx diagnose:*), Bash(gh auth status:*), Bash(git status:*), Bash(gh repo view:*), Bash(git remote get-url origin), Bash(git fetch:*), Bash(git merge-base:*), Bash(git diff:*), Bash(git rev-parse:*), Bash(gh pr view:*), Bash(git branch --show-current), Bash(git push -u origin HEAD:refs/heads/*), Bash(git log:*), Bash(gh pr create:*), Bash(gh pr checks:*), Bash(mktemp -d), Bash(printf:*), {{! tool('use_skill') !}}
---
...
**Step 0 — Load references.** {!% require_skill 'spec-tree:merging-standards' %!} It supplies the shared vocabulary. {!% require_skill 'spec-tree:commit-changes' %!} It supplies the commit type and scope classification. Follow /merging-standards `<reference_index>` and directly read its `merge-policy.md` reference before Step 1; invoking the compact loader alone does not load the tagged policy sections used below.
...
*(b) Evidence-auditor predicates.* Apply /merging-standards `<verification_dispatch_readiness>`, then dispatch every evidence Auditor /merging-standards `<authority_gates>` requires for the exact committed changeset: `{{! subagent_name('spec-tree', 'test-evidence-auditor') !}}` for changed `[test]` assertions, linked tests, or imported test-infrastructure artifacts; `{{! subagent_name('spec-tree', 'eval-evidence-auditor') !}}` for changed `[eval]` assertions, eval artifacts, or producer artifacts for eval-backed assertions. Handle rejected, failing, or unknown verdicts per /merging-standards `<auditor_verdicts>`, re-running deterministic verification, creating a new checkpoint after any repair, and re-running the relevant Auditor until the evidence predicate is clean.

*(c) Local review to convergence.* Apply /merging-standards `<verification_dispatch_readiness>`, then run the `{{! subagent_name('spec-tree', 'changes-reviewer') !}}` agent on the exact committed changeset — it runs in an isolated agent session, so the verdict is not biased by work performed in the Author's agent session. Invoke it per /merging-standards `<local_review_invocation>` with only the raw committed scope and no interpretive scope, severity pre-filter, or instruction on what to emphasize; the Reviewer reads the repository's own instructions and the shared taxonomy itself. The Reviewer emits findings only (no decision/verdict); process its findings by **validity and phase** per /merging-standards `<review_classification>` — this is the before-open phase:
...
The iteration accumulates commits on the branch — the eventual push at Step 4 sends them all. After every iteration that commits, re-run /merging-standards `<branch_hygiene>`, re-run local deterministic verification, re-run required artifact-auditor predicates for their changed surfaces, and re-run the local review — all `VERIFICATION_READINESS` predicates must hold together on the exact tree the push publishes, so loop until a single tree passes all predicates (the joint fixpoint of /manage-pr Step 6: a verification-driven fix is a diff the review has not seen, an artifact-audit fix changes its judged surface, and a review-driven fix is a tree verification has not covered). `VERIFICATION_READINESS` holds only when (a), (b), and (c) hold; only then proceed. The before-open pass is the strictest point in the lifecycle: every valid finding that belongs is applied here and only split-out work survives to the CI review, which on the open PR must show no unresolved valid `BLOCKING` or `DEBT` finding.
...
**Opened a PR gated on an earlier tree.** Claude established `VERIFICATION_READINESS`, then committed fixes during the convergence loop, and opened the PR without re-running deterministic verification, required artifact-auditor predicates, and local review on the final accumulated tree — so the opened diff was gated at an earlier state than the one CI receives. After every iteration that commits, re-run /merging-standards `<branch_hygiene>`, local deterministic verification, required artifact-auditor predicates, and the local review, treating `VERIFICATION_READINESS` as holding only when all predicates pass together on the exact tree the push publishes — never with the later-fixed predicate established before the last commit (Step 3).
...
**Convergence stall.** Claude repeated deterministic, artifact-audit, and review fixes without reaching one tree where all predicates held. Stop the loop when the next fix would expand the changeset beyond the requested scope, record the split-out concern in the owning node's coordination note, and run one final deterministic verification, required artifact-auditor predicates, and review on the narrowed branch before opening.
