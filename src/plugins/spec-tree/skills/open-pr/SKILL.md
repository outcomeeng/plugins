...
**Step 0 — Load references.** Use skill `spec-tree:merging-standards` for shared vocabulary and skill `spec-tree:commit-changes` for commit type/scope classification. Follow /merging-standards `<reference_index>` and directly read its `merge-policy.md` reference before Step 1; invoking the compact loader alone does not load the tagged policy sections used below.
