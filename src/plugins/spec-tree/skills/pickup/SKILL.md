...
3. When exactly one session exists, claim it directly. When several exist, present the first three ordered sessions in one single-select question with `{{! tool('ask_user') !}}`:
