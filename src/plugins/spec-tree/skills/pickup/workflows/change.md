...
**`gh api` stays inside the declared store.** The `gh api repos/*/issues/*` grants match by prefix and cannot bound the repository or method themselves, so this workflow passes them only the overlay's store: reads of `repos/<store>/issues/<N>/dependencies/blocked_by` and `repos/<store>/issues/<B> --jq .id`, and one write, `repos/<store>/issues/<N>/dependencies/blocked_by -X POST -F issue_id=…`. No other `-X` method, no other endpoint, and no other repository is ever passed to `gh api` here — that constraint is the whole containment.
