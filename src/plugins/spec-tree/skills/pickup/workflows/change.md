...
**Sliced.** Advance to Executable inside the Frame: settle consequential implementation Decisions, name the delivery evidence each Assertion carries, and write `## Activities` as an ordered checklist. Set `Maturity` to `Executable`. Execution still waits for the next step's checks.
