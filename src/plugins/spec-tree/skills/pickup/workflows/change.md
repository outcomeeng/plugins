...
**`gh api` stays inside the declared store.** The grant `gh api repos/*/issues/*/dependencies/*` ends in a wildcard segment that admits the appended `-X POST -F issue_id=…` the blocker write needs, and neither it nor `gh api repos/*/issues/* --jq .id` bounds the repository, so this workflow's own discipline is the containment: it passes them only the overlay's store — reads of `repos/<store>/issues/<N>/dependencies/blocked_by` and `repos/<store>/issues/<B> --jq .id`, and one write, `repos/<store>/issues/<N>/dependencies/blocked_by -X POST -F issue_id=…`. No other `-X` method, no other endpoint, and no other repository is ever passed to `gh api` here — that constraint is the whole containment.
