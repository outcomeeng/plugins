...
- `--list` presents candidates. List open Changes from the store with `gh issue list --repo <store> --state open --json number,title,assignees,url --limit 50`, then read Product, Maturity, and Status from their project items. Offer up to three through `{{! tool('ask_user') !}}`, Changes whose Product matches the overlay, Status is `Available`, and assignee list is empty: Executable first, then any Maturity, each labelled with number, title, and Maturity.
...
Claim through this exact sequence:
...
3. Re-read comments and take every `Claim:` posted after the newest `Handoff:` (or since issue creation when none exists). The earliest Claim wins. When another Claim wins, report `owned_elsewhere` naming that session and stop at the observed state; perform no cleanup mutation after losing the ordered transition.
4. When this session wins, write Status `Claimed` with `gh project item-edit` using the Status field and option ids from `gh project field-list`.
5. Read back the issue and project item. Execution may continue only when Product equals the overlay Product, Maturity is unchanged, Status is `Claimed`, the assignee list contains exactly the current account, and the earliest Claim after the latest Handoff is this session's exact comment.
...
- A claim is Status `Claimed`, exactly one assignee, and the earliest `Claim:` comment since the Change's last `Handoff:`; a session whose claim comment is not the earliest reports `owned_elsewhere` and executes nothing, and a Change whose Product or Status differs from the required Available state is never claimed.
