...
**Framed.** Slice: invoke `/slice` over the Frame's Nodes to select one independently integrable unit and its target repository; a person stays accountable, so present the Slice for approval before setting `Sliced`. Every blocker the Slice resolves — a Change that must be Applied first — is recorded in the store's issue-dependency graph, `gh api -X POST repos/<store>/issues/<N>/dependencies/blocked_by -F issue_id=<blocking issue's numeric id>` (the id from `gh api repos/<store>/issues/<B> --jq .id`), so the graph the Executable check reads is the graph refinement wrote; a Handoff's Blockers line mirrors that graph and never replaces it. Refinement may continue while blockers remain.
