...
5. If traversal exhausts the projection without reaching that resume-ready state, classify the handoff as `stale_or_superseded` and include the session evidence checked. Stop without asking the operator to search the tree, run `spx spec status`, choose a node, or supply a raw node path.
...
Otherwise, use `{{! tool('ask_user') !}}` with exactly one question and 2-3 mutually exclusive options. The options must come from the loaded context:
...
- Invoke `/handoff --no-session` if the user asks to close without creating a handoff. It archives the claimed sessions and does not put them back in the todo queue.
