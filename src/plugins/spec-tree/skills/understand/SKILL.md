...
description: >-
  ...
  {{! file('root_guide') !}} without loading this skill and all its references
  first.
...
6. **TWO NODE TYPES** — Enablers (infrastructure) and outcomes (hypothesis + assertions). No other node types exist. Read `${CLAUDE_SKILL_DIR}/references/node-types.md`.
...
2. Read the following references in full and point out any contradictions to the operator immediately:
   - `${CLAUDE_SKILL_DIR}/references/durable-map.md`
   - `${CLAUDE_SKILL_DIR}/references/node-types.md`
   - `${CLAUDE_SKILL_DIR}/references/assertion-types.md`
