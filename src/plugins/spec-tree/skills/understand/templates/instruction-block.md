...
**Use the multi-agent tool schema exactly.** The initial task goes in `message`; use `items` only when the task must pass structured mentions. Omit `fork_context`, `model`, `reasoning_effort`, and `service_tier` for typed agents. Full-history forks are incompatible with changing `agent_type` in this harness, and the named roles already carry their own model settings. Store every returned agent id verbatim. After spawning, continue only non-overlapping work while the subagent runs, then collect the result with `{{! tool('wait_agent', 'codex') !}}`. Resolve the typed close capability before cleanup; when `{{! tool('close_agent', 'codex') !}}` is absent, discover the available multi-agent close tool with `tool_search` and call its schema. Close every spawned agent immediately after collecting its final result. If no typed close capability exists or cleanup fails, record the exact discovery or close failure and block further agent dispatch until cleanup is possible; never silently leave a completed agent open.
