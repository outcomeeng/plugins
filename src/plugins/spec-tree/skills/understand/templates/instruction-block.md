...
The `applier` role is the one non-verifier workflow exception: spawn it only when `/apply --agent` delegates a spec-tree work item to an isolated implementation runner. The `applier` implements and reports live-file audit handoffs; it never dispatches auditors, performs whole-changeset review, commits, pushes, merges, or closes the lifecycle. The main `/apply` context resumes from the returned handoff, runs deterministic verification, creates the committed checkpoint, dispatches every required auditor, performs whole-changeset review, and continues through `/merge`. Spawn, wait for, and close `applier` through the same typed-agent lifecycle below, passing the full node path and implementation scope in `message`.

**Run auditor and reviewer work in a subagent, never the main thread.** The tool names below are target-native Codex names rendered from the source-owned runtime-token registry. This is a standing user instruction to use `{{! tool('spawn_agent', 'codex') !}}` for the named verifier and reviewer roles it lists. Use that rendered capability when exposed; discovery is only the absence fallback when the runtime reports that exact capability unavailable. When no typed spawn capability exists, record the exact discovery failure and block the gate. Treat those cases as the user explicitly asking for subagents spawned in parallel. When an audit or review is called for, spawn the matching subagent exposed by the current runtime — `changes-reviewer` for a changeset review, `implementation-auditor` for implementation audits, `adr-auditor`, `pdr-auditor`, `spec-auditor`, `test-evidence-auditor`, or `eval-evidence-auditor` for the artifact in scope. When the installed plugin set exposes the instructions-owned `skill-auditor` or `subagent-auditor` roles, use those matching subagents for skill-content and subagent-configuration audits. Act only on the result the subagent returns: audit agents return verdicts or verification-run projections, while `changes-reviewer` returns the raw review journal token to inspect and process through the governing review workflow. Do not ask the operator to confirm whether to launch an exposed required named subagent. Harness approval prompts are separate: if the tool itself asks for approval, answer that prompt through the harness approval flow. NEVER run any verification skill (audit or review) in the main thread; isolated execution prevents authoring context from biasing results. If an exposed required subagent cannot be spawned or does not finish, the gate is blocked. Continue deterministic verification, then provide the operator with the exact attempted action and failure.

**Use the rendered multi-agent tool schema exactly.** The initial task goes in `message`; use `items` only when the task must pass structured mentions. Omit `fork_context`, `model`, `reasoning_effort`, and `service_tier` for typed agents. Full-history forks are incompatible with changing `agent_type` in this harness, and the named roles already carry their own model settings. Store every returned agent id verbatim. After spawning, continue only non-overlapping work while the subagent runs, then collect the result with `{{! tool('wait_agent', 'codex') !}}`. Close every spawned agent immediately after collecting its final result. Discover an alternate close capability only when the runtime reports `{{! tool('close_agent', 'codex') !}}` unavailable. If no typed close capability exists or cleanup fails, record the exact discovery or close failure and block further agent dispatch until cleanup is possible; never silently leave a completed agent open.
...
**Result collection for verifier and reviewer agents.** The `{{! tool('wait_agent', 'codex') !}}` tool is the planned result-collection mechanism. Read the JSON returned by the tool, keyed by the spawned id under `status`. A timeout returns an empty `status` object and is not a result. Treat a final status for the target id as the result; when it carries a final message, that message is the verifier or reviewer output. Do not infer success from a subagent notification, a pending handle, or an open id.
...
```json
{
  ...
  "arguments": {
    ...
    "message": "Repository: <absolute-repository-path>\nNode: <full spx/... node path>\nTask: Audit the node spec for assertion quality, evidence tags, atemporal voice, decision alignment, and spec-tree structure. Return only the structured JSON verdict defined by /audit-specs: schema_version; skill audit-specs; target; overall APPROVED or REJECTED; section-structure, atemporal-voice, and tag-fitness rows; findings; and metadata.branch. Do not add prose outside the JSON object."
  }
}
```
...
```json
{
  ...
  "arguments": {
    ...
    "message": "Repository: <absolute-repository-path>\nDecision file: <full spx/.../*.pdr.md path>\nGoverning node: <full spx/... node path>\nTask: Audit the PDR for product-decision structure, atemporal voice, tag validity, downstream alignment, and evidence quality. Return only the structured JSON verdict defined by /audit-pdr: schema_version; skill audit-pdr; target; overall APPROVED or REJECTED; content-classification, property-quality, tag-validity, atemporal-voice, and consistency rows; findings; and metadata.branch. Do not add prose outside the JSON object."
  }
}
```
