...
- **auditing** (agentic) — spec, tests, and implementation against the Spec Tree methodology, and against the language skills.
- **validation** (deterministic) — spec format, and per-language tests and implementation, against tool config.
...
The assertion tag `[review]` is the legacy spelling of the `[audit]` tag and resolves to it during migration; it does not denote the `reviewing` type. The collision is between a tag and a type — `[review]`-the-tag names the auditing type, `reviewing`-the-type is the open-ended gate that backs no tag.
