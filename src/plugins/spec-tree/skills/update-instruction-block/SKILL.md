...
- The Step 1/Step 5 comparison confirms every existing file preserves independent content, a newly created counterpart receives the recorded seed content, and a two-file-absent start creates no independent content; successful `--write` followed by `--check` printing `current` confirms symlink replacement and retired `spx/` instruction-file removal.
