...
- A diverged `shared` region ends reconciled by whole-side replacement — git recency or the operator-selected side — and a recency tie, a one-sided region, a malformed shared fence, or a dirty root file was surfaced to the operator rather than guessed.
