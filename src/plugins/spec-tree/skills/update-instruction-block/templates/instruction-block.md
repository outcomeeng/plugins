...
**ALWAYS** invoke `/contextualize` on a spec node before discussing it and before reading or modifying any implementation it governs.
...
**🛑 STOP TRIGGER — after every compaction event:** the set of contextualized nodes is empty. **NEVER** read or modify implementation whose governing spec node has not been contextualized since that compaction, and **NEVER** discuss a node before contextualizing it. Implementation is every product artifact a spec node governs or must govern; its governing node is found by search under the live foundation marker — a path under `spx/<node>/` belongs to that node, any other path to the node whose `spx/**/tests/` file names it and whose spec links that test, several nodes resolving to their lowest common ancestor. Implementation with no governing spec is not read or modified; record the gap. An operational continuation — PR inspection, check wait, merge, deploy, release, `spx session` operations, occupancy proof — opens no implementation and triggers neither `/understand` nor `/contextualize`.
