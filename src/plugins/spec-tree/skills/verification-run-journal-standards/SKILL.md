...
<objective>
The shared run-journal projection: per-event builders plus event-prefix rollup and rendering for audit, review, and review-run inspection consumers.
</objective>
...
`render_review_run.py` is the provider module consumed by `/inspect-review-run`. It resolves a sealed review token's event prefix and renders the token, terminal status, full head/base identity, scope coverage, finding counts, and findings through `render_surface(events)`. The reference skill exposes the provider contract; the inspection workflow owns execution.
...
- `/inspect-review-run` consumes the review inspection provider through its own skill-local entrypoint; this reference skill remains read-only.
