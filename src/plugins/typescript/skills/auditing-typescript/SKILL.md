...
{!% require_skill 'typescript:standardizing-typescript' %!}
...
Automated tools catch syntax errors, type mismatches, and lint violations. Phase 3 catches: functions that do more than their name says, dead parameters required by no interface, IO tangled with logic, and designs that will break under change. The predict/verify protocol (Phase 3) is how these surface.
