...
2. **Add one section per edit.** Each section gets its own dedicated pass — take time with it. Think carefully about the layout, spacing, and how this section connects to what's already there.
...
5. **Update cross-section bindings** as sections are added. When a new section's element needs to bind to an element from a previous section (e.g., an arrow connecting sections), edit the earlier element's `boundElements` array at the same time.
...
Now run the render-view-fix loop from the `<render_validate>` section in SKILL.md. This is where the loop catches visual issues that aren't obvious from JSON — overlaps, clipping, imbalanced composition.

</section_workflow>

<section_boundaries>

Plan the sections around natural visual groupings from the diagram plan. A typical large diagram might split into:
...
- **Don't generate the entire diagram in one response.** Claude hits the output token limit and produces truncated, broken JSON. Even if the diagram is small enough to fit, splitting into sections produces better results.
- **Don't delegate to a separate agent** to generate the JSON. A separate agent won't have sufficient context about the skill's rules, and the coordination overhead negates any benefit.
