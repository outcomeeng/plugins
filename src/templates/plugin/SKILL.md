...
<objective>
The {{! plugin_name !}} plugin's resolved version and agent-delivery state reported or reconciled in the scope that carries its skills.
</objective>
...
{!% if plugin_name == 'spec-tree' %!}This plugin carries two changelogs. Read one only when the reader asks what changed.
...
The marketplace line is current through its topmost `##` heading. Report that date with the result.{!% else %!}This plugin carries one changelog:

| Line   | Records                     | Path                                     |
| ------ | --------------------------- | ---------------------------------------- |
| Plugin | what changed in this plugin | `${CLAUDE_SKILL_DIR}/../../CHANGELOG.md` |

Marketplace-wide events ship in the methodology plugin's marketplace changelog; this plugin carries no copy.{!% endif %!}
...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/place_agents.py" --home <selected-codex-home> --checkout <repository-root> --check
```
...
An interrupted `init` or `upgrade` leaves no partial definition: each destination and the ownership record are replaced atomically, and re-running `init` or `upgrade` recomputes every owned destination from the shipped definitions, adopting a destination that already carries the shipped bytes into the ownership record instead of reporting it as a collision.
...
prune: /Users/dev/.codex/agents/{{! plugin_name !}}_verifier.toml
collision: /Users/dev/.codex/agents/{{! plugin_name !}}_notes.toml (unrecorded)
```

`write` names a destination whose bytes differ from the shipped definition, `prune` a recorded `{{! plugin_name !}}` destination the plugin no longer ships, and `collision` a present file the ownership record does not authorize, with its cause: `symlink`, `unrecorded`, `owned by <plugin>`, `not a regular file`, or `digest mismatch`. A `scope-split directed-removal:` or `scope-split collision:` line names a checkout copy under `.codex/agents/` — matched by shipped bytes, by the `{{! plugin_name !}}` filename prefix, or by a `skills.config` entry naming a `{{! plugin_name !}}:` skill — and stops mutation. After the mutating form succeeds, the ownership record carries one entry per written destination:

```json
{ "destination": "agents/{{! plugin_name !}}_reviewer.toml", "plugin": "{{! plugin_name !}}", "digest": "<sha256 of the installed bytes>" }
```
...
- {!% if target == 'claude' %!}Every footprint verb reports manifest delivery and writes nothing.{!% else %!}`check` writes nothing and reports every drift, collision, and scope split.
...
- A final check reports no drift, collision, or scope split.{!% endif %!}
